...
public abstract class NodeAdapter implements MindMapNode {
  ...
  public NodeStyle getBareStyle() {
    return style;
  }

  /** A Node-Style like MindMapNode.STYLE_FORK or MindMapNode.STYLE_BUBBLE */
  public NodeStyle getStyle() {
    var returnedStyle = style; /* Style string returned */
    if (style == null) {
      if (this.isRoot()) {
        returnedStyle =
            NodeStyle.valueOf(
                getMapFeedback().getProperty(FreeMind.RESOURCES_ROOT_NODE_STYLE).toUpperCase());
      } else {
        var stdstyle =
            NodeStyle.valueOf(
                getMapFeedback().getProperty(FreeMind.RESOURCES_NODE_STYLE).toUpperCase());
        if (stdstyle.equals(NodeStyle.AS_PARENT)) {
          returnedStyle = getParentNode().getStyle();
        } else {
          returnedStyle = stdstyle;
        }
      }
    } else if (this.isRoot() && style.equals(NodeStyle.AS_PARENT)) {
      returnedStyle =
          NodeStyle.valueOf(
              getMapFeedback().getProperty(FreeMind.RESOURCES_ROOT_NODE_STYLE).toUpperCase());
    } else if (style.equals(NodeStyle.AS_PARENT)) {
      returnedStyle = getParentNode().getStyle();
    }

    // Handle the combined node style
    if (returnedStyle.equals(NodeStyle.COMBINED)) {
      if (this.isFolded()) {
    ...
    for (int i = 0; i < targetVector.size(); ++i) {
      if (targetVector.get(i) instanceof ArrowLinkAdapter) {
        XMLElement arrowLinkTargetElement =
            ((ArrowLinkAdapter) targetVector.get(i)).createArrowLinkTarget(registry).save();

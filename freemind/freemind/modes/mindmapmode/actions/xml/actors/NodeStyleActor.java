...
public class NodeStyleActor extends XmlActorAdapter {
  ...
  public void setStyle(MindMapNode node, NodeStyle style) {
    if (style == null) {
      execute(getActionPair(node, null));
      return;
    }
    for (int i = 0; i < NodeStyle.values().length; i++) {
      var dstyle = NodeStyle.values()[i];
  ...
  public ActionPair getActionPair(MindMapNode targetNode, NodeStyle style) {
  ...
  private NodeStyleFormatAction createNodeStyleFormatAction(MindMapNode selected, NodeStyle style) {
  ...
  public void act(XmlAction action) {
    if (action instanceof NodeStyleFormatAction) {
      ...
      var style = nodeStyleAction.getStyle();

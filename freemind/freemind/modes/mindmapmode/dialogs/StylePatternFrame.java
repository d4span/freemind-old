...
public class StylePatternFrame extends JPanel implements TextTranslator, PropertyChangeListener {
  ...
  private Vector<PropertyControl> getControls() {
    ...
    mNodeStyle = new ComboProperty(NODE_STYLE + ".tooltip", NODE_STYLE, possibleValues, this);
  ...
  public void setPattern(Pattern pattern) {
    ...
    setPatternControls(
        ...
        fmMain.getDefaultProperty(FreeMind.RESOURCES_BACKGROUND_COLOR));
    setPatternControls(
        pattern.getPatternNodeStyle(),
        mSetNodeStyle,
        mNodeStyle,
        NodeStyle.AS_PARENT.name().toLowerCase());
  ...
  private HashMap<String, Integer> getEdgeWidthTransformation() {
    ...
    transformator.put(EDGE_WIDTHS[0], new Integer(EdgeAdapter.WIDTH_PARENT));
  ...
  public Pattern getResultPattern(Pattern pattern) {
    ...
    pattern.setPatternScript(
        (PatternScript) getPatternResult(new PatternScript(), mSetScriptPattern, mScriptPattern));
    if (StylePatternFrameType.WITH_NAME_AND_CHILDS.equals(mType)) {
  ...
  public void propertyChange(PropertyChangeEvent pEvt) {
    ...
    if (mPropertyChangePropagation.containsKey(pEvt.getSource())) {
      ...
      PropertyControl bean = (PropertyControl) mPropertyChangePropagation.get(booleanProp);

...
public class OptionPanel implements TextTranslator {
  ...
  public OptionPanel(FreeMind fm, JDialog frame, OptionPanelFeedback feedback) {
    ...
    if (storage != null && storage instanceof OptionPanelWindowConfigurationStorage) {
      OptionPanelWindowConfigurationStorage oWindowSettings =
          (OptionPanelWindowConfigurationStorage) storage;
  ...
  private Vector<PropertyControl> getControls() {
    ...
    controls.add(
        new BooleanProperty(
            FreeMind.PROXY_USE_SETTINGS + TOOLTIP_EXT, FreeMind.PROXY_USE_SETTINGS));
    controls.add(new StringProperty(FreeMind.PROXY_HOST + TOOLTIP_EXT, FreeMind.PROXY_HOST));
    controls.add(
        new NumberProperty(FreeMind.PROXY_PORT + TOOLTIP_EXT, FreeMind.PROXY_PORT, 1, 65535, 1));
    controls.add(
        new BooleanProperty(
            FreeMind.PROXY_IS_AUTHENTICATED + TOOLTIP_EXT, FreeMind.PROXY_IS_AUTHENTICATED));
    controls.add(new StringProperty(FreeMind.PROXY_USER + TOOLTIP_EXT, FreeMind.PROXY_USER));
    controls.add(
        new PasswordProperty(FreeMind.PROXY_PASSWORD + TOOLTIP_EXT, FreeMind.PROXY_PASSWORD));
    controls.add(
        new StringProperty(FreeMind.PROXY_EXCEPTION + TOOLTIP_EXT, FreeMind.PROXY_EXCEPTION));
    ...
    controls.add(
        new BooleanProperty(
            FreeMind.RESOURCES_DON_T_OPEN_PORT + TOOLTIP_EXT, FreeMind.RESOURCES_DON_T_OPEN_PORT));
    controls.add(
        new BooleanProperty(
    ...
    controls.add(
        new ComboProperty(
            ...
            new String[] {
              NodeStyle.FORK.name().toLowerCase(),
              NodeStyle.BUBBLE.name().toLowerCase(),
              NodeStyle.COMBINED.name().toLowerCase()
            },
            this)); // fork
    ...
    controls.add(
        new NewTabProperty(
            "Keystrokes", form + "," + form)); // ", right:max(40dlu;p), 4dlu, 60dlu"));
    //
    ...
    controls.add(
        new KeyProperty(frame, null, "keystroke_plugins/ScriptingEngine.keystroke.evaluate"));
  ...
  private static Set<FreemindPropertyContributor> sContributors =
      new HashSet<FreemindPropertyContributor>();

 ...
 */
class ApplyFormatPlugin
/**
 */
    : MindMapNodeHookAdapter() {
    override fun invoke(rootNode: MindMapNode?) {
        // we dont need node.
        val focussed = getController()!!.selected
        val selected = getController()!!.selecteds
        val nodePattern = createPatternFromSelected(
                focussed!!, selected)
        val formatDialog = ChooseFormatPopupDialog(
                getController()!!.frame.getJFrame(), mindMapController!!,
        ...
        if (formatDialog.result == ChooseFormatPopupDialog.Companion.OK) {
            val pattern = formatDialog.pattern

 ...
 */
    ...
    override fun invoke(node: MindMapNode?) {
        ...
        if (timer == null) {
            timer = Timer()
            timer!!.schedule(TimerColorChanger(), 500, 500)
            nodeChanged(getNode())
        }
    }

    protected inner class TimerColorChanger internal constructor() : TimerTask() {
        init {
            colors.clear()
        ...
        override fun run() {
            SwingUtilities.invokeLater(Runnable {
                if (getNode() == null || getController()!!.isBlocked) return@Runnable
                getController()!!.view.acceptViewVisitor(getNode(), object : NodeViewVisitor {
                    override fun visit(view: NodeView?) {
                        if (!view!!.isVisible) {
                            return
                        }
                        val col = view.getMainView()!!.foreground
                        var index = -1
                        ...
                        view.getMainView()!!.foreground = colors[index] as Color
                    }
                })
            })
        }

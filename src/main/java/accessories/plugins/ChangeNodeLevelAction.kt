 ...
 */
	 ...
	 * java.util.List)
	 */
    override fun invoke(rootNode: MindMapNode?) {
        // we dont need node.
        var selectedNode: MindMapNode
        var selectedNodes: List<MindMapNode?>
        run {
            val focussed = mindMapController!!.selected!!
            val selecteds = mindMapController!!.selecteds
            selectedNode = focussed
            selectedNodes = selecteds
        }

        // bug fix: sort to make independent by user's selection:
        mindMapController!!.sortNodesByDepth(selectedNodes)
        if (selectedNode.isRoot) {
            mindMapController!!.controller!!.errorMessage(
                    getResourceString("cannot_add_parent_to_root"))
            return
        }
        val upwards = safeEquals("left",
        ...
        val selectedParent = selectedNode.parentNode
        for (node in selectedNodes) {
            if (node!!.parentNode !== selectedParent) {
                mindMapController!!.controller!!.errorMessage(
        ...
        // collect node ids:
        val selectedNodeId = selectedNode.getObjectId(getController())
        // WORKAROUND: Make target of local hyperlinks for the case, that ids
        // are not stored persistently.
        getMap()!!.linkRegistry!!.registerLocalHyperlinkId(selectedNodeId)
        val selectedNodesId = Vector<String?>()
        for (node in selectedNodes) {
            val nodeId = node!!.getObjectId(getController())
            // WORKAROUND: Make target of local hyperlinks for the case, that
            // ids are not stored persistently.
            getMap()!!.linkRegistry!!.registerLocalHyperlinkId(nodeId)
            selectedNodesId.add(nodeId)
        }
        if (upwards) {
            if (selectedParent!!.isRoot) {
                // change side of the items:
                val isLeft = selectedNode.isLeft
                val copy = mindMapController!!.cut(selectedNodes)
                mindMapController!!.paste(copy, selectedParent, false,
                        !isLeft)
                select(selectedNodeId, selectedNodesId)
                return
            }
            // determine child pos of parent
            val grandParent = selectedParent.parentNode
            val parentPosition = grandParent!!.getChildPosition(selectedParent)
            val isLeft = selectedParent.isLeft
            val copy = mindMapController!!.cut(selectedNodes)
            if (parentPosition == grandParent.childCount - 1) {
                mindMapController!!.paste(copy, grandParent, false, isLeft)
            } else {
                mindMapController!!.paste(
                        copy,
                        (grandParent
                                .getChildAt(parentPosition + 1) as MindMapNode), true, isLeft)
            }
            select(selectedNodeId, selectedNodesId)
        } else {
            val ownPosition = selectedParent!!.getChildPosition(selectedNode)
            // find node above the own nodes:
            var directSibling: MindMapNode? = null
            for (i in ownPosition - 1 downTo 0) {
                val sibling = selectedParent
                        .getChildAt(i) as MindMapNode
            ...
            if (directSibling == null) {
                ...
                for (i in ownPosition + 1 until selectedParent
                        .childCount) {
                    val sibling = selectedParent
                            .getChildAt(i) as MindMapNode
                    if (!selectedNodes.contains(sibling)
            ...
            if (directSibling != null) {
                ...
                val copy = mindMapController!!.cut(selectedNodes)
                mindMapController!!.paste(copy, directSibling, false,
    ...
    private fun select(selectedNodeId: String?, selectedNodesIds: List<String?>) {
        // get new nodes by object id:
        val newInstanceOfSelectedNode: MindMapNode = mindMapController
                .getNodeFromID(selectedNodeId!!)
        val newSelecteds: MutableList<MindMapNode?> = LinkedList()
        for (nodeId in selectedNodesIds) {
            newSelecteds.add(mindMapController!!.getNodeFromID(nodeId!!))
        }
        mindMapController!!.select(newInstanceOfSelectedNode, newSelecteds)
    }
}
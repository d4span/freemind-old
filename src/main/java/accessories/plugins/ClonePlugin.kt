...
class ClonePlugin : PermanentMindMapNodeHookAdapter(), NodeLifetimeListener, ClonePropertiesObserver {
    /**
     * This is the master list. [mCloneNodes][ClonePlugin.mCloneNodes] is
     * derived from it. It contains id strings.
     */
    private var mCloneNodeIds: HashSet<String>? = null

    /**
     * Includes the original node. This is a cached list with the MindMapNodes
     * belonging to the [mCloneNodeIds][ClonePlugin.mCloneNodeIds].
     */
    private var mCloneNodes: HashSet<MindMapNode?>? = null
    var cloneId: String? = null
        private set
    private var mDisabled = false
    private var mCloneItself: Boolean? = null
    private var mClonePropertiesHolder: CloneProperties? = null
    override fun invoke(node: MindMapNode?) {
        super.invoke(node)
        registerPlugin()
    }

    fun addClone(cloneNode: MindMapNode?) {
        mCloneNodeIds!!.add(mindMapController!!.getNodeID(cloneNode)!!)
        clearCloneCache()
    }
    ...
    private fun disablePlugin() {
        ...
        mindMapController!!.controller!!.errorMessage(
                mindMapController!!.getText("clone_plugin_impossible"))
    ...
    fun removeHook() {
        ...
        deregisterCloning()
        val selecteds = getVectorWithSingleElement(getNode())
        mindMapController
    ...
    override fun save(xml: XMLElement) {
        ...
        values[XML_STORAGE_CLONES] = cloneIdsAsString
        values[XML_STORAGE_CLONE_ID] = cloneId
        val cloneItselfValue = cloneItselfValue
        values[XML_STORAGE_CLONE_ITSELF] = cloneItselfValue
        logger!!.finest("Saved mCloneItself to $cloneItselfValue")
        saveNameValuePairs(values, xml)
        logger!!.fine("Saved clone plugin")
    }

    protected val cloneItselfValue: String
        protected get() = if (mClonePropertiesHolder
                        .isCloneItself()) CLONE_ITSELF_TRUE else CLONE_ITSELF_FALSE
    val cloneIdsAsString: String
        get() {
            val cloneIds = StringBuffer()
            for (cloneId in mCloneNodeIds!!) {
    ...
    override fun loadFrom(child: XMLElement?) {
        ...
        mCloneNodes = null
        mCloneNodeIds = HashSet()
        val values = loadNameValuePairs(child!!)
        val cloneIds = values[XML_STORAGE_CLONES]
        if (cloneIds != null) {
            val st = StringTokenizer(cloneIds, ",")
            while (st.hasMoreTokens()) {
                val cloneId = st.nextToken()
                mCloneNodeIds!!.add(cloneId)
            }
        }
        cloneId = values[XML_STORAGE_CLONE_ID]
        mCloneItself = if (values.containsKey(XML_STORAGE_CLONE_ITSELF)) {
            java.lang.Boolean.valueOf(safeEquals(CLONE_ITSELF_TRUE,
                    values[XML_STORAGE_CLONE_ITSELF]))
        } else {
    ...
    private fun registerPlugin() {
		 ...
		 */
        val originalNode = getNode()
        val cloneNodes = cloneNodes
        ...
        for (cloneNode in cloneNodes!!) {
            if (originalNode != null && originalNode.isDescendantOf(cloneNode!!)) {
                disablePlugin()
                return
            }
        }
        mindMapController!!.registerNodeLifetimeListener(this, false)
        val registration = registration
        if (cloneId == null) {
            // hmm, it seems, that I am the first. Let's generate an id:
            cloneId = registration.generateNewCloneId(null)
        }
        registration.registerClone(cloneId, this)
        // the clone list contains itself, too.
        addClone(getNode())
        mClonePropertiesHolder = registration
                .getCloneProperties(cloneId)
        if (mCloneItself != null) {
            mClonePropertiesHolder.setCloneItself(mCloneItself.toBoolean())
        }
        mClonePropertiesHolder!!.registerObserver(this)
    }

    protected val registration: ClonePasteAction.Registration
        protected get() = getPluginBaseClass() as ClonePasteAction.Registration

    private fun deregisterPlugin() {
        mClonePropertiesHolder!!.deregisterObserver(this)
        deregisterCloning()
        mindMapController!!.deregisterNodeLifetimeListener(this)
        // remove icon
        getNode()!!.setStateIcon(name, null)
        mindMapController!!.nodeRefresh(getNode())
    }

    protected fun deregisterCloning() {
        registration.deregisterClone(cloneId, this)
    }

    override fun onCreateNodeHook(node: MindMapNode?) {
        val cloneNodes = cloneNodes
        for (clone in cloneNodes!!) {
            for (clone2 in cloneNodes) {
                if (clone !== clone2) {
    ...
    override fun onPostDeleteNode(node: MindMapNode?, parent: MindMapNode?) {}// freemind.main.Resources.getInstance().logException(e);// is list up to date?

    /**
     * @return a list of [MindMapNode]s including the original node!
     */
    val cloneNodes: HashSet<MindMapNode?>?
        get() {
            // is list up to date?
            if (mCloneNodes != null) {
                for (cloneNode in mCloneNodes!!) {
            ...
            if (mCloneNodes!!.isEmpty()) {
                ...
                val it = mCloneNodeIds!!.iterator()
                while (it.hasNext()) {
                    val cloneId = it.next()
                    try {
                        mCloneNodes!!.add(mindMapController!!.getNodeFromID(
                                cloneId))
    ...
    private fun printNodeId(pCloneNode: MindMapNode?): String {
        return try {
            (mindMapController!!.getNodeID(pCloneNode) + ": '"
                    + pCloneNode!!.getShortText(mindMapController) + "'")
        } catch (e: Exception) {
     ...
     * @return
     */
    private fun printNodeIds(pTargets: Collection<MindMapNode?>?): String {
        val strings = Vector<String>()
        for (node in pTargets!!) {
            strings.add(printNodeId(node))
        }
        return "" + strings
    }

    private fun checkForChainError(originalNode: MindMapNode?, node: MindMapNode?,
                                   cloneNode: MindMapNode?) {
    ...
    fun removeClone(pCloneNode: MindMapNode?) {
        val nodeID = mindMapController!!.getNodeID(pCloneNode)!!
    ...
    override fun processUnfinishedLinks() {
        ...
        for (cloneNode in cloneNodes!!) {
            val hook = getHook(cloneNode)
            if (hook == null && cloneNode != null) {
                // add hook to clone partner:
                val selecteds = getVectorWithSingleElement<MindMapNode?>(cloneNode)
                // Transport the data to the plugin, as this method calls
                // invoke.
                val hookProperties = Properties()
                hookProperties.setProperty(XML_STORAGE_CLONE_ID, cloneId)
                hookProperties.setProperty(XML_STORAGE_CLONES,
                        cloneIdsAsString)
                hookProperties.setProperty(XML_STORAGE_CLONE_ITSELF, cloneItselfValue)
                mindMapController!!.addHook(cloneNode, selecteds,
	 ...
	 */
    override fun propertiesChanged(pCloneProperties: CloneProperties) {
        mCloneItself = java.lang.Boolean.valueOf(pCloneProperties.isCloneItself)
    }

    companion object {
        const val CLONE_ITSELF_FALSE = "false"
        const val CLONE_ITSELF_TRUE = "true"
        const val PLUGIN_LABEL = "accessories/plugins/ClonePlugin.properties"
        const val XML_STORAGE_CLONES = "CLONE_IDS"
        const val XML_STORAGE_CLONE_ID = "CLONE_ID"
        const val XML_STORAGE_CLONE_ITSELF = "CLONE_ITSELF"
        fun getHook(originalNode: MindMapNode?): ClonePlugin? {
            if (originalNode == null) {
                return null
            }
            for (hook in originalNode.activatedHooks) {
                if (hook is ClonePlugin) {
                    return hook
                }
            }
            return null

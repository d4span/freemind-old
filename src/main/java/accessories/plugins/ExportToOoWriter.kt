 ...
 * @author foltin
 */
class ExportToOoWriter : ExportHook() {
    /**
     *
     */
    init {
    ...
    override fun startupMapHook() {
        ...
        val chosenFile = chooseFile(getResourceString("file_type"), null /*
    ...
    fun exportToOoWriter(chosenFile: File?): Boolean {
        // get output:
        val writer = StringWriter()
        // get XML
        getController()!!.map!!.getFilteredXml(writer)
        val xslts = getResourceString("files")
     ...
     */
    @Throws(IOException::class)
    private fun applyXsltFile(xsltFileName: String, writer: StringWriter,
                              result: Result): Boolean {
        val xsltUrl = getResource(xsltFileName)
        if (xsltUrl == null) {
            Companion.logger!!.severe("Can't find $xsltFileName as resource.")
            throw IllegalArgumentException("Can't find " + xsltFileName
                    + " as resource.")
        }
        val xsltStream = xsltUrl.openStream()
        // System.out.println("set xsl");
        val xsltSource: Source = StreamSource(xsltStream)

        // create an instance of TransformerFactory
        return try {
            val reader = StringReader(writer.buffer
                    .toString())
            ...
            trans.setParameter(
                    ...
                    DateFormat.getDateInstance(DateFormat.SHORT).format(
                            Date()))
            trans.transform(StreamSource(reader), result)
            true
        } catch (e: Exception) {
    ...
    @Throws(IOException::class)
    fun exportToOoWriter(file: File?, writer: StringWriter, xslts: String?): Boolean {
        var resultValue = true
        val zipout = ZipOutputStream(FileOutputStream(file))
        val result: Result = StreamResult(zipout)
        val tokenizer = StringTokenizer(xslts, ",")
        while (tokenizer.hasMoreTokens()) {
            val token = tokenizer.nextToken()
            val files = token.split("->").toTypedArray()
            if (files.size == 2) {
                val entry = ZipEntry(files[1])
                zipout.putNextEntry(entry)
                resultValue = if (files[0].endsWith(".xsl")) {
                    Companion.logger!!.info("Transforming with xslt " + files[0]
                            + " to file " + files[1])
                    resultValue and applyXsltFile(files[0], writer, result)
                } else {
                    Companion.logger!!.info("Copying resource from " + files[0]
                            + " to file " + files[1])
                    resultValue and copyFromResource(files[0], zipout)
                }
                zipout.closeEntry()
            }
        }
     ...
     * @return true, if successful.
     */
    private fun copyFromResource(fileName: String, out: OutputStream): Boolean {
        // adapted from http://javaalmanac.com/egs/java.io/CopyFile.html
        // Copies src file to dst file.
        // If the dst file does not exist, it is created
        return try {
            Companion.logger!!.finest("searching for $fileName")
            val resource = getResource(fileName)
            if (resource == null) {
                Companion.logger!!.severe("Cannot find resource: $fileName")
                return false
            }
            ...
            Companion.logger!!.severe("File not found or could not be copied. "

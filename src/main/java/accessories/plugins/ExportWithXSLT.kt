 ...
 */
     ...
     */
    var isTransformResultWithoutError = false
        private set

    protected fun chooseFile(): File? {
        var nameExtension: String? = null
    ...
    override fun startupMapHook() {
        ...
        val model = getController()!!.map
        if (safeEquals(getResourceString("file_type"), "user")) {
            if (model == null) return  // there may be no map open
            if (model.file == null || model.isReadOnly) {
                if (mc!!.save()) {
                    export(model.file)
                    return
                } else return
            } else export(model.file)
        } else {
            val saveFile = chooseFile()
                    ?: // no file.
                    return
            try {
                isTransformResultWithoutError = true
                val transformErrors = transform(saveFile)
                if (transformErrors != null) {
                    JOptionPane.showMessageDialog(null,
                            getResourceString(transformErrors), "Freemind",
                            JOptionPane.ERROR_MESSAGE)
                    isTransformResultWithoutError = false
                } else {
                    if (safeEquals(getResourceString("load_file"), "true")) {
                        getController()!!.frame.openDocument(
                                fileToUrl(saveFile))
                    }
                }
            } catch (e: Exception) {
    ...
    fun transform(saveFile: File): String? {
        // get AREA:
        // create HTML image?
        val create_image = safeEquals(
                getResourceString("create_html_linked_image"), "true")
        val areaCode = getAreaCode(create_image)
        // XSLT Transformation
        val xsltFileName = getResourceString("xslt_file")
        var success = transformMapWithXslt(xsltFileName, saveFile, areaCode)
        if (!success) {
            // JOptionPane.showMessageDialog(null,
        ...
        if (success
            ...
            val directoryName = saveFile.absolutePath + "_files"
            success = createDirectory(directoryName)

            // copy files from the resources to the file system:
            if (success) {
                val files = getResourceString("files_to_copy")
                val filePrefix = getResourceString("file_prefix")
                copyFilesFromResourcesToDirectory(directoryName, files,
                        filePrefix)
            }
            // copy icons?
            if (success
                    && safeEquals(getResourceString("copy_icons"), "true")) {
                success = copyIcons(directoryName)
            }
            if (success
                    && safeEquals(getResourceString("copy_map"), "true")) {
    ...
    private fun copyMap(pDirectoryName: String): Boolean {
        val success = true
        // Generating output Stream
        val fileout = BufferedWriter(
                OutputStreamWriter(FileOutputStream(pDirectoryName
                        + File.separator + "map.mm")))
        getController()!!.map!!.getFilteredXml(fileout)
        return success
    }

    /**
     */
    private fun copyIcons(directoryName: String): Boolean {
        val success: Boolean
        val iconDirectoryName = directoryName + File.separatorChar + "icons"
    ...
    /**
     */
    private fun createImageFromMap(directoryName: String) {
        // in the test case, we don't have a viewer and skip the image.
        if (getController()!!.view == null) return
        // create image:
        val image = createBufferedImage()
        try {
            val out = FileOutputStream(directoryName
    ...
    /**
     */
    private fun copyIconsToDirectory(directoryName2: String) {
        val iconNames: Vector<String> = MindIcon.getAllIconNames()
        for (i in iconNames.indices) {
            val iconName = iconNames[i] as String
            val myIcon = MindIcon.factory(iconName)
            copyFromResource(MindIcon.getIconsPath(),
                    myIcon!!.iconBaseFileName, directoryName2)
        }
        val iconDir: File = File(Resources.getInstance().getFreemindDirectory(),
                "icons")
        if (iconDir.exists()) {
            val userIconArray = iconDir.list { dir, name -> name.matches(".*\\.png") }
            for (i in userIconArray.indices) {
                val iconName = userIconArray[i]
                if (iconName.length == 4) {
                    continue
                }
                copyFromFile(iconDir.absolutePath, iconName,
                        directoryName2)
            }
        }
    }
    ...
    /**
     */
    private fun copyFilesFromResourcesToDirectory(directoryName: String,
                                                  files: String, filePrefix: String) {
        val tokenizer = StringTokenizer(files, ",")
     ...
     */
    @Throws(IOException::class)
    private fun transformMapWithXslt(xsltFileName: String, saveFile: File,
                                     areaCode: String?): Boolean {
        val writer = mapXml
        val reader = StringReader(writer.buffer.toString())
        // search for xslt file:
        val xsltUrl = getResource(xsltFileName)
        if (xsltUrl == null) {
            logger!!.severe("Can't find $xsltFileName as resource.")
            throw IllegalArgumentException("Can't find " + xsltFileName
                    + " as resource.")
        }
        val xsltFile = xsltUrl.openStream()
        return transform(StreamSource(reader), xsltFile, saveFile, areaCode)
    }// get output:
    // get XML
    /**
     * @throws IOException
    ...
    private val mapXml: StringWriter
        private get() {
            // get output:
            val writer = StringWriter()
            // get XML
            getController()!!.map!!.getFilteredXml(writer)
            return writer
        }
     ...
     * @param create_image
     */
    private fun getAreaCode(create_image: Boolean): String? {
        var areaCode = ""
        if (create_image) {
            val root = getController()!!.map!!.rootNode
            val creator = ClickableImageCreator(root,
                    getController(),
    ...
    private fun export(file: File?) {
        val exp = ExportDialog(file, getController())
        exp.isVisible = true
    }

    @Throws(FileNotFoundException::class)
    fun transform(xmlSource: Source?, xsltStream: InputStream?,
                  resultFile: File, areaCode: String?): Boolean {
        // System.out.println("set xsl");
        val xsltSource: Source = StreamSource(xsltStream)
        // System.out.println("set result");
        val result: Result = StreamResult(FileOutputStream(resultFile))
        ...
        try {
            ...
            trans.setParameter("destination_dir", fileToRelativeUrlString(File(resultFile.absolutePath
                    + "_files/"), resultFile) + "/")
            trans.setParameter("area_code", areaCode)
            trans.setParameter("folding_type", getController()!!.frame
    ...
    companion object {
        private const val NAME_EXTENSION_PROPERTY = "name_extension"
    }
}
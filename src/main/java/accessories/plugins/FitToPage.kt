 ...
 */
    ...
    override fun startupMapHook() {
        ...
        view = getController()!!.view
        if (view == null) return
        zoom()
        EventQueue.invokeLater { scroll() }
    }

    private fun shift(coord1: Int, size1: Int, coord2: Int, size2: Int): Int {
        return coord1 - coord2 + (size1 - size2) / 2
    }

    private fun scroll() {
        val rect = view!!.innerBounds
        val viewer = view!!.visibleRect
    ...
    private fun zoom() {
        val rect = view!!.innerBounds
        // calculate the zoom:
        val oldZoom: Double = getController()!!.view.getZoom()
        val viewPort = view!!.parent as JViewport
        val viewer = viewPort.extentSize
        logger!!.info("Found viewer rect=" + viewer.height + "/" + rect.height
                + ", " + viewer.width + "/" + rect.width)
        var newZoom = viewer.width * oldZoom / (rect.width + 0.0)
        val heightZoom = viewer.height * oldZoom / (rect.height + 0.0)
        if (heightZoom < newZoom) {
            newZoom = heightZoom
        }
        logger!!.info("Calculated new zoom $newZoom")
        getController()!!.controller.setZoom(newZoom.toFloat())
    }
}
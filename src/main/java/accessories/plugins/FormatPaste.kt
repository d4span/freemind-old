 ...
 * @author foltin
 */
class FormatPaste : MindMapNodeHookAdapter() {
    override fun invoke(node: MindMapNode?) {
        super.invoke(node)
        val actionType = getResourceString("actionType")
        if (actionType == "copy_format") {
            copyFormat(node)
        } else {
            pasteFormat(node)
        }
    ...
    /**
     */
    private fun pasteFormat(node: MindMapNode?) {
        if (pattern == null) {
            JOptionPane.showMessageDialog(mindMapController!!.frame
                    .contentPane,
                    getResourceString("no_format_copy_before_format_paste"),
                    "" /* =Title */, JOptionPane.ERROR_MESSAGE)
            return
        }
        mindMapController!!.applyPattern(node!!, pattern)
    }

    /**
     */
    private fun copyFormat(node: MindMapNode?) {
        pattern = createPatternFromNode(node!!)
    }

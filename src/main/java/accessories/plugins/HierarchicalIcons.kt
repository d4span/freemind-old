...
/**  */
class HierarchicalIcons
/**
 *
 */
    : PermanentMindMapNodeHookAdapter(), UndoEventReceiver {
    private val nodeIconSets = HashMap<MindMapNode?, TreeSet<String?>>()
    override fun shutdownMapHook() {
        // remove all icons:
        val root = mindMapController!!.rootNode
        removeIcons(root)
    ...
    /**
     */
    private fun removeIcons(node: MindMapNode?) {
        node!!.setStateIcon(name, null)
        mindMapController!!.nodeRefresh(node)
    ...
    private fun setStyle(node: MindMapNode?) {
        ...
        val iconSet = TreeSet<String?>()
        val i: Iterator<MindMapNode?>? = node!!.childrenUnfolded()
        while (i!!.hasNext()) {
            val child = i.next()
            addAccumulatedIconsToTreeSet(child, iconSet, nodeIconSets[child])
        }
        ...
        if (nodeIconSets.containsKey(node)) {
            val storedIconSet = nodeIconSets[node]!!
            if (storedIconSet == iconSet) {
                dirty = false
            }
        }
        nodeIconSets[node] = iconSet
        if (dirty) {
            if (iconSet.size > 0) {
                // create multiple image:
                val image = MultipleImage(0.75f)
                for (iconName in iconSet) {
                    // logger.info("Adding icon "+iconName + " to node "+
                    // node.toString());
                    val icon = MindIcon.factory(iconName)
                    image.addImage(icon!!.icon)
                }
                node.setStateIcon(name, image)
            } else {
                node.setStateIcon(name, null)
            }
            mindMapController!!.nodeRefresh(node)
    ...
    /**
     */
    private fun addAccumulatedIconsToTreeSet(child: MindMapNode?,
                                             iconSet: TreeSet<String?>, childsTreeSet: TreeSet<String?>?) {
        for (icon in child!!.icons) {
            iconSet.add(icon!!.getName())
        }
        if (childsTreeSet == null) return
        for (iconName in childsTreeSet) {
    ...
    override fun onRemoveChildren(removedChild: MindMapNode?, oldDad: MindMapNode?) {
    ...
    /**
     */
    private fun gatherLeavesAndSetStyle(node: MindMapNode?) {
    ...
    /**
     */
    private fun gatherLeavesAndSetParentsStyle(node: MindMapNode?) {
        if (node!!.childCount == 0) {
            // call setStyleRecursive for all parents:
            if (node.parentNode != null) {
                setStyleRecursive(node.parentNode)
            }
            return
        }
        val i: Iterator<MindMapNode?>? = node.childrenUnfolded()

 ...
 */
class ImportMindmanagerFiles : ModeControllerHookAdapter() {
    override fun startupMapHook() {
        super.startupMapHook()
        val type = "mmap"
        val component: Container = getController()!!.frame.getContentPane()
        val filter = ExportHook.ImageFilter(type, null /*
																			 * No
																			 * description
																			 * so
																			 * far
																			 */)
        val chooser = getController()!!.getFileChooser(filter)
    ...
    private fun importMindmanagerFile(file: File) {
        ...
        try {
            ...
            while (`in`.available() != 0) {
                ...
                val xsltFileName = "accessories/mindmanager2mm.xsl"
                val xsltUrl = getResource(xsltFileName)
                if (xsltUrl == null) {
                    logger!!.severe("Can't find " + xsltFileName
                            + " as resource.")
                    throw IllegalArgumentException("Can't find $xsltFileName as resource.")
                }
                val xsltFile = xsltUrl.openStream()
                val xml = transForm(StreamSource(`in`), xsltFile)
                if (xml != null) {
    ...
    fun transForm(xmlSource: Source?, xsltStream: InputStream?): String? {

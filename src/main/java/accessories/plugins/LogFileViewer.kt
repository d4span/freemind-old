...
import freemind.modes.mindmapmode.actions.xml.*
import java.awt.*
import java.io.*
...
class LogFileViewer : MindMapHookAdapter(), MapModuleChangeObserver, LogReceiver {
    class Registration(controller: ModeController, map: MindMap?) : HookRegistration {
        ...
        fun togglePrintAction() {
            if (!mPrintActionHandler.containsKey(modeController)) {
                val printActionHandler = PrintActionHandler(
                        modeController)
                modeController.actionRegistry.registerHandler(
                        printActionHandler)
                mPrintActionHandler[modeController] = printActionHandler
            } else {
                modeController.actionRegistry
                        .deregisterHandler(
                                (mPrintActionHandler[modeController] as ActionHandler?)!!)
            }
        }

        val isPrintActionActive: Boolean
            get() = mPrintActionHandler.containsKey(modeController)
        ...
        companion object {
             ...
             */
            private val mPrintActionHandler = HashMap<MindMapController, PrintActionHandler>()
        }
    }
    ...
    override var mindMapController: MindMapController? = null
        private set
    private var mLogFileViewer: JDialog? = null
    var closeAction: CloseAction? = null
        private set
    private var mTextArea: JTextArea? = null
    private var mMenuBar: JMenuBar? = null
    private var mUpdateTextAreaThread: UpdateTextAreaThread? = null
    private var mSimpleFormatter: SimpleFormatter? = null

    inner class CloseAction : AbstractAction(getResourceString("LogFileViewer_close")) {
        override fun actionPerformed(arg0: ActionEvent) {
            disposeDialog()
        }
    }

    private inner class PrintOperationAction : AbstractAction(getResourceString("LogFileViewer.PrintOperationAction")), MenuItemSelectedListener {
        /*
		 * (non-Javadoc)
		 * 
		 * @see
        ...
        override fun actionPerformed(pE: ActionEvent) {
            registration.togglePrintAction()
        }
        ...
        override fun isSelected(pCheckItem: JMenuItem?, pAction: Action?): Boolean {
            return registration.isPrintActionActive
        }
    }

    private inner class SetLogLevelAction(private val mLevel: Level) : AbstractAction(getResourceString("LogFileViewer.SetLogLevelAction_"
            + pLevel.getName())), MenuItemSelectedListener {
        /*
		 * (non-Javadoc)
		 * 
		 * @see
		 ...
		 */
        override fun actionPerformed(pE: ActionEvent) {
            baseHandler!!.level = mLevel
            val loggerList = mindMapController!!.frame.loggerList
            for (otherLogger in loggerList) {
                otherLogger.level = mLevel
            }
        }
		 ...
		 */
        override fun isSelected(pCheckItem: JMenuItem?, pAction: Action?): Boolean {
            return baseHandler!!.level == mLevel
        }
    }
    ...
    override fun startupMapHook() {
        ...
        mSimpleFormatter = SimpleFormatter()
        // retrieve content
        val pathname = (mindMapController!!.frame
                .freemindDirectory
                + File.separator
                + FreeMind.LOG_FILE_NAME + ".0")
        val logFileContents = getFile(File(pathname))
        // done.
        mindMapController!!.controller!!.mapModuleManager
                .addListener(this)
        mLogFileViewer = JDialog(getController()!!.frame.getJFrame(),
                false)
        mLogFileViewer!!.title = (getResourceString("LogFileViewer_title")
                + pathname)
        mLogFileViewer
                .setDefaultCloseOperation(WindowConstants.DO_NOTHING_ON_CLOSE)
        mLogFileViewer!!.addWindowListener(object : WindowAdapter() {
        ...
        addEscapeActionToDialog(mLogFileViewer!!, CloseAction())
        /** Menu  */
        ...
        val printOperationAction: Action = PrintOperationAction()
        ...
        val loggerItem = JMenu(
                getResourceString("MapControllerPopupDialog.LogLevels"))
        menuHolder.addMenu(loggerItem, "main/loglevel/.")
        val levels = arrayOf(Level.FINEST, Level.FINER, Level.FINE, Level.INFO, Level.WARNING, Level.SEVERE, Level.OFF)
        for (i in levels.indices) {
            val level = levels[i]
            menuHolder.addAction(SetLogLevelAction(level),
                    "main/loglevel/setLogLevel_" + level.name)
        }
        menuHolder.updateMenus(mMenuBar!!, "main/")
        mLogFileViewer!!.jMenuBar = mMenuBar
        mLogFileViewer!!.setSize(400, 400)
        mLogFileViewer!!.layout = BorderLayout()
        mTextArea = JTextArea(logFileContents)
        mTextArea!!.isEditable = false
        mTextArea!!.caret.isVisible = true
        // scroll at the end
        mTextArea!!.caretPosition = logFileContents!!.length
        mLogFileViewer!!.add(JScrollPane(mTextArea), BorderLayout.CENTER)
        // restore preferences:
        // Retrieve window size and column positions.
        val storage = mindMapController
                .decorateDialog(mLogFileViewer,
                        WINDOW_PREFERENCE_STORAGE_PROPERTY) as LogFileViewerConfigurationStorage
        if (storage != null) {
            // retrieve_additional_data_here
        }
        mLogFileViewer!!.isVisible = true
        mUpdateTextAreaThread = UpdateTextAreaThread()
    ...
    protected val baseLogger: Logger
        protected get() = Companion.logger!!.parent
     ...
     */
    protected val baseHandler: LogFileLogHandler?
        protected get() {
            for (i in Companion.logger!!.handlers.indices) {
                val handler = Companion.logger!!.handlers[i]
                if (handler is LogFileLogHandler) {
                    return handler
                }
            }
            return null
    ...
    fun disposeDialog() {
        ...
        val storage = LogFileViewerConfigurationStorage()
        // put_additional_data_here
        mindMapController!!.storeDialogPositions(mLogFileViewer, storage,
                WINDOW_PREFERENCE_STORAGE_PROPERTY)
        mindMapController!!.controller!!.mapModuleManager
                .removeListener(this)
        mLogFileViewer!!.isVisible = false
	 ...
	 * .logging.LogRecord)
	 */
    override fun receiveLog(record: LogRecord?) {
        val msg = mSimpleFormatter!!.format(record)
        mUpdateTextAreaThread!!.addToInbox(msg)
    }

    private inner class UpdateTextAreaThread : Thread() {
        var mInbox = Vector<String>()
        private var mCommitSuicide = false
        private var mSuicided = false
                ...
                if (!queue.isEmpty()) {
                    EventQueue.invokeLater {
                        try {
                            ...
                            val length = mTextArea!!.document.length
                            val atEnd = mTextArea!!.caretPosition == length
                            mTextArea!!.document.insertString(length, msg, null)
                            if (atEnd) {
                                // if at end, scroll again to the end
                                mTextArea!!.caretPosition = mTextArea!!.document
                                        .length
                            }
                        } catch (ex: Exception) {
        ...
        fun addToInbox(msg: String) {
            synchronized(mInbox) { mInbox.add(msg) }
        }
    }

    companion object {
        private val WINDOW_PREFERENCE_STORAGE_PROPERTY = LogFileViewer::class.java
                .name

 ...
 */
    ...
    override fun startupMapHook() {
        ...
        val formatDialog = ManagePatternsPopupDialog(
                frame.jFrame, mindMapController!!)
        // formatDialog.pack();
        ...
        if (formatDialog.result == ChooseFormatPopupDialog.Companion.OK) {
            try {
                ...
                val patternFile: File = getController()!!.frame.getPatternsFile()
                savePatterns(FileWriter(patternFile), formatDialog.patternList)
                mindMapController!!.loadPatterns(mindMapController!!.patternReader)
                // TODO: seems to be a bad hack:
                mindMapController!!.frame.freeMindMenuBar!!.updateMenus(mindMapController!!)
            } catch (e: Exception) {
                JOptionPane.showMessageDialog(null, e.localizedMessage)
            }
        }
    }
}
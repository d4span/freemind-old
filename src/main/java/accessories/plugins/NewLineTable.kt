 ...
 * @date 11.03.2015
 */
class NewLineTable : ScalableJTable() {
    ...
    override fun getValueAt(row: Int, column: Int): Any {
        return if (row < super.getRowCount()) {
            super.getValueAt(row, column)
        } else ""
        // value to display in new line
    }

    override fun convertRowIndexToModel(viewRowIndex: Int): Int {
        return if (viewRowIndex < super.getRowCount()) {
            super.convertRowIndexToModel(viewRowIndex)
        } else super.getRowCount()
    ...
    override fun setValueAt(aValue: Any, row: Int, column: Int) {
        if (row < super.getRowCount()) {
            ...
            val attribute = NodeAttributeTableRegistration.AttributeHolder()
            when (column) {
                NodeAttributeTableRegistration.Companion.KEY_COLUMN -> {
                    attribute.mKey = aValue as String
                    attribute.mValue = ""
                }
                NodeAttributeTableRegistration.Companion.VALUE_COLUMN -> {
                    // this couldn't be happen, as the new-value-field is write protected.
                    attribute.mKey = ""
                    attribute.mValue = aValue as String
                }
            }
            val position = (model as NodeAttributeTableRegistration.AttributeTableModel).addAttributeHolder(attribute, true)
            // fix selection after sorting
            EventQueue.invokeLater {
                val table = this@NewLineTable
                val selRow = table.convertRowIndexToView(position)
     ...
     */
    override fun isCellEditable(pRow: Int, pColumn: Int): Boolean {
        return if (pRow < super.getRowCount()) {
            super.isCellEditable(pRow, pColumn)
        } else pColumn == NodeAttributeTableRegistration.Companion.KEY_COLUMN
    }
}
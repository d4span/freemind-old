...
import accessories.plugins.NodeAttributeTableRegistration.AttributeManagerimport
...
import java.lang.IllegalArgumentExceptionimport

java.util.*
import java.util.logging.Loggerimport
 ...
 */
    ...
    private class NodeHolder(pNode: MindMapNode?,
                             pMindMapController: MindMapController) {
        var mNodeId: String?
        var mMapModuleName: String

        init {
            mNodeId = pNode!!.getObjectId(pMindMapController)
            val mapModule = pMindMapController.mapModule
                    ?: throw IllegalArgumentException("MapModule not present to controller "
                            + pMindMapController)
         ...
         */
        fun getNode(pController: Controller?): MindMapNode? {
            val modeController = getModeController(pController)
            return modeController?.getNodeFromID(mNodeId)
        }

        fun getModeController(pController: Controller?): ModeController? {
            var modeController: ModeController? = null
            val mapModule = getMapModule(pController)
            if (mapModule != null) {
                modeController = mapModule.modeController
            }
            return modeController
        }

        fun getMapModule(pController: Controller?): MapModule? {
            var mapModule: MapModule? = null
            val mapModules = pController!!.mapModuleManager!!.mapModules
            for (mapModuleName in mapModules.keys) {
                if (mapModuleName != null
                        && mapModuleName == mMapModuleName) {
                    mapModule = mapModules[mapModuleName]
                    break
                }
            }
            return mapModule
        ...
        fun isIdentical(pNode: MindMapNode?,
                        pMindMapController: MindMapController): Boolean {
            val id = pNode!!.getObjectId(pMindMapController)
            val mapModule = pMindMapController.mapModule
            return if (mapModule != null) {
                id == mNodeId
            } else false
        }
    }

    class Registration(controller: ModeController, map: MindMap?) : HookRegistration, NodeSelectionListener, MenuItemEnabledListener {
        ...
        override fun onFocusNode(pNode: NodeView?) {
            ...
            if (!sPreventRegistration) {
                ...
                if (sCurrentPosition > 0
                        && (sNodeVector[sCurrentPosition - 1] as NodeHolder)
                                .isIdentical(pNode!!.model, controller)) {
                ...
                if (sCurrentPosition != sNodeVector.size) {
                     ...
                     */
                    for (i in sNodeVector.size - 1 downTo sCurrentPosition) {
                        sNodeVector.removeElementAt(i)
                    }
                }
                // logger.info("Adding " + pNode + " at " + sCurrentPosition);
                sNodeVector.add(NodeHolder(pNode!!.model, controller))
                sCurrentPosition++
        ...
        override fun isEnabled(pItem: JMenuItem?, pAction: Action?): Boolean {
            ...
            return if ("accessories/plugins/NodeHistoryBack.properties"
                    == hookName) {
                // back is only enabled if there are already some nodes to go
                // back ;-)
                sCurrentPosition > 1
            } else {
                sCurrentPosition < sNodeVector.size
            }
        }
    ...
    override fun invoke(node: MindMapNode?) {
        super.invoke(node)
        val modeController = mindMapController!!
        val direction = getResourceString("direction")
        ...
        // printVector();
        val nodeHolder = sNodeVector[sCurrentPosition - 1] as NodeHolder
        val mainController: Controller? = getController()!!.controller
        val toBeSelected = nodeHolder.getNode(mainController)
        var changeModule = false
        var newModule: MapModule? = null
        if (nodeHolder.getModeController(mainController) !== mindMapController) {
            changeModule = true
        ...
        val fChangeModule = changeModule
        val fNewModule = newModule
        ...
        EventQueue.invokeLater(Runnable {
            var c: ModeController = modeController
            if (fChangeModule) {
                val res = mainController!!.mapModuleManager
                ...
                c = fNewModule!!.modeController
            }
            if (!toBeSelected!!.isRoot) {
                c.setFolded(toBeSelected.parentNode!!, false)
            }
            val nodeView: NodeView? = c.getNodeView(toBeSelected)
            if (nodeView != null) {
                c.select(nodeView)
                sPreventRegistration = false
            }
    ...
    private fun printVector() {
        val sb = StringBuffer("\n")
        var i = 0
        for (holder in sNodeVector) {
            sb.append((if (sCurrentPosition - 1 == i) "==>" else "   ")
                    + "Node pos " + i + " is "
                    + holder.getNode(mindMapController!!.controller))
            sb.append("\n")
            i++
        }
        logger!!.info("""
    ...
    companion object {
        /** Of NodeHolder  */
        private val sNodeVector = Vector<NodeHolder>()
        private var sCurrentPosition = 0
        private var sPreventRegistration = false
    }
}
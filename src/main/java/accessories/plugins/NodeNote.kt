 ...
 * @author foltin
 */
class NodeNote : MindMapNodeHookAdapter() {
    override fun startupMapHook() {
        super.startupMapHook()
        val foldingType = getResourceString("command")
        // get registration:
        ...
        if (foldingType == "jump") {
            ...
            // show hidden window:
            if (!registration.splitPaneVisible) {
                ...
                mindMapController!!.obtainFocusForSelected()
            }
        }
    }
     ...
     * @return
     */
    private val registration: NodeNoteRegistration
        private get() = getPluginBaseClass() as NodeNoteRegistration

    // the split pane isn't visible. show it.
    private val splitPaneToScreen: Unit
        private get() {
            val registration = registration
            if (!registration.splitPaneVisible) {
                // the split pane isn't visible. show it.
            ...
            NodeNoteRegistration.Companion.getHtmlEditorPanel().getMostRecentFocusOwner()
                    .requestFocus()
        }

    private fun setShowSplitPaneProperty(pValue: Boolean) {
        mindMapController!!.setProperty(FreeMind.RESOURCES_SHOW_NOTE_PANE,
                if (pValue) "true" else "false")
    }

    companion object {
        const val NODE_NOTE_PLUGIN = "accessories/plugins/NodeNote.properties"
        const val EMPTY_EDITOR_STRING = "<html>\n  <head>\n\n  </head>\n  <body>\n    <p>\n      \n    </p>\n  </body>\n</html>\n"
        const val EMPTY_EDITOR_STRING_ALTERNATIVE = "<html>\n  <head>\n    \n  </head>\n  <body>\n    <p>\n      \n    </p>\n  </body>\n</html>\n"
        const val EMPTY_EDITOR_STRING_ALTERNATIVE2 = "<html>\n  <head>\n    \n    \n  </head>\n  <body>\n    <p>\n      \n    </p>\n  </body>\n</html>\n"
    }
}
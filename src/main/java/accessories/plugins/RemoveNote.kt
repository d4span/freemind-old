 ...
 * @author foltin
 */
class RemoveNote : MindMapNodeHookAdapter() {
    override fun invoke(rootNode: MindMapNode?) {
        ...
        val showResult = OptionalDontShowMeAgainDialog(
                ...
                "really_remove_notes", "confirmation", mindMapController!!,
    ...
    private fun removeNote(node: MindMapNode?) {
        if (mindMapController!!.selected === node) {
            NodeNoteRegistration.Companion.getHtmlEditorPanel()
                    .setCurrentDocumentContent("")
        }
        mindMapController!!.setNoteText(node!!, null)
    }

    class Registration(controller: ModeController, map: MindMap?) : HookRegistration, MenuItemEnabledListener {

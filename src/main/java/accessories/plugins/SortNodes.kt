 ...
 */
    ...
    private inner class NodeTextComparator : Comparator<MindMapNode> {
        private var mNegative = false
        override fun compare(node1: MindMapNode, node2: MindMapNode): Int {
            val nodeText1 = node1.plainTextContent
            val nodeText2 = node2.plainTextContent
            val retValue = nodeText1.compareTo(nodeText2, ignoreCase = true)
            return if (mNegative) {
                -retValue
            } else retValue
        }
    ...
    override fun invoke(node: MindMapNode?) {
        ...
        val sortVector = Vector<MindMapNode>()
        // put in all children of the node
        sortVector.addAll(node!!.children)
        val comparator = NodeTextComparator()
        var last: MindMapNode? = null
        var isOrdered = true
        for (listNode in sortVector) {
            if (last != null) {
        ...
        for (child in sortVector) {
            val childList = getVectorWithSingleElement(child)
            val cut = mindMapController!!.cut(childList)
            // paste directly again causes that the node is added as the last
            // one.
            mindMapController!!.paste(cut, node)
        }
        getController()!!.select(node, getVectorWithSingleElement<MindMapNode?>(node))

...
/**  */
class ChooseFormatPopupDialog(caller: JFrame?, private val mController: MindMapController,
                              dialogTitle: String, pattern: Pattern?, private val mNode: MindMapNode?) : JDialog(caller), TextTranslator, KeyListener {
    /**
     * @author foltin
     * @date 21.02.2014
     */
    private inner class DemoMapFeedback : ExtendedMapFeedbackAdapter() {
        ...
        override fun getDefaultFont(): Font {
            return mController.controller!!.defaultFont!!
        }
    }
    ...
    private var mDemoFrame: MapView? = null
    private var mDemoNode: MindMapNode? = null
    private var mDemoNodeMapFeedback: DemoMapFeedback? = null
     ...
     * @return void
     */
    private fun initialize(dialogTitle: String) {
        title = mController.getText(dialogTitle)
    ...
    private fun getJContentPane(): JPanel {
        if (jContentPane == null) {
			 ...
			 */jContentPane!!.add(JScrollPane(stylePatternFrame),
                    GridBagConstraints(0, 0, 2, 1, 2.0, 8.0,
                            GridBagConstraints.WEST, GridBagConstraints.BOTH,
                            Insets(0, 0, 0, 0), 0, 0))
            jContentPane!!.add(JScrollPane(demoFrame),
                    GridBagConstraints(0, 1, 2, 1, 1.0, 1.0,
                            GridBagConstraints.WEST, GridBagConstraints.BOTH,
                            Insets(0, 0, 0, 0), 0, 0))
            jContentPane!!.add(getJOKButton(), GridBagConstraints(0, 2, 1, 1,
                    1.0, 0.0, GridBagConstraints.EAST, GridBagConstraints.NONE,
                    Insets(0, 0, 0, 0), 0, 0))
            jContentPane!!.add(getJCancelButton(), GridBagConstraints(1, 2,
     ...
     * @return
     */
    private val demoFrame: Component
        private get() {
            if (mDemoFrame == null) {
                mDemoNodeMapFeedback = DemoMapFeedback()
                val mMap = MindMapMapModel(mDemoNodeMapFeedback)
                mDemoNodeMapFeedback.mMap = mMap
                val readerCreator = StringReaderCreator(
                ...
                try {
                    ...
                    mMap.root = root
                    mDemoNode = root!!.getChildAt(0) as MindMapNode
                } catch (e: XMLParseException) {
                    Resources.getInstance().logException(e)
                } catch (e: IOException) {
                    Resources.getInstance().logException(e)
                }
                mDemoFrame = MapView(mMap, mDemoNodeMapFeedback!!)
                mDemoFrame!!.centerNode(mDemoFrame!!.getNodeView(mDemoNode))
            }
            return mDemoFrame!!
        }
    private val stylePatternFrame: Component
        private get() {
            if (mStylePatternFrame == null) {
                mStylePatternFrame = object : StylePatternFrame(this, mController,
                        StylePatternFrameType.WITHOUT_NAME_AND_CHILDS) {
                    var mResetPattern = removeAllPattern
                    override fun propertyChange(pEvt: PropertyChangeEvent) {
                        super.propertyChange(pEvt)
                        if (mNode != null) {
                            mDemoNode!!.text = mNode.text
                        }
                        mDemoNodeMapFeedback!!.select(mDemoNode, getVectorWithSingleElement(mDemoNode))
                        mDemoNodeMapFeedback!!.applyPattern(mDemoNode!!, mResetPattern)
                        val pattern: Pattern = mStylePatternFrame.resultPattern
    ...
    fun getPattern(copyIntoPattern: Pattern?): Pattern {
    ...
    override fun keyTyped(keyEvent: KeyEvent) {
        println("keyTyped: $keyEvent")
    }

    companion object {
        const val CANCEL = -1
        const val OK = 1
        private const val WINDOW_PREFERENCE_STORAGE_PROPERTY = "accessories.plugins.dialogs.ChooseFormatPopupDialog.window_storage"

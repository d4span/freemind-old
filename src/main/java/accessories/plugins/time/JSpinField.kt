...
java.awt.*import java.lang.Exception
 ...
 */
open class JSpinField @JvmOverloads constructor(min: Int = Int.MIN_VALUE, max: Int = Int.MAX_VALUE) : JPanel(), ChangeListener, CaretListener, ActionListener, FocusListener {
    protected var spinner: JSpinner
    ...
    init {
        ...
        textField!!.text = Integer.toString(value)
        textField!!.addFocusListener(this)
        spinner = object : JSpinner() {
            private static
            val serialVersionUID = -6287709243342021172L
            private val textField = JTextField()
            override fun getPreferredSize(): Dimension {
                val size = super.getPreferredSize()
                return Dimension(size.width,
                        textField.preferredSize.height)
            }
        }
        spinner.editor = textField
        spinner.addChangeListener(this)
        // spinner.setSize(spinner.getWidth(), textField.getHeight());
        add(spinner, BorderLayout.CENTER)
    }

    fun adjustWidthToMaximumValue() {
        val testTextField = JTextField(Integer.toString(maximum))
        val width = testTextField.preferredSize.width
        val height = testTextField.preferredSize.height
     ...
     */
    protected fun setValue(newValue: Int, updateTextField: Boolean,
                           firePropertyChange: Boolean) {
        val oldValue = value
        value = if (newValue < minimum) {
            minimum
     ...
     * @see .getValue
     */
    open fun setValue(newValue: Int) {
     ...
     */
    fun setHorizontalAlignment(alignment: Int) {
    ...
    override fun caretUpdate(e: CaretEvent) {
        try {
            val testValue = Integer.valueOf(textField!!.text).toInt()
            if (testValue >= minimum && testValue <= maximum) {
                textField!!.foreground = darkGreen
    ...
    override fun actionPerformed(e: ActionEvent) {
        if (textField!!.foreground == darkGreen) {
            setValue(Integer.valueOf(textField!!.text).toInt())
        }
    }
    ...
    override fun setEnabled(enabled: Boolean) {
		 ...
		 */if (!enabled) {
            textField!!.background = UIManager
    ...
    companion object {
        private const val serialVersionUID = 1694904792717740650L
        ...
        fun main(s: Array<String>) {
            val frame = JFrame("JSpinField")
            frame.contentPane.add(JSpinField())
            frame.pack()

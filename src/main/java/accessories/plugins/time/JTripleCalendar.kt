 ...
 */
class JTripleCalendar(var currentMonthPosition: Int, private var mCurrentDate: Calendar?) : JPanel(), PropertyChangeListener {
    /** Contains a mapping info panel -> month distance of the panel to upper left corner. */
    private val mInfoPanels = HashMap<JSwitchableCalendar, Int>()
    private var mCurrentlyActivePanel: JSwitchableCalendar? = null
    private val mGridLayout: GridBagLayout
    protected fun getConstraints(row: Int, column: Int): GridBagConstraints {
        return GridBagConstraints(2 * column,
                row, 1, 1, 1, 1, GridBagConstraints.CENTER,
                GridBagConstraints.BOTH, Insets(0, 0, 0, 10), 50, 10)
    }

    /**
     * A calendar widget that can be switched on/off (activated/deactivated).
     */
    class JSwitchableCalendar : JCalendar() {
        override var date: Calendar?
            get() = super.date
            set(calendar) {
                val year = calendar!![Calendar.YEAR]
                val month = calendar[Calendar.MONTH]
                if (monthChooser!!.month != month) {
                    monthChooser!!.month = month
                }
                if (yearChooser.year != year) {
                    yearChooser.year = year
                }
                if (dayChooser.year != year) {
                    dayChooser.year = year
                }
                if (dayChooser.month != month) {
                    dayChooser.month = month
                }
                dayChooser!!.setDay(calendar[Calendar.DAY_OF_MONTH])
            }
		 ...
		 */
        override fun createJDayChooser(pWeekOfYearVisible: Boolean): JDayChooser? {
            return object : JDayChooser(weekOfYearVisible) {
                var mStandardSelectedColor: Color? = null
                override fun init() {
                    super.init()
                    mStandardSelectedColor = Color(160, 160, 160)
                    /**
                ...
                override fun setEnabled(pEnabled: Boolean) {
                    selectedColor = if (pEnabled) {
                        mStandardSelectedColor
                    } else {
                        // no color selection
        ...
        override fun getCalendar(): Calendar? {
            return dayChooser.temporaryCalendar
        }

        val calendarWidget: JDayChooser
            get() = dayChooser!!
    }

    private fun createInfoPanel(): JSwitchableCalendar {
        return JSwitchableCalendar()
    }

    private var mIgnoreChangeEvent = false

    init {
        if (mCurrentDate == null) {
            mCurrentDate = GregorianCalendar.getInstance()
        }
        val panelDate = mCurrentDate!!.clone() as Calendar
        panelDate.add(Calendar.MONTH, -currentMonthPosition)
        name = "JTripleCalendar"
        mGridLayout = GridBagLayout()
        layout = mGridLayout
        var monthIndex = 0
        mIgnoreChangeEvent = true
        try {
            for (column in 0 until AMOUNT_OF_COLUMNS) {
                for (row in 0 until AMOUNT_OF_ROWS) {
                    ...
                    mInfoPanels[infoPanel] = monthIndex
                    val constraints = getConstraints(row, column)
                    add(infoPanel, constraints)
                    if (monthIndex == currentMonthPosition) {
                ...
                if (column < AMOUNT_OF_COLUMNS - 1) {
                    val constraints = GridBagConstraints(2 * column + 1, 0, 1, AMOUNT_OF_ROWS,
                            0, 0, GridBagConstraints.CENTER, GridBagConstraints.VERTICAL,
                            Insets(0, 0, 0, 10), 0, 0)
                    val sep = JSeparator()
                    sep.preferredSize = Dimension(5, 2)
                    sep.orientation = JSeparator.VERTICAL
                    add(sep, constraints)
                }
            }
    ...
    override fun propertyChange(evt: PropertyChangeEvent) {
        ...
        try {
            ...
            if (safeEquals(evt.propertyName, JCalendar.Companion.CALENDAR_PROPERTY)) {
                val calendar = evt.newValue as Calendar
...
//				Tools.printStackTrace();
                if (source === mCurrentlyActivePanel) {
                    ...
                    for (infoPanel in mInfoPanels.keys) {
                        if (source === infoPanel) {
    ...
    fun setCalendar(gregorianCalendar: Calendar) {
        try {
            mIgnoreChangeEvent = true
    ...
    protected fun setCalendarInternally(gregorianCalendar: Calendar) {
...
//		System.out.println("Comparing new date " + gregorianCalendar.getTime() + " with active panel time " + mCurrentDate.getTime());
        val dist = mCurrentDate!![Calendar.MONTH] - gregorianCalendar[Calendar.MONTH] +
                12 * (mCurrentDate!![Calendar.YEAR] - gregorianCalendar[Calendar.YEAR])
        if (dist == 0) {
            propagateDate(gregorianCalendar)
        } else {
            // different month/year selected. Test, if present on the screen:
            val newIndex = currentMonthPosition - dist
            if (newIndex < 0 || newIndex >= AMOUNT_OF_COLUMNS * AMOUNT_OF_ROWS) {
                ...
                for (infoPanel in mInfoPanels.keys) {
                    if (mInfoPanels[infoPanel] == newIndex) {
    ...
    fun exchangeCalendars(infoPanel: JSwitchableCalendar,
                          gregorianCalendar: Calendar?) {
        markSelectedMonth(mCurrentlyActivePanel, false)
        mCurrentlyActivePanel = infoPanel
        currentMonthPosition = mInfoPanels[infoPanel]!!
        markSelectedMonth(infoPanel, true)
        infoPanel.calendar = gregorianCalendar
        propagateDate(gregorianCalendar)
        infoPanel.calendarWidget.setFocus()
    }

    fun markSelectedMonth(infoPanel: JSwitchableCalendar?, pSelected: Boolean) {
        if (pSelected) {
            infoPanel!!.isEnabled = true
            infoPanel.border = LineBorder(Color.red, 4)
        } else {
            infoPanel!!.isEnabled = false
            infoPanel.background = null
            infoPanel.border = null
        }
    }

    fun propagateDate(gregorianCalendar: Calendar?) {
        for (infoPanel in mInfoPanels.keys) {
            val monthDistance = mInfoPanels[infoPanel]
            val tmpCalendar = gregorianCalendar!!.clone() as Calendar
            tmpCalendar.add(Calendar.MONTH, -currentMonthPosition + monthDistance!!)
            infoPanel.date = tmpCalendar
        }
        mCurrentDate = mCurrentlyActivePanel!!.calendar
    }

    val calendar: Calendar?
        get() = mCurrentlyActivePanel!!.calendar
    var date: Date?
        get() = mCurrentlyActivePanel.getDate()
        set(date) {
            mCurrentlyActivePanel.setDate(date)
        }
    val dayChooser: JDayChooser?
        get() = mCurrentlyActivePanel.getDayChooser()
    val yearChooser: JYearChooser?
        get() = mCurrentlyActivePanel.getYearChooser()

    companion object {
        private const val AMOUNT_OF_ROWS = 3
        private const val AMOUNT_OF_COLUMNS = 4
        @JvmStatic
        fun main(args: Array<String>) {
            Resources.createInstance(FreeMindMainMock())
            val frame = JFrame("JTripleCalendar")
            frame.defaultCloseOperation = JFrame.DISPOSE_ON_CLOSE
            val jcalendar = JTripleCalendar(4, null)
            ...
            frame.addWindowFocusListener(object : WindowAdapter() {
                override fun windowGainedFocus(e: WindowEvent) {
                    ...
                    jcalendar.dayChooser.getSelectedDay().requestFocus()
                }
            })

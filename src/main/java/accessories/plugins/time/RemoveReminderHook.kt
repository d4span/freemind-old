 ...
 */
    ...
    override fun invoke(node: MindMapNode?) {
        ...
        if (hook != null) {
            val selected = Arrays.asList(*arrayOf(node))
            // adding the hook the second time, it is removed.
            mindMapController!!.addHook(node, selected, TimeManagement.Companion.REMINDER_HOOK_NAME, null)
        }
    }
}
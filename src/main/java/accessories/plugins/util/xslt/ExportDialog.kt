...
class ExportDialog(nxmlFile: File?, pController: ModeController?) : JFrame(pController!!.frame.getResourceString("ExportDialog.2")) {
    internal inner class ExportListener : ActionListener {
        private var parent: ExportDialog? = null
        var exitSystem = true
        private var cancel = false
        var xe: XmlExporter? = null

        constructor(m: ExportDialog?) {
            parent = m
        }

        constructor(m: ExportDialog?, pCancel: Boolean) {
            parent = m
            cancel = pCancel
        }
    ...
    protected var fieldXsltFileName: JTextField? = null
    protected var fieldTargetFileName: JTextField? = null
    protected var xmlFile: File? = null
    private val mController: ModeController?

    init {
        xmlFile = nxmlFile
        mController = pController
        background = Color.lightGray
        addWindowListener(WindowClosingAdapter(false))
        ...
        val lastXsltFileName: String = mController!!.frame.getProperty(
                ACCESSORIES_PLUGINS_UTIL_XSLT_EXPORT_DIALOG_STORE_XSLT)
        val lastTargetFileName: String = mController.frame.getProperty(
                ACCESSORIES_PLUGINS_UTIL_XSLT_EXPORT_DIALOG_STORE_TARGET)
        // Zwei Labels und zwei Textfelder
        gbc = makegbc(0, 0, 1, 1)
        gbc.fill = GridBagConstraints.NONE
        val label = JLabel(getResourceString("ExportDialog.3")) //$NON-NLS-1$
        ...
        fieldXsltFileName = JTextField(lastXsltFileName)
        fieldXsltFileName!!.columns = 20
        gbl.setConstraints(fieldXsltFileName, gbc)
        contentPane.add(fieldXsltFileName)
        gbc = makegbc(0, 1, 1, 1)
        gbc.fill = GridBagConstraints.NONE
        val labeli = JLabel(getResourceString("ExportDialog.4")) //$NON-NLS-1$
        ...
        gbc.weightx = 100.0
        gbc.fill = GridBagConstraints.HORIZONTAL
        fieldTargetFileName = JTextField(lastTargetFileName)
        fieldTargetFileName!!.columns = 20
        gbl.setConstraints(fieldTargetFileName, gbc)
        contentPane.add(fieldTargetFileName)

        // XSL-Button
        val xslbutton = JButton(getResourceString("ExportDialog.5")) //$NON-NLS-1$
        gbc = makegbc(2, 0, 1, 1)
        gbc.fill = GridBagConstraints.NONE
        // gbc.anchor = GridBagConstraints.SOUTHEAST;
        gbl.setConstraints(xslbutton, gbc)
        xslbutton.addActionListener(FileChooseListener(0,
                fieldXsltFileName, xslbutton, xmlFile))
        contentPane.add(xslbutton)
        // export-Button
        val exportbutton = JButton(getResourceString("ExportDialog.6")) //$NON-NLS-1$
        gbc = makegbc(2, 1, 1, 1)
        gbc.fill = GridBagConstraints.NONE
        // gbc.anchor = GridBagConstraints.SOUTHEAST;
        exportbutton.addActionListener(FileChooseListener(1,
                fieldTargetFileName, exportbutton, xmlFile))
        gbl.setConstraints(exportbutton, gbc)
        contentPane.add(exportbutton)

        // ok-Button
        val button = JButton(getResourceString("ExportDialog.7")) //$NON-NLS-1$
        gbc = makegbc(2, 2, 1, 1)
        gbc.fill = GridBagConstraints.NONE
        // gbc.anchor = GridBagConstraints.SOUTHEAST;
        button.addActionListener(ExportListener(this))
        gbl.setConstraints(button, gbc)
        contentPane.add(button)

        // cancel-Button
        val cbutton = JButton(getResourceString("ExportDialog.8")) //$NON-NLS-1$
        gbc = makegbc(1, 2, 1, 1)
        gbc.anchor = GridBagConstraints.EAST
        gbc.fill = GridBagConstraints.NONE
        // gbc.anchor = GridBagConstraints.SOUTHEAST;
        gbl.setConstraints(cbutton, gbc)
        cbutton.addActionListener(ExportListener(this, true))
    ...
    private fun makegbc(x: Int, y: Int, width: Int, height: Int): GridBagConstraints {
    ...
    private fun getResourceString(pString: String): String {
        return mController!!.frame.getResourceString(pString)
    }

    internal inner class FileChooseListener(wit: Int, jt: JTextField?, c: Component?,
                                            mmFile: File?) : ActionListener {
        private var parent: Component? = null
        private var jtf: JTextField? = null
        private var kind = 0
        private val xslch = getResourceString("ExportDialog.9") //$NON-NLS-1$
        private val expch = getResourceString("ExportDialog.10") //$NON-NLS-1$
        ...
        override fun actionPerformed(e: ActionEvent) {
            var filter: ExampleFileFilter? = null
            if (kind == 0) {
                filter = ExampleFileFilter(
                        String("xsl"), getResourceString("ExportDialog.13")) //$NON-NLS-1$ //$NON-NLS-2$
            }
            val chooser = mController!!.getFileChooser(filter)
            ...
            if (returnVal == JFileChooser.APPROVE_OPTION) {
                try {
                    if (kind == 0) {
                        if (!File(chooser.selectedFile
                                        .absolutePath).exists()) {
                            val Message: Any = getResourceString("ExportDialog.14") //$NON-NLS-1$
                            JOptionPane
                                    .showMessageDialog(
                                            null,
                                            Message,
    ...
    companion object {
        private const val ACCESSORIES_PLUGINS_UTIL_XSLT_EXPORT_DIALOG_STORE_TARGET = "accessories.plugins.util.xslt.ExportDialog.store.target" //$NON-NLS-1$
        private const val ACCESSORIES_PLUGINS_UTIL_XSLT_EXPORT_DIALOG_STORE_XSLT = "accessories.plugins.util.xslt.ExportDialog.store.xslt" //$NON-NLS-1$
    }
}
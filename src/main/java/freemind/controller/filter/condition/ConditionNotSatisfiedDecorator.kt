 ...
 */(private val originalCondition: Condition?) : Condition {
    ...
    override val listCellRendererComponent: JComponent?
        get() {
            ...
            val not = Tools.removeMnemonic(Resources.getInstance()
                    .getResourceString("filter_not"))
            val text = "$not "
            component.add(JLabel(text))
            val renderer = originalCondition
    ...
    companion object {
        ...
        fun load(element: XMLElement): Condition {
            ...
            val cond: Condition = FilterController.Companion.getConditionFactory().loadCondition(

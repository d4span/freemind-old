...
class PreviewDialog(title: String?, protected var view: MapView?, pPageFormat: PageFormat?) : JDialog(JOptionPane.getFrameForComponent(view), title, true), ActionListener {
    ...
    private fun getButton(name: String?, iconName: String,
                          action: AbstractAction?): JButton {
        var result: JButton? = null
        ...
        result = if (action != null) {
            if (icon != null) action.putValue(Action.SMALL_ICON, ImageFactory.getInstance().createIcon(imageURL))
    ...
    init {
        val preview = Preview(view, 1, pPageFormat)
        ...
        pageNumber = JLabel("- 1 -")
        val button = getButton("Back24.gif", BrowseAction(
        ...
        toolbar.add(getButton("Forward24.gif", BrowseAction(preview,
                pageNumber, 1)))
        toolbar.add(JToolBar.Separator())
        toolbar.add(getButton("ZoomIn24.gif", ZoomAction(preview,
                DEFAULT_ZOOM_FACTOR_STEP)))
        toolbar.add(getButton("ZoomOut24.gif", ZoomAction(preview,
                -DEFAULT_ZOOM_FACTOR_STEP)))
        toolbar.add(JToolBar.Separator())
        val dialog = JPanel()

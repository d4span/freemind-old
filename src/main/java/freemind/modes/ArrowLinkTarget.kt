 ...
 * @date 27.10.2012
 */
class ArrowLinkTarget
/**
 * @param pSource
 * @param pTarget I am the target!
 * @param pMapFeedback
 */
(pSource: MindMapNode?, pTarget: MindMapNode?,
 pMapFeedback: MapFeedback?) : ArrowLinkAdapter(pSource, pTarget, pMapFeedback) {
    var sourceLabel: String? = null
    override fun save(): XMLElement? {
        val arrowLink = super.save()
        arrowLink!!.name = "linktarget"
        if (sourceLabel != null) {
            arrowLink.setAttribute("SOURCE", sourceLabel!!)
        }
        return arrowLink
    }
	 ...
	 */
    override fun changeInclination(pMap: MapView, pOriginX: Int, pOriginY: Int,
                                   pDeltaX: Int, pDeltaY: Int) {
    }

    fun createArrowLinkAdapter(pRegistry: MindMapLinkRegistry?): ArrowLinkAdapter {
        val linkAdapter: ArrowLinkAdapter = MindMapArrowLinkModel(source, target, mMapFeedback)
        copy(linkAdapter)
        return linkAdapter
    }
}
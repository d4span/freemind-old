...
abstract class CloudAdapter(target: MindMapNode?, pMapFeedback: MapFeedback?) : LineAdapter(target, pMapFeedback), MindMapCloud {
    /**
     * calculates the cloud iterative level which is importent for the cloud
     * size
     */
    private fun calcIterativeLevel(target: MindMapNode?) {
        iterativeLevel = 0
        if (target != null) {
            var parentNode = target.parentNode
            while (parentNode != null) {
                val cloud = parentNode.cloud
                if (cloud != null) {
                    iterativeLevel = cloud.iterativeLevel + 1
                    break
                }
    ...
    override fun setTarget(target: MindMapNode?) {
        super.setTarget(target)
    }

    override val exteriorColor: Color?
        get() = getColor()!!.darker()

    /** gets iterative level which is required for painting and layout.  */
    override fun getIterativeLevel(): Int {
        if (iterativeLevel == -1) {
    ...
    override fun changeIterativeLevel(deltaLevel: Int) {
        if (iterativeLevel != -1) {
            iterativeLevel = iterativeLevel + deltaLevel
        }
    }

    private override var iterativeLevel: Int
    ...
    init {
        ...
        iterativeLevel = -1
        if (listener == null) {
            listener = CloudAdapterListener()
            Controller.addPropertyChangeListener(listener!!)
        }
    }

    override fun save(): XMLElement? {
        val cloud = XMLElement()
        cloud.name = "cloud"
        if (style != null) {
            cloud.setAttribute("STYLE", style!!)
        }
        if (color != null) {
            cloud.setAttribute("COLOR", colorToXml(color)!!)
        }
        if (width != LineAdapter.Companion.DEFAULT_WIDTH) {
    ...
    protected class CloudAdapterListener : FreemindPropertyListener {
        override fun propertyChanged(propertyName: String, newValue: String,
                                     oldValue: String) {
            if (propertyName == FreeMind.RESOURCES_CLOUD_COLOR) {
                standardColor = xmlToColor(newValue)
            }
            if (propertyName == standardStylePropertyString) {
    ...
    companion object {
        protected val standardStylePropertyString = "standardcloudstyle"
            protected get() = Companion.field
        private var standardColor: Color? = null
        private var standardStyle: String? = null
        private var listener: CloudAdapterListener? = null
        val DEF_STROKE: Stroke = BasicStroke(3)
    }
}
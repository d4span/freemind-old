...
abstract class EdgeAdapter(target: MindMapNode?, pMapFeedback: MapFeedback?) : LineAdapter(target, pMapFeedback), MindMapEdge {
    // private static Color standardEdgeColor = new Color(0);
    init {
        NORMAL_WIDTH = WIDTH_PARENT
        if (listener == null) {
            listener = EdgeAdapterListener()
    ...
    override fun getWidth(): Int {
        return if (width == WIDTH_PARENT) {
            if (getTarget()!!.isRoot) {
                WIDTH_THIN
            } else source.getEdge().width
        } else width
    }

    override val realWidth: Int
        get() = width
    ...
    override fun getStyle(): String? {
        return if (style == null) {
            if (getTarget()!!.isRoot) {
                mapFeedback.getProperty(standardStylePropertyString)
            } else source.getEdge().style
        } else style
    }
    ...
    // Private Methods
    // ///////
    private val source: MindMapNode?
        private get() = target.parentNode
    ...
    override fun save(): XMLElement? {
        if (style != null || color != null || width != WIDTH_PARENT) {
            ...
            if (color != null) {
                edge.setAttribute("COLOR", colorToXml(color)!!)
            }
            if (width != WIDTH_PARENT) {
                if (width == WIDTH_THIN) edge.setAttribute("WIDTH", EDGE_WIDTH_THIN_STRING) else edge.setAttribute("WIDTH", Integer.toString(width))
            }
            return edge
        }
        return null
    ...
    protected override var standardColor: Color?
        protected get() = Companion.standardColor
        protected set(standardColor) {
            Companion.standardColor = standardColor
        }
    protected override var standardStyle: String?
        protected get() = Companion.standardStyle
        protected set(standardStyle) {
            Companion.standardStyle = standardStyle
        }

    protected class EdgeAdapterListener : FreemindPropertyListener {
        override fun propertyChanged(propertyName: String, newValue: String,
                                     oldValue: String) {
            if (propertyName == FreeMind.RESOURCES_EDGE_COLOR) {
                standardColor = xmlToColor(newValue)
            }
            if (propertyName == FreeMind.RESOURCES_EDGE_STYLE) {
	 ...
	 */
    override val styleAsInt: Int
        get() {
            val edgeStyle = getStyle()
            return if (safeEquals(edgeStyle, EDGESTYLE_LINEAR)) {
                INT_EDGESTYLE_LINEAR
            } else if (safeEquals(edgeStyle, EDGESTYLE_BEZIER)) {
                INT_EDGESTYLE_BEZIER
            } else if (safeEquals(edgeStyle, EDGESTYLE_SHARP_LINEAR)) {
                INT_EDGESTYLE_SHARP_LINEAR
            } else if (safeEquals(edgeStyle, EDGESTYLE_SHARP_BEZIER)) {
                INT_EDGESTYLE_SHARP_BEZIER
            } else {
                throw IllegalArgumentException("Unknown Edge Style $edgeStyle")
            }
        }

    companion object {
        const val EDGE_WIDTH_THIN_STRING = "thin"
        private var standardColor: Color? = null
        private var standardStyle: String? = null
        private var listener: EdgeAdapterListener? = null
        const val WIDTH_PARENT = -1
        const val WIDTH_THIN = 0
        const val EDGESTYLE_LINEAR = "linear"
        const val EDGESTYLE_BEZIER = "bezier"
        const val EDGESTYLE_SHARP_LINEAR = "sharp_linear"
        const val EDGESTYLE_SHARP_BEZIER = "sharp_bezier"
        val EDGESTYLES = arrayOf(EDGESTYLE_LINEAR,
                EDGESTYLE_BEZIER, EDGESTYLE_SHARP_LINEAR, EDGESTYLE_SHARP_BEZIER)
        const val INT_EDGESTYLE_LINEAR = 0
        const val INT_EDGESTYLE_BEZIER = 1
        const val INT_EDGESTYLE_SHARP_LINEAR = 2
        const val INT_EDGESTYLE_SHARP_BEZIER = 3
    }
}
 ...
 * @date 21.02.2014
 */
class ExtendedMapFeedbackImpl : ExtendedMapFeedbackAdapter() {
    override var map: MindMap? = null
    override fun newNode(pUserObject: Any?, pMap: MindMap?): MindMapNode? {
        return MindMapNodeModel(pUserObject, pMap)
    }
    ...
    override fun copy(pNode: MindMapNode?, pSaveInvisible: Boolean): Transferable {
        ...
        try {
            (pNode as MindMapNodeModel?)!!.save(stringWriter, map
                    .getLinkRegistry(), pSaveInvisible, true)
        } catch (e: IOException) {
        }
        val nodeList = getVectorWithSingleElement(getNodeID(pNode))
        return MindMapNodesSelection(stringWriter.toString(), null,

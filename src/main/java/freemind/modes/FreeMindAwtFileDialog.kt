 ...
 * @date 23.02.2012
 */
class FreeMindAwtFileDialog : FileDialog(null as Frame?), FreeMindFileDialog {
    private class NullFilter : FileFilter() {
        /* (non-Javadoc)
		 * @see javax.swing.filechooser.FileFilter#accept(java.io.File)
		 */
        override fun accept(pF: File): Boolean {
            return true
        }
    ...
    private inner class DirFilter : FileFilter() {
        override fun accept(pF: File): Boolean {
    ...
    private inner class FileOnlyFilter : FileFilter() {
        override fun accept(pF: File): Boolean {
            return pF.isFile
        }
    ...
    private inner class FileAndDirFilter : FileFilter() {
        override fun accept(pF: File): Boolean {
    ...
    private val mFilter: FreeMindFilenameFilter
    private var mDirectoryResultListener: DirectoryResultListener? = null

    /**
     * @author foltin
     * @date 27.02.2012
     */
    private inner class FreeMindFilenameFilter
    /**
     * @param pFilter
     */
        : FilenameFilter {
         ...
         *
         */
        private var mCustomFilter: FileFilter = NullFilter()

        /**
         * Filter for dirs, files or both.
         */
        private var mPrincipalFilter: FileFilter = NullFilter()
        override fun accept(pDir: File, pName: String): Boolean {
            val file = File(pDir, pName)
            return mPrincipalFilter.accept(file) && mCustomFilter.accept(file)
        }

        fun setCustomFilter(pFilter: FileFilter) {
            mCustomFilter = pFilter
        }

        fun setPrincipalFilter(pPrincipalFilter: FileFilter) {
    ...
    protected val returnValue: Int
        protected get() = if (file == null) JFileChooser.CANCEL_OPTION else JFileChooser.APPROVE_OPTION

    /* (non-Javadoc)
	 * @see freemind.modes.FreeMindFileDialog#showOpenDialog(java.awt.Component)
	 */
    @Throws(HeadlessException::class)
    override fun showOpenDialog(pParent: Component): Int {
        mode = LOAD
        isVisible = true
	 ...
	 */
    @Throws(HeadlessException::class)
    override fun showSaveDialog(pParent: Component): Int {
        mode = SAVE
        isVisible = true
        callDirectoryListener()
        return returnValue
	 ...
	 */
    override fun setDialogTitle(pDialogTitle: String?) {
        title = pDialogTitle
    }
	 ...
	 */
    override fun addChoosableFileFilter(pFilter: FileFilter) {
	 ...
	 */
    override fun setFileSelectionMode(pMode: Int) {
        System.setProperty(APPLE_AWT_FILE_DIALOG_FOR_DIRECTORIES, "false")
        when (pMode) {
            JFileChooser.DIRECTORIES_ONLY -> {
                mFilter.setPrincipalFilter(DirFilter())
                System.setProperty(APPLE_AWT_FILE_DIALOG_FOR_DIRECTORIES, "true")
            }
            JFileChooser.FILES_ONLY -> mFilter.setPrincipalFilter(FileOnlyFilter())
            JFileChooser.FILES_AND_DIRECTORIES -> mFilter.setPrincipalFilter(FileAndDirFilter())
            else -> mFilter.setPrincipalFilter(NullFilter())
        }
    }
	 ...
	 */
    override var isMultiSelectionEnabled: Boolean
        get() = false
        set(pB) {
            require(!pB) { "Not implemented yet." }
        }

    /* (non-Javadoc)
	 * @see freemind.modes.FreeMindFileDialog#getSelectedFiles()
	 */
    override val selectedFiles: Array<File?>?
        get() {
	 ...
	 */
    override var selectedFile: File
        get() = File(directory, file)
        set(pFile) {
            super.setFile(pFile.name)
        }

    /* (non-Javadoc)
	 * @see freemind.modes.FreeMindFileDialog#setCurrentDirectory(java.io.File)
	 */
    override fun setCurrentDirectory(pDir: File?) {
        if (pDir == null) {
            return
        }
        logger!!.info("Setting dir to $pDir")
        super.setDirectory(pDir.absolutePath)
    }
	 ...
	 */
    override fun registerDirectoryResultListener(
            pDirectoryResultListener: DirectoryResultListener?) {
	 ...
	 */
    override fun addChoosableFileFilterAsDefault(pFilter: FileFilter) {
        addChoosableFileFilter(pFilter)
    }

    companion object {
        private const val APPLE_AWT_FILE_DIALOG_FOR_DIRECTORIES = "apple.awt.fileDialogForDirectories"
        protected var logger: Logger? = null
        @Throws(IOException::class)
        @JvmStatic
        fun main(args: Array<String>) {
            val dialog = FreeMindAwtFileDialog()
            dialog.setFileSelectionMode(JFileChooser.DIRECTORIES_ONLY)
            dialog.showOpenDialog(null)
            var selectedFile = dialog.selectedFile
            println("Dir '" + dialog.directory + "', File: '" + dialog.file + "', selected File: '" + selectedFile + "'")
            System.exit(0)
            dialog.showSaveDialog(null)
            selectedFile = dialog.selectedFile
            println("Dir '" + dialog.directory + "', File: '" + dialog.file + "', selected File: '" + selectedFile + "'")
            val streamResult = StreamResult(FileOutputStream(selectedFile))
            streamResult.outputStream.write("bla".toByteArray())
            streamResult.outputStream.close()
            println("File exists: " + selectedFile.exists())
        }
    }
}
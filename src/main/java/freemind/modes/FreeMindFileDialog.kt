 ...
 * @date 23.02.2012
 */
interface FreeMindFileDialog {
    interface DirectoryResultListener {
        fun setChosenDirectory(pDir: File?)
    }

    @Throws(HeadlessException::class)
    fun showOpenDialog(pParent: Component): Int

    @Throws(HeadlessException::class)
    fun showSaveDialog(pParent: Component): Int
    fun setDialogTitle(pDialogTitle: String?)

    /**
     * Sets the default file filter (that one that is activated at showup).
     * @see .addChoosableFileFilter
     */
    fun addChoosableFileFilterAsDefault(pFilter: FileFilter)

    /**
     * Adds a further file filter for optional use. It is not selected by default, but this is UI dependent.
     * @see .addChoosableFileFilterAsDefault
     */
    fun addChoosableFileFilter(pFilter: FileFilter)

    /**
     * @param pMode JFileChooser.DIRECTORIES_ONLY, JFileChooser.FILES_ONLY, JFileChooser.FILES_AND_DIRECTORIES
     */
    fun setFileSelectionMode(pMode: Int)
     ...
     */
    fun setCurrentDirectory(pLastCurrentDir: File?)
    fun registerDirectoryResultListener(pDirectoryResultListener: DirectoryResultListener?)
}
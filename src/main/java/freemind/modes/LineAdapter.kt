...
abstract class LineAdapter(protected var target: MindMapNode?, var mapFeedback: MapFeedback?) : MindMapLine {
    protected var NORMAL_WIDTH = 1

    // recursive attributes. may be accessed directly by the save() method.
    protected override var color: Color? = null
    protected override var style: String? = null
    protected override var width: Int
    ...
    /**
     */
    protected fun updateStandards() {
        if (standardColor == null) {
            val stdColor = mapFeedback!!.getProperty(
                    standardColorPropertyString)
            standardColor = if (stdColor != null && stdColor.length == 7) {
                xmlToColor(stdColor)
            } else {
                Color.RED
            }
    ...
    override fun getWidth(): Int {
        return if (width == DEFAULT_WIDTH) NORMAL_WIDTH else width
    }
     ...
     * but... fc.
     */
    fun getTarget(): MindMapNode? {
        return target
    }

    override fun clone(): Any {
        return try {
            val link = super.clone() as LineAdapter
            // color, ...
            link.color = if (color == null) null else Color(color!!.rgb)
            link
    ...
    /**
     * See [LineAdapter.setStandardColor]
     */
    /**
     ...
     */
    protected abstract var standardColor: Color?
    protected abstract var standardStyle: String?
    protected abstract val standardStylePropertyString: String
    protected abstract val standardColorPropertyString: String

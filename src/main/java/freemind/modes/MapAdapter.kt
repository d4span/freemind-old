...
abstract class MapAdapter(override var mapFeedback: MapFeedback?) : DefaultTreeModel(null), MindMap {
    /**
     * denotes the amount of changes since the last save. The initial value is
     * zero, such that new models are not to be saved.
     */
    protected var numberOfChangesSinceLastSave = 0
    ...
    private var mFileTime: Long = 0
    override var filter: Filter? = null
    private val mMapSourceChangedObserverSet = HashSet<MapSourceChangedObserver>()
    private val mTimerForFileChangeObservation: Timer

    protected inner class FileChangeInspectorTimerTask : TimerTask() {
        override fun run() {
            var shouldFire = false
            var lastModified: Long = 0
            // minimal synchronized block:
            synchronized(this@MapAdapter) {
            ...
            if (shouldFire) {
                for (observer in mMapSourceChangedObserverSet) {
                    logger!!.info("File " + file
                            + " changed on disk as it was last modified at "
                            + Date(lastModified))
                    try {
                        val changeAccepted = observer.mapSourceChanged(this@MapAdapter)
                        mFileTime = if (!changeAccepted) {
    ...
    private val fileTime: Long
        private get() {
            val lastModified: Long
            val fileName: File?
            fileName = file
            lastModified = fileName?.lastModified() ?: 0
            return lastModified
        }
    ...
    protected fun cancelFileChangeObservationTimer() {
    ...
    /**
     */
    private fun removeNodes(node: MindMapNode?) {
        node!!.removeAllHooks()
        mapFeedback!!.fireNodePreDeleteEvent(node)
        // and all children:
    ...
    override val isSaved: Boolean
        get() = numberOfChangesSinceLastSave == 0
     ...
     */
    override fun setSaved(saved: Boolean): Boolean {
        var setTitle = false
        if (saved) {
            numberOfChangesSinceLastSave = 0
            setTitle = true
        } else {
            if (numberOfChangesSinceLastSave == 0) {
                setTitle = true
            }
            ++numberOfChangesSinceLastSave
        }
        return setTitle
    }

    override val rootNode: MindMapNode
        get() = getRoot() as MindMapNode
     ...
     * around.
     */
    override fun changeRoot(newRoot: MindMapNode) {
        if (newRoot === rootNode) {
            return
        }
        val left = newRoot.isLeft
        var node: MindMapNode? = newRoot
        // collect parents (as we remove them from their parents...)
        val parents = Vector<MindMapNode?>()
        ...
        val it: Iterator<MindMapNode?> = parents.iterator()
        while (it.hasNext()) {
            node = it.next()
            val parent = node.getParentNode()
            // remove parent
    ...
    override val uRL: URL?
        get() = if (file != null) fileToUrl(file) else null

    protected fun getText(textId: String?): String? {
        return mapFeedback!!.getResourceString(textId)
    }
	 ...
	 */
    override fun insertNodeInto(pNewChild: MindMapNode?,
                                pParent: MindMapNode?, pIndex: Int) {
        super.insertNodeInto(pNewChild, pParent, pIndex)
        // call hooks
        mapFeedback!!.fireRecursiveNodeCreateEvent(pNewChild)
    }
    ...
    override fun nodeChanged(node: TreeNode) {
        mapFeedback!!.nodeChanged(node as MindMapNode)
    }

    override fun nodeRefresh(node: TreeNode?) {
        mapFeedback!!.nodeRefresh(node as MindMapNode?)
    }
     ...
     */
    fun nodeChangedInternal(node: TreeNode?) {
        if (node != null) {
     ...
     */
    override fun fireTreeNodesInserted(source: Any, path: Array<Any>,
                                       childIndices: IntArray, children: Array<Any>) {
        ...
        e = fireTreeNodesInserted(source, path, childIndices, children,
                listeners, e)
        val node = path[path.size - 1] as MindMapNode
        fireTreeNodesInserted(source, path, childIndices, children, node
                .listeners.listenerList, e)
    }

    private fun fireTreeNodesInserted(source: Any, path: Array<Any>,
                                      childIndices: IntArray, children: Array<Any>, listeners: Array<Any>,
                                      e: TreeModelEvent?): TreeModelEvent? {
        var e = e
    ...
    override fun fireTreeNodesRemoved(source: Any, path: Array<Any>,
                                      childIndices: IntArray, children: Array<Any>) {
        // Guaranteed to return a non-null array
        val listeners = listenerList.listenerList
        var e: TreeModelEvent? = null
    ...
    private fun fireTreeNodesRemoved(source: Any, path: Array<Any>,
                                     childIndices: IntArray, children: Array<Any>, listeners: Array<Any>,
                                     e: TreeModelEvent?): TreeModelEvent? {
        var e = e
        var i = listeners.size - 2
    ...
    override fun fireTreeStructureChanged(source: Any, path: Array<Any>,
                                          childIndices: IntArray, children: Array<Any>) {
        // Guaranteed to return a non-null array
        val listeners = listenerList.listenerList
        var e: TreeModelEvent? = null
    ...
    private fun fireTreeStructureChanged(source: Any,
                                         path: Array<Any>, childIndices: IntArray, children: Array<Any>,
                                         listeners: Array<Any>, e: TreeModelEvent?): TreeModelEvent? {
        var e = e
        var i = listeners.size - 2
        while (i >= 0) {
            if (listeners[i] === TreeModelListener::class.java) {
    ...
    override fun fireTreeNodesChanged(source: Any, path: Array<Any>,
                                      childIndices: IntArray, children: Array<Any>) {
        // Guaranteed to return a non-null array
        val listeners = listenerList.listenerList
        var e: TreeModelEvent? = null
    ...
    private fun fireTreeNodesChanged(source: Any, path: Array<Any>,
                                     childIndices: IntArray, children: Array<Any>, listeners: Array<Any>,
                                     e: TreeModelEvent?): TreeModelEvent? {
        var e = e
        var i = listeners.size - 2
    ...
    override fun registerMapSourceChangedObserver(
        ...
        if (pGetEventIfChangedAfterThisTimeInMillies != 0L && mFileTime > pGetEventIfChangedAfterThisTimeInMillies) {
            try {
                // Issue event here.
    ...
    override val icons: SortedListModel
        get() {
            val mapIcons: SortedMapListModel
            mapIcons = SortedMapListModel()
            addIcons(mapIcons, rootNode)
            return mapIcons
        }
     ...
     * @param pRootNode
     */
    private fun addIcons(pMapIcons: SortedMapListModel, pNode: MindMapNode?) {
        pMapIcons.addAll(pNode.getIcons())
        val iterator = pNode!!.childrenUnfolded()
    ...
    override fun createNodeTreeFromXml(pReader: Reader?, pIDToTarget: HashMap<String?, NodeAdapter?>): MindMapNode? {
        val xmlAdapter = XMLElementAdapter(mapFeedback, Vector(), pIDToTarget)
        xmlAdapter.parseFromReader(pReader)
        xmlAdapter.processUnfinishedLinks(linkRegistry)
        return xmlAdapter.mapChild
    }
    ...
    init {
        ...
        filter = DefaultFilter(NoFilteringCondition.createCondition(),
                true, false)
        mTimerForFileChangeObservation = Timer()
        mTimerForFileChangeObservation.schedule(
                FileChangeInspectorTimerTask(),
                INTERVAL_BETWEEN_FILE_MODIFICATION_TIME_CHECKS.toLong(),
                INTERVAL_BETWEEN_FILE_MODIFICATION_TIME_CHECKS.toLong())
    }

    class DontAskUserBeforeUpdateAdapter : AskUserBeforeUpdateCallback {
        /* (non-Javadoc)
		 * @see freemind.modes.XMLElementAdapter.AskUserBeforeUpdateCallback#askUserForUpdate()
		 */
        override fun askUserForUpdate(): Boolean {
            return true
        }
    }

    @Throws(XMLParseException::class, IOException::class)
    override fun loadTree(pReaderCreator: ReaderCreator,
                          pAskUserBeforeUpdateCallback: AskUserBeforeUpdateCallback): MindMapNode? {
        var versionInfoLength: Int
        versionInfoLength = EXPECTED_START_STRINGS[0].length
        // reading the start of the file:
        val buffer = readFileStart(pReaderCreator.createReader(),
                versionInfoLength)
        // the resulting file is accessed by the reader:
        var reader: Reader? = null
        for (i in EXPECTED_START_STRINGS.indices) {
        ...
        if (reader == null) {
            if (!isHeadless) {
                val showResult = pAskUserBeforeUpdateCallback.askUserForUpdate()
                require(showResult) { "We should not open the reader $pReaderCreator" }
            }
            reader = getUpdateReader(pReaderCreator.createReader(),
                    FREEMIND_VERSION_UPDATER_XSLT)
            if (reader == null) {
                // something went wrong on update:
                // FIXME: Translate me.
                mapFeedback!!.out("Error on conversion. Continue without conversion. Some elements may be lost!")
                reader = getActualReader(pReaderCreator.createReader())
            }
        }
        return try {
            ...
            val errorMessage = "Error while parsing file:$ex"
            System.err.println(errorMessage)
            Resources.getInstance().logException(ex)
            val result = createNodeAdapter(this, null)
    ...
    companion object {
        const val MAP_INITIAL_START = "<map version=\""
        const val FREEMIND_VERSION_UPDATER_XSLT = "freemind/modes/mindmapmode/freemind_version_updater.xslt"

        /**
         * The current version and all other version that don't need XML update for
         * sure.
         */
        val EXPECTED_START_STRINGS = arrayOf(
                MAP_INITIAL_START + FreeMind.XML_VERSION + "\"",
                MAP_INITIAL_START + "0.7.1\"")
        private const val INTERVAL_BETWEEN_FILE_MODIFICATION_TIME_CHECKS = 5000
        protected var logger: Logger? = null
        var sDontAskInstance = DontAskUserBeforeUpdateAdapter()
    }
}
...
freemind.controller.filter.Filterimport freemind.main.*
...
java.net.URLimport java.util.HashMapimport javax.swing.tree.TreeNode
interface MindMap : TreeModel {
    val rootNode: MindMapNode

    // nodeChanged has moved to the modeController. (fc, 2.5.2004)
    fun nodeChanged(node: TreeNode)
    fun nodeRefresh(node: TreeNode?)
    val mapFeedback: MapFeedback?
    fun getAsPlainText(mindMapNodes: List<MindMapNode?>?): String?
    fun getAsRTF(mindMapNodes: List<MindMapNode?>?): String?
    fun getAsHTML(mindMapNodes: List<MindMapNode?>?): String?
     ...
     */
    @Throws(IOException::class)
    fun getFilteredXml(fileout: Writer)
     ...
     */
    val restorable: String?
    fun getPathToRoot(node: TreeNode?): Array<TreeNode?>?

    /**
     * @return returns the link registry associated with this mode, or null, if
     * no registry is present.
     */
    val linkRegistry: MindMapLinkRegistry?
     ...
     * changed).
     */
    fun setSaved(isSaved: Boolean): Boolean
    ...
    interface MapSourceChangedObserver {
        ...
        fun mapSourceChanged(pMap: MindMap?): Boolean
    }
     ...
     */
    fun registerMapSourceChangedObserver(
     ...
     * [MindMap.registerMapSourceChangedObserver]
     * )
     */
    fun deregisterMapSourceChangedObserver(
     ...
     * around.
     */
    fun changeRoot(newRoot: MindMapNode)

    /**
     * @return a list of all icons present in the mindmap. Convenience method
     * for filters.
     */
    val icons: SortedListModel
    fun createNodeAdapter(pMap: MindMap?, nodeClass: String?): NodeAdapter
    fun createEdgeAdapter(node: NodeAdapter?): EdgeAdapter?
    fun createCloudAdapter(node: NodeAdapter?): CloudAdapter?
    fun createArrowLinkAdapter(source: NodeAdapter?,
                               target: NodeAdapter?): ArrowLinkAdapter?

    fun createArrowLinkTarget(source: NodeAdapter?, target: NodeAdapter?): ArrowLinkTarget?

    @Throws(XMLParseException::class, IOException::class)
    fun loadTree(pReaderCreator: ReaderCreator, pAskUserBeforeUpdateCallback: AskUserBeforeUpdateCallback): MindMapNode?

    @Throws(XMLParseException::class, IOException::class)
    fun createNodeTreeFromXml(pReader: Reader?, pIDToTarget: HashMap<String?, NodeAdapter?>): MindMapNode?
    fun createEncryptedNode(additionalInfo: String?): NodeAdapter?
    fun insertNodeInto(pNode: MindMapNode?,
                       pParentNode: MindMapNode?, pIndex: Int)

    fun removeNodeFromParent(node: MindMapNode?)
}
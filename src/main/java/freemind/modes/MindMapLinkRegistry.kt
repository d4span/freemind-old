...
class MindMapLinkRegistry {
     ...
     * @date 23.01.2012
     */
    private inner class SynchronousVector : Vector<MindMapLink?>() {
        /*
		 * (non-Javadoc)
		 * 
		 ...
		 */
        @Synchronized
        override fun add(pE: MindMapLink?): Boolean {
            val add = super.add(pE)
            if (pE is MindMapLink) {
                val source = pE.source
                if (!mSourceToLinks.containsKey(source)) {
                    mSourceToLinks[source] = Vector()
                }
                mSourceToLinks[source]!!.add(pE)
            }
            return add
        }
		 ...
		 */
        @Synchronized
        override fun removeElementAt(pIndex: Int) {
            val link = get(pIndex)
            val source = link.getSource()
            val vector = mSourceToLinks[source]
            if (vector != null) {
                vector.remove(link)
                if (vector.isEmpty()) {
    ...
    protected var mSourceToLinks = HashMap<MindMapNode?, Vector<MindMapLink?>>()
    // //////////////////////////////////////////////////////////////////////////////////////
    // // Attributes /////
    // //////////////////////////////////////////////////////////////////////////////////////
    /** MindMapNode = Target -> ID.  */
    protected var mTargetToId: HashMap<MindMapNode?, String>

    /** MindMapNode-> ID.  */
    protected var mIdToTarget: HashMap<String?, MindMapNode?>

    /** id -> vector of links whose TargetToID.get(target) == id.  */
    protected var mIdToLinks: HashMap<String?, Vector<MindMapLink?>?>

    /** id -> link  */
    protected var mIdToLink: HashMap<String?, MindMapLink?>

    /** id  */
    protected var mLocallyLinkedIds: HashSet<String?>
     ...
     * labeled.
     */
    fun generateUniqueID(proposedID: String?): String {
     ...
     */
    fun generateUniqueLinkId(proposedID: String?): String {
        return generateID(proposedID, mIdToLink, "Arrow_ID_")
    }

    fun registerLinkTarget(pTarget: MindMapNode?): String {
     ...
     */
    fun registerLinkTarget(pTarget: MindMapNode?, pProposedID: String?): String {
     ...
     */
    @JvmOverloads
    fun _registerLinkTarget(target: MindMapNode?, proposedID: String? = null): String {
        // id already exists?
        if (mTargetToId.containsKey(target)) {
            val id = mTargetToId[target]
            if (id != null) return id
            // blank state.
            // is equal to no state.
        }
        // generate new id:
        val newId = generateUniqueID(proposedID)
        mTargetToId[target] = newId
        mIdToTarget[newId] = target
     ...
     */
    fun getState(node: MindMapNode?): String? {
        return if (mTargetToId.containsKey(node)) mTargetToId[node] else null
    }
     ...
     * the argument.
     */
    fun getTargetForId(ID: String?): MindMapNode? {
        val target: Any? = mIdToTarget[ID]
        return target as MindMapNode?
    }

    /** @return a Vector of [MindMapLink]s
     */
    private fun getAssignedLinksVector(newId: String): Vector<MindMapLink?>? {
        // look, if target is already present:
        val vec: Vector<MindMapLink?>?
        if (mIdToLinks.containsKey(newId)) {
            vec = mIdToLinks[newId]
        } else {
            vec = SynchronousVector()
            mIdToLinks[newId] = vec
        }
    ...
    fun deregisterLinkTarget(target: MindMapNode?) {
        // deregister all links :
        val links = getAllLinks(target)
        for (i in links.indices.reversed()) {
            deregisterLink(links[i])
        }
        // and process my sons:
        val e = target!!.childrenUnfolded()
        while (e!!.hasNext()) {
            val child = e.next()
            deregisterLinkTarget(child)
        }
        val id = getState(target)
        if (id != null) {
     ...
     */
    @Throws(IllegalArgumentException::class)
    fun registerLink(link: MindMapLink?) {
        require(!(link.getSource() == null || link.getTarget() == null
                || link.getDestinationLabel() == null)) { "Illegal link specification.$link" }
        val source = link.getSource()
        val target = link.getTarget()
        logger!!.fine("Register link (" + link + ") from source node:" + source
                + " to target " + target)
        val id = _registerLinkTarget(target)
        val vec = getAssignedLinksVector(id)
        // already present?
        for (i in vec!!.indices) {
            if (vec[i] === link) return
        }
        vec.add(link)
        var uniqueId = link.getUniqueId()
        if (uniqueId == null) {
            (link as LinkAdapter?).setUniqueId(generateUniqueLinkId(uniqueId))
            uniqueId = link.getUniqueId()
        }
        if (mIdToLink.containsKey(uniqueId)) {
            if (mIdToLink[uniqueId] !== link) {
        ...
        mIdToLink[link.getUniqueId()] = link
    }

    fun deregisterLink(link: MindMapLink?) {
        val target = link.getTarget()
        val id = _registerLinkTarget(target)
        val vec = getAssignedLinksVector(id)
        for (i in vec!!.indices.reversed()) {
     ...
     * the argument.
     */
    fun getLinkForId(pId: String?): MindMapLink? {
        return if (mIdToLink.containsKey(pId)) {
            mIdToLink[pId]
        } else null
    }

    /**
     * @return Returns a Vector of [MindMapNode]s that point to the given
     * target node.
     */
    fun getAllSources(target: MindMapNode?): Vector<MindMapNode?> {
        ...
        if (id != null) {
            val vec = getAssignedLinksVector(id)
            for (i in vec!!.indices) {
                returnValue.add(vec[i].getSource())
            }
        }
        return returnValue
    }
     ...
     */
    fun getAllLinks(node: MindMapNode?): Vector<MindMapLink?> {
        val returnValue = Vector<MindMapLink?>()
    ...
    /** @return returns all links to this node as [MindMapLink] vector.
     */
    fun getAllLinksIntoMe(target: MindMapNode?): Vector<MindMapLink?> {
        val returnValue = Vector<MindMapLink?>()
        val id = getState(target)
        if (id != null) {
            val vec = getAssignedLinksVector(id)
            /* "clone" */returnValue.addAll(vec!!)
        }
        return returnValue
    }

    /** @return returns all links from this node as [MindMapLink] vector.
     */
    fun getAllLinksFromMe(source: MindMapNode?): Vector<MindMapLink?> {
        val returnValue = Vector<MindMapLink?>()
        val vec: Collection<MindMapLink?>? = mSourceToLinks[source]
        if (vec != null) {
    ...
    fun getLabel(target: MindMapNode?): String? {
        return getState(target)
    }

    fun registerLocalHyperlinkId(pTargetId: String?) {
        mLocallyLinkedIds.add(pTargetId)
    }

    fun isTargetOfLocalHyperlinks(pTargetId: String?): Boolean {

...
class FindAction(private val controller: ControllerAdapter) : FreemindAction("find", "images/filefind.png", controller) {
    ...
    val findFromText: String
        get() {
            val plainNodeText: String = HtmlTools.htmlToPlain(findFromNode.toString())
                    .replaceAll("\n", " ")
    ...
    fun displayDialog() {
        mDialog = null
        mDialog = JDialog(controller.frame.jFrame,
        ...
        addEscapeActionToDialog(mDialog!!, cancelAction)
        ...
        contentPane.add(JLabel(controller.getText("find_what")),
        ...
        contentPane.add(mSearchField, GridBagConstraints(2, 0, 10, 1, 1.0,
                1.0, GridBagConstraints.WEST, GridBagConstraints.BOTH,
                Insets(5, 5, 0, 0), 0, 0))
        val findImage: ImageIcon = ImageFactory.getInstance().createIcon(Resources.getInstance()
                .getResource("images/filefind_big.png"))
        contentPane.add(JLabel(findImage), GridBagConstraints(0, 0, 1,
        ...
        setLabelAndMnemonic(mFindInNotesTooBox!!, null)
        contentPane.add(mFindInNotesTooBox, GridBagConstraints(0, 2, 3, 1,
        ...
        contentPane.add(okButton, GridBagConstraints(2, 3, 1, 1, 1.0, 1.0,
                GridBagConstraints.WEST, GridBagConstraints.BOTH, Insets(5,
                5, 0, 0), 0, 0))
        val cancelButton = JButton(
                controller.getText("ExtendedFindDialog.cancel"))
        setLabelAndMnemonic(cancelButton, null)
        cancelButton.addActionListener(cancelAction)
        contentPane.add(cancelButton, GridBagConstraints(3, 3, 1, 1, 1.0,
                1.0, GridBagConstraints.WEST, GridBagConstraints.BOTH,
                Insets(5, 5, 0, 0), 0, 0))
        mDialog!!.rootPane.defaultButton = okButton
        mDialog!!.pack()
        setDialogLocationRelativeTo(mDialog!!, controller.selectedView)
    ...
    class FindNextAction(private val controller: ControllerAdapter, private val find: FindAction) : FreemindAction("find_next", controller) {
        override fun actionPerformed(e: ActionEvent) {
    ...
    fun find(node: MindMapNode?, subterms: MutableCollection<String>,
    ...
    private fun find(nodes: LinkedList<MindMapNode?>?,
                     subterms: Collection<String>, caseSensitive: Boolean): Boolean {
        // Precondition: if !caseSensitive then >>what<< is in lowercase.
        val searchInNotesToo: Boolean = Resources.getInstance().getBoolProperty(
                FreeMind.RESOURCES_SEARCH_IN_NOTES_TOO)
        if (!findNodesUnfoldedByLastFind!!.isEmpty()) {

            // if (false) {
            val i: ListIterator<MindMapNode?> = findNodesUnfoldedByLastFind!!.listIterator(findNodesUnfoldedByLastFind!!.size)
            while (i.hasPrevious()) {
                val node = i.previous()
                try {
    ...
    fun prepareTextContent(caseSensitive: Boolean, nodeText: String?): String {
        ...
        if (!caseSensitive) {
            nodeText = nodeText.lowercase(Locale.getDefault())
        }
        return nodeText
    }
    ...
    fun displayNode(node: MindMapNode?, nodesUnfoldedByDisplay: ArrayList<MindMapNode?>?) {
        ...
        val path: Array<Any?>? = controller.map.getPathToRoot(node)

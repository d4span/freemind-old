...
class EncryptedMindMapNode(userObject: Any?, map: MindMap?) : MindMapNodeModel(userObject, map) {
    /**
    ...
    init {
        if (encryptedIcon == null) {
            encryptedIcon = MindIcon.Companion.factory("encrypted").getIcon()
        }
        if (decryptedIcon == null) {
            decryptedIcon = MindIcon.Companion.factory("decrypted").getIcon()
        }
        if (map != null) {
            updateIcon()
        }
     ...
     */
    fun decrypt(givenPassword: StringBuffer): Boolean {
        ...
        if (!isDecrypted) {
            try {
                var node: MindMapNode? = null
                val childXml = decryptXml(encryptedContent, password)
                // is it a map at all?
                if (childXml!!.startsWith(MapAdapter.Companion.MAP_INITIAL_START)) {
                    node = getNodeFromXml(childXml)
                } else {
                    // old handling up to version 0.9.0_rc8:
                    val childs: Array<String> = childXml
                    ...
                    for (j in childs.indices) {
                        val nodeContent = childs[j]
    ...
    private fun getNodeFromXml(childXml: String?): MindMapNode? {
        // the loadTree method performs an automatical version update.
        return getMap().loadTree(
                StringReaderCreator(childXml!!), MapAdapter.Companion.sDontAskInstance)
    }

    private val mindMapMapModel: MindMapMapModel?
        private get() = mapFeedback.map as MindMapMapModel

    /**
     */
    fun checkPassword(givenPassword: StringBuffer): Boolean {
        if (password != null) {
            if (!equals(givenPassword, password!!)) {
                NodeAdapter.Companion.logger!!.warning("Wrong password supplied (cached!=given).")
                return false
            }
            return true
        }
        // new password:
        val decryptedNode = decryptXml(encryptedContent, givenPassword)
        // FIXME: Better test needed.
        if (decryptedNode == null) {
            NodeAdapter.Companion.logger!!.warning("Wrong password supplied (deciphered text is null).")
            return false
        }
        if (!decryptedNode.startsWith("<node ")) {
            ...
            if (!HtmlTools.getInstance().isWellformedXml(decryptedNode)) {
                NodeAdapter.Companion.logger!!.warning("Wrong password supplied (malformed deciphered text).")
                return false
            }
        }
    ...
    /**
     */
    private fun equals(givenPassword: StringBuffer, password2: StringBuffer): Boolean {
        if (givenPassword.length != password!!.length) return false
        for (i in 0 until password2.length) {
            val c1 = password2[i]
            val c2 = givenPassword[i]
    ...
    override fun childrenFolded(): ListIterator<MindMapNode?>? {
        return if (isAccessible) {
            super.childrenFolded()
        } else Vector<MindMapNode?>().listIterator()
    }

    override fun childrenUnfolded(): ListIterator<MindMapNode?>? {
        return if (isAccessible || isShuttingDown) {
            super.childrenUnfolded()
        } else Vector<MindMapNode?>().listIterator()
    }
    ...
    fun setPassword(password: StringBuffer?) {
    ...
    override var additionalInfo: String?
        get() = if (isStoringEncryptedContent) {
            null
    ...
    override fun save(writer: Writer?, registry: MindMapLinkRegistry?,
        ...
        val oldIsVisible = isAccessible
        isAccessible = false
        var ret: XMLElement? = null
        ret = try {
            super.save(writer, registry, saveHidden, saveChildren)
        } finally {
            isAccessible = oldIsVisible
        }
        return ret
    }
     ...
     */
    @Throws(IOException::class)
    private fun generateEncryptedContent() {
        isStoringEncryptedContent = true
        encryptedContent = try {
            val sWriter = StringWriter()
            mindMapMapModel!!.getXml(sWriter, true, this)
            val childXml = sWriter.buffer
            encryptXml(childXml)
        } finally {
            isStoringEncryptedContent = false
        }
    }

    /**
     */
    private fun encryptXml(childXml: StringBuffer): String? {
        try {
            // Create encrypter/decrypter class
            // FIXME: Use char[] instead of toString.
            val encrypter = SingleDesEncrypter(password!!)
        ...
        throw IllegalArgumentException("Can't encrypt the node.")
    }

    /**
     * @return null if the password is wrong.
     */
    private fun decryptXml(encryptedString: String?, pwd: StringBuffer?): String? {
        val encrypter = SingleDesEncrypter(pwd!!)
     ...
     */
    fun setShuttingDown(isShuttingDown: Boolean) {
        this.isShuttingDown = isShuttingDown
    }

    override fun insert(pChild: MutableTreeNode, pIndex: Int) {
        if (isAccessible) {
    ...
    override val isWriteable: Boolean
        get() = isAccessible && super.isWriteable()

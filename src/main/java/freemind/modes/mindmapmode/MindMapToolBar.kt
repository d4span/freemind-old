...
class MindMapToolBar(private val c: MindMapController) : FreeMindToolBar(), ZoomListener {
    /**
     ...
     */
    private inner class FreeMindComboBox(pItems: Array<String?>?) : JComboBox<String?>(pItems) {
        override fun getMaximumSize(): Dimension {
            return preferredSize
        }
    }

    private val fonts: JComboBox<String>
    private val size: JComboBox<String>
    private val iconToolBarScrollPane: JAutoScrollBarPane
    private val iconToolBar: JToolBar
    private var fontSize_IgnoreChangeEvent = false
    private var fontFamily_IgnoreChangeEvent = false
    private var color_IgnoreChangeEvent = false
    private val fontsListener: ItemListener
    private val sizeListener: ItemListener
    private val zoom: JComboBox<String?>
    private val userDefinedZoom: String?
    private val colorCombo: JColorCombo
    private var userDefinedCounter = 1
    ...
    init {
        ...
        fonts = FreeMindComboBox(availableFonts)
        fonts.setFocusable(false)
        size = FreeMindComboBox(sizes)
        size.setFocusable(false)
        iconToolBar = FreeMindToolBar()
        iconToolBarScrollPane = JAutoScrollBarPane(iconToolBar)
        iconToolBar.setOrientation(VERTICAL)
        iconToolBar.setRollover(true)
        iconToolBar.setLayout(GridLayout(0, c.getIntProperty(FreeMind.ICON_BAR_COLUMN_AMOUNT, 1)))
        iconToolBarScrollPane.verticalScrollBar.unitIncrement = 100
        fontsListener = ItemListener { e ->
            ...
            c.fontFamily!!.actionPerformed(e.item as String)
            fontFamily_IgnoreChangeEvent = false
        }
        fonts.addItemListener(fontsListener)
        sizeListener = ItemListener { e -> // System.err.println("ce:"+e);
            if (e.stateChange != ItemEvent.SELECTED) {
                return@ItemListener
            }
            ...
            // call action:
            c.fontSize!!.actionPerformed(e.item as String)
        }
        size.addItemListener(sizeListener)
        userDefinedZoom = c.getText("user_defined_zoom")
        zoom = FreeMindComboBox(c.controller.zooms)
        zoom.setSelectedItem("100%")
        zoom.addItem(userDefinedZoom)
        // Focus fix.
        zoom.setFocusable(false)
        zoom.addItemListener { e -> // todo: dialog with user zoom value, if user zoom is chosen.
            // change proposed by dimitri:
            if (e.stateChange == ItemEvent.SELECTED) {
                setZoomByItem(e.item)
            }
        }
        colorCombo = JColorCombo()
        colorCombo.isFocusable = false
        colorCombo.addItemListener(ItemListener { e ->
            if (color_IgnoreChangeEvent) {
                return@ItemListener
            }
            if (e.stateChange == ItemEvent.SELECTED) {
                color_IgnoreChangeEvent = true
                setFontColorByItem(e.item as ColorPair)
                color_IgnoreChangeEvent = false
            }
        })
    }

    private fun setZoomByItem(item: Any) {
        if (item as String == userDefinedZoom) return
        val dirty = item
        val cleaned = dirty.substring(0, dirty.length - 1)
        // change representation ("125" to 1.25)
        val zoomValue = cleaned.toFloat() / 100f // nothing to do...
        // remove '%' sign
        c.setZoom(zoomValue)
    }

    private fun setFontColorByItem(pItem: ColorPair) {
    ...
    protected val controller: Controller?
        protected get() = c.controller

    fun update(holder: StructuredMenuHolder) {
        this.removeAll()
        holder.updateMenus(this, "mindmapmode_toolbar/")
        addIcon("images/list-add-font.png")
        fonts.maximumRowCount = 30
        add(fonts)
...
//		size.setEditable(true);
        ...
        add(size)
        val label = addIcon("images/format-text-color.png")
        label.toolTipText = Resources.getInstance().getText("mindmapmode_toolbar_font_color")
        add(colorCombo)
        ...
        for (i in c.iconActions.indices) {
            iconToolBar.add(c.iconActions[i] as Action)
        }
    }

    fun addIcon(iconPath: String?): JLabel {
        add(Separator())
        val label = JLabel(ImageFactory.getInstance().createIcon(iconPath))
        label.text = " "
        add(label)
    ...
    fun selectFontSize(fontSize: String?) // (DiPo)
    {
    ...
    val leftToolBar: Component
        get() = iconToolBarScrollPane

    fun selectFontName(fontName: String?) // (DiPo)
    {
        if (fontFamily_IgnoreChangeEvent) {
            return
        }
        fontFamily_IgnoreChangeEvent = true
        fonts.isEditable = true
        fonts.selectedItem = fontName
        fonts.isEditable = false
        fontFamily_IgnoreChangeEvent = false
    }

    fun setAllActions(enabled: Boolean) {
        fonts.isEnabled = enabled
	 ...
	 */
    override fun setZoom(f: Float) {
        logger!!.fine("setZoomComboBox is called with $f.")
        val toBeFound = getItemForZoom(f)
        for (i in 0 until zoom.itemCount) {
            if (toBeFound == zoom.getItemAt(i)) {
                // found
                zoom.selectedItem = toBeFound
                return
            }
        }
        zoom.selectedItem = userDefinedZoom
    }

    private fun getItemForZoom(f: Float): String {
        return (f * 100f).toInt().toString() + "%"
    }

    fun startup() {
        c.registerZoomListener(this)
    }

    fun shutdown() {
        c.deregisterZoomListener(this)
    }

    fun selectColor(pColor: Color?) {
        var pColor = pColor
        if (pColor == null) {
            pColor = MapView.standardNodeTextColor
        }
        color_IgnoreChangeEvent = true
        for (i in 0 until colorCombo.model.size) {
            val pair = colorCombo.model.getElementAt(i)!!
            if (pair.color == pColor) {
        ...
        val pair = ColorPair(pColor!!, "user$userDefinedCounter",
                Resources.getInstance().format(
                        "mindmapmode_toolbar_font_color_user_defined", arrayOf<Any>(userDefinedCounter)))
        userDefinedCounter++
        colorCombo.addItem(pair)
        colorCombo.selectedItem = pair
        color_IgnoreChangeEvent = false
    }

    companion object {
        private val sizes = arrayOf<String?>("8", "10", "12", "14", "16", "18",
                "20", "24", "28")

 ...
 * @author foltin
 */
class AddArrowLinkAction(modeController: MindMapController) : MindmapAction("paste_as_link", "images/designer.png", modeController) {
    ...
    override fun actionPerformed(e: ActionEvent) {
        val selected = modeController.getSelected()
        ...
        for (destination in nodesFromClipboard) {
            if (selected !== destination) {
                mindMapController.addLink(selected, destination)
            } else {
                // give an error afterwards?
                identicalError = true
                FreemindAction.Companion.logger!!.warning("Can't create a link from the node '$selected' to itself. Skipped.")
            }
        }
        if (identicalError) {
            modeController.controller.errorMessage(
    ...
    override fun isEnabled(pItem: JMenuItem?, pAction: Action?): Boolean {
        return (super.isEnabled(pItem, pAction)
                && modeController != null
                && !getMindMapNodesFromClipboard(modeController).isEmpty())
    }
}
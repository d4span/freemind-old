...
class ApplyPatternAction(controller: MindMapController, pattern: Pattern?) : NodeGeneralAction(controller, null /* no text */, null /* = no icon */), SingleNodeOperation {
    interface ExternalPatternAction : MindMapControllerPlugin {
        fun act(node: MindMapNode?, pattern: Pattern?)
    }
    ...
    override fun apply(map: MindMapMapModel?, node: MindMapNodeModel) {
        StylePatternFactory.applyPattern(node, pattern, mindMapController.patternsList, mindMapController.plugins, mindMapController)
    }

    companion object {
        fun edgeWidthIntToString(value: Int): String? {
            if (value == LineAdapter.Companion.DEFAULT_WIDTH) {
                return null
            }
            return if (value == EdgeAdapter.Companion.WIDTH_THIN) {
                EdgeAdapter.Companion.EDGE_WIDTH_THIN_STRING

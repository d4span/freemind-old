...
freemind.view.mindmapview.*import java.util.*
import java.util.regex.Pattern

//
//Node editing
//
class EditAction(private val mMindMapController: MindMapController) : MindmapAction("edit_node", mMindMapController) {
    private var mCurrentEditDialog: EditNodeBase? = null
    override fun actionPerformed(arg0: ActionEvent) {
        mMindMapController.edit(null, false, false)
    }

    // edit begins with home/end or typing (PN 6.2)
    fun edit(e: KeyEvent?, addNew: Boolean, editLong: Boolean) {
        val selectedNodeView = mMindMapController.view.selected
        if (selectedNodeView != null) {
            if (e == null || !addNew) {
                edit(selectedNodeView, selectedNodeView, e, false, false,
                        editLong)
            } else if (!mMindMapController.isBlocked) {
                mMindMapController.addNew(mMindMapController.selected,
                        MindMapActions.Companion.NEW_SIBLING_BEHIND, e)
            }
            e?.consume()
        }
    }

    fun edit(node: NodeView?, prevSelected: NodeView?,
             firstEvent: KeyEvent?, isNewNode: Boolean,
             parentFolded: Boolean, editLong: Boolean) {
        if (node == null) {
            return
        }
        val map = node.map
        ...
        mMindMapController.isBlocked = true // locally "modal" stated
        var text = node.model.toString()
        val htmlEditingOption = mMindMapController.controller
                .getProperty("html_editing_option")
        val isHtmlNode = HtmlTools.isHtmlNode(text)
        val isLongNode = node.isLong

        // do we need a decision if plain or HTML editing?
        var useRichTextInNewLongNodes = if (isHtmlNode) "true" else "false"
        ...
        if (!isHtmlNode && !isLongNode && editLong) {
            ...
            val showResult = OptionalDontShowMeAgainDialog(
                    mMindMapController.frame.jFrame,
                    mMindMapController.selectedView,
                    "edit.edit_rich_text",
                    "edit.decision",
                    mMindMapController,
                    ...
                    .show().result
            useRichTextInNewLongNodes = if (showResult == JOptionPane.OK_OPTION) "true" else "false"
        }
        // useRichTextInNewLongNodes =
        // c.getController().getProperty("use_rich_text_in_new_long_nodes");
        val editHtml = (isHtmlNode
                || editLong && safeEquals(useRichTextInNewLongNodes,
                "true"))
        val editInternalWysiwyg = (editHtml
                && safeEquals(htmlEditingOption, "internal-wysiwyg"))
        val editExternal = (editHtml
                && safeEquals(htmlEditingOption, "external"))
        if (editHtml && !isHtmlNode) {
            text = HtmlTools.plainToHTML(text)
        }
        if (editInternalWysiwyg) {
            val editNodeWYSIWYG = EditNodeWYSIWYG(node, text,
                    ...
                    object : EditControl {
                        ...
                        override fun split(newText: String, position: Int) {
                            mMindMapController.splitNode(node.model,
                                    position, newText)
                            cancel()
                        }
                    }) // focus fix
        ...
        if (editExternal) {
            val editNodeExternalApplication = EditNodeExternalApplication(
        ...
        if (isLongNode || editLong) {
            val nodeEditDialog = EditNodeDialog(node, text,
                    firstEvent, mMindMapController,
                    object : EditControl {
                        override fun cancel() {
                            mMindMapController.isBlocked = false
                        ...
                        override fun ok(newText: String) {
                            mMindMapController.setNodeText(node.model, newText)
                            cancel()
                        }
        ...
        // inline editing:
        val textfield = EditNodeTextField(node, text,
                firstEvent, mMindMapController, object : EditControl {
            override fun cancel() {
                if (isNewNode) { // delete also the node and set focus
                    // to the parent
                    mMindMapController.view
                            .selectAsTheOnlyOneSelected(node)
                    val nodeList = Vector<MindMapNode?>()
                    nodeList.add(node.model)
    ...
    private fun setHtmlText(node: NodeView, newText: String) {
        val body = HTML_HEAD.matcher(newText).replaceFirst("")
        mMindMapController.setNodeText(node.model, body)
    }

    companion object {
        private val HTML_HEAD = Pattern.compile(

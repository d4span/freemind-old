...
class IconAction(private val modeController: MindMapController, _icon: MindIcon?,
                 removeLastIconAction: RemoveIconAction?) : MindmapAction(_icon.getDescription(), _icon.getIcon(), modeController), IconInformation {
    var mindIcon: MindIcon?

    init {
        putValue(SHORT_DESCRIPTION, _icon.getDescription())
        mindIcon = _icon
    }

    override fun actionPerformed(e: ActionEvent) {
        if (e.id == ActionEvent.ACTION_FIRST
                && (e.modifiers and ActionEvent.SHIFT_MASK
                        and ActionEvent.CTRL_MASK.inv() and ActionEvent.ALT_MASK.inv()) != 0) {
            removeAllIcons()
            addLastIcon()
            return
        }
        if (e == null
                || e.modifiers and (ActionEvent.CTRL_MASK or ActionEvent.ALT_MASK) == 0) {
            addLastIcon()
            return
        }
        // e != null
        if ((e.modifiers and ActionEvent.SHIFT_MASK.inv()
                        and ActionEvent.CTRL_MASK.inv() and ActionEvent.ALT_MASK) != 0) {
            removeIcon(false)
            return
        }
        if (e.modifiers and ActionEvent.SHIFT_MASK.inv() and ActionEvent.CTRL_MASK and ActionEvent.ALT_MASK.inv() != 0) {
            removeIcon(true)
            return
        }
    }

    private fun addLastIcon() {
        for (selected in modeController.selecteds) {
            addIconActor!!.addIcon(selected, mindIcon)
        }
    }

    private fun removeIcon(removeFirst: Boolean) {
        for (selected in modeController.selecteds) {
            addIconActor!!.removeIcon(selected, mindIcon, removeFirst)
        }
    }

    private fun removeAllIcons() {
        for (selected in modeController.selecteds) {
            if (selected.icons.size > 0) {
                modeController.removeAllIcons(selected)
            }
        }
    }

    protected fun createAddIconAction(node: MindMapNode?,
                                      icon: MindIcon?, iconIndex: Int): AddIconAction? {
        return addIconActor!!.createAddIconAction(node, icon, iconIndex)
    }

    protected val addIconActor: AddIconActor?
        protected get() = mindMapController.actorFactory.addIconActor
    val doActionClass: Class<AddIconAction>
        get() = AddIconAction::class.java
    override val keyStroke: KeyStroke?
        get() {
            val keystrokeResourceName = mindIcon.getKeystrokeResourceName()
            val keyStrokeDescription = mindMapController.frame
                    .getAdjustableProperty(keystrokeResourceName)
            return getKeyStroke(keyStrokeDescription)
        }
    override val description: String?
        get() = mindIcon.getDescription()

    override fun getIcon(): ImageIcon? {
        return mindIcon.getIcon()
    }

    override val keystrokeResourceName: String?
        get() = mindIcon.getKeystrokeResourceName()
}
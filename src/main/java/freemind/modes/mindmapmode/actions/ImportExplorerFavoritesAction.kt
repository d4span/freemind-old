...
class ImportExplorerFavoritesAction(private val controller: MindMapController) : MindmapAction("import_explorer_favorites", controller) {
    override fun actionPerformed(e: ActionEvent) {
        val chooser = controller.getFileChooser(null)
        chooser!!.setFileSelectionMode(JFileChooser.DIRECTORIES_ONLY)
        chooser.setDialogTitle(controller.getText("select_favorites_folder"))
        val returnVal = chooser.showOpenDialog(controller.frame
                .contentPane)
        if (returnVal == JFileChooser.APPROVE_OPTION) {
            val folder = chooser.selectedFile
            controller.frame.out("Importing Favorites ...")
            // getFrame().repaint(); // Refresh the frame, namely hide dialog
            // and show status
            // getView().updateUI();
            // Problem: the frame should be refreshed here, but I don't know how
            // to do it
            importExplorerFavorites(folder, controller.selected,  /*
																	 * redisplay=
																	 */
                    true)
            controller.frame.out("Favorites imported.")
        }
    }

    fun importExplorerFavorites(folder: File?, target: MindMapNode?,
                                redisplay: Boolean): Boolean {
        ...
        if (folder!!.isDirectory) {
            ...
            for (i in list.indices) {
                if (list[i].isDirectory) {
                    // Insert a new node
                    val nodeContent = list[i].name
                    val node = addNode(target, nodeContent)
                    //
                    val favoritesFoundInSubfolder = importExplorerFavorites(
                            list[i], node, false)
            ...
            for (i in list.indices) {
                if (!list[i].isDirectory
                        && getExtension(list[i]) == "url") {
                    favoritesFound = true
                    try {
                        val node = addNode(target,
                                removeExtension(list[i].name))
                        // For each line: Is it URL? => Set it as link
                        val `in` = BufferedReader(FileReader(
                                list[i]))
                        while (`in`.ready()) {
    ...
    /**
     */
    private fun addNode(target: MindMapNode?, nodeContent: String): MindMapNode? {
        val node = controller.addNewNode(target,

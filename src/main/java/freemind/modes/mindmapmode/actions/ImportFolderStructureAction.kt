...
class ImportFolderStructureAction(private val controller: MindMapController) : MindmapAction("import_folder_structure", controller) {
    init {
    ...
    override fun actionPerformed(e: ActionEvent) {
        ...
        if (returnVal == JFileChooser.APPROVE_OPTION) {
            ...
            try {
                ...
                importFolderStructure(folder, controller.selected,  /*
																		 * redisplay
																		 * =
																		 */
                        true)
    ...
    private val frame: FreeMindMain?
        private get() = controller.frame

    @Throws(MalformedURLException::class)
    fun importFolderStructure(folder: File?, target: MindMapNode?,
                              redisplay: Boolean) {
        logger!!.fine("Entering folder: $folder")
        if (folder!!.isDirectory) {
            frame!!.out(folder.name)
            ...
            if (list != null) {
                ...
                for (i in list.indices) {
                    if (list[i].isDirectory) {
                        // Insert a new node
                        val node = addNode(target, list[i].name,
                                fileToUrl(list[i]).toString())
    ...
    /**
     */
    private fun addNode(target: MindMapNode?, nodeContent: String,
                        link: String): MindMapNode? {

...
class NewChildAction(val modeController: MindMapController) : MindmapAction("new_child", "images/idea.png", modeController) {

    init {
        if (logger == null) {
            logger = modeController.frame.getLogger(NewChildAction::class.java.name)
        }
    }

    override fun actionPerformed(e: ActionEvent) {
        modeController.addNew(modeController.selected, MindMapActions.Companion.NEW_CHILD, null)
    }

    fun addNew(target: MindMapNode, newNodeMode: Int,
               e: KeyEvent?): MindMapNode? {
        var newNodeMode = newNodeMode
        var newNode: MindMapNode? = null
        when (newNodeMode) {
            MindMapActions.Companion.NEW_SIBLING_BEFORE, MindMapActions.Companion.NEW_SIBLING_BEHIND -> {
                run {
                    if (!target.isRoot) {
                        val parent = target.parentNode
                        var childPosition = parent!!.getChildPosition(target)
                        if (newNodeMode == MindMapActions.Companion.NEW_SIBLING_BEHIND) {
                            childPosition++
                        }
                        newNode = modeController.addNewNode(parent, childPosition, target.isLeft)
                        val nodeView = modeController.getNodeView(newNode)
                        modeController.select(nodeView)
                        modeController.edit!!.edit(nodeView, modeController.getNodeView(target), e, true, false,
                                false)
                        break
                    } else {
                        // fc, 21.8.07: we don't do anything here and get a new child
                        // instead.
                        newNodeMode = MindMapActions.Companion.NEW_CHILD
                        // @fallthrough
                    }
                }
                run {
                    if (target is EncryptedMindMapNode && !target.isAccessible) break
                    val parentFolded = target.isFolded
                    if (parentFolded) {
                        modeController.setFolded(target, false)
                    }
                    val position = if (modeController.getProperty("placenewbranches")
                            == "last") target.childCount else 0
                    newNode = addNewNode(target, position)
                    val nodeView = modeController.getNodeView(newNode)
                    if (newNodeMode == MindMapActions.Companion.NEW_CHILD) {
                        modeController.select(nodeView)
                    }
                    modeController.edit!!.edit(nodeView, modeController.getNodeView(target), e, true, parentFolded,
                            false)
                }
            }
            MindMapActions.Companion.NEW_CHILD, MindMapActions.Companion.NEW_CHILD_WITHOUT_FOCUS -> {
                if (target is EncryptedMindMapNode && !target.isAccessible) break
                val parentFolded = target.isFolded
                if (parentFolded) {
                    modeController.setFolded(target, false)
    ...
    protected fun addNewNode(parent: MindMapNode, index: Int): MindMapNode? {
        return modeController.addNewNode(parent, index, parent.isNewChildLeft)
    }

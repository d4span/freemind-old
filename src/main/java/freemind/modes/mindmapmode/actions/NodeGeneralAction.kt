...
open class NodeGeneralAction protected constructor(protected val modeController: MindMapController,
                                                   textID: String?, iconPath: String?) : AbstractXmlAction(null, if (iconPath != null) ImageFactory.getInstance().createIcon(
        modeController.getResource(iconPath)) else null, modeController) {
    var singleNodeOperation: SingleNodeOperation?
    private var mDoActionClass: Class<*>? = null
    ...
    protected fun setName(name: String?) {
        if (name != null) {
            putValue(NAME, name)
            putValue(SHORT_DESCRIPTION, removeMnemonic(name))
        }
    }

    constructor(modeController: MindMapController, textID: String?,
                iconPath: String?, singleNodeOperation: SingleNodeOperation?) : this(modeController, textID, iconPath) {
        this.singleNodeOperation = singleNodeOperation
    }

    constructor(modeController: MindMapController, textID: String?,
                iconPath: String?,
                actor: NodeActorXml?) : this(modeController, textID, iconPath) {
        addActor(actor)
    }
     ...
     */
    fun setSingleNodeOperation(singleNodeOperation: SingleNodeOperation?) {
        this.singleNodeOperation = singleNodeOperation
    }
	 ...
	 */
    public override fun xmlActionPerformed(e: ActionEvent?) {
        if (singleNodeOperation != null) {
            val it: ListIterator<*> = modeController.selecteds.listIterator()
            while (it
                            .hasNext()) {
                val selected = it.next() as MindMapNodeModel
                singleNodeOperation!!.apply(
                        modeController.map as MindMapMapModel,
                        selected)
            }
        } else {
            // xml action:
            ...
            while (it
                            .hasNext()) {
                val selected = it.next() as MindMapNodeModel
                val pair = getActionPair(selected)
                if (pair != null) {
                    doAction.addChoice(pair.doAction)
                    undo.addAtChoice(0, pair.undoAction)
                }
            }
            if (doAction.sizeChoiceList() == 0) return
            modeController.doTransaction(getValue(NAME) as String,
                    ActionPair(doAction, undo))
        }
    }
     ...
     */
    protected open fun getActionPair(selected: MindMapNodeModel): ActionPair? {
        var pair: ActionPair? = null
        pair = if (mDoActionClass != null) {
            val actorXml = mindMapController.actionRegistry.getActor(mDoActionClass)
            if (actorXml is NodeActorXml) {
                actorXml.apply(modeController.map,
                        selected)
            } else {
                throw IllegalArgumentException("ActorXml $actorXml is not a NodeActorXml.")
            }
        } else {
            throw IllegalArgumentException("doActionClass not defined.")
        }
        return pair
    }

    protected fun setDoActionClass(pDoActionClass: Class<*>?) {
        mDoActionClass = pDoActionClass
    }

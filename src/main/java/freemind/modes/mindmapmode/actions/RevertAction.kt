...
class RevertAction
/**
 */(private override val mindMapController: MindMapController) : MindmapAction("RevertAction", null as String?, mindMapController) {
    /*
	 * (non-Javadoc)
	 * 
	 * @see
    ...
    override fun actionPerformed(arg0: ActionEvent) {
        try {
            val map = mindMapController.map
            val file = map.file
            if (file == null) {
                JOptionPane.showMessageDialog(mindMapController.view,
                        mindMapController.getText("map_not_saved"), "FreeMind",
                        JOptionPane.ERROR_MESSAGE)
                return
            }
            // FIXME: Make action from MindMapActions out of it.
            mindMapController.actorFactory.revertActor.revertMap(map, file)
        } catch (e: IOException) {

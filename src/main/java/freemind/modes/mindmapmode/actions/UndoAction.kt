...
open class UndoAction protected constructor(private var controller: MindMapController, text: String?, icon: Icon?,
                                            mode: MindMapController) : AbstractXmlAction(text, icon, mode) {
    /**
     */
    var isUndoAction: Boolean
        private set
    protected var actionPairList = Vector<ActionPair>()
    private var timeOfLastAdd: Long = 0
    private var actionFrameStarted = false

    constructor(controller: MindMapController) : this(controller, controller.getText("undo"), ImageFactory.getInstance().createIcon(
            controller.getResource("images/undo.png")), controller) {
        this.controller = controller
        if (logger == null) {
            logger = controller.frame
                    .getLogger(this.javaClass.toString())
        }
    }
	 ...
	 */
    override fun xmlActionPerformed(arg0: ActionEvent?) {
        if (actionPairList.size > 0) {
            val pair = actionPairList[0] as ActionPair
            informUndoPartner(pair)
            actionPairList.removeAt(0)
            undoDoAction(pair)
        }
        if (actionPairList.size == 0) {
            // disable undo
    ...
    /**
     */
    protected open fun informUndoPartner(pair: ActionPair) {
        this.controller.redo!!.add(pair.reverse())
        this.controller.redo!!.isEnabled = true
    }

    protected fun undoDoAction(pair: ActionPair) {
        logger!!.info("Undo, doing: " + printXmlAction(pair.undoAction))
        logger!!.info("Redo, would: " + printXmlAction(pair.doAction))
        isUndoAction = true
        this.controller.doTransaction("Undo",
                ActionPair(pair.undoAction, pair.doAction))
        isUndoAction = false
    }
	 ...
	 */
    override fun setEnabled(arg0: Boolean) {
        if (arg0) super.setEnabled(actionPairList.size != 0) else super.setEnabled(false)
    }

    fun add(pair: ActionPair?) {
        val dcDo = deepCopy(pair.getDoAction())
        val dcUndo = deepCopy(pair.getUndoAction())
        ...
        if (actionPairList.size > 0
            ...
            var firstPair = actionPairList[0] as ActionPair
            val action: CompoundAction
            val remedia: CompoundAction
            if (firstPair.doAction !is CompoundAction
                    || firstPair.undoAction !is CompoundAction) {
                action = CompoundAction()
                action.addChoice(firstPair.doAction)
                remedia = CompoundAction()
                remedia.addChoice(firstPair.undoAction)
                actionPairList.removeAt(0)
                actionPairList.add(0, ActionPair(action, remedia))
                firstPair = actionPairList[0] as ActionPair
            } else {
                action = firstPair.doAction as CompoundAction
                remedia = firstPair.undoAction as CompoundAction
            }
            action.addChoice(dcDo)
            remedia.addAtChoice(0, dcUndo)
        } else {
            val storagePair = ActionPair(dcDo, dcUndo)
            actionPairList.add(0, storagePair)
            // and cut vector, if bigger than given size:
            var maxEntries = 100
            try {
                maxEntries = controller.frame.getProperty(
                        "undo_levels").toInt()
            } catch (e: NumberFormatException) {
                Resources.getInstance().logException(e)
            }
            while (actionPairList.size > maxEntries) {
                actionPairList.removeAt(actionPairList.size - 1) // remove
                // last elt
            }
        }
        startActionFrame()
    ...
    fun print() {
        ...
        var j = 0
        for (pair in actionPairList) {
            logger!!.info("line " + j++ + " = "
                    + printXmlAction(pair.doAction))
        }
    }

    companion object {
        private const val TIME_TO_BEGIN_NEW_ACTION: Long = 100

...
class ActionRegistry {
     ...
     * action.
     */
    private val registeredHandler: Vector<ActionHandler>

    /** This set denotes all filters for XmlActions.  */
    private val registeredFilters: Vector<ActionFilter>

    /** HashMap of Action class -> actor instance.  */
    private val registeredActors: HashMap<Class<*>?, ActorXml?>
    private var undoActionHandler: UndoActionHandler? = null
    ...
    init {
        ...
        registeredHandler = Vector()
        registeredFilters = Vector()
        registeredActors = HashMap()
    }

    /**
     * The handler is put in front. Thus it is called before others are called.
     */
    fun registerHandler(newHandler: ActionHandler) {
    ...
    fun deregisterHandler(newHandler: ActionHandler) {
        registeredHandler.remove(newHandler)
    }

    fun registerFilter(newFilter: ActionFilter) {
        if (!registeredFilters.contains(newFilter)) {
            if (newFilter is FinalActionFilter) {
                /* Insert as the last one here. */
                registeredFilters.insertElementAt(newFilter,
                        registeredFilters.size)
            } else if (newFilter is FirstActionFilter) {
                /* Insert as the first one here. */
                registeredFilters.insertElementAt(newFilter, 0)
            } else {
                /* Insert before FinalActionFilters */
                var index = 0
                val it: Iterator<ActionFilter> = registeredFilters.iterator()
                while (it.hasNext()) {
                    val filter = it.next()
                    if (filter is FinalActionFilter) {
                        break
                    }
                    index++
                }
    ...
    fun deregisterFilter(newFilter: ActionFilter) {
        registeredFilters.remove(newFilter)
    }

    private fun startTransaction(name: String?) {
        val i: Iterator<ActionHandler> = registeredHandler.iterator()
        while (i.hasNext()) {
            val handler = i.next()
    ...
    private fun endTransaction(name: String?) {
        val i: Iterator<ActionHandler> = registeredHandler.iterator()
        while (i.hasNext()) {
            val handler = i.next()
    ...
    /**
     * @return see [.executeAction]
     */
    fun doTransaction(pName: String?, pPair: ActionPair?): Boolean {
        startTransaction(pName)
        val result = executeAction(pPair)
        endTransaction(pName)
        return result
    }
     ...
     * returns false.
     */
    private fun executeAction(pair: ActionPair?): Boolean {
        ...
        var filteredPair = pair
        // first filter:
        run {
            val i: Iterator<ActionFilter> = registeredFilters.iterator()
            while (i.hasNext()) {
                val filter = i.next()
                filteredPair = filter.filterAction(filteredPair)
            }
        }
        val aArray: Array<Any> = registeredHandler.toTypedArray()
        for (i in aArray.indices) {
            val handler = aArray[i] as ActionHandler
            try {
    ...
    /**
     */
    fun registerActor(actor: ActorXml?, action: Class<*>?) {
        registeredActors[action] = actor
    }

    /**
     */
    fun deregisterActor(action: Class<*>?) {
        registeredActors.remove(action)
    }

    fun getActor(action: XmlAction?): ActorXml? {
        val i: Iterator<Class<*>?> = registeredActors.keys.iterator()
        while (i.hasNext()) {
            val actorClass = i.next()
            if (actorClass!!.isInstance(action)) {
                return registeredActors[actorClass]
            }
        }
        throw IllegalArgumentException("No actor present for xmlaction"
                + action!!.javaClass)
    }

    fun getActor(actionClass: Class<*>): ActorXml? {
        if (registeredActors.containsKey(actionClass)) {
            return registeredActors[actionClass]
        }
        throw IllegalArgumentException("No actor present for xmlaction"
                + actionClass)
    }

    fun registerUndoHandler(undoActionHandler: UndoActionHandler?) {

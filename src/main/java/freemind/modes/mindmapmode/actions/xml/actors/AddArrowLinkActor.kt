...
(pMapFeedback: ExtendedMapFeedback) : XmlActorAdapter(pMapFeedback) {
    override fun act(action: XmlAction?) {
        if (action is AddArrowLinkXmlAction) {
            ...
            val source: MindMapNode? = getNodeFromID(arrowAction.node!!)
            val target: MindMapNode? = getNodeFromID(arrowAction.destination!!)
            if (source === target) {
                XmlActorAdapter.Companion.logger!!.warning("Can't create link between itself. ($source).")
                return
            }
            ...
            val linkModel = MindMapArrowLinkModel(source,
            ...
            if (arrowAction.startInclination != null) {
                linkModel.startInclination = xmlToPoint(arrowAction
                        .startInclination)
            }
            // register link.
    ...
    override val doActionClass: Class<*>
        get() = AddArrowLinkXmlAction::class.java

    private fun getActionPair(source: MindMapNode, target: MindMapNode): ActionPair {
        val doAction = createAddArrowLinkXmlAction(source,
    ...
    fun createAddArrowLinkXmlAction(
            source: MindMapNode?, target: MindMapNode?, proposedID: String?): AddArrowLinkXmlAction {

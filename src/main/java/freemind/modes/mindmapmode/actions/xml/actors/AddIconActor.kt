...
(pMapFeedback: ExtendedMapFeedback) : XmlActorAdapter(pMapFeedback) {
    ...
    override fun act(action: XmlAction?) {
        if (action is AddIconAction) {
            ...
            val node: MindMapNode? = getNodeFromID(iconAction
                    .node!!)
            val iconName = iconAction.iconName
            val position = iconAction.iconPosition
            val icon: MindIcon = MindIcon.Companion.factory(iconName)
    ...
    override val doActionClass: Class<*>
        get() = AddIconAction::class.java

    fun createAddIconAction(node: MindMapNode?, icon: MindIcon?,
                            iconPosition: Int): AddIconAction {
    ...
    private fun getAddLastIconActionPair(node: MindMapNode?, icon: MindIcon?): ActionPair {
        val iconIndex: Int = MindIcon.Companion.LAST
        return getAddIconActionPair(node, icon, iconIndex)
    }

    private fun getAddIconActionPair(node: MindMapNode?, icon: MindIcon?,
                                     iconIndex: Int): ActionPair {
        val doAction = createAddIconAction(node, icon, iconIndex)
        val undoAction: XmlAction? = xmlActorFactory.removeIconActor.createRemoveIconXmlAction(
                node, iconIndex)
    ...
    private fun getToggleIconActionPair(node: MindMapNode, icon: MindIcon): ActionPair {
        val iconIndex = iconFirstIndex(node,
                icon.name!!)
     ...
     * @param removeFirst
     */
    private fun getRemoveIconActionPair(node: MindMapNode?, icon: MindIcon?,
                                        removeFirst: Boolean): ActionPair? {
        val iconIndex = if (removeFirst) iconFirstIndex(
                node!!, icon!!.name!!) else iconLastIndex(
                node!!, icon!!.name!!)
    ...
    private fun getRemoveIconActionPair(node: MindMapNode?, icon: MindIcon?,
                                        iconIndex: Int): ActionPair {
        val doAction: XmlAction? = xmlActorFactory.removeIconActor.createRemoveIconXmlAction(
                node, iconIndex)
    ...
    fun removeIcon(node: MindMapNode?, icon: MindIcon?, removeFirst: Boolean) {
        val removeIconActionPair = getRemoveIconActionPair(node,
                icon, removeFirst) ?: return
        exMapFeedback.doTransaction(

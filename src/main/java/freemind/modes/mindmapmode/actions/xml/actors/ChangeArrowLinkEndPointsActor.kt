...
(pMapFeedback: ExtendedMapFeedback) : XmlActorAdapter(pMapFeedback) {
    ...
    /**
     */
    private fun getActionPair(link: MindMapArrowLink, startPoint: Point?,
                              endPoint: Point?): ActionPair {
        return ActionPair(createArrowLinkPointXmlAction(link, startPoint,
    ...
    override fun act(action: XmlAction?) {
        if (action is ArrowLinkPointXmlAction) {
            ...
            val link = linkRegistry
                    .getLinkForId(pointAction.id) as MindMapArrowLink
            link.startInclination = xmlToPoint(pointAction
                    .startPoint)
            link.endInclination = xmlToPoint(pointAction.endPoint)
    ...
    override val doActionClass: Class<*>
        get() = ArrowLinkPointXmlAction::class.java

    private fun createArrowLinkPointXmlAction(
            arrowLink: MindMapArrowLink, startPoint: Point?, endPoint: Point?): ArrowLinkPointXmlAction {
        val action = ArrowLinkPointXmlAction()
        action.startPoint = PointToXml(startPoint)
        action.endPoint = PointToXml(endPoint)

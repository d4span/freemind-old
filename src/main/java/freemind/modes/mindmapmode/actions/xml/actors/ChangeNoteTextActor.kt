...
class ChangeNoteTextActor(pMapFeedback: ExtendedMapFeedback) : XmlActorAdapter(pMapFeedback) {
    override fun act(action: XmlAction?) {
        if (action is EditNoteToNodeAction) {
            ...
            val node: MindMapNode? = getNodeFromID(noteTextAction
                    .node!!)
            val newText = noteTextAction.text
            val oldText = node.getNoteText()
            if (!safeEquals(newText, oldText)) {
    ...
    override val doActionClass: Class<*>
        get() = EditNoteToNodeAction::class.java

    fun createEditNoteToNodeAction(node: MindMapNode?,
                                   text: String?): EditNoteToNodeAction {
    ...
    fun setNoteText(node: MindMapNode, text: String?) {
        ...
        if (safeEquals(text, oldNoteText)) {
            // they are equal.
            return
        }
        XmlActorAdapter.Companion.logger!!.fine("Old Note Text:'$oldNoteText, new:'$text'.")
        XmlActorAdapter.Companion.logger!!.fine(compareText(oldNoteText, text))
        val doAction = createEditNoteToNodeAction(node, text)
        val undoAction = createEditNoteToNodeAction(node,
                oldNoteText)

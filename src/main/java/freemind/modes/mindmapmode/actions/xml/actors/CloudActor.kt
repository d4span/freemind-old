...
(pMapFeedback: ExtendedMapFeedback) : NodeXmlActorAdapter(pMapFeedback) {
    override val doActionClass: Class<*>
        get() = AddCloudXmlAction::class.java
    ...
    private fun getActionPair(selected: MindMapNode?, enable: Boolean): ActionPair {
        val cloudAction = createAddCloudXmlAction(selected,
                enable, null)
        var undocloudAction: AddCloudXmlAction? = null
        undocloudAction = if (selected.getCloud() != null) {
            createAddCloudXmlAction(selected, true, selected
                    .getCloud().color)
        } else {
    ...
    private fun createAddCloudXmlAction(selected: MindMapNode?,
                                        enable: Boolean, color: Color?): AddCloudXmlAction {
        val nodecloudAction = AddCloudXmlAction()
        nodecloudAction.node = getNodeID(selected)
        nodecloudAction.enabled = enable
        nodecloudAction.color = colorToXml(color)
    ...
    override fun act(action: XmlAction?) {
        if (action is AddCloudXmlAction) {
            ...
            val node: MindMapNode? = getNodeFromID(nodecloudAction.node!!)
            if (node.getCloud() == null == nodecloudAction.enabled) {
                if (nodecloudAction.enabled) {
                    if (node!!.isRoot) {
                        return
                    }
                    val cloudModel = MindMapCloudModel(node,
                            exMapFeedback)
                    node.cloud = cloudModel
                    if (nodecloudAction.color != null) {
                        val color = xmlToColor(nodecloudAction
                                .color)

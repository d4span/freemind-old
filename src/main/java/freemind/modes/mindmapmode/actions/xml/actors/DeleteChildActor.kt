...
(pMapFeedback: ExtendedMapFeedback) : XmlActorAdapter(pMapFeedback) {
    ...
    override fun act(action: XmlAction?) {
        ...
        val selectedNode: MindMapNode? = getNodeFromID(deleteNodeAction!!.node!!)
    ...
    override val doActionClass: Class<*>
        get() = DeleteNodeAction::class.java

    fun deleteNode(selectedNode: MindMapNode?) {
        require(!selectedNode!!.isRoot) { "Root node can't be deleted" }
        val newId = getNodeID(selectedNode)
        val copy = exMapFeedback.copy(selectedNode, true)
        val coord = NodeCoordinate(selectedNode,
                selectedNode.isLeft)
        // Undo-action
        var pasteNodeAction: PasteNodeAction? = null
        pasteNodeAction = exMapFeedback.actorFactory.pasteActor.getPasteNodeAction(copy,
                coord, null as UndoPasteNodeAction?)
        val deleteAction = getDeleteNodeAction(newId)
        exMapFeedback.doTransaction(doActionClass.name,

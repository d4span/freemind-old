...
(pMapFeedback: ExtendedMapFeedback) : XmlActorAdapter(pMapFeedback) {
    override val doActionClass: Class<*>
        get() = EdgeStyleFormatAction::class.java
    ...
    fun setEdgeStyle(node: MindMapNode, style: String?) {
        if (safeEquals(style, getStyle(node))) {
            return
        }
        if (style != null) {
            var found = false
            // check style:
            for (i in EdgeAdapter.Companion.EDGESTYLES.indices) {
                val possibleStyle: String = EdgeAdapter.Companion.EDGESTYLES.get(i)
                if (safeEquals(style, possibleStyle)) {
                    found = true
                    break
                }
            }
    ...
    private fun createNodeStyleFormatAction(
            selected: MindMapNode, style: String?): EdgeStyleFormatAction {
    ...
    override fun act(action: XmlAction?) {
        if (action is EdgeStyleFormatAction) {
            ...
            val node: MindMapNode? = getNodeFromID(edgeStyleAction.node!!)
            val newStyle = edgeStyleAction.style
            val edge = node.getEdge()
            if (!safeEquals(if (edge!!.hasStyle()) edge.style else null,
                            newStyle)) {

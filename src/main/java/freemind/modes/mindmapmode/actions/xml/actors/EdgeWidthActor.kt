...
(pMapFeedback: ExtendedMapFeedback) : XmlActorAdapter(pMapFeedback) {
    override val doActionClass: Class<*>
        get() = EdgeWidthFormatAction::class.java
    ...
    private fun createEdgeWidthFormatAction(
            selected: MindMapNode, width: Int): EdgeWidthFormatAction {
    ...
    override fun act(action: XmlAction?) {
        if (action is EdgeWidthFormatAction) {
            ...
            val node: MindMapNode? = getNodeFromID(edgeWithAction.node!!)
            val width = edgeWithAction.width
            val edge = node.getEdge() as EdgeAdapter

...
(pMapFeedback: ExtendedMapFeedback) : NodeXmlActorAdapter(pMapFeedback) {
    override fun act(action: XmlAction?) {
        ...
        if (!node!!.isRoot) node.parentNode.vGap = moveAction.vGap
        exMapFeedback.nodeChanged(node)
    }

    override val doActionClass: Class<*>
        get() = MoveNodeXmlAction::class.java

    override fun apply(model: MindMap?, selected: MindMapNode?): ActionPair? {
        // reset position
        return if (selected!!.isRoot) null else getActionPair(selected, NodeAdapter.Companion.VGAP, NodeAdapter.Companion.HGAP, 0)
    }

    private fun getActionPair(selected: MindMapNode?, parentVGap: Int,
                              hGap: Int, shiftY: Int): ActionPair {
        val moveAction = moveNode(selected, parentVGap, hGap,
                shiftY)
        val undoAction = moveNode(selected, selected
                .getParentNode().vGap, selected.getHGap(),
                selected.getShiftY())
        return ActionPair(moveAction, undoAction)
    }

    private fun moveNode(selected: MindMapNode?, parentVGap: Int,
                         hGap: Int, shiftY: Int): MoveNodeXmlAction {
    ...
    fun moveNodeTo(node: MindMapNode, parentVGap: Int, hGap: Int,
                   shiftY: Int) {

...
(pMapFeedback: ExtendedMapFeedback) : XmlActorAdapter(pMapFeedback) {
    fun setNodeColor(node: MindMapNode?, color: Color?) {
        ...
        val doAction = createNodeColorFormatAction(node,
                color)
        val undoAction = createNodeColorFormatAction(node,
                node.getColor())
        execute(ActionPair(doAction, undoAction))
    }

    fun createNodeColorFormatAction(node: MindMapNode?,
                                    color: Color?): NodeColorFormatAction {
        val nodeAction = NodeColorFormatAction()
        nodeAction.node = getNodeID(node)
        nodeAction.color = colorToXml(color)
        return nodeAction
    }
    ...
    override fun act(action: XmlAction?) {
        if (action is NodeColorFormatAction) {
            ...
            val color = xmlToColor(nodeColorAction.color)
            val node: MindMapNode? = getNodeFromID(nodeColorAction
                    .node!!)
            val oldColor = node.getColor()
    ...
    override val doActionClass: Class<*>
        get() = NodeColorFormatAction::class.java
}
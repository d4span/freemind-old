...
(pMapFeedback: ExtendedMapFeedback) : XmlActorAdapter(pMapFeedback) {
    override val doActionClass: Class<*>
        get() = NodeStyleFormatAction::class.java
    ...
    fun setStyle(node: MindMapNode, style: String?) {
        ...
        for (i in MindMapNode.Companion.NODE_STYLES.indices) {
            val dstyle: String = MindMapNode.Companion.NODE_STYLES.get(i)
            if (safeEquals(style, dstyle)) {
    ...
    private fun createNodeStyleFormatAction(
            selected: MindMapNode, style: String?): NodeStyleFormatAction {
    ...
    override fun act(action: XmlAction?) {
        if (action is NodeStyleFormatAction) {
            ...
            val node: MindMapNode? = getNodeFromID(nodeStyleAction.node!!)
            val style = nodeStyleAction.style
            if (!safeEquals(if (node!!.hasStyle()) node.bareStyle else null,
                            style)) {

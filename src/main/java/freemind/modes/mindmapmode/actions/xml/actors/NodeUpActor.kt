...
(pMapFeedback: ExtendedMapFeedback) : XmlActorAdapter(pMapFeedback) {
    ...
    fun moveNodes(selected: MindMapNode, selecteds: List<MindMapNode?>?, direction: Int) {
        val doAction = createMoveNodesAction(selected, selecteds,
                direction)
        val undoAction = createMoveNodesAction(selected, selecteds,
                -direction)
        execute(ActionPair(
                doAction, undoAction))
    }
    ...
    private fun _moveNodes(selected: MindMapNode?, selecteds: List<MindMapNode?>, direction: Int) {
        val comparator = if (direction == -1) null else Comparator<Int> { i1, i2 -> i2 - i1 }
        if (!selected!!.isRoot) {
            ...
            for (node in selecteds) {
                if (node!!.parent !== parent) {
                    XmlActorAdapter.Companion.logger!!.warning("Not all selected nodes (here: "
            ...
            for (newInt in range) {
                if (Math.abs(newInt - last) > 1) {
                    XmlActorAdapter.Companion.logger!!.warning("Not adjacent nodes. Skipped. ")
                    return
                }
                last = newInt
            }
            for (position in range) {
                // from above:
                moveNodeTo(sortedChildren[position], parent, direction)
            }
        }
    }
     ...
     */
    private fun moveNodeTo(newChild: MindMapNode?, parent: MindMapNode?,
                           direction: Int): Int {
        val model = exMapFeedback.map
        val index = model!!.getIndexOfChild(parent, newChild)
        var newIndex = index
        val maxIndex = parent!!.childCount
        val sortedNodesIndices = getSortedSiblings(parent)
        var newPositionInVector = (sortedNodesIndices.indexOf(newChild)
                + direction)
        ...
        newIndex = model.getIndexOfChild(parent, sortedNodesIndices[newPositionInVector])
        exMapFeedback.removeNodeFromParent(newChild)
        exMapFeedback.insertNodeInto(newChild, parent, newIndex)
    ...
    override fun act(action: XmlAction?) {
        if (action is MoveNodesAction) {
            ...
            val selected: MindMapNode? = getNodeFromID(moveAction
                    .node!!)
            val selecteds = Vector<MindMapNode?>()
            val i: Iterator<NodeListMember> = moveAction.listNodeListMemberList.iterator()
            while (i.hasNext()) {
                val node = i.next()
    ...
    override val doActionClass: Class<*>
        get() = MoveNodesAction::class.java

    private fun createMoveNodesAction(selected: MindMapNode,
                                      selecteds: List<MindMapNode?>?, direction: Int): MoveNodesAction {

...
(pMapFeedback: ExtendedMapFeedback) : XmlActorAdapter(pMapFeedback) {
    override fun act(action: XmlAction?) {
        if (action is RemoveAttributeAction) {
            val removeAttributeAction = action
            val node = getNodeFromID(removeAttributeAction.node!!)
            val position = removeAttributeAction.position
    ...
    override val doActionClass: Class<*>
        get() = RemoveAttributeAction::class.java

    fun getActionPair(selected: MindMapNode, pPosition: Int): ActionPair {
        val setAttributeAction = getRemoveAttributeAction(selected,
                pPosition)
        val undoRemoveAttributeAction: NodeAction?
        undoRemoveAttributeAction = if (pPosition == selected.attributeTableLength - 1) {
            xmlActorFactory.addAttributeActor.getAddAttributeAction(
                    selected, selected.getAttribute(pPosition))
        } else {
            xmlActorFactory.insertAttributeActor.getInsertAttributeAction(
                    selected, pPosition, selected.getAttribute(pPosition))
        }
        return ActionPair(setAttributeAction, undoRemoveAttributeAction)
    }
     ...
     * @return
     */
    fun getRemoveAttributeAction(pSelected: MindMapNode?,
                                 pPosition: Int): RemoveAttributeAction {
        val removeAttributeAction = RemoveAttributeAction()
        removeAttributeAction.node = getNodeID(pSelected)
        removeAttributeAction.position = pPosition
        return removeAttributeAction
    }

    fun removeAttribute(pNode: MindMapNode, pPosition: Int) {
        val actionPair = getActionPair(pNode, pPosition)
        execute(actionPair)
    }
}
...
(pMapFeedback: ExtendedMapFeedback) : XmlActorAdapter(pMapFeedback) {
    ...
    fun revertMap(map: MindMap?, file: File) {
        val doAction = createRevertXmlAction(file)
        val undoAction = createRevertXmlAction(
                map, null, file.name)
        execute(ActionPair(doAction, undoAction))
    }

    fun openXmlInsteadOfMap(xmlFileContent: String?) {
        try {
            val doAction = createRevertXmlAction(xmlFileContent,
                    null, null)
            val undoAction = createRevertXmlAction(
                    exMapFeedback.map, null, null)
            execute(ActionPair(doAction, undoAction))
        } catch (e: IOException) {
    ...
    @Throws(IOException::class)
    fun createRevertXmlAction(file: File): RevertXmlAction {
        val fileName = file.absolutePath
        val f = FileReader(file)
        val buffer = StringBuffer()
        var c: Int
        while (f.read().also { c = it } != -1) {
    ...
    @Throws(IOException::class)
    fun createRevertXmlAction(map: MindMap?, fileName: String?,
                              filePrefix: String?): RevertXmlAction {
        val writer = StringWriter()
        map!!.getXml(writer)
        return createRevertXmlAction(writer.buffer.toString(), fileName,
                filePrefix)
    }
     ...
     * fileName is null.
     */
    fun createRevertXmlAction(xmlPackedFile: String?,
                              fileName: String?, filePrefix: String?): RevertXmlAction {
        val revertXmlAction = RevertXmlAction()
        revertXmlAction.localFileName = fileName
        revertXmlAction.map = xmlPackedFile
        revertXmlAction.filePrefix = filePrefix
    ...
    override fun act(action: XmlAction?) {
        if (action is RevertXmlAction) {
            try {
                val revertAction = action

                // close the old map.
                exMapFeedback.close(true)
                if (revertAction.localFileName != null) {
                    exMapFeedback.load(File(revertAction
                            .localFileName))
                } else {
                    // the map is given by xml. we store it and open it.
                    var filePrefix = exMapFeedback
                            .getResourceString("freemind_reverted")
                    if (revertAction.filePrefix != null) {
                        filePrefix = revertAction.filePrefix
                    }
                    val xmlMap = revertAction.map
                    val tempFile = File.createTempFile(filePrefix,
                            FreeMindCommon.FREEMIND_FILE_EXTENSION,
                            File(Resources.getInstance().getFreemindDirectory()))
                    val fw = FileWriter(tempFile)
                    fw.write(xmlMap)
                    fw.close()
                    exMapFeedback.load(tempFile)
                }
            } catch (e: Exception) {
                Resources.getInstance().logException(e)
            }
    ...
    override val doActionClass: Class<*>
        get() = RevertXmlAction::class.java
}
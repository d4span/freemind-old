 ...
 * @author foltin
 */
class StrikethroughNodeActor
/**
 * @param pMapFeedback
 */
(pMapFeedback: ExtendedMapFeedback) : NodeXmlActorAdapter(pMapFeedback) {
    override fun act(action: XmlAction?) {
        if (action is StrikethroughNodeAction) {
            val strikethroughact = action
            val node = getNodeFromID(strikethroughact.node!!)
            if (node!!.isStrikethrough != strikethroughact.strikethrough) {
                node.isStrikethrough = strikethroughact.strikethrough
    ...
    override val doActionClass: Class<*>
        get() = StrikethroughNodeAction::class.java

    override fun apply(model: MindMap?, selected: MindMapNode?): ActionPair? {
        // every node is set to the inverse of the focussed node.
        val Strikethrough = getSelected().isStrikethrough
        return getActionPair(selected, !Strikethrough)
    }

    private fun getActionPair(selected: MindMapNode?, Strikethrough: Boolean): ActionPair {
        val StrikethroughAction = toggleStrikethrough(selected, Strikethrough)
        val undoStrikethroughAction = toggleStrikethrough(selected, selected!!.isStrikethrough)
        return ActionPair(StrikethroughAction, undoStrikethroughAction)
    }

    private fun toggleStrikethrough(selected: MindMapNode?, strikethrough: Boolean): StrikethroughNodeAction {
        val StrikethroughAction = StrikethroughNodeAction()
        StrikethroughAction.node = getNodeID(selected)
        StrikethroughAction.strikethrough = strikethrough

 ...
 * @date 10.04.2014
 */
class ToggleFoldedActor
/**
 * @param pMapFeedback
 */
(pMapFeedback: ExtendedMapFeedback) : XmlActorAdapter(pMapFeedback) {
    fun toggleFolded(listIterator: ListIterator<*>?) {
        val fold: Boolean = CommonToggleFoldedAction.Companion.getFoldingState(CommonToggleFoldedAction.Companion.reset(listIterator))
        val doAction = createFoldAction(
                CommonToggleFoldedAction.Companion.reset(listIterator), fold, false)
        val undoAction = createFoldAction(
                CommonToggleFoldedAction.Companion.reset(listIterator), !fold, true)
        execute(ActionPair(doAction, undoAction))
    }

    private fun createFoldAction(iterator: ListIterator<MindMapNode>?,
                                 fold: Boolean, undo: Boolean): CompoundAction {
        val comp = CompoundAction()
        // sort selectedNodes list by depth, in order to guarantee that sons
        // are deleted first:
        while (iterator!!.hasNext()) {
            val node = iterator.next()
            val foldAction = createSingleFoldAction(fold, node, undo)
            if (foldAction != null) {
                if (!undo) {
                    comp
        ...
        XmlActorAdapter.Companion.logger!!.finest("Compound contains " + comp.sizeChoiceList()
                + " elements.")
        return comp
    }
     ...
     */
    private fun createSingleFoldAction(fold: Boolean, node: MindMapNode,
                                       undo: Boolean): FoldAction? {
        var foldAction: FoldAction? = null
        if (undo && node.isFolded == fold
            ...
            if (node.hasChildren()
                ...
                foldAction.folded = fold
                foldAction.node = getNodeID(node)
            }
        }
        return foldAction
    }
    ...
    override fun act(action: XmlAction?) {
        if (action is FoldAction) {
            val foldAction = action
            val node: MindMapNode? = getNodeFromID(foldAction
                    .node!!)
            val folded = foldAction.folded
            ...
            if (node.isFolded != folded) {
                ...
                exMapFeedback.map.nodeStructureChanged(node)
            }
    ...
    override val doActionClass: Class<*>
        get() = FoldAction::class.java

    /**
     */
    fun setFolded(node: MindMapNode, folded: Boolean) {
        val doAction = createSingleFoldAction(folded, node, false)
        val undoAction = createSingleFoldAction(!folded, node, true)

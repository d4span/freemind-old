 ...
 * @date 27.03.2014
 */
class UnderlineActor
/**
 * @param pMapFeedback
 */
(pMapFeedback: ExtendedMapFeedback) : NodeXmlActorAdapter(pMapFeedback) {
    override fun act(action: XmlAction?) {
        val underlinedact = action as UnderlinedNodeAction?
        val node = getNodeFromID(underlinedact!!.node!!)
        if (node!!.isUnderlined != underlinedact.underlined) {
            node.isUnderlined = underlinedact.underlined
            exMapFeedback.nodeChanged(node)
        }
    }

    override val doActionClass: Class<*>
        get() = UnderlinedNodeAction::class.java

    override fun apply(model: MindMap?, selected: MindMapNode?): ActionPair? {
        // every node is set to the inverse of the focussed node.
        val underlined = exMapFeedback.selected.isUnderlined
        return getActionPair(selected, !underlined)
    }

    private fun getActionPair(selected: MindMapNode?, underlined: Boolean): ActionPair {
        val underlinedAction = toggleUnderlined(selected,
                underlined)
        val undoUnderlinedAction = toggleUnderlined(selected,
                !underlined)
        return ActionPair(underlinedAction, undoUnderlinedAction)
    }

    private fun toggleUnderlined(selected: MindMapNode?,
                                 underlined: Boolean): UnderlinedNodeAction {
        val underlinedAction = UnderlinedNodeAction()
        underlinedAction.node = getNodeID(selected)
        underlinedAction.underlined = underlined
        return underlinedAction
    }

    fun setUnderlined(node: MindMapNode?, underlined: Boolean) {

 ...
 */
class MindMapNodeMotionListener(private val c: MindMapController) : NodeMotionAdapter() {
    override fun mouseMoved(e: MouseEvent) {}
    var dragStartingPoint: Point? = null
        private set
    private var originalParentVGap = 0
    private var originalHGap = 0
    ...
    override fun mouseDragged(e: MouseEvent) {
        ...
        if (e.modifiersEx and InputEvent.BUTTON1_DOWN_MASK == InputEvent.BUTTON1_DOWN_MASK) {
            val motionListenerView = e
                    .source as NodeMotionListenerView
            val nodeView = getNodeView(e)
            val mapView = nodeView.map
            val node = nodeView.model
            val point = e.point
            convertPointToAncestor(motionListenerView, point,
                    JScrollPane::class.java)
            if (!isActive) {
                setDragStartingPoint(point, node)
            } else {
                if (e.modifiersEx and InputEvent.CTRL_DOWN_MASK == 0) {
                    val nodeShiftY = getNodeShiftY(point, node,
                            dragStartingPoint)
                    val hGap = getHGap(point, node, dragStartingPoint)
                    node.shiftY = nodeShiftY
                    node.hGap = hGap
                } else {
                    val parentNode = nodeView.visibleParentView
                            .model
                    parentNode.vGap = getVGap(point, dragStartingPoint)
                    c.modeController.nodeRefresh(parentNode)
                }
                dragStartingPoint = point
                c.modeController.nodeRefresh(node)
            }
            val mapPoint = e.point
            convertPointToAncestor(motionListenerView, mapPoint, mapView)
            val isEventPointVisible = mapView.visibleRect.contains(
                    mapPoint)
            if (!isEventPointVisible) {
                val r = Rectangle(mapPoint)
                val bounds = mapView.bounds
                mapView.scrollRectToVisible(r)
                val bounds2 = mapView.bounds
                val diffx = bounds2.x - bounds.x
                val diffy = bounds2.y - bounds.y
                try {
                    mapPoint.translate(diffx, diffy)
                    // here, there are strange cases, when the mouse moves away.
                    // Workaround.
                    if (mapView.visibleRect.contains(mapPoint)) {
                        Robot().mouseMove(e.xOnScreen + diffx,
                                e.yOnScreen + diffy)
                    }
                } catch (e1: AWTException) {
                    Resources.getInstance().logException(e1)
                }
                dragStartingPoint!!.x += (if (node.hGap < 0) 2 else 1) * diffx
                dragStartingPoint!!.y += (if (node.shiftY < 0) 2 else 1) * diffy
            }
        }
    }

    private fun getVGap(dragNextPoint: Point, dragStartingPoint: Point?): Int {
        var oldVGap = originalParentVGap
        val vGapChange = ((dragNextPoint.y - dragStartingPoint!!.y) / c
                .view.zoom).toInt()
        oldVGap = Math.max(0, oldVGap - vGapChange)
        return oldVGap
    }

    private fun getHGap(dragNextPoint: Point, node: MindMapNode,
                        dragStartingPoint: Point?): Int {
        var oldHGap = node.hGap
        var hGapChange = ((dragNextPoint.x - dragStartingPoint!!.x) / c
                .view.zoom).toInt()
    ...
    private fun getNodeShiftY(dragNextPoint: Point, pNode: MindMapNode,
                              dragStartingPoint: Point?): Int {
        var shiftY = pNode.shiftY
        val shiftYChange = ((dragNextPoint.y - dragStartingPoint!!.y) / c
                .view.zoom).toInt()
        shiftY += shiftYChange
        return shiftY
    }
    ...
    override fun mouseClicked(e: MouseEvent) {
        if (e.button == 1 && e.clickCount == 2) {
            if (e.modifiersEx == 0) {
                val nodeV = getNodeView(e)
                val node = nodeV.model
                c.moveNodePosition(node, node.vGap, NodeAdapter.Companion.HGAP, 0)
                return
            }
            if (e.modifiersEx == InputEvent.CTRL_DOWN_MASK) {
                val nodeV = getNodeView(e)
                val node = nodeV.model
                c.moveNodePosition(node, NodeAdapter.Companion.VGAP, node.hGap,
                        node.shiftY)
                return
            }
        }
    ...
    /**
     */
    private fun getNodeView(e: MouseEvent): NodeView {
        return (e.source as NodeMotionListenerView).movedView
    }
    ...
    override fun mouseEntered(e: MouseEvent) {
        ...
        if (!isActive) {
            val v = e.source as NodeMotionListenerView
            v.setMouseEntered()
        }
    }
    ...
    override fun mouseReleased(e: MouseEvent) {
        ...
        val v = e.source as NodeMotionListenerView
        if (!v.contains(e.x, e.y)) v.setMouseExited()
        if (!isActive) return
        val nodeV = getNodeView(e)
        val point = e.point
        convertPointToAncestor(nodeV, point, JScrollPane::class.java)
        // move node to end position.
        val node = nodeV.model
        val parentNode = nodeV.model.parentNode
        val parentVGap = parentNode.vGap
        val hgap = node.hGap
        val shiftY = node.shiftY
        resetPositions(node)
    ...
    /**
     */
    private fun resetPositions(node: MindMapNode) {
        node.parentNode.vGap = originalParentVGap
        node.hGap = originalHGap
        node.shiftY = originalShiftY
    }

    val isActive: Boolean
        get() = dragStartingPoint != null

    fun setDragStartingPoint(point: Point?, node: MindMapNode?) {
        dragStartingPoint = point
        if (point != null) {
            originalParentVGap = node.getParentNode().vGap
            originalHGap = node.getHGap()
            originalShiftY = node.getShiftY()
        } else {
            originalShiftY = 0
            originalHGap = originalShiftY
            originalParentVGap = originalHGap
        }
    }

...
import freemind.preferences.layout.KeyEventTranslator.translateKeyEvent
 ...
 */
class GrabKeyDialog @JvmOverloads constructor(
    private val fmMain: FreeMindMain?,
    parent: Dialog?,
    binding: KeyBinding,
    allBindings: Vector<KeyBinding>,
    debugBuffer: Buffer?,
    private val modifierMask: Int = 0
) : JDialog(parent, /* FIXME: getText */"grab-key.title", true) {
    ...
    // {{{ Private members
    // {{{ Instance variables
    private var shortcut: InputPane? = // this is a bad hack
        null
    ...
    private fun init(binding: KeyBinding, allBindings: Vector<KeyBinding>, debugBuffer: Buffer?) {
        ...
        contentPane = content
        JLabel(
            if (debugBuffer == null) getText("grab-key.caption") + " " + binding.label // FIXME: getText("grab-key.caption")+new String[] {
            // binding.label })
    ...
    internal inner class InputPane : JTextField() {
        ...
        public override fun processKeyEvent(_evt: KeyEvent) {
            ...
            if (evt == null) return
            evt.consume()
            val key = translateKeyEvent(evt) ?: return
            if (debugBuffer != null) {
                debugBuffer!!.insert()
            }
        ...
        private fun canClose(): Boolean {
            ...
            val answer = JOptionPane.showConfirmDialog(
                ...
                Resources.getInstance().format(
                    "GrabKeyDialog.grab-key.duplicate-shortcut", arrayOf(bindingReset!!.name)
                ),
                null,
    ...
    companion object {
        ...
        fun toString(evt: KeyEvent): String {
            ...
            return (
                id + ",keyCode=0x" + Integer.toString(evt.keyCode, 16) +

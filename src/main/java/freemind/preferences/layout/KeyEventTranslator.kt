...
object KeyEventTranslator {
    ...
    fun translateKeyEvent(evt: KeyEvent): Key? {
        ...
        when (evt.id) {
            KeyEvent.KEY_PRESSED -> {
                ...
                returnValue = if (keyCode >= KeyEvent.VK_0 && keyCode <= KeyEvent.VK_9 ||
                    ...
                    if (KeyEventWorkaround.ALTERNATIVE_DISPATCHER) return null else {
                        Key(
                            modifiersToString(modifiers), // fc, 12.5.2005: changed to upper case as Freemind seems to
                            // need this.
                            '\u0000'.digitToInt(), keyCode.toChar().uppercaseChar()
                        )
                    }
                } else {
            ...
            KeyEvent.KEY_TYPED -> {
                ...
                returnValue =
                    if (modifiers and InputEvent.ALT_GRAPH_MASK == 0 && evt.getWhen() - KeyEventWorkaround.lastKeyTime < 750 && KeyEventWorkaround.modifiers and ignoreMods.inv() != 0) {
    ...
    fun modifiersToString(mods: Int): String? {
        ...
        if (mods and InputEvent.ALT_MASK != 0) {
            if (buf == null) buf = StringBuffer() else buf.append(GrabKeyDialog.MODIFIER_SEPARATOR)
    ...
    class Key(var modifiers: String?, var key: Int, var input: Char) {
        ...
        override fun toString(): String {
            return (
                (if (modifiers == null) "" else modifiers) + "<" +

...
import freemind.common.XmlBindingTools.Companion.instance
...
class OptionPanel(fm: FreeMind, frame: JDialog, feedback: OptionPanelFeedback) : TextTranslator {
    ...
    fun buildPanel() {
        ...
        var rightLayout: FormLayout? // add rows dynamically
    ...
    override fun getText(pKey: String?): String? {
        ...
        return fmMain!!.getResourceString("OptionPanel.$pKey")
    }
    ...
    private val allButtons: Collection<JButton>
        get() = tabButtonMap.values

    private inner class ChangeTabAction(
    ...
    class NewTabProperty @JvmOverloads constructor(
        override val label: String,
        override val description: String = DEFAULT_LAYOUT_FORMAT
    ) : PropertyControl {
    ...
    private inner class KeyProperty(frame: JDialog?, override var description: String?, override var label: String) :
         ...
         */
        init {
            kb = createBinding(getText(label), label, fmMain!!.getAdjustableProperty(label))
            mButton.addActionListener {
                val dialog = GrabKeyDialog(
                    fmMain, frame,
                    GrabKeyDialog.KeyBinding(label, label, value, false),
        ...
        override fun layout(
            ...
            if (labelText == null) labelText = pTranslator!!.getText(label)
    ...
    private fun getControls(): Vector<PropertyControl?> {
            ...
            ComboProperty(/**
                ...
                object : TextTranslator {
                    override fun getText(pKey: String?): String? {
                        ...
                        return Tools.removeTranslateComment(
                            this@OptionPanel
                                .getText(pKey)
                        )
                    }
        ...
        controls.add(
            BooleanProperty(
                FreeMind.PROXY_IS_AUTHENTICATED +
                    TOOLTIP_EXT,
                FreeMind.PROXY_IS_AUTHENTICATED
            )
        )
        ...
        controls.add(
            StringProperty(
                FreeMind.PROXY_EXCEPTION + TOOLTIP_EXT,
        ...
        controls.add(
            NumberProperty(
                null, "last_opened_list_length", 0,
                200, 1
            )
        ) // 25
        controls.add(
            BooleanProperty(
                FreeMindCommon.LOAD_LAST_MAP +
                    TOOLTIP_EXT,
                FreeMindCommon.LOAD_LAST_MAP
            )
        ) // true
        controls.add(
            BooleanProperty(
                FreeMind.RESOURCES_DON_T_OPEN_PORT +
                    TOOLTIP_EXT,
                FreeMind.RESOURCES_DON_T_OPEN_PORT
            )
        )
        controls.add(
            BooleanProperty(
                ...
                FreeMindCommon.LOAD_LAST_MAPS_AND_LAYOUT
            )
        ) // true
        controls.add(
            BooleanProperty(
                "experimental_file_locking_on.tooltip", "experimental_file_locking_on"
            )
        ) // false
        controls.add(NextLineProperty())
        ...
        controls.add(
            StringProperty(
                "browsemode_initial_map.tooltip",
        ...
        controls.add(
            StringProperty(
                "time_for_automatic_save.tooltip",
        ...
        controls.add(
            StringProperty(
                "path_to_automatic_saves.tooltip",
        ...
        controls.add(
            ComboProperty(
                ...
                FreeMind.RESOURCES_NODE_STYLE, MindMapNode.NODE_STYLES, this
            )
        ) // as_parent
        controls.add(
            ComboProperty(
        ...
        controls.add(
            ColorProperty(
                ...
                FreeMind.RESOURCES_EDGE_COLOR, fmMain?.getDefaultProperty(FreeMind.RESOURCES_EDGE_COLOR),
                this
            )
        ) // #808080
        controls.add(
            ColorProperty(
                ...
                FreeMind.RESOURCES_LINK_COLOR, fmMain?.getDefaultProperty(FreeMind.RESOURCES_LINK_COLOR),
                this
            )
        ) // #b0b0b0
        controls.add(
        ...
        controls.add(
            ColorProperty(
                ...
                FreeMind.RESOURCES_CLOUD_COLOR, fmMain?.getDefaultProperty(FreeMind.RESOURCES_CLOUD_COLOR),
                this
            )
        ) // #f0f0f0
        controls.add(NextLineProperty())
        ...
        controls.add(
            StringProperty(
                "defaultfont.tooltip", "defaultfont"
            )
        ) // SansSerif
        ...
        controls.add(
            NumberProperty(
                ...
                "defaultfontsize", 1,
                96 /* taken from openoffice as maximum. */, 1
            )
        ) // 12
        // controls.add(new StringProperty(null, "defaultfontsize")); // 12
        controls.add(
            NumberProperty(
                "max_node_width.tooltip",
                "max_node_width", 1, Int.MAX_VALUE, 1
            )
        ) // 600
        controls.add(
            NumberProperty(
                "max_tooltip_width.tooltip",
                "max_tooltip_width", 1, Int.MAX_VALUE, 1
            )
        ) // 600
        controls.add(NextLineProperty())
        ...
        val translatedLafNames: MutableList<String?> = mutableListOf()
        lafNames[0] = "default"
        ...
        controls.add(
            ComboProperty(
                "lookandfeel.tooltip",
                FreeMind.RESOURCE_LOOKANDFEEL, lafNames, translatedLafNames
            )
        ) // default
        controls.add(
            NumberProperty(
        ...
        controls.add(
            ComboProperty(
                ...
                FreeMind.J_SPLIT_PANE_SPLIT_TYPE,
                arrayOf(
                    FreeMind.VERTICAL_SPLIT_BELOW,
                    FreeMind.HORIZONTAL_SPLIT_RIGHT
                ),
                this
            )
        )
        ...
        controls.add(
            BooleanProperty(
                ...
                FreeMindCommon.CREATE_THUMBNAIL_ON_SAVE
            )
        ) // true
        controls.add(
            NumberProperty(
        ...
        controls.add(
            NumberProperty(
                FreeMind.TOOLTIP_DISPLAY_TIME +
                    ".tooltip",
                FreeMind.TOOLTIP_DISPLAY_TIME, 0, Int.MAX_VALUE, 1
            )
        ) // 4000
        ...
        controls.add(
            KeyProperty(
                frame, null,
                FreeMind.KEYSTROKE_PREVIOUS_MAP
            )
        ) // control
        // LEFT
        ...
        controls.add(
            BooleanProperty(
                "disable_cursor_move_paper.tooltip",
                "disable_cursor_move_paper"
            )
        ) // false
        controls.add(
            BooleanProperty(
                "enable_leaves_folding.tooltip",
                "enable_leaves_folding"
            )
        ) // false
        controls.add(
            StringProperty(
                "foldingsymbolwidth.tooltip",
                "foldingsymbolwidth"
            )
        ) // 6
        controls.add(NextLineProperty())
        ...
        controls.add(
            BooleanProperty(
                "disable_key_type.tooltip",
                "disable_key_type"
            )
        ) // false
        controls.add(
            BooleanProperty(
                "key_type_adds_new.tooltip",
                "key_type_adds_new"
            )
        ) // false
        controls.add(NextLineProperty())
        ...
        controls.add(
            NumberProperty(
                ...
                "time_for_delayed_selection", 1, Int.MAX_VALUE, 1
            )
        ) // 500
        controls.add(NextLineProperty())
        ...
        controls.add(
            NumberProperty(
                "undo_levels.tooltip", "undo_levels",
                2, 1000, 1
            )
        )
        /***********************************************************************
    ...
    private fun createBinding(
        ...
        var theShortcut: String? = shortcut
        if (theShortcut != null && theShortcut.isEmpty()) theShortcut = null
        val binding = GrabKeyDialog.KeyBinding(name, label, theShortcut, false)

...
class ImageFactory {
    ...
    fun createIcon(pFilePath: String): ImageIcon {
        if (scalingFactorPlain == 200) {
            // test for existence  of a scaled icon:
            if (pFilePath.endsWith(".png")) {
                try {
                    val url: URL = Resources.getInstance().getResource(pFilePath.replace(".png$".toRegex(), "_32.png"))

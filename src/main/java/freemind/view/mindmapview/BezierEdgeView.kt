...
class BezierEdgeView : EdgeView() {
    ...
    private fun update() {
        ...
        var sourceSign = 1
        if (getSource().isRoot
                && !VerticalRootNodeViewLayout.Companion.USE_COMMON_OUT_POINT_FOR_ROOT_NODE) {
            sourceSign = 0
        }
        val xctrl = map.getZoomed(sourceSign * sign * XCTRL)
        val childXctrl = map.getZoomed(-1 * sign * CHILD_XCTRL)
        graph.setCurve(start!!.x.toFloat(), start!!.y.toFloat(), (start!!.x + xctrl).toFloat(), start!!.y.toFloat(), (end!!.x
    ...
    override fun paint(g: Graphics2D) {
        ...
        if (isTargetEclipsed) {
            ...
            g.stroke = EdgeView.Companion.getEclipsedStroke()
            g.draw(graph)
            g.stroke = stroke
            g.color = color
        }

...
abstract class EdgeView {
    ...
    protected fun createEnd() {
        end = target.getMainViewInPoint()
        convertPointToAncestor(target!!.mainView!!, end!!, source!!)
    }

    protected open fun createStart() {
        start = source!!.getMainViewOutPoint(target, end)
        convertPointToAncestor(source!!.mainView!!, start!!, source!!)
    }
    ...
    val stroke: Stroke
        get() {
            ...
            return if (width == EdgeAdapter.WIDTH_THIN) {
                DEF_STROKE
            } else BasicStroke(width * map!!.zoom,
                    BasicStroke.CAP_BUTT, BasicStroke.JOIN_MITER)
        }
    val width: Int
        get() = model.width
    protected val map: MapView?
        protected get() = target.getMap()
    protected val isTargetEclipsed: Boolean
        protected get() = target!!.isParentHidden
        ...
        protected val eclipsedStroke: Stroke?
            protected get() {
                if (ECLIPSED_STROKE == null) {
                    ...
                    ECLIPSED_STROKE = BasicStroke(3.0f, BasicStroke.CAP_BUTT,

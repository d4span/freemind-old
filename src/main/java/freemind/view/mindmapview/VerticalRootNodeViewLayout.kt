...
class VerticalRootNodeViewLayout : NodeViewLayoutAdapter() {
    override fun layout() {
        ...
        val contentHeight = Math.max(rightContentHeight,
                leftContentHeight)
        val x = Math.max(spaceAround, -childHorizontalShift)
        if (view.isContentVisible) {
            content.isVisible = true
            val contentPreferredSize = content
                    .preferredSize
            ...
            val childVerticalShift = Math.min(rightChildVerticalShift,
                    leftChildVerticalShift)
            val y = Math.max(spaceAround, -childVerticalShift)
            content.setBounds(x, y, contentPreferredSize.width,
    ...
    override fun getMainViewOutPoint(view: NodeView, targetView: NodeView?,
        ...
        convertPointFromAncestor(view, p, mainView!!)

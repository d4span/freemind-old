 ...
 * @author foltin
 */
class DatabaseConnector : DatabaseBasics() {
    /**
     *
     */
    override fun startupMapHook() {
        ...
        try {
            ...
            if (connectionHook != null) {
                ...
                DatabaseBasics.Companion.logger!!.info("Deregister filter, so that the hook isn't reported to the database.")
                val updateThread = connectionHook.updateThread
                updateThread!!.deregisterFilter()
                updateThread.removeUser()
                DatabaseBasics.Companion.logger!!.info("Shutting down the permanent hook.")
                DatabaseBasics.Companion.togglePermanentHook(controller)
                return
            }
            ...
            val hostProperty = StringProperty(DatabaseBasics.Companion.HOST_DESCRIPTION,
                    DatabaseBasics.Companion.HOST)
            ...
            val connection = DriverManager.getConnection(
                    "jdbc:hsqldb:hsql://" + hostProperty.value + ":"
                            + portProperty!!.value + "/xdb", "sa", password)
            ...
            mUpdateThread.host = hostProperty.value
    ...
    private val isConnected: DatabaseConnectionHook?
        private get() {
    ...
    override val role: Int?
        get() = DatabaseBasics.Companion.ROLE_SLAVE

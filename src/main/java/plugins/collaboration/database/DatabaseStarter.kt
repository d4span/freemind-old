 ...
 * @author foltin
 */
class DatabaseStarter : DatabaseBasics(), PermanentNodeHook, DontSaveMarker {
    private var mTempDbFile: File? = null
    ...
    override fun startupMapHook() {
        ...
        val passwordProperty = StringProperty(
                DatabaseBasics.Companion.PASSWORD_DESCRIPTION, DatabaseBasics.Companion.PASSWORD)
        val passwordProperty2 = StringProperty(
                DatabaseBasics.Companion.PASSWORD_VERIFICATION_DESCRIPTION, DatabaseBasics.Companion.PASSWORD_VERIFICATION)
        ...
        dialog.setUp(controls, object : FormDialogValidator {
            override val isValid: Boolean
                get() {
                    DatabaseBasics.Companion.logger!!.finest("Output valid?")
                    return safeEquals(passwordProperty.value,
                            passwordProperty2.value)
                }
        })
        if (!dialog.isSuccess) return
        val password = passwordProperty.value
        setPortProperty(portProperty)
        // start server:
        DatabaseBasics.Companion.logger!!.info("Start server...")
        try {
            mTempDbFile = File.createTempFile("collaboration_database",
                    ".hsqldb", File(controller.frame
                    .freemindDirectory))
            // mTempDbFile.deleteOnExit();
            DatabaseBasics.Companion.logger!!.info("Start server in file $mTempDbFile")
            val server = Thread {
                Server.main(arrayOf("-database.0",
                        "file:$mTempDbFile", "-dbname.0", "xdb", "-port",
                        portProperty!!.value, "-no_system_exit", "true"))
            }
            server.start()
            Thread.sleep(1000)
            DatabaseBasics.Companion.logger!!.info("Connect...")
            Class.forName("org.hsqldb.jdbcDriver")
            val url = ("jdbc:hsqldb:hsql://localhost:"
                    + portProperty!!.value + "/xdb")
            DatabaseBasics.Companion.logger!!.info("Connecting to $url")
            val connection = DriverManager.getConnection(url, "sa", "")
            mUpdateThread = UpdateThread(connection, controller)
            mUpdateThread.host = hostName
            mUpdateThread.port = portProperty.value
            mUpdateThread.setupTables(password)
            DatabaseBasics.Companion.logger!!.info("Starting update thread...")
            mUpdateThread.start()
        } catch (e: Exception) {
            Resources.getInstance().logException(e)
            // TODO: Need a better message here.
            controller.controller!!.errorMessage(e.localizedMessage)
    ...
    override fun shutdownMapHook() {
        if (mUpdateThread != null) {
            ...
            DatabaseBasics.Companion.logger!!.info("Remove temporary database files.")
            mTempDbFile!!.delete()
            File(mTempDbFile!!.absoluteFile.toString() + ".script").delete()
            File(mTempDbFile!!.absoluteFile.toString() + ".properties").delete()
        }
    ...
    override val role: Int?
        get() = DatabaseBasics.Companion.ROLE_MASTER

...
abstract class SocketBasics : MindMapNodeHookAdapter(), MapTitleContributor, FirstActionFilter {
    ...
    override var mapFeedback: ExtendedMapFeedback
        protected get() = mindMapController!!
        set(mapFeedback) {
            super.mapFeedback = mapFeedback
        }
    ...
    protected var portProperty: NumberProperty
        protected get() {
            ...
            // fill values:
            portProperty.value = (""
                    + mapFeedback.getIntProperty(
                    PORT_PROPERTY, 9001))
            return portProperty
        }
        protected set(portProperty) {
            mapFeedback.setProperty(PORT_PROPERTY,
                    portProperty.value)
        }
    abstract val port: Int
    override fun getMapTitle(pOldTitle: String?, pMapModule: MapModule?,
                             pModel: MindMap?): String? {
        if (pMapModule!!.modeController !== mapFeedback) {
            return pOldTitle
        }
        val userInfo = getMasterInformation(mapFeedback) ?: return pOldTitle
        return (pOldTitle
                + Resources.getInstance().format(
                TITLE, arrayOf<Any?>(role,
                userInfo.masterHostname,
                userInfo.masterIp, userInfo.masterPort,
                userInfo.userIds)))
    }

    abstract fun getMasterInformation(pController: ExtendedMapFeedback?): CollaborationUserInformation?

    /**
     * Deep search inside the [XmlAction] to find a hook (i.e. myself).
     ...
     * @return
     */
    private fun visit(pAction: XmlAction?, pSearchString: String): Boolean {
        if (pAction is CompoundAction) {
            ...
            while (it
                            .hasNext()) {
                val action = it.next()
                result = result or visit(action, pSearchString)
            }
            return result
        }
        if (pAction is HookNodeAction) {
            if (safeEquals(pAction.hookName, pSearchString)) {
                return true
            }
        }
        return false
    ...
    override fun filterAction(pPair: ActionPair?): ActionPair? {
        ...
        if (visit(pPair.doAction, SLAVE_HOOK_LABEL)) {
            return pPair
        }
        if (visit(pPair.doAction, MASTER_HOOK_LABEL)) {
            return pPair
        }
        val doAction = marshall(pPair.doAction)
        val undoAction = marshall(
                pPair.undoAction)
        logger!!.info("Require lock for command: $doAction")
        try {
            val lockId = lock(userName, mapFeedback)
            /*
			 * Blocking broadcast call: Client: send to master (who broadcasts
			 * the command afterwards), Master: send to all clients.
			 */broadcastCommand(doAction, undoAction, lockId, mapFeedback)
        } catch (e: UnableToGetLockException) {
            Resources.getInstance().logException(e)
            return emptyActionPair
        } catch (e: Exception) {
            Resources.getInstance().logException(e)
            return emptyActionPair
        } finally {
    ...
    val emptyActionPair: ActionPair
        get() = ActionPair(CompoundAction(), CompoundAction())

    class UnableToGetLockException : Exception()
     ...
     */
    @Throws(UnableToGetLockException::class, InterruptedException::class)
    protected abstract fun lock(pUserName: String?, pController: ExtendedMapFeedback?): String?
    ...
    protected abstract fun broadcastCommand(pDoAction: String?,
                                            pUndoAction: String?, pLockId: String?, pController: ExtendedMapFeedback?)

    /**
     * Unlocks the previous lock
     * @param pController TODO
     */
    protected abstract fun unlock(pController: ExtendedMapFeedback?)
    fun registerFilter() {
        logger!!.info("Registering filter")
        mapFeedback.actionRegistry.registerFilter(this)
    }

    fun deregisterFilter() {
        logger!!.info("Deregistering filter")
        mapFeedback.actionRegistry.deregisterFilter(this)
    }

    open fun executeTransaction(pair: ActionPair?, pController: ExtendedMapFeedback?) {
        mFilterEnabled = false
        try {
            mapFeedback.doTransaction("update", pair)
        } finally {
            mFilterEnabled = true
        }
    }
    ...
    companion object {
         ...
         *
         */
        private const val PLUGINS_COLLABORATION_SOCKET = "plugins/collaboration/socket/"
        const val MASTER_HOOK_LABEL = (PLUGINS_COLLABORATION_SOCKET
                + "socket_master_plugin")
        const val SLAVE_HOOK_LABEL = (PLUGINS_COLLABORATION_SOCKET
                + "socket_slave_plugin")
        const val SLAVE_STARTER_LABEL = (PLUGINS_COLLABORATION_SOCKET
                + "socket_slave_starter_plugin")
        const val SLAVE_PUBLISH_LABEL = (PLUGINS_COLLABORATION_SOCKET
                + "socket_publish_map_plugin")
        protected val ROLE_MASTER = Integer.valueOf(0)
        protected val ROLE_SLAVE = Integer.valueOf(1)
        private const val PORT_PROPERTY = "plugins.collaboration.socket.port"
        private const val SOCKET_BASICS_CLASS = "plugins.collaboration.socket.SocketBasics"
        protected const val PASSWORD = SOCKET_BASICS_CLASS + ".password"
        protected const val PASSWORD_DESCRIPTION = (SOCKET_BASICS_CLASS
                + ".password.description")
        protected const val PASSWORD_VERIFICATION = (SOCKET_BASICS_CLASS
                + ".password_verification")
        protected const val PASSWORD_VERIFICATION_DESCRIPTION = (SOCKET_BASICS_CLASS
                + ".password_verification_description")
        protected const val HOST = SOCKET_BASICS_CLASS + ".host"
        protected const val HOST_DESCRIPTION = (SOCKET_BASICS_CLASS
                + ".host.description")
        protected const val PORT = SOCKET_BASICS_CLASS + ".port"
        protected const val PORT_DESCRIPTION = PORT + ".description"
        protected const val TITLE = SOCKET_BASICS_CLASS + ".title"
        protected const val UNKNWON_HOST_EXCEPTION_MESSAGE = (SOCKET_BASICS_CLASS
                + ".unknown_host_exception")
        protected const val CONNECT_EXCEPTION_MESSAGE = (SOCKET_BASICS_CLASS
                + ".connection_exception")
        protected const val SOCKET_CREATION_EXCEPTION_MESSAGE = (SOCKET_BASICS_CLASS
                + ".socket_creation_exception")
        const val SOCKET_TIMEOUT_IN_MILLIES = 500

 ...
 * @date 03.05.2014
 */
class StandaloneMindMapMaster(pFreeMindMain: FreeMindMainMock?, private val mBaseFilePath: File,
                              pPassword: String, pPort: Int) : SocketMaster() {
    private var mServer: ServerSocket? = null
    private var mMasterThread: MasterThread? = null

    private inner class MasterThread : TerminateableThread("StandaloneMaster") {
        private var mLastTimeUserInformationSent: Long = 0
        private var mLastSaveAction: Long = 0
        ...
        init {
            ...
            Runtime.getRuntime().addShutdownHook(Thread {
                println("Automatic save on exit starts...")
                for (mapName in mFileMap.keys) {
                    val extendedMapFeedback = mFileMap[mapName]
                    val map = extendedMapFeedback!!.map
                    val file: File? = map!!.file
                    println("Looking for map $file to be saved...")
                    if (!map!!.isSaved) {
                        try {
                            // save map:
                            map!!.save(file)
        ...
        override fun processAction(): Boolean {
            try {
                ...
                val communication = ServerCommunication(
                        this@StandaloneMindMapMaster, client, null, false)
                communication.start()
            ...
            synchronized(mSessions) {
                for (mapName in mFileMap.keys) {
                    ...
                    synchronized(sessionData!!.mLockMutex) {
                        ...
                        // regular save action:
                        if (!sessionData.mLockEnabled && now - mLastSaveAction > Companion.TIME_BETWEEN_SAVE_ACTIONS_IN_MILLIES) {
                            val map = extendedMapFeedback!!.map
                            val file: File? = map!!.file
                            TerminateableThread.Companion.logger!!.fine("Checking map $file for save action needed.")
                            mLastSaveAction = now
                            if (!map!!.isSaved) {
                                // save map:
                                TerminateableThread.Companion.logger!!.info("Saving map $file now.")
                                map!!.save(file)
                            } else {
                                TerminateableThread.Companion.logger!!.fine("No save necessary.")
                            }
                        }
                    }
                }
        ...
        companion object {
            ...
            private const val TIME_BETWEEN_SAVE_ACTIONS_IN_MILLIES: Long = 60000
	 ...
	 * ()
	 */
    override val mindMapController: MindMapController
        get() {
            throw IllegalArgumentException("No controller here.")
        }
     ...
     */
    init {
        val fileList = mBaseFilePath.list { pDir, pName -> pName.endsWith(".mm") }
        for (i in fileList.indices) {
            val fileName = fileList[i]
            val file = File(mBaseFilePath, fileName)
            logger!!.info("Loading $fileName")
            val readerCreator = FileReaderCreator(file)
        ...
        try {
            mServer = ServerSocket(pPort)
            mServer.setSoTimeout(SocketBasics.Companion.SOCKET_TIMEOUT_IN_MILLIES)
    ...
    companion object {
        ...
        fun main(args: Array<String>) {
            ...
            StandaloneMindMapMaster(FreeMindMainMock(), File(path), "aa", 9001)
        }
    }
}
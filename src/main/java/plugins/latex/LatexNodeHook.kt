...
import freemind.modes.mindmapmode.actions.xml.ActionFilter.FinalActionFilterimport
...
import plugins.collaboration.database.DatabaseStarterimport
 ...
 */
class LatexNodeHook : PermanentMindMapNodeHookAdapter() {
    private var equation = "\\mbox{I}^\\u000cgcolor{ff0000}{\\heartsuit}\\mbox{HotEqn}"
    private val viewers: MutableSet<Component>

    /**
     */
    init {
        viewers = LinkedHashSet()
    }
    ...
    private fun deleteViewer(nodeView: NodeView?) {
        if (viewers.isEmpty()) {
            return
        }
        val contentPane = nodeView!!.getContentPane()
        val componentCount = contentPane!!.componentCount
        for (i in 0 until componentCount) {
            val component = contentPane.getComponent(i)
    ...
    override fun invoke(node: MindMapNode?) {
        for (view in mindMapController!!.view!!.getViewers(node)) {
            createViewer(view)
        }
        super.invoke(node)
    }

    private fun createViewer(view: NodeView?) {
        val comp = JZoomedHotEqn(this)
        viewers.add(comp)
    ...
    fun setContent(key: String?, content: String) {
        equation = content
        val iterator: Iterator<*> = viewers.iterator()
        while (iterator.hasNext()) {
            val comp = iterator.next() as JZoomedHotEqn
            comp.setModel(this)
        }

 ...
 * @date 14.12.2011
 */
class AddMapImageToNodeAction : MindMapNodeHookAdapter() {
    /* (non-Javadoc)
    ...
    override fun invoke(pNode: MindMapNode?) {
        ...
        for (node in selecteds) {
            ...
            if (hook != null) {
                FreeMindMapController.Companion.addPictureToNode(hook, mindMapController)
            }
        }
    }

    companion object {
        const val NODE_CONTEXT_PLUGIN_NAME = "plugins/map/MapDialog_AddMapImageToNodeAction.properties"
    }
}
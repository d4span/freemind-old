 ...
 * @date 28.08.2012
 */
abstract class MapMarkerBase(protected var mMapDialog: MapDialog) : JLabel(), MapMarker {
    /**
     * @param pSel
     */
    var isSelected = false
    protected var mBulletColor = Color.BLACK
    protected var mSelectedBackgroundColor = Color.GRAY
    protected var mBackgroundColor = Color.WHITE
    protected var mTextWidthShorteningPerZoom = floatArrayOf(0f, 0f, 0.1f, 0.2f,
            0.3f, 0.4f, 0.5f, 0.75f, 0.8f, 0.9f, 0.95f, 0.97f)
    protected var mTextHeightShorteningPerZoom = floatArrayOf(0f, 0f, 0.0f, 0.0f,
    ...
    override fun paint(pGraphics: Graphics, position: Point, pRadio: Int) {
        val g = pGraphics.create()
        paintCenter(g, position)
        if (isSelected) {
            g.color = mSelectedBackgroundColor
        } else {
            g.color = mBackgroundColor
        }
        val newPoint = adjustToTextfieldLocation(position)
        val map = mMapDialog.map
        var inversZoom = (map.freeMindMapController.maxZoom
                - map!!.zoom)
        inversZoom = Math.min(mTextWidthShorteningPerZoom.size - 1, inversZoom)
        val destWidth = Math.min(width, map.width)
        val normalWidth = (destWidth * (1f - mTextWidthShorteningPerZoom[inversZoom])).toInt()
        val destHeight = Math.min(height, map.height)
        val normalHeight = (destHeight * (1f - mTextHeightShorteningPerZoom[inversZoom])).toInt()
        val node_y = newPoint.y
        val node_x = newPoint.x
        g.fillRect(node_x, node_y, normalWidth, normalHeight)
        g.color = mBulletColor
    ...
    protected open fun paintCenter(g: Graphics, position: Point) {
        g.color = mBulletColor
     ...
     * 0,0 is likely a hit...).
     */
    fun checkHit(pX: Int, pY: Int): Boolean {
        var x = pX
        var y = pY
        // translation:
        x -= CIRCLE_RADIUS
        y += CIRCLE_RADIUS
        return if (x >= 0 && y >= 0 && x <= width && y <= height) true else pX * pX + pY * pY <= CIRCLE_RADIUS * CIRCLE_RADIUS
    ...
    override fun setLayer(pLayer: Layer) {
        sLayer = pLayer
    }
    ...
    override fun getStyleAssigned(): Style {
        return sStyle
    }
    ...
    override fun getBackColor(): Color {
    ...
    companion object {
         ...
         *
         */
        private const val CIRCLE_DIAMETER = CIRCLE_RADIUS * 2
        protected var logger: Logger? = null
        private val sBasicStroke = BasicStroke()
        private var sLayer = Layer("basis")
        private val sStyle = Style()
        fun adjustToTextfieldLocation(position: Point?): Point {

...
java.awt.*
class MapSearchMarkerLocation(pMapDialog: MapDialog, pNewPlace: Place) : MapMarkerBase(pMapDialog) {
    val place: Place
	 ...
	 * java.awt.Point)
	 */
    override fun paintCenter(pG: Graphics, pPosition: Point) {
        if (isSelected) {
            val g2 = pG as Graphics2D
            val oldStroke = g2.stroke
            g2.stroke = BasicStroke(4)
            val xo: Int = pPosition.x - MapMarkerBase.Companion.CIRCLE_RADIUS * CIRCLE_SELECTED_FACTOR
            val xu: Int = pPosition.x + MapMarkerBase.Companion.CIRCLE_RADIUS * CIRCLE_SELECTED_FACTOR
            val yo: Int = pPosition.y - MapMarkerBase.Companion.CIRCLE_RADIUS * CIRCLE_SELECTED_FACTOR
            val yu: Int = pPosition.y + MapMarkerBase.Companion.CIRCLE_RADIUS * CIRCLE_SELECTED_FACTOR
    ...
    override fun toString(): String {
        return ("MapSearchMarkerLocation for search text "
                + place.displayName + " at " + lat + " " + lon)
    }

    override fun setLat(pLat: Double) {
        place.lat = pLat
    }

    override fun setLon(pLon: Double) {
        place.lon = pLon
    }

    companion object {
        const val CIRCLE_SELECTED_FACTOR = 2
    }
}
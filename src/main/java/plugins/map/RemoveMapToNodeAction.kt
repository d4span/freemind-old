 ...
 * @date 13.12.2011
 */
class RemoveMapToNodeAction : MindMapNodeHookAdapter() {
    /* (non-Javadoc)
    ...
    override fun invoke(pNode: MindMapNode?) {
        ...
        for (node in selecteds) {
            val hook: MapNodePositionHolderBase = MapNodePositionHolder.Companion.getHook(node)
            if (hook != null) {
                val nodeSelected = Arrays.asList(*arrayOf(node))
                // double add = remove
                mindMapController!!.addHook(node, nodeSelected,
    ...
    companion object {
        const val NODE_CONTEXT_PLUGIN_NAME = "plugins/map/MapDialog_RemoveMapToNode.properties"
    }
}
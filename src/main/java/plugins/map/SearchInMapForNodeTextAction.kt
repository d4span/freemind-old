 ...
 * @date 1.4.2012
 */
class SearchInMapForNodeTextAction : MindMapNodeHookAdapter() {
    /*
	 * (non-Javadoc)
	 * 
	 * @see
    ...
    override fun invoke(pNode: MindMapNode?) {
        ...
        for (node in selecteds) {
            ...
            val plainText = HtmlTools.htmlToPlain(node!!.text!!)
            if (sb.length != 0) {
                sb.append(" ")
            }
            sb.append(plainText)
        }
        val searchText = sb.toString()
        ...
        val showResult = OptionalDontShowMeAgainDialog(
                ...
                object : TextTranslator {
                    override fun getText(pKey: String?): String {
                        var text = mindMapController
                                .getText(pKey)
                        if (pKey.equals(REALLY_SEARCH_FOR_NODE_TEXT_IN_WEB)) {
                            val messageArguments = arrayOf<Any>(searchText)
                            val formatter = MessageFormat(text)
        ...
        if (registration != null) {
            // is the map open?
            var mapDialog = registration.mapDialog
            if (mapDialog == null) {
                // if not, open it!
                mindMapController!!.createModeControllerHook(
                        MapDialog.Companion.MAP_HOOK_NAME)
            }
            mapDialog = registration.mapDialog
            if (mapDialog != null) {
    ...
    companion object {
         ...
         *
         */
        private const val REALLY_SEARCH_FOR_NODE_TEXT_IN_WEB = "really_search_for_node_text_in_web"
        const val NODE_CONTEXT_PLUGIN_NAME = "plugins/map/MapDialog_SearchInMapForNodeTextAction.properties"
    }
}
 ...
 * @date 12.12.2011
 */
class ShowMapToNodeAction : MindMapNodeHookAdapter() {
    /*
	 * (non-Javadoc)
	 * 
	 * @see
    ...
    override fun invoke(pNode: MindMapNode?) {
        ...
        if (registration != null) {
            ...
            if (mapDialog == null) {
                // if not, open it!
                mindMapController!!.createModeControllerHook(MapDialog.Companion.MAP_HOOK_NAME)
            }
            mapDialog = registration.mapDialog
            if (mapDialog != null) {
                mapDialog.freeMindMapController.showSelectedNodes()
                mapDialog.mapDialog.requestFocus()
            } else {
    ...
    companion object {
        const val NODE_CONTEXT_PLUGIN_NAME = "plugins/map/MapDialog_ShowMapToNode.properties"
    }
}
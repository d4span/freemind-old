...
class TileImage : ImageObserver {
    private var mTiles: Array<Array<Tile?>>? = null
    private var mTilesPresent = false
    var isImageCreated = false
        private set
    private var mImage: BufferedImage? = null
    private var mWaitingForCallbacks = 0
    private var mDx = 0
    private var mDy = 0
    val isLoaded: Boolean
        get() {
            if (!mTilesPresent) return false
            for (i in mTiles!!.indices) {
                val tiles = mTiles!![i]
                for (j in tiles.indices) {
                    val tile = tiles[j]
                    if (!tile!!.isLoaded && !tile.hasError()) {
                        println("Tile " + tile + " is not loaded:"
                                + tile.status)
                        return false
                    }
                }
    ...
    private fun createImage() {
        val tileImage00 = mTiles!![0][0]!!.image
        val height = tileImage00.height
        val width = tileImage00.width
        mImage = BufferedImage(height * mTiles!![0].size, width
                * mTiles!!.size, BufferedImage.TYPE_INT_RGB)
        val graphics = mImage!!.graphics as Graphics2D
        for (i in mTiles!!.indices) {
            val tiles = mTiles!![i]
            for (j in tiles.indices) {
                val tile = tiles[j]
                val done = graphics.drawImage(tile!!.image, i
                        * height, j * width, this)
                if (!done) {
    ...
    val isDrawingDone: Boolean
        get() = mWaitingForCallbacks <= 0
    ...
    fun load(pCodedImage: String?) {
        try {
            mImage = ImageIO.read(ByteArrayInputStream(fromBase64(pCodedImage!!)))
            mTilesPresent = false
            isImageCreated = true
        } catch (e: IOException) {
    ...
    fun load(pFileName: File?) {
        try {
            mImage = ImageIO.read(FileInputStream(pFileName))
            mTilesPresent = false
            isImageCreated = true
        } catch (e: IOException) {
            Resources.getInstance().logException(e)
     ...
     * @param pDx
     */
    fun setTiles(pDimension: Int, pX: Int, pY: Int, pZoom: Int,
                 mTileController: TileController, pLogger: Logger, pDx: Int, pDy: Int) {
        mDx = pDx
        mDy = pDy
        mTiles = Array(pDimension) { arrayOfNulls(pDimension) }
        for (i in 0 until pDimension) {
            for (j in 0 until pDimension) {
	 ...
	 * int, int, int, int)
	 */
    override fun imageUpdate(pImg: Image, pInfoflags: Int, pX: Int, pY: Int,
                             pWidth: Int, pHeight: Int): Boolean {
        mWaitingForCallbacks--
        if (isDrawingDone) {
            drawCross()
        }

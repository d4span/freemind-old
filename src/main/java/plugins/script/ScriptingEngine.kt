 ...
 * @author foltin
 */
class ScriptingEngine : MindMapHookAdapter() {
    interface ErrorHandler {
        fun gotoLine(pLineNumber: Int)
    }

    override fun startupMapHook() {
        super.startupMapHook()
        val node = mindMapController!!.map!!.rootNode
        val booleanHolder = BooleanHolder(false)
        // check for installed script:
        val scriptLocation = getResourceString("ScriptLocation")
    ...
    private fun performExternalScript(pScriptLocation: String,
                                      pNode: MindMapNode, pBooleanHolder: BooleanHolder) {
        // get cookies from base plugin:
        val reg = getPluginBaseClass() as ScriptingRegistration
        val scriptContent = getFile(File(pScriptLocation)) ?: return
        executeScript(pNode, pBooleanHolder, scriptContent,
                mindMapController!!, object : ErrorHandler {
            override fun gotoLine(pLineNumber: Int) {}
        }, System.out, reg.scriptCookies)
    }

    private fun performScriptOperation(node: MindMapNode?,
                                       pAlreadyAScriptExecuted: BooleanHolder) {
        // depth first:
        val iter: Iterator<MindMapNode?>? = node!!.childrenUnfolded()
        while (iter!!.hasNext()) {
            val element = iter.next()
            performScriptOperation(element, pAlreadyAScriptExecuted)
        }
        // FIXME: Scripts
        val attributes = null ?: return
...
//		}
    ...
    companion object {
        ...
        private val sScriptCookies: HashMap<*, *> = HashMap<Any, Any>()
        var logger: Logger? = null
        fun findLineNumberInString(resultString: String?, lineNumber: Int): Int {
            var lineNumber = lineNumber
            val pattern = Pattern.compile(
                    ".*@ line ([0-9]+).*", Pattern.DOTALL)
            val matcher = pattern.matcher(resultString)
            if (matcher.matches()) {
                lineNumber = matcher.group(1).toInt()
            }
            return lineNumber
        }
         ...
         */
        fun executeScript(node: MindMapNode?,
                          pAlreadyAScriptExecuted: BooleanHolder, script: String?,
                          pMindMapController: MindMapController,
                          pErrorHandler: ErrorHandler, pOutStream: PrintStream,
                          pScriptCookies: HashMap<*, *>?): Boolean {
            // ask user if first script:
            var script = script
            val frame = pMindMapController.frame
            if (!pAlreadyAScriptExecuted.value) {
                val showResult = OptionalDontShowMeAgainDialog(
                        frame.jFrame, pMindMapController.selectedView,
                        "really_execute_script", "confirmation",
                        pMindMapController,
                        StandardPropertyHandler(
                                pMindMapController.controller!!,
            ...
            pAlreadyAScriptExecuted.value = true
            val binding = Binding()
            ...
            if (script!!.startsWith("=")) {
                ...
                val indexOfEquals = script.indexOf('=')
                if (indexOfEquals > 0) {
                    val start = script.substring(0, indexOfEquals)
		 ...
		 */
            ...
            var value: Any? = null
            var e1: GroovyRuntimeException? = null
            var e2: Throwable? = null
            var filePerm = isPreferenceTrue(executeWithoutFileRestriction)
            var networkPerm = isPreferenceTrue(executeWithoutNetworkRestriction)
            var execPerm = isPreferenceTrue(executeWithoutExecRestriction)
            if (isPreferenceTrue(signedScriptsWithoutRestriction)) {
                val isSigned = SignedScriptHandler().isScriptSigned(script,
                        pOutStream)
                if (isSigned) {
                    filePerm = true
                    networkPerm = true
            ...
            val scriptingSecurityManager = ScriptingSecurityManager(
                    filePerm, networkPerm, execPerm)
            val securityManager = System
                    .getSecurityManager() as FreeMindSecurityManager
            val compilerConfig = CompilerConfiguration()
            compilerConfig
                    .addCompilationCustomizers(ASTTransformationCustomizer(
                            ThreadInterrupt::class.java))
            try {
                ...
                val shell: GroovyShell = object : GroovyShell(null, binding,
                        compilerConfig) {
                    /**
                    ...
                    override fun evaluate(`in`: Reader, fileName: String): Any {
                        ...
                        return try {
                            ...
                            if (script != null) {
                                InvokerHelper.removeClass(script!!.javaClass)
                ...
                /* restore preferences (and assure that the values are unchanged!). */frame.setProperty(
                        FreeMind.RESOURCES_EXECUTE_SCRIPTS_WITHOUT_ASKING,
		 ...
		 */if (e1 != null) {
                val resultString = e1.message
                pOutStream.print("message: $resultString")
                val module = e1.module
                val astNode = e1.node
                var lineNumber = -1
                lineNumber = module?.lineNumber
                        ?: (astNode?.lineNumber ?: findLineNumberInString(resultString, lineNumber))
            ...
            if (e2 != null) {
                ...
                pOutStream.print(e2.message)
                val cause = if (e2.cause != null) e2.cause!!
                        .message else ""
                val message = if (e2.message != null) e2.message else ""
                val errorMessage = (e2.javaClass.name
                        + ": "
                        + cause
                        + (if (cause!!.length != 0 && message!!.length != 0) ", " else "") + message)
                pOutStream.println(errorMessage)
                if (e2 !is InterruptedException) {
                    EventQueue.invokeLater { pMindMapController.controller!!.errorMessage(errorMessage) }
                }
                return false
            }

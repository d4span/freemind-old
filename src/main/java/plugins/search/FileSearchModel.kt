 ...
 */
class FileSearchModel internal constructor(logger: Logger?) {
    private enum class FileAttribute {
    ...
    fun doSearch(q: Query?, hitsPerPage: Int, searcher: IndexSearcher?): TopDocs {
    ...
    fun getQuery(querystring: String?): Query {
        // the "title" arg specifies the default field to use
        // when no field is explicitly specified in the query.
        return QueryParser(Version.LUCENE_46,
                FileAttribute.contents.name, analyzer).parse(querystring)
    }

    @Throws(IOException::class)
    fun getSearcher(index: Directory?): IndexSearcher {
     ...
     */
    @Throws(IOException::class)
    fun indexFileOrDirectory(vararg fileName: String): Directory {
        val index: Directory = RAMDirectory()
        val config = IndexWriterConfig(Version.LUCENE_46,
                analyzer)
    ...
    @Throws(IOException::class)
    fun indexFileOrDirectory(vararg fileName: File): Directory {
        val index: Directory = RAMDirectory()
        val config = IndexWriterConfig(Version.LUCENE_46,
                analyzer)
        val writer = IndexWriter(index, config)
     ...
     */
    @Throws(IOException::class)
    fun indexFileOrDirectory(fileName: String): Directory {
        return indexFileOrDirectory(*arrayOf(fileName))
    }

    @Throws(IOException::class)
    fun indexFileOrDirectory(fileName: File): Directory {
        return indexFileOrDirectory(*arrayOf(fileName))
    }
     ...
     */
    @Throws(IOException::class)
    private fun indexFileOrDirectoryCore(fileName: String, writer: IndexWriter): Int {
        // ===================================================
        ...
        if (!file.exists()) {
            throw FileNotFoundException("Can't open $fileName")
        }
        addFiles(file)
    ...
    @Throws(IOException::class)
    private fun indexFileOrDirectoryCore(fileName: File, writer: IndexWriter): Int {
        // ===================================================
        // gets the list of files in a folder (if user has submitted
        // the name of a folder) or gets a single file name (is user
        // has submitted only the file name)
    ...
    @Throws(IOException::class)
    fun indexFiles(writer: IndexWriter): Int {
        ...
        for (f in queue) {
            ...
            try {
                ...
                // ===================================================
                fr = FileReader(f)
                doc.add(TextField(FileAttribute.contents.name, fr))
                doc.add(StringField(FileAttribute.path.name, f.path,
                        Field.Store.YES))
                doc.add(StringField(FileAttribute.filename.name, f
        ...
        val newNumDocs = writer.numDocs()
        val added = newNumDocs - originalNumDocs
        _logger!!.info("$added documents added.")
        queue.clear()
        return added
    }

    fun getPath(d: Document): String {
        return d[FileAttribute.path.name]
    }

    fun getFilename(d: Document): String {
        return d[FileAttribute.filename.name]
    }

    @Throws(IOException::class)
    fun getFilepathsFromSearchResults(searcher: IndexSearcher,
                                      results: TopDocs): Array<String?> {
        val scoreDocs = results.scoreDocs
        val fileNames = arrayOfNulls<String>(scoreDocs.size)
        for (i in scoreDocs.indices) {
            val scoreDoc = scoreDocs[i]
            val d = searcher.doc(scoreDoc.doc)
    ...
    companion object {
        const val FREEMIND_FILENAME_SUFFIX = ".mm"

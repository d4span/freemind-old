...
class Search(private val _logger: Logger?) {
    private var indexer: FileSearchModel? = null
    private var searcher: IndexSearcher? = null
    private var hits: Array<ScoreDoc>
    @Throws(IOException::class, ParseException::class)
    fun runSearch(searchString: String, mapsFiles: Array<File?>?): Array<SearchResult?> {
        indexer = FileSearchModel(_logger)
        _logger!!.fine("runSearch :$searchString in $mapsFiles")
        val index = indexer.indexFileOrDirectory(*mapsFiles)
        searcher = indexer!!.getSearcher(index)
        _logger.fine("Run search")
        val query = indexer!!.getQuery(searchString)
        val reader = DirectoryReader.open(index)
        val results = indexer!!.doSearch(query, reader.numDocs(), searcher)
        hits = results!!.scoreDocs
        _logger.fine("Returned: " + hits.size + " results")
        val listData = arrayOfNulls<SearchResult>(hits.size)
        for (i in hits.indices) {
            var d: Document
            try {
                val docId = hits[i].doc
                d = searcher!!.doc(docId)
                listData[i] = SearchResult(i + 1, hits[i].doc,
                        indexer!!.getFilename(d), indexer!!.getPath(d),
                        hits[i].score, hits[i].shardIndex)
                _logger.fine(i.toString() + " " + listData[i])
            } catch (e: IOException) {
                _logger.warning("Failed:" + e.message)
                e.printStackTrace()
            }
        }
        return listData
    ...
    inner class SearchResult(val rank: Int, val docId: Int, val fileName: String, val path: String,
                             val score: Float, val shardIndex: Int) {

 ...
 * @author foltin
 */
class Base64Tests : FreeMindTestBase() {
    @Throws(Exception::class)
    fun testDifferentBase64ers() {
        val input = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ"
        val output = encode64(input.toByteArray())
        println(output)
        // byte[] output2 = CommonsCodecBase64.encodeBase64(input.getBytes());
        // String string2 = new String(output2);
        // System.out.println(string2);
        val expected = "YWJjZGVmZ2hpamtsbW5vcHFyc3R1dnd4eXpBQkNERUZHSElKS0xNTk9QUVJTVFVWV1hZWg=="
        assertEquals(expected, output)
        val back = String(decode64(output)!!)

 ...
 * @date 08.04.2013
 */
class FreeMindTaskTests : FreeMindTestBase() {
    /* (non-Javadoc)
     ...
     */
    (pFrame: JFrame?) : FreeMindTask(pFrame!!, Companion.AMOUNT_OF_TIME, "TestTask") {
        var i = Companion.AMOUNT_OF_TIME
        ...
        override fun processAction(): Boolean {
            sleep(200)
            i--
            mProgressDescription = ProgressDescription("Format {0}", arrayOf(i))
            return true
        }
        ...
        companion object {
             ...
             *
             */
            private const val AMOUNT_OF_TIME = 10
        }
    }
    ...
    fun testTestTask() {
        val frame = JFrame("Hi")
        val task: TestTask = TestTask(frame)
        val button = JButton("Hello from FreeMind")
        frame.add(button, BorderLayout.CENTER)
        button.addActionListener { task.i = -100 }
        frame.setSize(500, 300)
        frame.defaultCloseOperation = JFrame.EXIT_ON_CLOSE
        frame.isVisible = true
        // the task is called by the event queue.
        EventQueue.invokeAndWait { task.start() }

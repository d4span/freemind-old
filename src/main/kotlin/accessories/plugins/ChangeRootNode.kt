 ...
 * @date 01.10.2011
 */
class ChangeRootNode : MindMapNodeHookAdapter() {
    override fun invoke(node: MindMapNode?) {
        // we dont need node.
        val focussed = mindMapController!!.selected!!
        val rootNode = mindMapController!!.rootNode
        mindMapController!!.doTransaction(
                TRANSACTION_NAME,
                ActionPair(getAction(focussed), getAction(rootNode)))
    }
     ...
     */
    private fun getAction(pNode: MindMapNode): XmlAction {
        val action = ChangeRootNodeAction()
        action.node = mindMapController!!.getNodeID(pNode)
        return action
    }

    class Registration(controller: ModeController, map: MindMap) : HookRegistration, MenuItemEnabledListener, ActorXml {
        ...
        override fun isEnabled(pItem: JMenuItem?, pAction: Action?): Boolean {
            return controller.selecteds.size == 1
        }
        ...
        override fun register() {
            controller.actionRegistry.registerActor(this,
                    doActionClass)
        }
        ...
        override fun act(pAction: XmlAction?) {
            if (pAction is ChangeRootNodeAction) {
                val focussed: MindMapNode = controller.getNodeFromID(pAction.node)
                if (focussed.isRoot) {
                ...
                while (!nodes.isEmpty()) {
                    val child = nodes.firstElement() as MindMapNode
                    logger.fine("Removing viewers for $child")
                    nodes.removeAt(0)
                    nodes.addAll(child.children)
                    val viewers: Collection<NodeView> = Vector(view!!.getViewers(child))
                    for (viewer in viewers) {
                        view.removeViewer(child, viewer)
                    }
                }
                for (i in view!!.componentCount - 1 downTo 0) {
                    val comp = view.getComponent(i)
                    if (comp is NodeView
                            || comp is NodeMotionListenerView) {
                        view.remove(comp)
                    }
                }
                view.initRoot()
                ...
                controller.select(focussed,
                        getVectorWithSingleElement<MindMapNode?>(focussed))
		 ...
		 */
        override val doActionClass: Class<ChangeRootNodeAction>
            get() = ChangeRootNodeAction::class.java
    }

    companion object {
        private const val TRANSACTION_NAME = "ChangeRootNode"
    }
}
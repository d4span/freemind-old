 ...
 * @author foltin
 */
class CreationModificationPlugin
/**
 *
 */
    : PermanentMindMapNodeHookAdapter() {
    private var tooltipFormat: String? = null
    private fun setStyle(node: MindMapNode?) {
        ...
        val formatter = MessageFormat(tooltipFormat)
        val message = formatter.format(messageArguments)
        setToolTip(node, name, message)
        logger!!.finest(this.toString() + "Tooltip for " + node + " with parent "
                + node.parentNode + " is " + message)
    }
     ...
     *
     */
    private fun removeToolTipRecursively(node: MindMapNode?) {

 ...
 * @date 11.10.2013
 */
class DeactivateCloneAction : MindMapNodeHookAdapter(), MenuItemEnabledListener {
    /*
	 * (non-Javadoc)
	 * 
	 * @see
    ...
    override fun invoke(pNode: MindMapNode?) {
        super.invoke(pNode)
        var hook: ClonePlugin = ClonePlugin.Companion.getHook(pNode)
        if (hook != null) {
            // has it been removed due to the deregister in the meantime?
            hook = ClonePlugin.Companion.getHook(pNode)
            if (hook != null) {
                hook.removeHook()
            }
        }
    ...
    override fun isEnabled(pItem: JMenuItem?, pAction: Action?): Boolean {
        return registration.isEnabled(pItem, pAction)
    }

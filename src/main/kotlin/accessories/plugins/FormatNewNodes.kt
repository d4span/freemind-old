 ...
 * @author foltin
 */
class FormatNewNodes(controller: ModeController, map: MindMap?) : ActionHandler, ActionFilter, HookRegistration {
    private val controller: MindMapController
    private val logger: Logger
    private val formatActions: HashMap<String, XmlAction>

    init {
        this.controller = controller as MindMapController
        logger = controller.frame.getLogger(this.javaClass.name)
        formatActions = HashMap()
    }
    ...
    private fun detectFormatChanges(doAction: XmlAction?) {
        ...
        } else if (doAction is FormatNodeAction) {
            formatActions[doAction.javaClass.name] = doAction
        }
    }
    ...
    override fun filterAction(pair: ActionPair?): ActionPair? {
        if (pair!!.doAction is NewNodeAction) {
            val newNodeAction = pair.doAction as NewNodeAction?
            // add to a compound the newNodeAction and the other formats we
            // have:
            val compound = CompoundAction()
            compound.addChoice(newNodeAction)
            for (formatAction in formatActions.values) {
                // deep copy:
                val copiedFormatAction = deepCopy(formatAction) as FormatNodeAction
                copiedFormatAction.node = newNodeAction!!.newId
                compound.addChoice(copiedFormatAction)
            }
            return ActionPair(compound, pair.undoAction)
        }
        return pair
    }
}
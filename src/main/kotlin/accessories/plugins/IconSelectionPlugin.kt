 ...
 */
class IconSelectionPlugin
/**
 */
    : MindMapNodeHookAdapter() {
    override fun invoke(rootNode: MindMapNode?) {
        // we dont need node.
        val focussed: NodeView? = getController()!!.selectedView
        ...
        val frame = getController()!!.frame as FreeMind
        val selectionDialog = IconSelectionPopupDialog(
                frame.jFrame, actions, frame)
        val mapView = controller.view
        mapView!!.scrollNodeToVisible(focussed, 0)
        selectionDialog.pack()
        setDialogLocationRelativeTo(selectionDialog, focussed)
        selectionDialog.isModal = true
        selectionDialog.isVisible = true
        // process result:
        val result = selectionDialog.result
        if (result >= 0) {
            val action = actions[result] as Action?
            action!!.actionPerformed(ActionEvent(action, 0, "icon",
                    selectionDialog.modifiers))
        }
    } // /* (non-Javadoc)

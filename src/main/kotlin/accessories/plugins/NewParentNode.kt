 ...
 */
    ...
    override fun invoke(rootNode: MindMapNode?) {
        ...
        mindMapController!!.sortNodesByDepth(selecteds)
        if (focussed.isRoot) {
            if (selecteds.size == 1) {
                // only root is selected. we try to create a new root:
                val children = Vector(rootNode!!.children)
                // copy only root.
                val rootContent = mindMapController!!.copySingle()
                // and paste it directly again.
                mindMapController!!.paste(rootContent, rootNode)
                val childrenNew = Vector(rootNode.children)
                /*
				 * look for the new node as the difference between former
				 * children and new children.
				 */
                var rootCopy: MindMapNode? = null
                var found = false
                val it: Iterator<MindMapNode> = childrenNew.iterator()
                while (it.hasNext()) {
                    rootCopy = it.next()
                ...
                mindMapController!!.clearNodeContents(rootNode)
    ...
    private fun moveToNewParent(rootNode: MindMapNode?,
                                selectedNode: MindMapNode, selectedNodes: List<MindMapNode?>): MindMapNode? {
        // Create new node in the position of the selectedNode
        val selectedParent = selectedNode.parentNode
        val childPosition = selectedParent!!.getChildPosition(selectedNode)
        val newNode = mindMapController!!.addNewNode(selectedParent,
                childPosition, selectedNode.isLeft)
        return moveToOtherNode(rootNode, selectedNodes, selectedParent, newNode)
    }

    private fun moveToOtherNode(rootNode: MindMapNode?,
                                nodesToBeMoved: List<MindMapNode?>, selectedParent: MindMapNode?, newNode: MindMapNode?): MindMapNode? {
        ...
        val copy = mindMapController!!.cut(nodesToBeMoved)
        mindMapController!!.paste(copy, newNode!!)
        mindMapController!!.select(selectedParent, getVectorWithSingleElement(selectedParent))

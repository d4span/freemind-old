 ...
 *
 */
    : PermanentMindMapNodeHookAdapter(), ActionHandler {
    ...
    override fun invoke(node: MindMapNode?) {
        ...
        if (alreadyUsed == false) {
            color = Color.YELLOW
            // new register:
            mindMapController!!.actionRegistry.registerHandler(this)
            alreadyUsed = true
        }
    }

    override fun shutdownMapHook() {
        mindMapController!!.actionRegistry.deregisterHandler(this)
    ...
    override fun executeAction(action: XmlAction?) {
        if (action is EditNodeAction) {
            // there is an edit action.
            val node = mindMapController!!.getNodeFromID(
                    action.node!!)

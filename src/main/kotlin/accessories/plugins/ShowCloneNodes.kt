        ...
        if (!newSelecteds.isEmpty()) {
            EventQueue.invokeLater {
                mindMapController.select(
                        newSelecteds[0], newSelecteds)
            }
        }
    }

    protected fun addClonesToList(newSelecteds: Vector<MindMapNode?>, node: MindMapNode?) {
        val hook: ClonePlugin = ClonePlugin.Companion.getHook(node)
        if (hook != null) {
            // original found. 
            val clones = hook.cloneNodes

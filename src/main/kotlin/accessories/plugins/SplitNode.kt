 ...
 */
    ...
    override fun invoke(node: MindMapNode?) {
        super.invoke(node)
        val list = mindMapController!!.selecteds
        for (next in list) {
            splitNode(next)
        }
    }

    private fun splitNode(node: MindMapNode?) {
        if (node!!.isRoot) {
            return
        }
        val text = node.toString()
        val parts = splitNode(text)
        if (parts == null || parts.size == 1) {
            return
        }
        val c = mindMapController!!
        var firstPartNumber = 0
        while (parts[firstPartNumber] == null) {
            firstPartNumber++
        }
        c.setNodeText(node, parts[firstPartNumber])
        val parent = node.parentNode
        val nodePosition = parent!!.getChildPosition(node) + 1
        for (i in parts.size - 1 downTo firstPartNumber + 1) {
            val lowerNode = c.addNewNode(parent, nodePosition,
                    node.isLeft)
            val part = parts[i] ?: continue
            lowerNode!!.color = node.color
            lowerNode.font = node.font
    ...
    private fun splitNode(text: String): Array<String?>? {
        if (text.startsWith("<html>")) {
            ...
            try {
                kit.read(buf, doc, 0)
                val parent = getParentElement(doc) ?: return null
                val elementCount = parent.elementCount
                var notEmptyElementCount = 0
                parts = arrayOfNulls(elementCount)
                for (i in 0 until elementCount) {
                    val current = parent.getElement(i)
                    val start = current.startOffset
                    val end = current.endOffset
                    val paragraphText = doc
                            .getText(start, end - start).trim { it <= ' ' }
                    if (paragraphText.length > 0) {
                        val out = StringWriter()
                        FixedHTMLWriter(out, doc, start, end - start)
                                .write()
    ...
    private fun getParentElement(doc: HTMLDocument): Element? {
        val htmlRoot = doc.defaultRootElement
        var parentCandidate = htmlRoot.getElement(htmlRoot
                .elementCount - 1)
        do {
            if (parentCandidate.elementCount > 1) {
                return parentCandidate
            }
            parentCandidate = parentCandidate.getElement(0)
        } while (!(parentCandidate.isLeaf || parentCandidate.name
                        .equals("p-implied", ignoreCase = true)))
        return null
    }
}
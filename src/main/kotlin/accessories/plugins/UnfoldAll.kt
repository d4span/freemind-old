 ...
 */
    ...
    class Registration(controller: ModeController, map: MindMap) : HookRegistration, MouseWheelEventHandler {
        private val controller: MindMapController
        private val mMap: MindMap
        private val logger: Logger
        private val hookInstance: UnfoldAll
        ...
        override fun handleMouseWheelEvent(e: MouseWheelEvent?): Boolean {
            if (e!!.modifiers and InputEvent.ALT_MASK != 0) {
                logger.info("handleMouseWheelEvent entered.")
                val rootNode = mMap.rootNode
                if (e.wheelRotation > 0) {
                    hookInstance.unfoldOneStage(rootNode)
                } else {
                    // this is to avoid having selected nodes getting folded.
                    controller.select(controller.view!!.root)
    ...
    override fun invoke(node: MindMapNode?) {
        var node = node
        super.invoke(node)
        val foldState = xmlToBoolean(getResourceString("foldingState"))
        val foldingType = getResourceString("foldingType")
        val applyTo = getResourceString("applyTo")
        if ("root" == applyTo) {
            node = mindMapController!!.rootNode
        }
        if (foldingType == "All") {
            if (foldState) {
                foldAll(node)
    ...
    /**
     */
    protected fun unfoldOneStage(node: MindMapNode?) {
        var minDepth = getMinDepth(node)
        if (minDepth < Int.MAX_VALUE) minDepth++
        unfoldStageN(node, minDepth)
    }

    /**
     */
    protected fun foldOneStage(node: MindMapNode?) {
        foldStageN(node, getMaxDepth(node) - 1)
    }

    /**
     */
    protected fun foldAll(node: MindMapNode?) {
    ...
    fun unfoldAll(node: MindMapNode?) {
     ...
     * node to start from.
     */
    fun foldLastBranches(node: MindMapNode?) {
        var nodeHasChildWhichIsLeave = false
        run {
            val i: Iterator<MindMapNode?>? = node!!.childrenUnfolded()
            while (i!!.hasNext()) {
                val child = i.next()
    ...
    protected fun setFolded(node: MindMapNode?, state: Boolean) {
        if (node!!.hasChildren() && node.isFolded != state) {
            mindMapController!!.setFolded(node, state)
        }
    }

    fun unfoldStageN(node: MindMapNode?, stage: Int) {
        val k = depth(node)
        if (k < stage) {
            setFolded(node, false)
            val i: Iterator<MindMapNode?>? = node!!.childrenUnfolded()
    ...
    fun foldStageN(node: MindMapNode?, stage: Int) {
        val k = depth(node)
        if (k < stage) {
            setFolded(node, false)
            val i: Iterator<MindMapNode?>? = node!!.childrenUnfolded()
    ...
    fun getMinDepth(node: MindMapNode?): Int {
        if (node!!.isFolded) return depth(node)
        if (!node.hasChildren()) return Int.MAX_VALUE
        var k = Int.MAX_VALUE
        val i: Iterator<MindMapNode?>? = node.childrenUnfolded()
        while (i!!.hasNext()) {
            val l = getMinDepth(i.next())
            if (l < k) k = l
        }
        return k
    }
    ...
    /**
     */
    protected fun getMaxDepth(node: MindMapNode?): Int {
        if (node!!.isFolded || !node.hasChildren()) return depth(node)
        var k = 0
        val i: Iterator<MindMapNode?>? = node.childrenUnfolded()
        while (i!!.hasNext()) {
            val l = getMaxDepth(i.next())
            if (l > k) k = l
        }
        return k
    }

    protected fun depth(node: MindMapNode?): Int {
        return if (node!!.isRoot) 0 else depth(node.parent as MindMapNode) + 1
    }
}
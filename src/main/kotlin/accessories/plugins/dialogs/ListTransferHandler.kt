...
class ListTransferHandler : TransferHandler() {
    var source: JList<*>? = null
    ...
    override fun importData(c: JComponent, t: Transferable): Boolean {
        val target = c as JList<*>
        var alist: List<*>? = null
        if (!canImport(c, t.transferDataFlavors)) {
            return false
        }
        alist = try {
            t.getTransferData(localListFlavor) as List<*>
        } catch (ufe: UnsupportedFlavorException) {
            println("importData: unsupported data flavor")
            return false
        } catch (ioe: IOException) {
            println("importData: I/O exception")
            return false
        }
        ...
        // and has no effect.
        if (source == target) {
            if (indices != null && index >= indices!![0] - 1 && index <= indices!![indices!!.size - 1]) {
                indices = null
                return true
            }
        }
    ...
    override fun exportDone(c: JComponent, data: Transferable, action: Int) {
        if (action == MOVE && indices != null) {
            ...
            for (i in indices!!.indices.reversed()) model.remove(indices!![i])
        }
        indices = null
        addIndex = -1
        addCount = 0
    ...
    private fun hasLocalListFlavor(flavors: Array<DataFlavor>): Boolean {
    ...
    override fun canImport(c: JComponent, flavors: Array<DataFlavor>): Boolean {
        return if (hasLocalListFlavor(flavors)) {
            true
        } else false
    }
    ...
    override fun createTransferable(c: JComponent): Transferable {
        if (c is JList<*>) {
            ...
            return ListTransferable(source!!.selectedValuesList)
        }
        return null
    }
    ...
    inner class ListTransferable(private val data: List<*>) : Transferable {
    ...
    companion object {
        private var localListFlavor: DataFlavor? = null
        private var dataFlavors: Array<DataFlavor>

        init {
            try {
                localListFlavor = DataFlavor(DataFlavor.javaJVMLocalObjectMimeType + ";class=java.util.List")
                dataFlavors = arrayOf(localListFlavor!!)

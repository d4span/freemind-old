 ...
 * @date 05.07.2015
 */
class CalendarMarkingEvaluator(private var mCalendarMarkings: CalendarMarkings?) : ICalendarMarkingEvaluator {
    private val mCache = HashMap<Long, CalendarMarking?>()

    private interface RepetitionHandler {
        fun getFirst(pStartDate: Calendar?, pMarking: CalendarMarking): Calendar?
        fun getNext(pDay: Calendar?, pMarking: CalendarMarking): Calendar?
    }

    private abstract class BasicRepetitionHandler : RepetitionHandler {
        fun compareIfStillBefore(pDay: Calendar, pMarking: CalendarMarking): Calendar? {
            if (pMarking.endDate > 0) {
                val cal2 = Calendar.getInstance()
                cal2.timeInMillis = pMarking.endDate
                return if (pDay.compareTo(cal2) <= 0) {
                    pDay
                } else {
                    null
                }
        ...
        fun shiftToBeAfterStartDate(pFirstDate: Calendar?,
                                    pMarking: CalendarMarking?, pUserStartDate: Calendar): Calendar? {
            var pFirstDate = pFirstDate
            while (pFirstDate != null && pUserStartDate.compareTo(pFirstDate) > 0) {
                // first occurrence is after the start date. we shift unless done.
                val millies = pFirstDate.timeInMillis
                pFirstDate = getNext(pFirstDate, pMarking!!)
                require(millies < pFirstDate!!.timeInMillis) { "Next doesn't work: " + pFirstDate.time }
            }
            return pFirstDate
        }
    }

    private abstract class DirektBeginnerHandler : BasicRepetitionHandler() {
        override fun getFirst(pStartDate: Calendar?, pMarking: CalendarMarking): Calendar? {
            return pStartDate
        }
    }

    private class NeverHandler : DirektBeginnerHandler() {
        override fun getNext(pDay: Calendar?, pMarking: CalendarMarking): Calendar? {
            return null
        }
    }

    private class WeeklyHandler : DirektBeginnerHandler() {
        override fun getNext(pDay: Calendar?, pMarking: CalendarMarking): Calendar? {
            pDay!!.add(Calendar.WEEK_OF_YEAR, pMarking.repeatEachNOccurence)
            return compareIfStillBefore(pDay, pMarking)
        }
    }

    private class WeeklyEveryNthDayHandler : DirektBeginnerHandler() {
        override fun getFirst(pStartDate: Calendar?,
                              pMarking: CalendarMarking): Calendar? {
            val clone = pStartDate!!.clone() as Calendar
            pStartDate[Calendar.DAY_OF_WEEK] = pMarking.firstOccurence
            return shiftToBeAfterStartDate(pStartDate, pMarking, clone)
        }

        override fun getNext(pDay: Calendar?, pMarking: CalendarMarking): Calendar? {
            val clone = pDay!!.clone() as Calendar
            pDay.add(Calendar.DAY_OF_WEEK, pMarking.repeatEachNOccurence)
            if (clone[Calendar.WEEK_OF_YEAR] != pDay[Calendar.WEEK_OF_YEAR]) {
                // we switched the week. Let's start from new:
                pDay[Calendar.DAY_OF_WEEK] = pMarking.firstOccurence
            }
            return compareIfStillBefore(pDay, pMarking)!!
        }
    }

    private class MonthlyHandler : DirektBeginnerHandler() {
        override fun getNext(pDay: Calendar?, pMarking: CalendarMarking): Calendar? {
            pDay!!.add(Calendar.MONTH, pMarking.repeatEachNOccurence)
            return compareIfStillBefore(pDay, pMarking)
        }
    }

    private class MonthlyEveryNthDayHandler : BasicRepetitionHandler() {
        override fun getFirst(pStartDate: Calendar?,
                              pMarking: CalendarMarking): Calendar? {
            val clone = pStartDate!!.clone() as Calendar
            pStartDate[Calendar.DAY_OF_MONTH] = pMarking.firstOccurence
            return shiftToBeAfterStartDate(pStartDate, pMarking, clone)
        }

        override fun getNext(pDay: Calendar?, pMarking: CalendarMarking): Calendar? {
            val clone = pDay!!.clone() as Calendar
            pDay.add(Calendar.DAY_OF_MONTH, pMarking.repeatEachNOccurence)
            if (clone[Calendar.MONTH] != pDay[Calendar.MONTH]) {
                // we switched the month. Let's start from new:
                pDay[Calendar.DAY_OF_MONTH] = pMarking.firstOccurence
            }
            return compareIfStillBefore(pDay, pMarking)
        }
    }

    private class MonthlyEveryNthWeekHandler : BasicRepetitionHandler() {
        override fun getFirst(pStartDate: Calendar?,
                              pMarking: CalendarMarking): Calendar? {
            val clone = pStartDate!!.clone() as Calendar
            pStartDate[Calendar.WEEK_OF_MONTH] = pMarking.firstOccurence
            return shiftToBeAfterStartDate(pStartDate, pMarking, clone)
        }

        override fun getNext(pDay: Calendar?, pMarking: CalendarMarking): Calendar? {
            val clone = pDay!!.clone() as Calendar
            pDay.add(Calendar.WEEK_OF_MONTH, pMarking.repeatEachNOccurence)
            if (clone[Calendar.MONTH] != pDay[Calendar.MONTH]) {
                // we switched the month. Let's start from new:
                pDay[Calendar.WEEK_OF_MONTH] = pMarking.firstOccurence
            }
            return compareIfStillBefore(pDay, pMarking)
        }
    }

    private class YearlyHandler : DirektBeginnerHandler() {
        override fun getNext(pDay: Calendar?, pMarking: CalendarMarking): Calendar? {
            pDay!!.add(Calendar.YEAR, pMarking.repeatEachNOccurence)
            return compareIfStillBefore(pDay, pMarking)
        }
    }

    private class YearlyEveryNthDayHandler : BasicRepetitionHandler() {
        override fun getFirst(pStartDate: Calendar?,
                              pMarking: CalendarMarking): Calendar? {
            val clone = pStartDate!!.clone() as Calendar
            pStartDate[Calendar.DAY_OF_YEAR] = pMarking.firstOccurence
            return shiftToBeAfterStartDate(pStartDate, pMarking, clone)
        }

        override fun getNext(pDay: Calendar?, pMarking: CalendarMarking): Calendar? {
            val clone = pDay!!.clone() as Calendar
            pDay.add(Calendar.DAY_OF_YEAR, pMarking.repeatEachNOccurence)
            if (clone[Calendar.YEAR] != pDay[Calendar.YEAR]) {
                // we switched the year. Let's start from new:
                pDay[Calendar.DAY_OF_YEAR] = pMarking.firstOccurence
            }
            return compareIfStillBefore(pDay, pMarking)
        }
    }

    private class YearlyEveryNthWeekHandler : BasicRepetitionHandler() {
        override fun getFirst(pStartDate: Calendar?,
                              pMarking: CalendarMarking): Calendar? {
            val userStartDate = pStartDate!!.clone() as Calendar
            pStartDate[Calendar.WEEK_OF_YEAR] = pMarking.firstOccurence
        ...
        override fun getNext(pDay: Calendar?, pMarking: CalendarMarking): Calendar? {
            ...
            if (clone[Calendar.YEAR] != pDay[Calendar.YEAR]) {
                // we switched the year. Let's start from new:
                pDay[Calendar.WEEK_OF_YEAR] = pMarking.firstOccurence
                if (clone[Calendar.YEAR] == pDay[Calendar.YEAR]) {
    ...
    private class YearlyEveryNthMonthHandler : BasicRepetitionHandler() {
        override fun getFirst(pStartDate: Calendar?,
                              pMarking: CalendarMarking): Calendar? {
            val clone = pStartDate!!.clone() as Calendar
            pStartDate[Calendar.MONTH] = pMarking.firstOccurence
            return shiftToBeAfterStartDate(pStartDate, pMarking, clone)
        }
        ...
        override fun getNext(pDay: Calendar?, pMarking: CalendarMarking): Calendar? {
            ...
            if (clone[Calendar.YEAR] != pDay[Calendar.YEAR]) {
                // we switched the year. Let's start from new:
                pDay[Calendar.MONTH] = pMarking.firstOccurence
            }
            return compareIfStillBefore(pDay, pMarking)
        }
    }

    private class DailyHandler : DirektBeginnerHandler() {
        override fun getNext(pDay: Calendar?, pMarking: CalendarMarking): Calendar? {
            pDay!!.add(Calendar.DAY_OF_YEAR, pMarking.repeatEachNOccurence)
            return compareIfStillBefore(pDay, pMarking)
        }
    ...
    init {
        ...
        if (sHandlerMap == null) {
            sHandlerMap = HashMap()
            sHandlerMap!![CalendarMarking.NEVER] = NeverHandler()
            sHandlerMap!![CalendarMarking.DAILY] = DailyHandler()
            sHandlerMap!![CalendarMarking.WEEKLY] = WeeklyHandler()
            sHandlerMap!![CalendarMarking.WEEKLY_EVERY_NTH_DAY] = WeeklyEveryNthDayHandler()
            sHandlerMap!![CalendarMarking.MONTHLY] = MonthlyHandler()
            sHandlerMap!![CalendarMarking.MONTHLY_EVERY_NTH_DAY] = MonthlyEveryNthDayHandler()
            sHandlerMap!![CalendarMarking.MONTHLY_EVERY_NTH_WEEK] = MonthlyEveryNthWeekHandler()
            sHandlerMap!![CalendarMarking.YEARLY] = YearlyHandler()
            sHandlerMap!![CalendarMarking.YEARLY_EVERY_NTH_DAY] = YearlyEveryNthDayHandler()
            sHandlerMap!![CalendarMarking.YEARLY_EVERY_NTH_WEEK] = YearlyEveryNthWeekHandler()
            sHandlerMap!![CalendarMarking.YEARLY_EVERY_NTH_MONTH] = YearlyEveryNthMonthHandler()
        }
    }

    fun getAtLeastTheFirstNEntries(n: Int): Set<Calendar> {
        val retValue: MutableSet<Calendar> = TreeSet()
        for (i in 0 until mCalendarMarkings!!.sizeCalendarMarkingList()) {
            var count = 0
            val marking = mCalendarMarkings!!.getCalendarMarking(i)
            // common error for self written entries:
            if (marking.repeatEachNOccurence == 0) {
                marking.repeatEachNOccurence = 1
            }
            // get first occurrence:
            var firstDay = Calendar.getInstance()
            firstDay!!.timeInMillis = marking.startDate
            val repeatType = marking.repeatType
            if (!sHandlerMap!!.containsKey(repeatType)) {
                logger!!.severe("Repeat type $repeatType unknown.")
                continue
            }
            val handler = sHandlerMap
                    .get(repeatType)
            ...
            while (count < n) {
                retValue.add(firstDay!!.clone() as Calendar)
	 ...
	 */
    override fun isMarked(pCalendar: Calendar?): CalendarMarking? {
        var pCalendar = pCalendar
        val millies = pCalendar!!.timeInMillis
        if (mCache.containsKey(millies)) {
            return mCache[millies]
        }
        pCalendar = pCalendar.clone() as Calendar
        for (i in 0 until mCalendarMarkings!!.sizeCalendarMarkingList()) {
            val marking = mCalendarMarkings!!.getCalendarMarking(i)
            // common error for self written entries:
            if (marking.repeatEachNOccurence == 0) {
            ...
            firstDay!!.timeInMillis = marking.startDate
            val handler = sHandlerMap
                    .get(marking.repeatType)
            firstDay = handler!!.getFirst(firstDay, marking)
            if (firstDay == null) {
                continue
            }
            if (equal(pCalendar, firstDay)) {
                mCache[millies] = marking
                return marking
            }
    ...
    private fun equal(pCalendar: Calendar?, pOtherDay: Calendar): Boolean {
        return pCalendar!![Calendar.YEAR] == pOtherDay[Calendar.YEAR] && pCalendar[Calendar.MONTH] == pOtherDay[Calendar.MONTH] && pCalendar[Calendar.DAY_OF_MONTH] == pOtherDay[Calendar.DAY_OF_MONTH]
    }
    ...
    fun printDate(firstDay: Calendar?) {
        if (firstDay != null) {
            println(DateFormat.getDateInstance().format(
                    firstDay.time))
        }
    }

    override fun changeMarkings(pMarkings: CalendarMarkings?) {
        mCalendarMarkings = pMarkings
        mCache.clear()
    }

    companion object {
        private var sHandlerMap: HashMap<String?, RepetitionHandler>? = null

 ...
 * @author foltin
 */
class FlatNodeTableFilterModel(private val mTableModel: TableModel?,
                               /**
                                * The column that contains the NodeHolder items
                                */
                               private val mNodeTextColumn: Int, private val mNoteTextColumn: Int) : AbstractTableModel() {
    private var mFilterRegexp: String? = null

    /**
     * Contains indices or rows matching the filter criteria.
     */
    private var mIndexArray: ArrayList<Int>? = null
     ...
     */
    init {
        mTableModel!!.addTableModelListener(TableModelHandler())
        resetFilter()
    }
    ...
    fun setFilter(filterRegexp: String?) {
    ...
    private fun updateIndexArray() {
        val newIndexArray = ArrayList<Int>()
        for (i in 0 until mTableModel!!.rowCount) {
            val nodeContent = mTableModel.getValueAt(i, mNodeTextColumn) as TimeList.NodeHolder
            if (mPattern!!.matcher(nodeContent.toString()).matches()) {
                // add index to array:
                newIndexArray.add(i)
            } else {
                // only check notes, when not already a hit.
                val noteContent = mTableModel.getValueAt(i,
                        mNoteTextColumn) as NotesHolder
    ...
    override fun getColumnCount(): Int {
        return mTableModel!!.columnCount
    }

    override fun getColumnName(pColumnIndex: Int): String {
    ...
    override fun getValueAt(row: Int, column: Int): Any {
        require(!(row < 0 || row >= rowCount)) { "Illegal Row specified: $row" }
        val origRow = mIndexArray!![row]
        return mTableModel!!.getValueAt(origRow, column)
    }

    private inner class TableModelHandler : TableModelListener {
        override fun tableChanged(arg0: TableModelEvent) {
//			updateIndexArray();
            fireTableDataChanged()
        }
    }

 ...
 */
class JMonthChooser @JvmOverloads constructor(hasSpinner: Boolean = true) : JPanel(), ItemListener, ChangeListener {
    /** true, if the month chooser has a spinner component  */
    protected var hasSpinner: Boolean
    private var locale: Locale
    private var month = 0

    // needed for comparison
    private var dayChooser: JDayChooser? = null
    private var yearChooser: JYearChooser? = null
    private val comboBox: JComboBox<String>?
    private var spinner: JSpinner? = null
    private val initialized: Boolean
    private var localInitialize = false
    /**
    ...
    init {
        ...
        initialized = true
        setMonth(Calendar.getInstance()[Calendar.MONTH])
    }
    ...
    override fun stateChanged(e: ChangeEvent) {
        val model = (e
                .source as JSpinner).model as SpinnerNumberModel
...
//		changeMonth(increase);
    ...
    fun changeMonth(increase: Boolean) {
        var month = getMonth()
        if (increase) {
            month += 1
            if (month == 12) {
     ...
     */
    private fun setMonth(newMonth: Int, select: Boolean) {
     ...
     * the day chooser
     */
    fun setDayChooser(dayChooser: JDayChooser?) {
    ...
    override fun updateUI() {
        val testSpinner = JSpinner()
        if (spinner != null) {
            if ("Windows" == UIManager.getLookAndFeel().id) {
                spinner.setBorder(testSpinner.border)
            } else {
                spinner.setBorder(EmptyBorder(0, 0, 0, 0))
            }
        }
    }
    ...
    companion object {
        ...
        private const val serialVersionUID = -2028361332231218527L
        ...
        fun main(s: Array<String>) {
            val frame = JFrame("MonthChooser")
            frame.contentPane.add(JMonthChooser())
            frame.pack()

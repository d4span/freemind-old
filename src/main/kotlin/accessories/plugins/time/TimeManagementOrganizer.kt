 ...
 * @author foltin
 */
class TimeManagementOrganizer(private val controller: ModeController, map: MindMap?) : HookRegistration, MenuItemEnabledListener {
    ...
    override fun isEnabled(item: JMenuItem?, action: Action?): Boolean {
        if (action is NodeHookAction) {
            val hookName = action.hookName
            if (hookName == "plugins/time/RemoveReminderHook.java") {
                var visible = false
                for (node in controller.selecteds) {
                    if (getHook(node) != null) {
                        visible = true
                    }
                }
    ...
    companion object {
        /**
         */
        fun getHook(node: MindMapNode?): ReminderHookBase? {
            for (element in node!!.activatedHooks) {
                if (element is ReminderHookBase) {
                    return element
                }
            }
            return null

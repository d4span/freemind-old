...
/**  */
class ClickableImageCreator(private val root: MindMapNode,
                            modeController: ModeController?, private val linkFormatter: String) {
    class AreaHolder {
    ...
    var area = Vector<AreaHolder>()
    private val modeController: ModeController?
    private var innerBounds: Rectangle? = null
     ...
     */
    init {
        mapView = modeController!!.view
        innerBounds = mapView?.innerBounds ?: // test case: give any bounds:
    ...
    fun generateHtml(): String {
        val htmlArea = StringBuffer()
        val i: Iterator<AreaHolder> = area.iterator()
        while (i.hasNext()) {
            val holder = i.next()
            val formatter = MessageFormat(linkFormatter)
            val replacement = formatter.format(arrayOf<Any?>(holder.href, holder.link))
    ...
    /**
     */
    private fun createArea(node: MindMapNode? = root) {
        if (mapView == null) {
            return
        }
        val nodeView = mapView.getNodeView(node)
        if (nodeView != null) {
            val holder = AreaHolder()
            ...
            holder.link = if (node.link != null) node.link else ""
            val contentXY = mapView.getNodeContentLocation(nodeView)
            val content = nodeView.content
            holder.coordinates.x = (contentXY.x - innerBounds!!.minX).toInt()
            holder.coordinates.y = (contentXY.y - innerBounds!!.minY).toInt()
            holder.coordinates.width = content!!.width
            holder.coordinates.height = content.height
            area.add(holder)

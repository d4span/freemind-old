...
class LastStateStorageManagement(pXml: String?) {
    ...
    fun changeOrAdd(pStore: MindmapLastStateStorage) {
        ...
        while (it?.hasNext() ?: false) {
            ...
            if (Tools.safeEquals(pStore.restorableName,
                ...
                store?.x = pStore.x
                store?.y = pStore.y
                val listCopy = Vector<NodeListMember?>(pStore.listNodeListMemberList)
    ...
    val lastOpenList: List<MindmapLastStateStorage?>
        get() {
            ...
            while (it?.hasNext() ?: false) {
                val store = it?.next()
                if ((store?.tabIndex ?: Int.MIN_VALUE) >= 0) {
                    ret.add(store)
                }
            }
            Collections.sort(ret) { store0, store1 -> (store0?.tabIndex ?: Int.MIN_VALUE) - (store1?.tabIndex ?: Int.MIN_VALUE) }
            return ret
        }
    var lastFocussedTab: Int?
        get() = mLastStatesMap?.lastFocusedTab
        set(pIndex) {
            mLastStatesMap?.lastFocusedTab = pIndex
        }

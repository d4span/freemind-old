 ...
 */(private val attribute: String?, value: String?,
    ignoreCase: Boolean, private val comparationResult: Int?, private val succeed: Boolean) : CompareConditionAdapter(value, ignoreCase) {
    /*
	 * (non-Javadoc)
	 * 
	 * @see
    ...
    override fun checkNode(c: Controller?, node: MindMapNode): Boolean {
        for (i in 0 until node.attributeTableLength) {
            try {
                ...
                if (attribute2.name == attribute && succeed == (compareTo(attribute2.value
                                .toString()) == comparationResult)) return true
    ...
    companion object {
        ...
        fun load(element: XMLElement?): Condition {
            return AttributeCompareCondition(
                    ...
                    element?.getStringAttribute(CompareConditionAdapter.Companion.VALUE),

 ...
 */(private val conditions: Array<Any?>) : Condition {
    ...
    override val listCellRendererComponent: JComponent?
        get() {
            ...
            i = 1
            while (i < conditions.size) {
                val and = Tools.removeMnemonic(Resources.instance
    ...
    companion object {
        ...
        fun load(element: XMLElement?): Condition {
            ...
            val conditions = arrayOfNulls<Any>(children?.size ?: 0)
            for (i in conditions.indices) {
                val cond = FilterController.Companion.conditionFactory
                        ?.loadCondition(children?.get(i) as XMLElement)
                conditions[i] = cond
            }

 ...
 */(private val conditions: Array<Any?>) : Condition {
    ...
    override val listCellRendererComponent: JComponent?
        get() {
            ...
            var rendererComponent = cond.getListCellRendererComponent()
            rendererComponent!!.isOpaque = false
            component.add(rendererComponent)
            var i: Int
            i = 1
            while (i < conditions.size) {
                val and = Tools.removeMnemonic(Resources.getInstance()
    ...
    companion object {
        ...
        fun load(element: XMLElement): Condition {
            ...
            for (i in conditions.indices) {
                val cond: Condition = FilterController.Companion.getConditionFactory()

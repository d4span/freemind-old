...
class SortedMapVector {
    private class MapElement(val key: Comparable<*>, val value: Any)
    ...
    fun add(key: Comparable<*>, value: Any): Int {
    ...
    fun getValue(index: Int): Any {
        return (elements[index] as MapElement).getValue()
    }

    fun getValue(key: Comparable<*>): Any {
        val index = findElement(key)
        if (index and ELEMENT_NOT_FOUND_FLAG == 0) return (elements[index] as MapElement).getValue()
        throw NoSuchElementException()
    }

    fun getKey(index: Int): Comparable<*> {
        return (elements[index] as MapElement).getKey()
    }

    fun containsKey(key: Comparable<*>): Boolean {
        val index = findElement(key)
        return index and ELEMENT_NOT_FOUND_FLAG == 0
    }

    fun indexOf(key: Comparable<*>): Int {
        val index = findElement(key)
        return if (index and ELEMENT_NOT_FOUND_FLAG == 0) index else -1
    }

    private fun findElement(key: Comparable<*>, first: Int = 0, size: Int = size()): Int {
        if (size == 0) return first or ELEMENT_NOT_FOUND_FLAG
        val halfSize = size / 2
        val middle = first + halfSize
        val middleElement = elements[middle]
        var comparationResult = key.compareTo(middleElement.getKey())
        val last = first + size - 1
        return if (comparationResult < 0) {
            if (halfSize <= 1) {
                if (middle != first) comparationResult = key.compareTo(elements[first].getKey())
            ...
            if (halfSize <= 1) {
                if (middle != last) comparationResult = key.compareTo(elements[last].getKey())
    ...
    fun remove(key: Comparable<*>): Boolean {
        val index = findElement(key)
        if (index and ELEMENT_NOT_FOUND_FLAG == 0) {

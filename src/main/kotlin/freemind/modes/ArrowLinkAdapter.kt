...
abstract class ArrowLinkAdapter(source: MindMapNode?, target: MindMapNode?,
                                pMapFeedback: MapFeedback?) : LinkAdapter(source, target, pMapFeedback), MindMapArrowLink {
    /** the zero is the start point of the line;  */
    protected override var startInclination: Point? = null

    /** the zero is the end point of the line;  */
    protected override var endInclination: Point? = null
    protected override var startArrow: String?
    protected override var endArrow: String?
    override var showControlPointsFlag = false
        protected set

    init {
        startArrow = MindMapArrowLink.Companion.ARROW_NONE
        endArrow = MindMapArrowLink.Companion.ARROW_DEFAULT
    }

    override fun getStartInclination(): Point? {
        return if (startInclination == null) null else Point(startInclination)
    }

    override fun getEndInclination(): Point? {
        return if (endInclination == null) null else Point(endInclination)
    }

    override fun getStartArrow(): String? {
        return startArrow
    }

    override fun getEndArrow(): String? {
        return endArrow
    }

    override fun setStartInclination(startInclination: Point?) {
        this.startInclination = startInclination
    }

    override fun setEndInclination(endInclination: Point?) {
        this.endInclination = endInclination
    }

    fun setStartArrow(startArrow: String?) {
        if (startArrow == null || startArrow.uppercase(Locale.getDefault()) == ARROW_NONE_UC) {
            this.startArrow = MindMapArrowLink.Companion.ARROW_NONE
            return
        } else if (startArrow.uppercase(Locale.getDefault()) == ARROW_DEFAULT_UP) {
            this.startArrow = MindMapArrowLink.Companion.ARROW_DEFAULT
            return
        }
        // dont change:
    ...
    fun setEndArrow(endArrow: String?) {
        if (endArrow == null || endArrow.uppercase(Locale.getDefault()) == ARROW_NONE_UC) {
            this.endArrow = MindMapArrowLink.Companion.ARROW_NONE
            return
        } else if (endArrow.uppercase(Locale.getDefault()) == ARROW_DEFAULT_UP) {
            this.endArrow = MindMapArrowLink.Companion.ARROW_DEFAULT
            return
        }
        // dont change:
    ...
    override fun clone(): Any {
        val arrowLink = super.clone() as ArrowLinkAdapter
        // now replace the points:
        arrowLink.startInclination = if (startInclination == null) null else Point(startInclination!!.x, startInclination!!.y)
        arrowLink.endInclination = if (endInclination == null) null else Point(
                endInclination!!.x, endInclination!!.y)
        arrowLink.startArrow = if (startArrow == null) null else String(
                startArrow)
        arrowLink.endArrow = if (endArrow == null) null else String(endArrow)
        return arrowLink
    }

    override fun showControlPoints(bShowControlPointsFlag: Boolean) {
        showControlPointsFlag = bShowControlPointsFlag
    }

    override fun save(): XMLElement? {
        val arrowLink = XMLElement()
        arrowLink.name = "arrowlink"
        if (style != null) {
            arrowLink.setAttribute("STYLE", style!!)
        }
        if (uniqueId != null) {
            arrowLink.setAttribute("ID", uniqueId)
        }
        if (color != null) {
            arrowLink.setAttribute("COLOR", colorToXml(color)!!)
        }
    ...
    fun createArrowLinkTarget(pRegistry: MindMapLinkRegistry?): ArrowLinkTarget {
        val linkTarget = ArrowLinkTarget(source, target, mMapFeedback)
        linkTarget.sourceLabel = pRegistry!!.getLabel(source)
        copy(linkTarget)
        return linkTarget
    }

    protected fun copy(linkTarget: ArrowLinkAdapter) {
        linkTarget.uniqueId = uniqueId
    ...
    companion object {
         ...
         *
         */
        private const val ARROW_DEFAULT_UP = "DEFAULT"

        /**
         *
         */
        private const val ARROW_NONE_UC = "NONE"
    }
}
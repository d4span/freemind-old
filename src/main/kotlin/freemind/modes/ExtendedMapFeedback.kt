 ...
 * @date 16.03.2014
 */
interface ExtendedMapFeedback : MapFeedback, MindMapActions {
    /**
    ...
    fun select(pFocussed: MindMapNode?, pSelecteds: List<MindMapNode?>?)
     ...
     * @param pIndex
     */
    fun insertNodeInto(pNewNode: MindMapNode?, pParent: MindMapNode?, pIndex: Int)
     ...
     * @return the new node.
     */
    fun newNode(pUserObject: Any?, pMap: MindMap?): MindMapNode?

    /**
     * @param pSelectedNode
     */
    fun removeNodeFromParent(pSelectedNode: MindMapNode?)

    /**
     * @return the factory used to create all xml actors.
     */
    val actorFactory: XmlActorFactory?
     ...
     */
    fun copy(pNodeList: List<MindMapNode?>?, pSaveInvisible: Boolean): Transferable
    fun setWaitingCursor(waiting: Boolean)
    fun nodeStyleChanged(pNode: MindMapNode?)
    ...
    fun load(pFile: File?): MapFeedback

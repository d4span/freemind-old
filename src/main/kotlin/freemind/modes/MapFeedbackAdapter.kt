 ...
 * @date 07.02.2014
 */
abstract class MapFeedbackAdapter : MapFeedback, ViewFeedback {
	 ...
	 */
    override fun firePreSaveEvent(pNode: MindMapNode?) {}
	 ...
	 */
    override fun nodeChanged(pNode: MindMapNode?) {
        if (map != null) {
            map.isSaved = false
        }
    }
	 ...
	 */
    override fun nodeRefresh(pNode: MindMapNode?) {}
	 ...
	 */
    override fun paste(pNode: MindMapNode?, pParent: MindMapNode?) {}
	 ...
	 */
    override fun getIntProperty(pKey: String?, pDefaultValue: Int): Int {
    ...
    override fun getFontThroughMap(font: Font): Font? {
        if (!fontMap.containsKey(font.toString())) {
            fontMap[font.toString()] = font
        }
        return fontMap[font.toString()]
    }
    ...
    override fun createNodeHook(pLoadName: String?, pNode: MindMapNode): NodeHook? {
        val hook = PermanentNodeHookSubstituteUnknown(pLoadName!!)
        hook.setMap(map)
        pNode.addHook(hook)
        return hook
    }
	 ...
	 */
    override fun invokeHooksRecursively(pNode: MindMapNode?, pModel: MindMap?) {
        val i: Iterator<*>? = pNode!!.childrenUnfolded()
        while (i!!.hasNext()) {
            val child = i.next() as NodeAdapter
            invokeHooksRecursively(child, map)
        }
        for (hook in pNode.hooks) {
    ...
    override fun setFolded(pModel: MindMapNode, pB: Boolean) {}
    override fun onViewCreatedHook(pNewView: NodeView) {}
	 ...
	 */
    open fun select(node: NodeView?) {}

    /* (non-Javadoc)
	 * @see freemind.view.mindmapview.ViewFeedback#getNodeView(freemind.modes.MindMapNode)
	 */
    open fun getNodeView(node: MindMapNode?): NodeView? {
        return null
    }

    /**
     * This class sortes nodes by ascending depth of their paths to root. This
     * is useful to assure that children are cutted **before ** their
     * fathers!!!.
     *
     * Moreover, it sorts nodes with the same depth according to their position
     * relative to each other.
     */
    protected inner class NodesDepthComparator : Comparator<MindMapNode> {
        /* the < relation. */
        override fun compare(n1: MindMapNode, n2: MindMapNode): Int {
            val path1: Array<Any?>? = map.getPathToRoot(n1)
            val path2: Array<Any?>? = map.getPathToRoot(n2)
            val depth = path1!!.size - path2!!.size
            if (depth > 0) return -1
            if (depth < 0) return 1
            return if (n1.isRoot) 0 else n1.parentNode.getChildPosition(n1)
                    - n2.parentNode.getChildPosition(n2)
        }
    }

    override fun sortNodesByDepth(inPlaceList: List<MindMapNode?>) {
        Collections.sort(inPlaceList, NodesDepthComparator())
        logger!!.finest("Sort result: $inPlaceList")
    }

    override fun registerMouseWheelEventHandler(pHandler: MouseWheelEventHandler) {}
    override fun deRegisterMouseWheelEventHandler(pHandler: MouseWheelEventHandler) {}

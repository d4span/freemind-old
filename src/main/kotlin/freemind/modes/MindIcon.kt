...
class MindIcon : Comparable<MindIcon>, IconInformation {
    ...
    private var number = UNKNOWN
        private get() {
            if (field == UNKNOWN) {
                field = allIconNames!!.indexOf(name)
            }
            if (field == UNKNOWN) {
                field = nextNumber--
            }
     ...
     */
    private var associatedIcon: ImageIcon?
    ...
    /**
     */
    private constructor(name: String?, icon: ImageIcon) {
        setName(name)
        associatedIcon = icon
    }
    ...
    fun getName(): String? {
        ...
        return if (name == null) "notfound" else name
    }
    ...
    override val description: String?
        get() {
            val resource: String = String("icon_" + getName())
            return Resources.getInstance().getResourceString(resource, resource)
        }
    val iconFileName: String
        get() = iconsPath + iconBaseFileName
    val iconBaseFileName: String
        get() = getName() + ".png"// As standard icon not found, try user's
    ...
    override var icon: ImageIcon?
        get() {
            ...
            if (iconNotFound == null) {
                iconNotFound = ImageFactory.getInstance().createIcon(Resources.getInstance().getResource(
                        "images/IconNotFound.png"))
            }
            if (associatedIcon != null) return associatedIcon
            return if (name != null) {
                var imageURL: URL? = Resources.getInstance().getResource(
                        iconFileName)
                if (imageURL == null) { // As standard icon not found, try user's
                    try {
                        val file: File = File(Resources.getInstance()
                ...
                var icon = if (imageURL == null) iconNotFound else ImageFactory.getInstance().createIcon(
                        imageURL)
                icon = icon
                icon
            } else {
        ...
        protected set(_associatedIcon) {
            associatedIcon = _associatedIcon
        }
    val unscaledIcon: ImageIcon?
        get() {
            if (associatedIcon is ScalableImageIcon) {
                val scalableIcon = associatedIcon as ScalableImageIcon
                return scalableIcon.unscaledIcon
            }
	 ...
	 */
    override fun compareTo(icon: MindIcon): Int {
        val i1 = number
        val i2 = icon.number
    ...
    /**
     */
    val rendererComponent: JComponent
        get() {
            if (component == null) {
    ...
    override val keystrokeResourceName: String?
        get() = "keystroke_icon_$name"
    override val keyStroke: KeyStroke?
        get() = null

    companion object {
        const val PROPERTY_STRING_ICONS_LIST = "icons.list"
        private val mAllIconNames: Vector<String?>? = null
        private var iconNotFound: ImageIcon? = null

        /**
         * Set of all created icons. Name -> MindIcon
         */
        private val createdIcons = HashMap<String?, MindIcon>()
        private const val UNKNOWN = -1
        const val LAST = UNKNOWN
        var nextNumber = UNKNOWN - 1
        val iconsPath: String
            get() = "images/icons/"
        val allIconNames: Vector<String?>?
            get() {
                if (mAllIconNames != null) return mAllIconNames
                val mAllIconNames = Vector<String?>()
                val icons: String = Resources.getInstance().getProperty(
                        PROPERTY_STRING_ICONS_LIST)
                val tokenizer = StringTokenizer(icons, ";")
        ...
        fun factory(iconName: String?): MindIcon? {
            if (createdIcons.containsKey(iconName)) {
                return createdIcons[iconName]
            }
            val icon = MindIcon(iconName)
            createdIcons[iconName] = icon
            return icon
        }
        ...
        fun factory(iconName: String?, icon: ImageIcon?): MindIcon? {
            ...
            val mindIcon = MindIcon(iconName, icon)
            allIconNames!!.add(iconName)
            createdIcons[iconName] = mindIcon
            return mindIcon
        }
    }
}
...
import freemind.common.NextLinePropertyimport
...
import freemind.controller.actions.generated.instance.MindmapLastStateStorageimport
...
freemind.view.mindmapview.*import java.awt.*
import java.awt.event.*
import java.io.*
import java.net.URLimport

javax.swing.filechooser.FileFilter
interface ModeController : TextTranslator, MapFeedback, ViewFeedback {
    /**
     * @param file
    ...
    fun load(file: File?): MapFeedback?

    /**
     * Opens a link in * the opened map * another map * another file.
     */
    fun loadURL(relative: String?)
    fun save(file: File?): Boolean
    fun newMap(): ModeController?
    ...
    fun saveAs(): Boolean
    fun open()
    fun close(force: Boolean, mapModuleManager: MapModuleManager): Boolean

    // activation methods:
    fun startupController()
    fun shutdownController()

    // end activation methods.
    // listener -> controller handling
    fun doubleClick(e: MouseEvent)
    fun plainClick(e: MouseEvent)
     ...
     */
    override fun sortNodesByDepth(inPlaceList: List<MindMapNode?>)
     ...
     */
    fun extendSelection(e: MouseEvent?): Boolean
    ...
    override fun onViewCreatedHook(newView: NodeView)
    override fun onViewRemovedHook(newView: NodeView)
    ...
    interface NodeSelectionListener {
         ...
         */
        fun onSelectionChange(pNode: NodeView?, pIsSelected: Boolean)
    }
     ...
     * listener.
     */
    fun registerNodeSelectionListener(listener: NodeSelectionListener,
                                      pCallWithCurrentSelection: Boolean)

    fun deregisterNodeSelectionListener(listener: NodeSelectionListener)
    ...
    interface NodeLifetimeListener {
         ...
         */
        fun onCreateNodeHook(node: MindMapNode?)

        /**
         * Is sent before a node is deleted (on map shutdown, too).
         */
        fun onPreDeleteNode(node: MindMapNode?)

        /**
         * Is sent before after a node is deleted (on map shutdown, this event
         * is *not* send).
         */
        fun onPostDeleteNode(node: MindMapNode?, parent: MindMapNode?)
    }
     ...
     * TODO
     */
    fun registerNodeLifetimeListener(listener: NodeLifetimeListener,
                                     pFireCreateEvent: Boolean)

    fun deregisterNodeLifetimeListener(listener: NodeLifetimeListener)
     ...
     */
    fun displayNode(node: MindMapNode?)

    /**
     * Node is displayed and selected as the only one selected. It is moved to
     * the center of the screen.
     */
    fun centerNode(node: MindMapNode?)
    fun getLinkShortText(node: MindMapNode): String?
    val modeToolBar: JToolBar?

    /** For the toolbar on the left hand side of the window.  */
    val leftToolBar: Component?

    /** Use this method to get menus to the screen.  */
    fun updateMenus(holder: StructuredMenuHolder)
    fun updatePopupMenu(holder: StructuredMenuHolder?)
    val popupMenu: JPopupMenu?
    fun showPopupMenu(e: MouseEvent?)

    /** This returns a context menu for an object placed in the background pane.  */
    fun getPopupForModel(obj: Any?): JPopupMenu?
    val frame: FreeMindMain
    var view: MapView?
    abstract override val map: MindMap?

    /**
     * This method must only be used by the model itself at creation time. Don't
     * use this method.
     */
    fun setModel(model: MapAdapter?)
    val mode: Mode?
    ...
    fun getNodeView(node: MindMapNode?): NodeView?
    fun refreshMap()
    fun copy(node: MindMapNode?, saveInvisible: Boolean): Transferable
    fun copy(): Transferable?
    fun copySingle(): Transferable?
    fun copy(selectedNodes: List<MindMapNode?>, copyInvisible: Boolean): Transferable?
    fun getFileChooser(filter: FileFilter?): FreeMindFileDialog
     ...
     */
    fun setToolTip(node: MindMapNode, key: String?, value: String?)

    companion object {
        const val NODESEPARATOR = "<nodeseparator>"
    }
}
 ...
 */
class ModesCreator(private val c: Controller) {
    /**
     * Contains translated mode name => Mode instances
     */
    private var mCreatedModes: MutableMap<String?, Mode?>? = null

    /**
     * Contains a name translation. Mode name => Class Name
     */
    private var modesTranslation: MutableMap<String, String>? = null
    val allModes: Set<String>
        get() {
            if (logger == null) {
                logger = c.frame.getLogger(this.javaClass.name)
            }
            if (mCreatedModes == null) {
                mCreatedModes = TreeMap()
                modesTranslation = HashMap()
                val modestring = c.frame.getProperty("modes_since_0_8_0")
                val tokens = StringTokenizer(modestring, ",")
                while (tokens.hasMoreTokens()) {
                    val modename = tokens.nextToken()
                    val modeAlias = tokens.nextToken()
                    mCreatedModes[modename] = null
                    modesTranslation[modeAlias] = modename
                }
                logger!!.info("Modes:" + mCreatedModes.keys)
            }
            return modesTranslation!!.keys
        }

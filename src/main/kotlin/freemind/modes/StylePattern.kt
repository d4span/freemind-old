...
class StylePattern {
    ...
    var nodeBackgroundColor: Color? = null
    /**
     ...
     */
    var nodeFontFamily: String? = null
    /**
     ...
     */
    var nodeFontItalic: Boolean? = null
    /**
    ...
    var childrenStylePattern: StylePattern? = null

    /**
     * Empty constructor
     */
    constructor() {}
    constructor(elm: XMLElement, justConstructedPatterns: List<StylePattern>) {
    ...
    constructor(node: MindMapNode) {
        ...
        nodeBackgroundColor = node.backgroundColor
        nodeStyle = node.style
        nodeFontBold = node.isBold
        nodeFontItalic = node.isItalic
        nodeFontSize = if (node.fontSize == null) null else Integer.valueOf(node
                .fontSize)
        nodeFontFamily = node.fontFamilyName
        nodeIcon = null
        // appliesToNodeIcon = node.getIcons().size()>0;
        // nodeIcon = (MindIcon)
        // (node.getIcons().size()==0?null:node.getIcons().get(0));
        edgeColor = node.edge.color
        edgeStyle = node.edge.style
        edgeWidth = node.edge.width
    }

    override fun toString(): String {
        return """
            node: $nodeColor, $nodeBackgroundColor, $nodeStyle, $nodeFontFamily, $nodeFontSize, $nodeIcon, $text, 
            edge: $edgeColor, $edgeStyle, $edgeWidth
            """.trimIndent()
    }

    val appliesToEdge: Boolean
        get() = edgeColor != null || edgeStyle != null || edgeWidth != null
    val appliesToNode: Boolean
        get() = nodeBackgroundColor != null || nodeColor != null || nodeStyle != null
    val appliesToNodeFont: Boolean
        get() = nodeFontBold != null || nodeFontFamily != null || nodeFontItalic != null || nodeFontSize != null
    val appliesToNodeIcon: Boolean
        get() = nodeIcon != null
    val appliesToChildren: Boolean
        get() = childrenStylePattern != null

    protected fun loadPattern(pattern: XMLElement, justConstructedPatterns: List<StylePattern>) {
        // PATTERN
        if (pattern.getStringAttribute("name") != null) {
            name = pattern.getStringAttribute("name")
        }
        if (safeEquals(pattern.getStringAttribute("recursive"), "true")) {
            recursive = true
        }
        val i: Iterator<XMLElement> = pattern.getChildren()!!.iterator()
        while (i.hasNext()) {
            ...
            if (child.name == "node") {
                if (child.getStringAttribute("color") != null
                    ...
                    nodeColor = xmlToColor(child
                            .getStringAttribute("color"))
                }
                if (child.getStringAttribute("background_color") != null
                        && child.getStringAttribute("background_color")!!
                                .length == 7) {
                    nodeBackgroundColor = xmlToColor(child
                            .getStringAttribute("background_color"))
                }
                if (child.getStringAttribute("style") != null) {
                    nodeStyle = child.getStringAttribute("style")
                }
                if (child.getStringAttribute("icon") != null) {
                    nodeIcon = if (child.getStringAttribute("icon") == "none") null else MindIcon.Companion.factory(child.getStringAttribute("icon"))
                }
                text = child.getStringAttribute("text")
                val j: Iterator<XMLElement> = child.getChildren()!!.iterator()
                while (j.hasNext()) {
                    val nodeChild = j.next()
                    // FONT
                    if (nodeChild.name == "font") {
                        if (nodeChild.getStringAttribute("name") != null) {
                            nodeFontFamily = nodeChild
                                    .getStringAttribute("name")
                        }
                        if (safeEquals(
                                        nodeChild.getStringAttribute("bold"), "true")) {
                            nodeFontBold = java.lang.Boolean.TRUE
                        }
                        if (safeEquals(
                                        nodeChild.getStringAttribute("italic"), "true")) {
                            nodeFontItalic = java.lang.Boolean.TRUE
                        }
                        // if (font.getProperty("underline")!=null &&
                        // nodeChild.getProperty("underline").equals("true"))
                        // setUnderlined(true);
                        if (nodeChild.getStringAttribute("size") != null) {
                            nodeFontSize = Integer.valueOf(nodeChild
            ...
            if (child.name == "edge") {
                if (child.getStringAttribute("style") != null) {
                    edgeStyle = child.getStringAttribute("style")
                }
                if (child.getStringAttribute("color") != null) {
                    edgeColor = xmlToColor(child
                            .getStringAttribute("color"))
                }
                if (child.getStringAttribute("width") != null) {
                    edgeWidth = if (child.getStringAttribute("width") == "thin") {
                        EdgeAdapter.Companion.WIDTH_THIN
                    } else {
                        child
                                .getStringAttribute("width")!!.toInt()
                    }
                }
            }
            ...
            if (child.name == "child") {
                if (child.getStringAttribute("pattern") != null) {
                    ...
                    val searchName = child.getStringAttribute("pattern")
                    var anythingFound = false
                    for (patternFound in justConstructedPatterns) {
                        if (patternFound.name == searchName) {
                            childrenStylePattern = patternFound
                            anythingFound = true
                            break
                        }
                    }
                    ...
                    if (name == searchName) {
                        childrenStylePattern = this
                        anythingFound = true
                    }
                    if (anythingFound == false) System.err.println("Cannot find the children "
                            + searchName + " to the pattern " + name)
                }
            }
        }
    }
    ...
    companion object {
        ...
        fun loadPatterns(file: File?): List<*> {
            return loadPatterns(BufferedReader(FileReader(file)))
        }

        @Throws(Exception::class)
        fun loadPatterns(reader: Reader?): List<*> {
            val list: MutableList<*> = LinkedList<Any?>()
            val parser = XMLElement()
            parser.parseFromReader(reader)
            val e = parser.enumerateChildren()
            while (e.hasMoreElements()) {
                list.add(StylePattern(e.nextElement() as XMLElement, list))
            }
            return list
        }
    }

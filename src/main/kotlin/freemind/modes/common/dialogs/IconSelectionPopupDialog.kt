...
class IconSelectionPopupDialog(caller: JFrame?, icons: Vector<IconInformation>,
                               freeMindMain: FreeMindMain) : JDialog(caller, freeMindMain.getResourceString("select_icon")), KeyListener, MouseListener {
    ...
    init {
        ...
        iconPanel.preferredSize = Dimension(xDimension * perIconSize,
                yDimension * perIconSize)
        iconPanel.minimumSize = Dimension(xDimension * perIconSize,
                yDimension * perIconSize)
        iconPanel.maximumSize = Dimension(xDimension * perIconSize,
                yDimension * perIconSize)
        iconPanel.size = Dimension(xDimension * perIconSize, yDimension
                * perIconSize)
    ...
    private fun canSelect(position: Position): Boolean {
        return (position.x >= 0 && position.x < xDimension
    ...
    private fun cursorLeft() {
        val newPosition = Position(selectedPosition.x - 1,
                selectedPosition.y)
    ...
    private fun cursorUp() {
        val newPosition = Position(selectedPosition.x,
    ...
    val modifiers: Int
        get() {
            var m = mModifiers
            if (mModifiers and (ActionEvent.SHIFT_MASK or InputEvent.SHIFT_DOWN_MASK) != 0) m = m or ActionEvent.SHIFT_MASK
    ...
    private fun findIndexByKeyEvent(keyEvent: KeyEvent): Int {
        for (i in icons.indices) {
            ...
            if (iconKeyStroke != null && (keyEvent.keyCode == iconKeyStroke.keyCode && keyEvent.keyCode != 0 && iconKeyStroke.modifiers and KeyEvent.SHIFT_MASK == keyEvent

...
java.net.URLimport javax.swing.*
class FileController(mode: Mode?) : ViewControllerAdapter(mode) {
    ...
    override val modeToolBar: JToolBar?
        get() = (mode as FileMode).toolbar

    override fun newModel(modeController: ModeController?): MapAdapter {
        val model = FileMapModel(frame, modeController)
        ...
        override fun actionPerformed(e: ActionEvent) {
            if (selected != null) {
                val map: MindMap = FileMapModel(
                        (selected as FileNodeModel).file, frame,  /*
    ...
    private inner class OpenPathAction internal constructor() : AbstractAction(controller.getResourceString("open")) {
        override fun actionPerformed(e: ActionEvent) {
            val inputValue = JOptionPane.showInputDialog(controller
                    .view.getSelected(), getText("open"), "")
            if (inputValue != null) {
                val newCenter = File(inputValue)
                if (newCenter.exists()) { // and is a folder
                    val map: MindMap = FileMapModel(newCenter, frame,  /*

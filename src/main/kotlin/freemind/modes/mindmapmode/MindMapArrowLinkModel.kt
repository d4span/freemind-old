...
class MindMapArrowLinkModel(source: MindMapNode?, target: MindMapNode?,
                            pMapFeedback: MapFeedback?) : ArrowLinkAdapter(source, target, pMapFeedback) {
	 ...
	 * int)
	 */
    override fun changeInclination(map: MapView, originX: Int, originY: Int,
                                   deltaX: Int, deltaY: Int) {
        var distSqToTarget = 0.0
        var distSqToSource = 0.0
        val targetView = map.getNodeView(getTarget())
        val sourceView = map.getNodeView(source)
        if (targetView != null && sourceView != null) {
            val targetLinkPoint = targetView
                    .getLinkPoint(getEndInclination())
            val sourceLinkPoint = sourceView
                    .getLinkPoint(getStartInclination())
            distSqToTarget = targetLinkPoint.distanceSq(originX.toDouble(), originY.toDouble())
            distSqToSource = sourceLinkPoint.distanceSq(originX.toDouble(), originY.toDouble())
        }
        if ((targetView == null || sourceView != null)
                && distSqToSource < distSqToTarget * 2.25) {
            val changedInclination = getStartInclination()
        ...
        if ((sourceView == null || targetView != null)
            ...
            val changedInclination = getEndInclination()
            changeInclination(deltaX, deltaY, targetView, changedInclination)
            setEndInclination(changedInclination)
        }
    }

    private fun changeInclination(deltaX: Int, deltaY: Int,
                                  linkedNodeView: NodeView?, changedInclination: Point?) {
        var deltaX = deltaX
        ...
        if (changedInclination.x != 0
                && Math.abs(changedInclination.y.toDouble() / changedInclination.x) < 0.015) {
            changedInclination.y = 0
        }
        val k = changedInclination.distance(0.0, 0.0)
        if (k < 10) {
            if (k > 0) {
                changedInclination.x = (changedInclination.x * 10 / k).toInt()
                changedInclination.y = (changedInclination.y * 10 / k).toInt()
            } else {

...
internal class MindMapHTMLWriter(private val fileout: Writer) {
    private var writeFoldingCode = false
    private val basedOnHeadings: Boolean
    ...
    fun saveHTML(mindMapNodes: List<MindMapNodeModel>) {
        fileout.write("<html>" + el + "<head>" + el)
        writeStyle()
        fileout.write(el + "</head>" + el + "<body>" + el)
        val iterator = mindMapNodes.iterator()
        while (iterator.hasNext()) {
            saveHTML(iterator.next(), "1", 0,  /* isRoot */true, true,  /* depth */1)
        }
    ...
    @Throws(IOException::class)
    fun saveHTML(rootNodeOfBranch: MindMapNodeModel) {
        ...
        val htmlExportFoldingOption = getProperty("html_export_folding")
        writeFoldingCode = ((htmlExportFoldingOption
                == "html_export_fold_currently_folded") && rootNodeOfBranch
                .hasFoldedStrictDescendant()
                || htmlExportFoldingOption == "html_export_fold_all")
    ...
    @Throws(IOException::class)
    private fun writeBodyWithFolding(rootNodeOfBranch: MindMapNodeModel) {
        writeJavaScript()
    ...
    @Throws(IOException::class)
    private fun saveHTML(model: MindMapNodeModel, parentID: String,
                         lastChildNumber: Int, isRoot: Boolean, treatAsParagraph: Boolean,
                         depth: Int): Int {
        // return lastChildNumber
        // Not very beautiful solution, but working at least and logical too.
        var lastChildNumber = lastChildNumber
        var createFolding = model.isFolded
        if (getProperty("html_export_folding") == "html_export_fold_all") {
            createFolding = model.hasChildren()
        }
        if (getProperty("html_export_folding") == "html_export_no_folding" || basedOnHeadings || isRoot) {
            createFolding = false
        }
        val heading = isHeading(model, depth)
        if (!treatAsParagraph && !basedOnHeadings) fileout.write("<li>") else {
            if (heading) {
        ...
        for (hook in model.activatedHooks) {
        ...
        if (link != null) {
            ...
            fileout.write("<a href=\"" + HtmlTools.unicodeToHTMLUnicodeEntity(link, false) + "\" target=\"_blank\"><span class=l>~</span>&nbsp;")
        }
        val fontStyle = fontStyle(model)
        if (fontStyle != "") {
        ...
        var treatChildrenAsParagraph = false
        val e: ListIterator<*>? = model.sortedChildrenUnfolded()
        while (e!!.hasNext()) {
            if ((e.next() as MindMapNodeModel).toString().length > 100) { // TODO:
                // replace
                // heuristic
                // constant
        ...
        if (getProperty("html_export_folding") ==
                "html_export_based_on_headings") {
            val e: ListIterator<*>? = model.sortedChildrenUnfolded()
            while (e!!.hasNext()) {
                val child = e.next() as MindMapNodeModel
    ...
    @Throws(IOException::class)
    private fun fontStyle(model: MindMapNodeModel): String {
        var fontStyle = ""
        if (model.color != null) {
            fontStyle += "color: " + colorToXml(model.color) + ";"
        }
        if (model.font != null && model.font.size != 0) {
            val defaultFontSize = getProperty("defaultfontsize").toInt()
            val procentSize = (model.font.size * 100 / defaultFontSize)
            if (procentSize != 100) {
                fontStyle += "font-size: $procentSize%;"
            }
        }
        if (model.font != null) {
            val fontFamily = model.font.family
    ...
    @Throws(IOException::class)
    private fun writeModelContent(model: MindMapNodeModel) {
    ...
    @Throws(IOException::class)
    private fun writeIcons(model: MindMapNodeModel) {
        for (i in model.icons.indices) {
            fileout.write("<img src=\""
                    + (model.icons[i] as MindIcon).iconFileName
                    + "\" alt=\""
                    + (model.icons[i] as MindIcon).description
                    + "\">")
        }
    }

    @Throws(IOException::class)
    private fun writeFoldingButtons(localParentID: String) {
    ...
    private fun isHeading(model: MindMapNodeModel, depth: Int): Boolean {
        return basedOnHeadings && model.hasChildren() && depth <= 6 && !hasHtml(model)
    }

    fun hasHtml(model: MindMapNodeModel): Boolean {
        return model.text.startsWith("<html>")
    }
    ...
    companion object {
        private val el = System.getProperty("line.separator")
        private fun convertSpecialChar(c: Char): String {
            val cvt: String
            cvt = when (c.code) {
                0xe4 -> "&auml;"
                0xf6 -> "&ouml;"
                ...
                0xdc -> "&Uuml;"
                0xdf -> "&szlig;"
                else -> "&#" + Integer.toString(c.code) + ";"
            }
            return cvt
        }

        private fun saveHTML_escapeUnicodeAndSpecialCharacters(text: String): String {
            val len = text.length

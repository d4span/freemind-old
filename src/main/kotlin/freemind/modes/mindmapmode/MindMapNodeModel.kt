...
java.io.*import java.util.HashMapimport
 ...
 */
open class MindMapNodeModel(userObject: Any?, pMap: MindMap?) : NodeAdapter(userObject, pMap) {
    //
    // Constructors
    //
    constructor(pMap: MindMap?) : this(null, pMap) {}

    init {
        children = LinkedList()
        edge = MindMapEdgeModel(this, mapFeedback)
    }
    ...
    override val plainTextContent: String
        get() = HtmlTools.htmlToPlain(toString())

    @Throws(IOException::class)
    fun saveTXT(fileout: Writer, depth: Int) {
        val plainTextContent = plainTextContent
    ...
    @Throws(IOException::class)
    private fun saveChildrenText(fileout: Writer, depth: Int) {
        val e: ListIterator<*>? = sortedChildrenUnfolded()
        while (e!!.hasNext()) {
    ...
    fun collectColors(colors: HashSet<Color?>) {
        if (color != null) {
            colors.add(getColor())
        }
        val e = childrenUnfolded()
        while (e!!.hasNext()) {
            (e.next() as MindMapNodeModel).collectColors(colors)
        }
    }

    private fun saveRFT_escapeUnicodeAndSpecialCharacters(text: String?): String {
        val len = text!!.length
    ...
    @Throws(IOException::class)
    fun saveRTF(fileout: Writer, depth: Int, colorTable: HashMap<Color?, Int>) {
        var pre = "{" + "\\li" + depth * 350
        val level: String
        level = if (depth <= 8) {
            "\\outlinelevel$depth"
        } else {
        ...
        if (font != null && font!!.size != 0) {
            fontsize = "\\u000cs" + Math.round(1.5 * getFont().size)
            pre += fontsize
        }
        ...
        if (this.toString().matches(" *")) {
            ...
            val text = saveRFT_escapeUnicodeAndSpecialCharacters(plainTextContent)
            if (link != null) {
                val link = saveRFT_escapeUnicodeAndSpecialCharacters(link)
                if (link == this.toString()) {
    ...
    @Throws(IOException::class)
    private fun saveChildrenRTF(fileout: Writer, depth: Int, colorTable: HashMap<Color?, Int>) {
        val e: ListIterator<*>? = sortedChildrenUnfolded()
        while (e!!.hasNext()) {
            val child = e.next() as MindMapNodeModel
            if (child.isVisible) {

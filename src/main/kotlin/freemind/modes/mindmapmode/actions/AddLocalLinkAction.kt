 ...
 * @author foltin
 */
class AddLocalLinkAction(modeController: MindMapController) : MindmapAction("paste_as_local_link", "images/stock_right.png", modeController) {
    ...
    override fun actionPerformed(e: ActionEvent) {
        val source = modeController.getSelected()
        val nodesFromClipboard = getMindMapNodesFromClipboard(modeController!!)
        if (nodesFromClipboard.size == 0) {
            modeController.controller.errorMessage(
                    modeController.getText("no_copied_nodes"))
            return
        }
        var first = true
        for (destination in nodesFromClipboard) {
            if (!first) {
                FreemindAction.Companion.logger!!.warning("Can't link the node '$source' to more than one destination. Only the last is used.")
            }
            if (source !== destination) {
                modeController.setLink(source, "#" + modeController.getNodeID(destination))
            } else {
                // hmm, give an error?
                FreemindAction.Companion.logger!!.warning("Can't link the node '$source' onto itself. Skipped.")
            }
            first = false
        }
    }
    ...
    override fun isEnabled(pItem: JMenuItem?, pAction: Action?): Boolean {
        return (super.isEnabled(pItem, pAction)
                && modeController != null
                && getMindMapNodesFromClipboard(modeController).size == 1)
    }
}
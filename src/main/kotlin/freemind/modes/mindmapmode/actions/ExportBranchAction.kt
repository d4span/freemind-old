...
/**  */
class ExportBranchAction(private val mMindMapController: MindMapController) : MindmapAction("export_branch_new", mMindMapController) {
    override fun actionPerformed(e: ActionEvent) {
        val node = mMindMapController
                .selected as MindMapNodeModel

        // if something is wrong, abort.
        if (mMindMapController.map == null || node == null || node.isRoot) {
            mMindMapController.frame.err("Could not export branch.")
            return
        }
        // If the current map is not saved yet, save it first.
        if (mMindMapController.map.file == null) {
            mMindMapController.frame.out(
        ...
        val chooser = mMindMapController.fileChooser
        chooser.selectedFile = File(getFileNameProposal(node)
                + FreeMindCommon.FREEMIND_FILE_EXTENSION)
        val returnVal = chooser!!.showSaveDialog(mMindMapController
                .selectedView)
        if (returnVal == JFileChooser.APPROVE_OPTION) {
            var chosenFile = chooser.selectedFile
            // Force the extension to be .mm
            val ext = getExtension(chosenFile!!.name)
            ...
            try {
                ...
                JOptionPane.showMessageDialog(mMindMapController.view,
			 ...
			 * to the new Map.
			 */
            val parent = node.parentNode as MindMapNodeModel
            // set a link from the new root to the old map
            val linkToNewMapString = fileToRelativeUrlString(
                    mMindMapController.model.file, chosenFile)
            mMindMapController.setLink(node, linkToNewMapString)
            val nodePosition = parent.getChildPosition(node)
            ...
            val newModeController = mMindMapController.mode
                    .createModeController()
            val newMap = MindMapMapModel(node,
                    newModeController)
            ...
            try {
                ...
                val message = expandPlaceholders(mMindMapController.getText("save_failed"),
                        chosenFile.name)
                mMindMapController.controller.errorMessage(message)
                return
            }
            // new node instead:
            val newNode = mMindMapController.addNewNode(parent,
                    nodePosition, node.isLeft)
            // TODO: Keep formatting of node.
            mMindMapController.setNodeText(newNode, node.text)
            val linkString = fileToRelativeUrlString(chosenFile, mMindMapController.model.file)

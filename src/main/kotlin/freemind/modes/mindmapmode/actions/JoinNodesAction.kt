...
class JoinNodesAction(private val controller: MindMapController) : MindmapAction("join_nodes", controller) {
    override fun actionPerformed(e: ActionEvent) {
        val selectedNode = controller.view.selected
                .model
        val selectedNodes = controller.view
                .selectedNodesSortedByY
    ...
    fun joinNodes(selectedNode: MindMapNode, selectedNodes: List<MindMapNode>) {
        ...
        var isHtml = false
        for (node in selectedNodes) {
            val nodeContent = node.toString()
            val isHtmlNode = HtmlTools.isHtmlNode(nodeContent)
    ...
    private fun addContent(content: String, isHtml: Boolean,
                           nodeContent: String, isHtmlNode: Boolean): String {
        var content = content
        var nodeContent = nodeContent
        if (isHtml) {
            val start = BODY_END.split(content, -2)
            content = start[0]
            if (!isHtmlNode) {
                val end = BODY_START.split(content, 2)
                nodeContent = end[0] + "<body><p>" + nodeContent + "</p>"
            }
        }
        if (isHtmlNode and (content != "")) {
            val end = BODY_START.split(nodeContent, 2)
            nodeContent = end[1]
            if (!isHtml) {
                content = end[0] + "<body><p>" + content + "</p>"
            }
        }
        if (!(isHtml || isHtmlNode || content == "")) {
            content += " "
        }
        content += nodeContent
        return content
    ...
    companion object {
        val BODY_START = Pattern.compile("<body>",
                Pattern.CASE_INSENSITIVE)
        val BODY_END = Pattern.compile("</body>",
                Pattern.CASE_INSENSITIVE)
    }
}
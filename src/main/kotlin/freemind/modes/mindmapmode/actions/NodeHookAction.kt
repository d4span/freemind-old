...
java.util.logging.Loggerimport javax.swing.*
class NodeHookAction(
        /**
         */
        override var hookName: String?, var controller: MindMapController) : MindmapAction(hookName, null as Icon?, controller), HookAction, MenuItemEnabledListener, MenuItemSelectedListener {

    init {
        if (logger == null) logger = controller.frame.getLogger(this.javaClass.name)
    }

    override fun actionPerformed(arg0: ActionEvent) {
        // check, which method of invocation:
        //
        controller.frame.setWaitingCursor(true)
        invoke(controller.selected,
                controller.selecteds)
        controller.frame.setWaitingCursor(false)
    }

    operator fun invoke(focussed: MindMapNode?, selecteds: List<MindMapNode?>?) {
        controller.addHook(focussed, selecteds, hookName, null)
    }
    ...
    private val hookFactory: HookFactory?
        private get() = controller.hookFactory
    ...
    override fun isEnabled(item: JMenuItem?, action: Action?): Boolean {
        if (!super.isEnabled(item, action) || controller.view == null) {
            return false
        }
        val factory = hookFactory
        val baseClass = factory!!.getPluginBaseClass(hookName)
        if (baseClass != null) {
            if (baseClass is MenuItemEnabledListener) {
                return baseClass.isEnabled(item, action)
            }
        }
        return true
    }
    ...
    override fun isSelected(pCheckItem: JMenuItem?, pAction: Action?): Boolean {
        ...
        val baseClass = factory!!.getPluginBaseClass(hookName)
        if (baseClass != null) {
            if (baseClass is MenuItemSelectedListener) {
                return baseClass.isSelected(pCheckItem, pAction)
            }
        }
        val focussed = controller.selected
        val selecteds = controller.selecteds
        val instMethod = getInstanciationMethod(hookName)
        // get destination nodes
        instMethod!!.getDestinationNodes(controller, focussed, selecteds)
        val adaptedFocussedNode = instMethod.getCenterNode(
                controller, focussed, selecteds)
        // test if hook already present
        return instMethod.isAlreadyPresent(hookName!!, adaptedFocussedNode)
    }

...
javax.swing.*
class NodeStyleAction(controller: MindMapController, private val mStyle: String?) : NodeGeneralAction(controller, mStyle, null), MenuItemSelectedListener {
    /* (non-Javadoc)
    ...
    override fun getActionPair(pSelected: MindMapNodeModel): ActionPair? {
        return mindMapController.actorFactory.nodeStyleActor.getActionPair(pSelected, mStyle)
    }

    override fun isSelected(pCheckItem: JMenuItem?, pAction: Action?): Boolean {
        val selected = modeController.selected
        return if (!selected.hasStyle()) false else safeEquals(mStyle, selected.style)
    }
}
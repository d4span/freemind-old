...
class AddHookActor(pMapFeedback: ExtendedMapFeedback) : XmlActorAdapter(pMapFeedback) {
    ...
    private val hookFactory: HookFactory?
        private get() = exMapFeedback.hookFactory
    ...
    fun addHook(focussed: MindMapNode?, selecteds: List<MindMapNode?>?, hookName: String?, pHookProperties: Properties?) {
        val doAction = createHookNodeAction(focussed, selecteds,
                hookName, pHookProperties)
        var undoAction: XmlAction? = null
    ...
    private fun createHookNodeUndoAction(focussed: MindMapNode?,
                                         selecteds: List<MindMapNode?>?, hookName: String?): XmlAction {
        val undoAction = CompoundAction()
        val hookNodeAction = createHookNodeAction(focussed,
        ...
        if (instMethod.isAlreadyPresent(hookName!!, adaptedFocussedNode)) {
            ...
            for (currentDestinationNode in destinationNodes) {
                ...
                for (hook in currentDestinationNode.activatedHooks) {
                        ...
                        if (hook !is DontSaveMarker) {
                            ...
                            if (child.countChildren() == 1) {
                                val parameters = child
                                        .getChildren()!!.firstElement() as XMLElement
                                if (safeEquals(parameters.name,
    ...
    fun createHookNodeAction(focussed: MindMapNode?,
                             selecteds: List<MindMapNode?>?, hookName: String?, pHookProperties: Properties?): HookNodeAction {
    ...
    override fun act(action: XmlAction?) {
        if (action is HookNodeAction) {
            ...
            val selected: MindMapNode? = getNodeFromID(
                    hookNodeAction.node!!)
            val selecteds = Vector<MindMapNode?>()
            val i: Iterator<NodeListMember> = hookNodeAction.listNodeListMemberList.iterator()
            while (i.hasNext()) {
            ...
            val it: Iterator<NodeChildParameter> = hookNodeAction.listNodeChildParameterList.iterator()
            while (it.hasNext()) {
                val childParameter = it.next()
                child.setAttribute(childParameter.key!!,
    ...
    private operator fun invoke(focussed: MindMapNode?, selecteds: List<MindMapNode?>?, hookName: String?,
        ...
        if (instMethod.isAlreadyPresent(hookName!!, adaptedFocussedNode)) {
            ...
            // add the hook
            for (currentDestinationNode in destinationNodes) {
                val hook = exMapFeedback.createNodeHook(hookName,
    ...
    protected val viewAbstraction: ViewAbstraction
        protected get() = exMapFeedback.viewAbstraction
     ...
     */
    private fun finishInvocation(focussed: MindMapNode?, selecteds: List<MindMapNode?>?, adaptedFocussedNode: MindMapNode, destinationNodes: Collection<MindMapNode>) {

...
(pMapFeedback: ExtendedMapFeedback) : XmlActorAdapter(pMapFeedback) {
    fun setCloudColor(node: MindMapNode?, color: Color?) {
        ...
        val undoAction = createCloudColorXmlAction(node,
                if (node.getCloud() == null) null else node.getCloud().color)
        execute(ActionPair(doAction, undoAction))
    }

    fun createCloudColorXmlAction(node: MindMapNode?,
                                  color: Color?): CloudColorXmlAction {
    ...
    override fun act(action: XmlAction?) {
        if (action is CloudColorXmlAction) {
            ...
            if (node.getCloud() != null) {
                selectedColor = node.getCloud().color
            }
            if (!safeEquals(color, selectedColor)) {
                (node.getCloud() as LineAdapter).color = color // null
    ...
    override val doActionClass: Class<*>
        get() = CloudColorXmlAction::class.java
}
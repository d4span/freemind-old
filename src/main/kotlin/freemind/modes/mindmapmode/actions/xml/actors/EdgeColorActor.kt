...
(pMapFeedback: ExtendedMapFeedback) : XmlActorAdapter(pMapFeedback) {
    fun setEdgeColor(node: MindMapNode?, color: Color?) {
        val doAction = createEdgeColorFormatAction(node,
                color)
        val undoAction = createEdgeColorFormatAction(node,
                (node.getEdge() as EdgeAdapter).realColor)
    ...
    override fun act(action: XmlAction?) {
        if (action is EdgeColorFormatAction) {
            ...
            val color = xmlToColor(edgeAction.color)
            val node: MindMapNode? = getNodeFromID(edgeAction.node!!)
            val oldColor = (node.getEdge() as EdgeAdapter).realColor
            if (!safeEquals(color, oldColor)) {
                (node.getEdge() as MindMapEdgeModel).color = color
                exMapFeedback.nodeChanged(node)
            }
        }
    }

    fun createEdgeColorFormatAction(node: MindMapNode?,
                                    color: Color?): EdgeColorFormatAction {
        val edgeAction = EdgeColorFormatAction()
        edgeAction.node = getNodeID(node)
        if (color != null) {
            edgeAction.color = colorToXml(color)
        }
        return edgeAction
    }
    ...
    override val doActionClass: Class<*>
        get() = EdgeColorFormatAction::class.java
}
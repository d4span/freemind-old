...
(pMapFeedback: ExtendedMapFeedback) : XmlActorAdapter(pMapFeedback) {
    ...
    fun getActionPair(node: MindMapNode, fontFamilyValue: String?): ActionPair {
        val fontFamilyAction = createFontNodeAction(node,
                fontFamilyValue)
        val undoFontFamilyAction = createFontNodeAction(node,
                node.fontFamilyName)
        return ActionPair(fontFamilyAction, undoFontFamilyAction)
    }

    private fun createFontNodeAction(node: MindMapNode,
                                     fontValue: String?): FontNodeAction {
    ...
    override fun act(action: XmlAction?) {
        if (action is FontNodeAction) {
            ...
            val node: MindMapNode? = getNodeFromID(fontFamilyAction.node!!)
            val fontFamily = fontFamilyAction.font
            if (!safeEquals(node.getFontFamilyName(), fontFamily)) {
                (node as NodeAdapter?)!!.establishOwnFont()
                node.setFont(exMapFeedback.getFontThroughMap(
                        Font(fontFamily, node.getFont().style, node
                                .getFont().size)))

...
(pMapFeedback: ExtendedMapFeedback) : XmlActorAdapter(pMapFeedback) {
    override val doActionClass: Class<*>
        get() = FontSizeNodeAction::class.java

    /**
     */
    fun setFontSize(node: MindMapNode, fontSizeValue: String?) {
        if (safeEquals(fontSizeValue, node.fontSize)) {
            return
        }
    ...
    fun getActionPair(node: MindMapNode, fontSizeValue: String?): ActionPair {
        val fontSizeAction = createFontSizeNodeAction(node,
                fontSizeValue)
        val undoFontSizeAction = createFontSizeNodeAction(node,
                node.fontSize)
        return ActionPair(fontSizeAction, undoFontSizeAction)
    }

    private fun createFontSizeNodeAction(node: MindMapNode,
                                         fontSizeValue: String?): FontSizeNodeAction {
    ...
    override fun act(action: XmlAction?) {
        if (action is FontSizeNodeAction) {
            ...
            val node: MindMapNode? = getNodeFromID(fontSizeAction.node!!)
            try {

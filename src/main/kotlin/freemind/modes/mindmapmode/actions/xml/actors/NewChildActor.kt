...
(pMapFeedback: ExtendedMapFeedback) : XmlActorAdapter(pMapFeedback) {
    ...
    override fun act(action: XmlAction?) {
        ...
        // call hooks:
        for (hook in parent!!.activatedHooks) {
            hook!!.onNewChild(newNode)
        }
        // done.
    }

    protected override val linkRegistry: MindMapLinkRegistry?
        protected get() = exMapFeedback.map.linkRegistry
    ...
    override val doActionClass: Class<*>
        get() = NewNodeAction::class.java

    fun addNewNode(parent: MindMapNode?, index: Int,
        ...
        val newNodeAction = getAddNodeAction(parent, index, newId,
                newNodeIsLeft)
        // Undo-action
        val deleteAction = exMapFeedback.actorFactory.deleteChildActor
                .getDeleteNodeAction(newId)
        exMapFeedback.doTransaction(exMapFeedback.getResourceString("new_child"),
                ActionPair(newNodeAction, deleteAction))
        return parent!!.getChildAt(index) as MindMapNode
    }

    fun getAddNodeAction(parent: MindMapNode?, index: Int,
                         newId: String?, newNodeIsLeft: Boolean): NewNodeAction {

 ...
 * @date 25.03.2014
 */
class RemoveAllIconsActor
/**
 * @param pMapFeedback
 */
(pMapFeedback: ExtendedMapFeedback) : NodeXmlActorAdapter(pMapFeedback) {
    fun createRemoveAllIconsXmlAction(
            node: MindMapNode?): RemoveAllIconsXmlAction {
        val action = RemoveAllIconsXmlAction()
        action.node = getNodeID(node)
        return action
    }

    override fun act(action: XmlAction?) {
        if (action is RemoveAllIconsXmlAction) {
            val node: MindMapNode? = getNodeFromID(action
                    .node!!)
            while (node.getIcons().size > 0) {
                node!!.removeIcon(MindIcon.Companion.LAST)
            }
    ...
    fun removeAllIcons(node: MindMapNode?) {
        exMapFeedback.doTransaction(
                this.javaClass.name, apply(exMapFeedback.map, node))
    }

    override val doActionClass: Class<*>
        get() = RemoveAllIconsXmlAction::class.java

    override fun apply(model: MindMap?, selected: MindMapNode?): ActionPair? {
        val undoAction = CompoundAction()
        for (icon in selected.getIcons()) {
            undoAction.addChoice(exMapFeedback.actorFactory.addIconActor.createAddIconAction(selected,
                    icon, MindIcon.Companion.LAST))
        }
        return ActionPair(createRemoveAllIconsXmlAction(selected),
                undoAction)
    }
}
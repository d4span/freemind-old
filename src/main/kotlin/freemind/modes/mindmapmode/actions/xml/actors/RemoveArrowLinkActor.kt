...
(pMapFeedback: ExtendedMapFeedback) : XmlActorAdapter(pMapFeedback) {
    ...
    /**
     */
    private fun getActionPair(arrowLink: MindMapLink?): ActionPair {
        return ActionPair(
    ...
    override fun act(action: XmlAction?) {
        if (action is RemoveArrowLinkXmlAction) {
            val removeAction = action
            val arrowLink = linkRegistry.getLinkForId(
                    removeAction.id)
                    ?: // strange: link not found:
                    throw IllegalArgumentException("Unknown link to id "
                            + removeAction.id + " should be deleted.")
            linkRegistry.deregisterLink(arrowLink)
            exMapFeedback.nodeChanged(arrowLink.source)
            exMapFeedback.nodeChanged(arrowLink.target)
        }
    }

    override val doActionClass: Class<*>
        get() = RemoveArrowLinkXmlAction::class.java

    fun createRemoveArrowLinkXmlAction(id: String?): RemoveArrowLinkXmlAction {
        val action = RemoveArrowLinkXmlAction()
        action.id = id
        return action
    }

    fun createAddArrowLinkXmlAction(link: MindMapLink?): AddArrowLinkXmlAction {
        val action = AddArrowLinkXmlAction()
        action.node = getNodeID(link.getSource())
        action.destination = getNodeID(link.getTarget())
        action.newId = link.getUniqueId()
        action.color = colorToXml(link.getColor())
        if (link is MindMapArrowLink) {
            val arrowLink = link
            action.endArrow = arrowLink.endArrow
            action.endInclination = PointToXml(arrowLink
                    .endInclination)
            action.startArrow = arrowLink.startArrow
            action.startInclination = PointToXml(arrowLink
                    .startInclination)
        }
        return action
    }

...
(pMapFeedback: ExtendedMapFeedback) : XmlActorAdapter(pMapFeedback) {
    ...
    override fun act(action: XmlAction?) {
        if (action is AddLinkXmlAction) {
            val linkAction = action
            val node = getNodeFromID(linkAction.node!!)
            node.link = linkAction.destination
            exMapFeedback.nodeChanged(node)
        }
    }

    override val doActionClass: Class<*>
        get() = AddLinkXmlAction::class.java

    private fun getActionPair(node: MindMapNode?, link: String?): ActionPair {
        return ActionPair(createAddLinkXmlAction(node, link),
                createAddLinkXmlAction(node, node.getLink()))
    }

    private fun createAddLinkXmlAction(node: MindMapNode?,
                                       link: String?): AddLinkXmlAction {
        val action = AddLinkXmlAction()

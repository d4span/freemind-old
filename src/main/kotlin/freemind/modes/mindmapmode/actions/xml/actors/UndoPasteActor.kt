...
(pMapFeedback: ExtendedMapFeedback) : XmlActorAdapter(pMapFeedback) {
    ...
    override fun act(pAction: XmlAction?) {
        if (pAction is UndoPasteNodeAction) {
            val undoAction = pAction
            val selectedNode: MindMapNode? = getNodeFromID(undoAction.node!!)
            var amount = undoAction.nodeAmount
            while (amount > 0) {
                val coordinate = NodeCoordinate(selectedNode,
                        undoAction.asSibling, undoAction.getIsLeft())
                val targetNode = coordinate.node
                xmlActorFactory.deleteChildActor.deleteWithoutUndo(targetNode)
                amount--
            }
        }
    }
    ...
    override val doActionClass: Class<*>
        get() = UndoPasteNodeAction::class.java
}
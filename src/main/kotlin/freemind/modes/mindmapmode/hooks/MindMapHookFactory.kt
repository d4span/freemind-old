 ...
 * @author foltin
 */
class MindMapHookFactory : HookFactoryAdapter() {
    /**
     *
     */
    init {
        ...
        allRegistrationInstances = HashMap()
    }
    ...
    override val possibleNodeHooks: Vector<String?>
        get() = searchFor(NodeHook::class.java, MindMapController::class.java)

    /**
     * @return a string vector with representatives for plugins.
     */
    override val possibleModeControllerHooks: Vector<String?>
        get() = searchFor(ModeControllerHook::class.java, MindMapController::class.java)

    /**
     * @return a string vector with representatives for plugins.
     */
    private fun searchFor(baseClass: Class<*>, mode: Class<*>): Vector<String?> {
        actualizePlugins()
        val returnValue = Vector<String?>()
        val modeName = mode.getPackage().name
        for (label in allPlugins!!) {
            val descriptor = getHookDescriptor(label)
    ...
    private fun actualizePlugins() {
        if (importWizard == null) {
            ...
            pluginInfo = HashMap()
            allPlugins = Vector()
            allRegistrations = HashSet()
            // the unmarshaller:
            val unmarshaller: IUnmarshallingContext = XmlBindingTools.getInstance()
                    .createUnmarshaller()
            // the loop
            for (xmlPluginFile in importWizard!!.CLASS_LIST) {
                if (xmlPluginFile.matches(pluginPrefixRegEx)) {
                    ...
                    val pluginURL: URL = Resources.getInstance().getFreeMindClassLoader().getResource(
                            xmlPluginFile)
                    // unmarshal xml:
                    ...
                    plugin = try {
                        ...
                        unmarshaller.unmarshalDocument(`in`,
                                null) as Plugin
                    } catch (e: Exception) {
                        // error case
                        Resources.getInstance().logException(e)
                    ...
                    // plugin is loaded.
                    for (obj in plugin!!.listChoiceList) {
                        if (obj is PluginAction) {
                            val action = obj
                            pluginInfo!![action.label] = HookDescriptorPluginAction(xmlPluginFile, plugin, action)
                            allPlugins!!.add(action.label)
                        } else if (obj is PluginRegistration) {
                            allRegistrations!!
                                    .add(HookDescriptorRegistration(xmlPluginFile, plugin, obj))
    ...
    override fun createModeControllerHook(hookName: String?): ModeControllerHook? {
        val descriptor = getHookDescriptor(hookName)
        return createJavaHook(hookName, descriptor) as ModeControllerHook?
    }

    private fun createJavaHook(hookName: String?,
                               descriptor: HookDescriptorPluginAction): MindMapHook? {
        return try {
            // constructed.
            val loader = descriptor.pluginClassLoader
            val hookClass = Class.forName(descriptor.className, true,
                    loader)
            val hook = hookClass.newInstance() as MindMapHook
            decorateHook(hookName, descriptor, hook)
            hook
        } catch (e: Throwable) {
            var path = ""
            for (plPath in descriptor.pluginClasspath) {
                path += plPath.jar + ";"
            }
            Resources.getInstance().logException(
                    e,
                    """
                        Error occurred loading hook: ${descriptor.className}
                        Classpath: $path
                        Exception:
                        """.trimIndent())
            null
    ...
    override fun createNodeHook(hookName: String?): NodeHook? {
        ...
        val descriptor = getHookDescriptor(hookName)
        return createJavaHook(hookName, descriptor) as NodeHook?
    }

    private fun decorateHook(hookName: String?,
                             descriptor: HookDescriptorPluginAction, hook: MindMapHook) {
        hook.setProperties(descriptor.properties)
        hook.name = hookName
        val pluginBaseClassSearcher: PluginBaseClassSearcher = object : PluginBaseClassSearcher {
            override val pluginBaseObject: Any?
                get() = getPluginBaseClass(descriptor)
        }
    ...
    /**
     */
    fun decorateAction(hookName: String?, action: AbstractAction) {
        val descriptor = getHookDescriptor(hookName)
        val name = descriptor.name
        if (name != null) {
            action.putValue(AbstractAction.NAME, name)
        } else {
            action.putValue(AbstractAction.NAME, descriptor.className)
        }
        val docu = descriptor.documentation
        if (docu != null) {
            action.putValue(AbstractAction.SHORT_DESCRIPTION, docu)
            action.putValue(AbstractAction.LONG_DESCRIPTION, docu)
        }
        val icon = descriptor.iconPath
        if (icon != null) {
            val imageIcon = ImageFactory.getInstance().createIcon(descriptor
                    .pluginClassLoader!!.getResource(icon))
            action.putValue(AbstractAction.SMALL_ICON, imageIcon)
        }
        val key = descriptor.keyStroke
        if (key != null) action.putValue(AbstractAction.ACCELERATOR_KEY,
    ...
    override fun getInstanciationMethod(hookName: String?): HookInstanciationMethod? {
        val descriptor = getHookDescriptor(hookName)
        return descriptor.instanciationMethod
    ...
    override val registrations: List<RegistrationContainer>
        get() {
            val mode = MindMapController::class.java
            actualizePlugins()
            val returnValue = Vector<RegistrationContainer>()
            for (descriptor in allRegistrations!!) {
                // PluginRegistration registration =
                // descriptor.getPluginRegistration();
                var modeFound = false
                for (possibleMode in descriptor.listPluginModeList) {
                    if (mode.getPackage().name
                            == possibleMode!!.className) {
                        modeFound = true
                    }
                }
                if (!modeFound) continue
                try {
                    val plugin = descriptor.pluginBase
                    val loader = descriptor.pluginClassLoader
                    val hookRegistrationClass = Class.forName(descriptor.className, true, loader)
                    val container = RegistrationContainer()
    ...
    /**
     */
    private fun getPluginBaseClass(descriptor: HookDescriptorPluginAction): Any? {
        // test if registration is present:
        var baseClass: Any? = null
        val label = descriptor.pluginBase.label
        if (allRegistrationInstances!!.containsKey(label)) {
            baseClass = allRegistrationInstances!![label]
        }
        return baseClass
    }
    ...
    /**
     */
    private fun getHookDescriptor(hookName: String?): HookDescriptorPluginAction {
        val descriptor = pluginInfo
                .get(hookName)
        require(!(hookName == null || descriptor == null)) { "Unknown hook name $hookName" }
        return descriptor
    }

    fun getMenuItem(pHookName: String?, pHookAction: AbstractAction?): JMenuItem {
        val descriptor = getHookDescriptor(pHookName)
        return if (descriptor.isSelectable) {
            JCheckBoxMenuItem(pHookAction)
        } else {
            JMenuItem(pHookAction)
        }
    }
    ...
    companion object {
         ...
         * subdirectories.
         */
        private const val pluginPrefixRegEx = ".*(accessories(/|\\\\)|)plugins(/|\\\\)[^/\\\\]*"

        // Logging:
        protected var logger: Logger? = null
        private var pluginInfo: HashMap<String?, HookDescriptorPluginAction>? = null
        private var allPlugins: Vector<String?>? = null
        private var importWizard: ImportWizard? = null

        /** Contains PluginRegistrationType -> PluginType relations.  */
        protected var allRegistrations: HashSet<HookDescriptorRegistration>? = null
    }
}
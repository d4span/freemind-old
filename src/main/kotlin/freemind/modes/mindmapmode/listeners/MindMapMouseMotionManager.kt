...
/**  */
class MindMapMouseMotionManager
/**
 *
 */(private val mController: MindMapController) : MapMouseMotionReceiver {
    var originX = -1
    var originY = -1
    var draggedLink: MindMapArrowLink? = null
    private var draggedLinkOldStartPoint: Point? = null
    private var draggedLinkOldEndPoint: Point? = null
    ...
    override fun mouseDragged(e: MouseEvent?) {
        ...
        if (originX >= 0) {
            if (draggedLink != null) {
                val deltaX = ((e.x - originX) / mController
                        .view.zoom).toInt()
                val deltaY = ((e.y - originY) / mController
                        .view.zoom).toInt()
                draggedLink!!.changeInclination(mapView, originX, originY,
                        deltaX, deltaY)
                originX = e.x
                originY = e.y
                mController.view.repaint()
            } else {
                mapView.scrollBy(originX - e.x, originY - e.y)
            }
        }
    }
    ...
    override fun mousePressed(e: MouseEvent?) {
        if (!mController.isBlocked && e!!.button == MouseEvent.BUTTON1) {
            ...
            originX = e.x
            originY = e.y
            draggedLink = mController.view.detectCollision(
    ...
    override fun mouseReleased(e: MouseEvent?) {
        ...
        if (draggedLink != null) {
            ...
            val draggedLinkNewStartPoint = draggedLink.getStartInclination()
            val draggedLinkNewEndPoint = draggedLink.getEndInclination()

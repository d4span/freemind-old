...
internal class ForkMainView : MainView() {
    override fun paint(graphics: Graphics) {
        ...
        g.stroke = BasicStroke(edgeWidth.toFloat())
        // Draw a standard node
        g.color = model.edge.color
        g.drawLine(0, height - edgeWidth / 2 - 1, width, height
                - edgeWidth / 2 - 1)
        g.color = oldColor
    ...
    protected override val mainViewWidthWithFoldingMark: Int
        protected get() {
            ...
            if (nodeView.getModel().isFolded) {
                width += (zoomedFoldingSymbolHalfWidth * 2
                        + zoomedFoldingSymbolHalfWidth)
            }
            return width
        }
    protected override val mainViewHeightWithFoldingMark: Int
        protected get() {
            var height = height
            if (nodeView.getModel().isFolded) {
    ...
    override val deltaX: Int
        get() = if (nodeView.getModel().isFolded && nodeView.isLeft) {
            super.getDeltaX() + zoomedFoldingSymbolHalfWidth * 3
    ...
    override val alignment: Int
        get() = NodeView.Companion.ALIGN_BOTTOM
    ...
    protected val edgeWidth: Int
        protected get() {
            val nodeModel = nodeView.getModel()
            val edge: MindMapEdge = nodeModel.edge

...
class LeftNodeViewLayout : NodeViewLayoutAdapter() {
    ...
    override fun layoutNodeMotionListenerView(nodeMotionView: NodeMotionListenerView) {
        val nodeView = nodeMotionView.movedView
        val content = nodeView.content
        location.x = content!!.width
        location.y = 0
        convertPointToAncestor(content, location, nodeMotionView.parent)
        nodeMotionView.location = location
        nodeMotionView.setSize(LISTENER_VIEW_WIDTH, content.height)
    }

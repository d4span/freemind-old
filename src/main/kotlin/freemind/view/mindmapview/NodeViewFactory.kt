...
private constructor() {
    private class ContentPane internal constructor() : JComponent() {
        init {
    ...
    fun getEdge(newView: NodeView): EdgeView {
        val edgeStyle: Int = newView.getModel().edge.styleAsInt
     ...
     */
    fun newNodeView(model: MindMapNode?, position: Int, map: MapView,
                    parent: Container): NodeView {
        val newView = NodeView(model, position, map, parent)
        if (model!!.isRoot) {
            val mainView: MainView = RootMainView()
            newView.mainView = mainView
            newView.layout = VerticalRootNodeViewLayout.Companion.getInstance()
        } else {
            newView.mainView = newMainView(model)
            if (newView.isLeft) {
                newView.layout = LeftNodeViewLayout.Companion.getInstance()
            } else {
                newView.layout = RightNodeViewLayout.Companion.getInstance()
            }
        }
    ...
    fun newContentPane(view: NodeView?): JComponent {
        return ContentPane()
    }

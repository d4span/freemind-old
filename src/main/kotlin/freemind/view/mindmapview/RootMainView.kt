...
internal class RootMainView : MainView() {
    ...
    override fun paint(graphics: Graphics) {
        ...
        if (nodeView.getModel() == null) return
        val renderingHint = nodeView.map.setEdgesRenderingHint(g)
        paintSelected(g)
        paintDragOver(g)
    ...
    override fun paintDragOver(graphics: Graphics2D) {
        ...
        if (draggedOver == NodeView.Companion.DRAGGED_OVER_SON) {
            ...
            graphics.fillRect(width / 4, 0, width - 1,
                    height - 1)
        } else if (draggedOver == NodeView.Companion.DRAGGED_OVER_SON_LEFT) {
            graphics.paint = GradientPaint((width * 3 / 4).toFloat(), 0,
                    nodeView.map.background, (width / 4).toFloat(), 0,
    ...
    override fun setDraggedOver(p: Point) {
        setDraggedOver(if (dropPosition(p.getX())) NodeView.Companion.DRAGGED_OVER_SON_LEFT else NodeView.Companion.DRAGGED_OVER_SON)
    }
    ...
    override val textWidth: Int
        get() = super.getTextWidth() - width / 10

...
class SharpBezierEdgeView  // controlpoint
    ...
    private fun update() {
        ...
        val w2 = w / 2
        line1.setCurve((start!!.x - deltaX).toFloat(), (start!!.y - deltaY).toFloat(), one!!.x - deltaX,
                one!!.y - deltaY, two!!.x, two!!.y - w2, end!!.x.toFloat(), end!!.y.toFloat())
        line2.setCurve(end!!.x.toFloat(), end!!.y.toFloat(), two!!.x, two!!.y + w2, one!!.x + deltaX, one!!.y
                + deltaY, (start!!.x + deltaX).toFloat(), (start!!.y + deltaY).toFloat())
        graph.reset()
    ...
    override fun paint(g: Graphics2D) {
        ...
        g.color = color
        g.paint = color
        g.stroke = EdgeView.Companion.DEF_STROKE
        g.fill(graph)
        g.draw(graph)
    }
    ...
    override fun createStart() {
        if (source!!.isRoot) {
            ...
            if (w2 == x02) {
                ...
                val delta = (map.zoom * (width / 2 + 1)).toDouble()
            ...
            convertPointToAncestor(mainView, start!!, source!!)
        } else {
            val delta = map.getZoomed(width / 2 + 1)
            super.createStart()
            deltaX = 0

...
class DatabaseRegistration(controller: ModeController, map: MindMap?) : HookRegistration, MenuItemSelectedListener, MenuItemEnabledListener {
    ...
    override fun isSelected(pCheckItem: JMenuItem?, pAction: Action?): Boolean {
        ...
        if (pAction is NodeHookAction) {
            if (pAction.hookName ==
                    DatabaseBasics.Companion.SLAVE_STARTER_NAME) {
                return isSlave
            }
        }
        return isMaster

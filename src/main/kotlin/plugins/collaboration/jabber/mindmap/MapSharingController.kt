 ...
 * Window&gt;Preferences&gt;Java&gt;Code Generation&gt;Code and Comments
 */
class MapSharingController(
        private val jabberConnectionWizardView: MapSharingWizardView,
        val controller: MindMapController?) {
    //The wizard is implemented as a state machine.
    private val STATE_NOT_CONNECTED = 1
    private val STATE_CONNECTED = 2
    private val STATE_SENT_SHARE_REQUEST_PENDING = 3

    /** Waiting for a user to accept or decline the sharing of a map.  */
    private val STATE_RECEIVED_SHARE_REQUEST_PENDING = 4
    private val STATE_SHARING_MAP = 5

    //TODO: Make Jabber server port configurable.
    private val JABBER_SERVER_PORT = 5222
    private var state = 0
    private val mapSharingWizardController: MapSharingController
    private var mapSharingListener: JabberListener? = null
    private var sender: JabberSender? = null
    private var mapSharingRequestingUser: String? = null
    private val frameTitle //The title of the FreeMind window.
            : String
     ...
     */
    private var mapContent: String? = null
    private var mapFileName: String? = null

    init {
        mapSharingWizardController = this //Need an instance of this in
    ...
    private fun addListeners() {
        jabberConnectionWizardView
                .addCloseButtonMouseListener(CloseButtonClickListener())
        jabberConnectionWizardView
                .addNextButtonMouseListener(NextButtonClickListener())
        jabberConnectionWizardView
                .addBackButtonMouseListener(BackButtonClickListener())
        jabberConnectionWizardView
                .addAcceptButtonMouseListener(AcceptButtonClickListener())
        jabberConnectionWizardView
                .addDeclineButtonMouseListener(DeclineButtonClickListener())
    }

    private inner class CloseButtonClickListener : MouseListener {
    ...
    private inner class NextButtonClickListener : MouseListener {
        override fun mouseClicked(arg0: MouseEvent) {
            if (state == STATE_CONNECTED) {
                sender = JabberSender(mapSharingListener!!.getSession(),
                        this@MapSharingController)
                // register action listener:
                controller.getActionFactory().registerFilter(sender)
                ...
                mapSharingListener = JabberListener(controller,
                        mapSharingWizardController, jabberConnectionWizardView
                        .servername, JABBER_SERVER_PORT,
                        jabberConnectionWizardView.username,
                        jabberConnectionWizardView.password)
                ...
                sender!!.sendMapSharingRequest(jabberConnectionWizardView
                        .username
                        + "@" + jabberConnectionWizardView.servername,
                        jabberConnectionWizardView.mapShareUserName)
    ...
    private inner class BackButtonClickListener : MouseListener {
        override fun mouseClicked(arg0: MouseEvent) {
            if (state == STATE_CONNECTED) {
                setState(STATE_NOT_CONNECTED)
    ...
    private inner class AcceptButtonClickListener : MouseListener {
        override fun mouseClicked(arg0: MouseEvent) {
            if (state == STATE_RECEIVED_SHARE_REQUEST_PENDING) {
                ...
                jabberConnectionWizardView.hide()
                val action: RevertXmlAction = controller!!.revertAction
                        .createRevertXmlAction(mapContent, null,
                                mapFileName)
                // no undo possible.
                val pair = ActionPair(action, null)
    ...
    private inner class DeclineButtonClickListener : MouseListener {
        override fun mouseClicked(arg0: MouseEvent) {
            if (state == STATE_RECEIVED_SHARE_REQUEST_PENDING) {
                setState(STATE_CONNECTED)
                sender!!.sendMapSharingInvitationResponse(
     ...
     * @param isAccepted
     */
    fun setMapShareRequestAccepted(userName: String, isAccepted: Boolean) {
    ...
    fun setMapSharingRequested(username: String?, mapContent: String?,
                               mapFileName: String?) {
     ...
     * @param username
     */
    fun setSharingStopped(username: String) {
        stopSharing()
    ...
    private fun setState(newState: Int) {
        state = newState
        when (newState) {
            STATE_CONNECTED ->             //TODO: Update resource files.
                controller!!.frame.setTitle("$frameTitle - Connected")
            STATE_NOT_CONNECTED -> controller!!.frame.setTitle(frameTitle)
            STATE_SENT_SHARE_REQUEST_PENDING -> {}
            STATE_RECEIVED_SHARE_REQUEST_PENDING -> {}
            STATE_SHARING_MAP ->             //TODO: Update resource files.
                controller!!.frame.setTitle("$frameTitle - Sharing Map")
            else -> {}
        }
    }
}
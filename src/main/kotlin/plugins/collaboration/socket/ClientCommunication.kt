 ...
 * @date 06.09.2012
 */
open class ClientCommunication(pName: String?, pClient: Socket,
                               pController: MindMapController?, val mPassword: String) : CommunicationBase(pName, pClient, pController, DataOutputStream(
        pClient.getOutputStream()), DataInputStream(
        pClient.getInputStream())) {
    private var mLockId: String? = null
    private val mLockIds = HashSet<String?>()
    private var mSocketConnectionHook: SocketConnectionHook? = null
    private var mReceivedGoodbye = false
    var userInfo: CollaborationUserInformation? = null
        private set
     ...
     */
    init {
        currentState = CommunicationBase.Companion.STATE_WAIT_FOR_WHO_ARE_YOU
    }
    ...
    override fun processCommand(pCommand: CollaborationActionBase) {
        if (pCommand is CollaborationGoodbye) {
            TerminateableThread.Companion.logger!!.info("Goodbye received from " + pCommand.userId)
            terminateSocket()
            return
        }
        var commandHandled = false
        if (pCommand is CollaborationUserInformation) {
            this.userInfo = pCommand
            commandHandled = true
        }
        if (pCommand is CollaborationWhoAreYou) {
            if (currentState != CommunicationBase.Companion.STATE_WAIT_FOR_WHO_ARE_YOU) {
                printWrongState(pCommand)
            }
            // check server version:
            val serverVersion = pCommand.serverVersion
            if (!safeEquals(ServerCommunication.Companion.SERVER_VERSION, serverVersion)) {
                val errorMessage: String = Resources.getInstance().format(
                        "Collaboration_wrong_server_version", arrayOf<Any?>(ServerCommunication.Companion.SERVER_VERSION,
                        serverVersion))
                val showResult = OptionalDontShowMeAgainDialog(mindMapController
                        .frame.jFrame, mindMapController!!.selectedView,
                        errorMessage, controller.getResourceString("confirmation"), object : TextTranslator {
        ...
        if (pCommand is CollaborationOffers) {
            if (currentState != CommunicationBase.Companion.STATE_WAIT_FOR_OFFER) {
                printWrongState(pCommand)
            }
            val collOffers = pCommand
            if (collOffers.getIsSingleOffer()) {
                // we directly choose the one and only offer:
                val size = collOffers.listCollaborationMapOfferList.size
                if (size != 1) {
                    TerminateableThread.Companion.logger!!.severe("Collaboration offers with $size offers. Expected exactly one.")
                    terminateSocket()
                    return
                }
                // send hello:
                val helloCommand = CollaborationHello()
                helloCommand.map = collOffers.getCollaborationMapOffer(0).map
                send(helloCommand)
                currentState = CommunicationBase.Companion.STATE_WAIT_FOR_WELCOME
            } else {
                // now, we have a bundle of different maps to offer to the user
                val mapChooserDialog = JDialog(mindMapController!!.frame.jFrame,
                        false)
                mapChooserDialog.contentPane.layout = GridBagLayout()
                val mapTitle: String = Resources.getInstance().format(
                        "MapChooserDialog_title", arrayOf<Any>(mSocket.inetAddress.hostAddress, mSocket.port))
                mapChooserDialog.title = mapTitle
                mapChooserDialog.defaultCloseOperation = WindowConstants.DO_NOTHING_ON_CLOSE
                mapChooserDialog.addWindowListener(object : WindowAdapter() {
                ...
                val cancelAction: AbstractAction = object : AbstractAction() {
                    override fun actionPerformed(pE: ActionEvent) {
                        TerminateableThread.Companion.logger!!.info("Map choosing action canceled.")
                        mapChooserDialog.dispose()
                        terminateSocketWithGoodbye()
                    }
                }
                addEscapeActionToDialog(mapChooserDialog, cancelAction)
                val mapList = JList<String>()
                mapList.selectionMode = ListSelectionModel.SINGLE_SELECTION
                mapList.setModel(object : AbstractListModel<String?>() {
                    override fun getSize(): Int {
                        return collOffers.listCollaborationMapOfferList.size
                    }

                    override fun getElementAt(pIndex: Int): String {
                        return collOffers.getCollaborationMapOffer(pIndex).map!!
                    }
                })
                val okAction: AbstractAction = object : AbstractAction() {
                    override fun actionPerformed(pE: ActionEvent) {
                        val selection = mapList.selectedIndex
                        if (selection < 0) {
                            return
                        }
                        // user has chosen the map:
                        TerminateableThread.Companion.logger!!.info("Map choosing action. selected index $selection")
                        mapChooserDialog.isVisible = false
                        // send hello:
                        val helloCommand = CollaborationHello()
                        helloCommand.map = collOffers.getCollaborationMapOffer(selection).map
                        send(helloCommand)
                        currentState = CommunicationBase.Companion.STATE_WAIT_FOR_WELCOME
                    }
                }
                mapChooserDialog.contentPane.add(
                        JScrollPane(mapList),
                        GridBagConstraints(0, 0, 2, 1, 10.0, 4.0,
                                GridBagConstraints.WEST, GridBagConstraints.BOTH,
                                Insets(5, 5, 0, 0), 0, 10))
                val okButton = JButton(
                        controller.getResourceString("ExtendedFindDialog.ok"))
                setLabelAndMnemonic(okButton, null)
                okButton.addActionListener(okAction)
                mapChooserDialog.contentPane.add(
                        okButton,
                        GridBagConstraints(0, 1, 1, 1, 1.0, 1.0,
                                GridBagConstraints.WEST, GridBagConstraints.BOTH,
                                Insets(5, 5, 0, 0), 0, 0))
                val cancelButton = JButton(
                        controller.getResourceString("ExtendedFindDialog.cancel"))
                setLabelAndMnemonic(cancelButton, null)
                ...
                mapChooserDialog.contentPane.add(
                        ...
                        GridBagConstraints(1, 1, 1, 1, 1.0, 1.0,
                                GridBagConstraints.WEST, GridBagConstraints.BOTH,
                                Insets(5, 5, 0, 0), 0, 0))
                mapChooserDialog.rootPane.defaultButton = okButton
                mapChooserDialog.pack()
                mapChooserDialog.isVisible = true
            }
            commandHandled = true
        }
        if (pCommand is CollaborationWelcome) {
            if (currentState != CommunicationBase.Companion.STATE_WAIT_FOR_WELCOME) {
                printWrongState(pCommand)
            }
            createNewMap(pCommand.map)
            currentState = CommunicationBase.Companion.STATE_IDLE
            commandHandled = true
        }
        if (pCommand is CollaborationWrongCredentials) {
            ...
            mindMapController!!.controller!!.errorMessage(
                    mindMapController!!.getText("socket_wrong_password"))
            commandHandled = true
        }
        if (pCommand is CollaborationWrongMap) {
            ...
            // Display error message!
            mindMapController!!.controller!!.errorMessage(
                    mindMapController!!.getText("socket_wrong_map"))
            commandHandled = true
        }
        if (pCommand is CollaborationTransaction) {
            ...
            synchronized(mLockIds) { removeResult = mLockIds.remove(trans.id) }
            if (!removeResult) {
                // it is not from me, so handle it:
                if (currentState != CommunicationBase.Companion.STATE_IDLE) {
                    printWrongState(pCommand)
                }
                if (mSocketConnectionHook != null) {
                    mSocketConnectionHook!!
                            .executeTransaction(getActionPair(trans), controller)
                }
            }
            commandHandled = true
        }
        if (pCommand is CollaborationReceiveLock) {
            if (currentState != CommunicationBase.Companion.STATE_WAIT_FOR_LOCK) {
                printWrongState(pCommand)
            }
            mLockId = pCommand.id
            synchronized(mLockIds) { mLockIds.add(mLockId) }
            currentState = CommunicationBase.Companion.STATE_LOCK_RECEIVED
            commandHandled = true
        }
        ...
        if (!commandHandled) {
            TerminateableThread.Companion.logger!!.warning("Received unknown message of type "
                    + pCommand.javaClass)
        }
    }

    protected open fun reactOnWhoAreYou() {
        // send hello:
        val getOffersCommand = CollaborationGetOffers()
        getOffersCommand.userId = userName
        getOffersCommand.password = mPassword
        send(getOffersCommand)
        currentState = CommunicationBase.Companion.STATE_WAIT_FOR_OFFER
    }
    ...
    fun terminateSocketWithGoodbye() {
        if (mSocketConnectionHook != null) {
    ...
    fun toggleHook() {
        SocketBasics.Companion.togglePermanentHook(mindMapController,
                SocketBasics.Companion.SLAVE_HOOK_LABEL)
    }
     ...
     */
    @Synchronized
    @Throws(InterruptedException::class, UnableToGetLockException::class)
    fun sendLockRequest(): String? {
        // TODO: Global lock needed?
        mLockId = null
        var lockTries = 0
        var timeout = 0
        while (lockTries < MAX_LOCK_RETRIES) {
            val lockRequest = CollaborationRequireLock()
            currentState = CommunicationBase.Companion.STATE_WAIT_FOR_LOCK
            if (!send(lockRequest)) {
                currentState = CommunicationBase.Companion.STATE_IDLE
                throw UnableToGetLockException()
            }
            val sleepTime: Int = CommunicationBase.Companion.ROUNDTRIP_TIMEOUT / CommunicationBase.Companion.ROUNDTRIP_ROUNDS
            timeout = CommunicationBase.Companion.ROUNDTRIP_ROUNDS
            while (currentState == CommunicationBase.Companion.STATE_WAIT_FOR_LOCK && timeout >= 0) {
                sleep(sleepTime.toLong())
                timeout--
            }
            if (currentState == CommunicationBase.Companion.STATE_LOCK_RECEIVED) {
                break
            }
            // error case, repeat lock question.
            lockTries++
            TerminateableThread.Companion.logger!!.info("Didn't receive a lock, current try: $lockTries")
    ...
    fun createNewMap(map: String?) {
        run {
            ...
            // deregisterFilter();
            TerminateableThread.Companion.logger!!.info("Restoring the map...")
            val newModeController = mindMapController
    ...
    protected fun registerClientCommunicationAtHook() {
        // tell him about this thread.
        val activatedHooks = mindMapController!!.rootNode.activatedHooks
        for (hook in activatedHooks) {
            if (hook is SocketConnectionHook) {
                var connHook: SocketConnectionHook? = null
                connHook = hook
                // Tell the hook about me
                connHook.setClientCommunication(this)
                /* register as listener, as I am a slave. */connHook!!.registerFilter()
                mSocketConnectionHook = connHook
                break
            }
        }
    }
    ...
    private val mindMapController: MindMapController?
        private get() = if (mController is MindMapController) {
            mController as MindMapController
        } else null
     ...
     */
    val port: Int
        get() = mSocket.localPort

    companion object {
        private const val MAX_LOCK_RETRIES = 5
        private const val LOCK_RETRY_SLEEP_TIME: Long = 1000
    }
}
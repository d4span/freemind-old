...
class FormDialog(pController: MindMapController) : JDialog(pController.frame.jFrame), PropertyChangeListener {
    interface FormDialogValidator {
        /**
    ...
    private val mController2: MindMapController
    var isSuccess = false
        private set
    private var mOkButton: JButton? = null
    private var mFormDialogValidator: FormDialogValidator? = null
    ...
    fun setUp(controls: Vector<PropertyControl?>) {
        setUp(controls, object : FormDialogValidator {
            override val isValid: Boolean
                get() = true
        })
    }

    fun setUp(controls: Vector<PropertyControl?>, pValidator: FormDialogValidator?) {
        mFormDialogValidator = pValidator
        isModal = true
        contentPane.layout = BorderLayout()
        val formLayout = FormLayout(
                "right:max(40dlu;p), 4dlu, 80dlu, 7dlu", "")
        val builder = DefaultFormBuilder(formLayout)
        builder.setDefaultDialogBorder()
        for (prop in controls) {
            prop!!.layout(builder, mController2)
            val bean = prop as PropertyBean?
            bean!!.addPropertyChangeListener(this)
        }
        contentPane.add(builder.panel, BorderLayout.CENTER)
        val cancelButton = JButton()
        setLabelAndMnemonic(cancelButton, getText("cancel"))
        cancelButton.addActionListener { closeWindow() }
        mOkButton = JButton()
        setLabelAndMnemonic(mOkButton!!, getText("ok"))
        mOkButton!!.addActionListener(object : ActionListener {
            override fun actionPerformed(arg0: ActionEvent) {
                isSuccess = true
                closeWindow()
            }
        })
        getRootPane().defaultButton = mOkButton
        contentPane.add(
                ButtonBarBuilder().addGlue().addButton(cancelButton).addButton(mOkButton).build(),
                BorderLayout.SOUTH)
        title = getText("enter_password_dialog")
        ...
        val actionSuccess: Action = object : AbstractAction() {
            override fun actionPerformed(arg0: ActionEvent) {
                isSuccess = true
                closeWindow()
            }
    ...
    override fun propertyChange(pEvt: PropertyChangeEvent) {
        ...
        mOkButton!!.isEnabled = mFormDialogValidator!!.isValid
    }

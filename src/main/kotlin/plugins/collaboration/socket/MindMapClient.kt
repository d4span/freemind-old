 ...
 * @author foltin
 */
class MindMapClient : SocketBasics() {
    override fun startupMapHook() {
        super.startupMapHook()
        val callType = getProperties()!!.getProperty("callType")
        val isOfPublishType = safeEquals(callType, "publish")
        val controller = mindMapController!!
        if (isOfPublishType && controller.map!!.file == null) {
        ...
        val hostProperty = StringProperty(SocketBasics.Companion.HOST_DESCRIPTION, SocketBasics.Companion.HOST)
        val portProperty = portProperty
        try {
            val connectionHook = isConnected
            if (connectionHook != null) {
                ...
                SocketBasics.Companion.togglePermanentHook(controller, SocketBasics.Companion.SLAVE_HOOK_LABEL)
                return
            }
            // get last value
            hostProperty.value = controller.frame.getProperty(
                    HOST_PROPERTY)
            ...
            controller.frame.setProperty(HOST_PROPERTY,
                    hostProperty.value)
            mPassword = passwordProperty.value
            val port = portProperty!!.intValue
            val serverConnection = Socket(hostProperty.value, port)
            serverConnection.soTimeout = SocketBasics.Companion.SOCKET_TIMEOUT_IN_MILLIES
            val clientCommunication: ClientCommunication
            // determine type: join or publish
            if (isOfPublishType) {
                logger!!.info("Starting client thread and publish map...")
                clientCommunication = object : ClientCommunication(
                        "Client Communication (publish)", serverConnection,
                        mindMapController, mPassword) {
                    override fun reactOnWhoAreYou() {
                        // send publish command
                        val publishCommand = CollaborationPublishNewMap()
                        publishCommand.userId = Tools.userName
                        publishCommand.password = mPassword
                        val map = mController.map
                        publishCommand.mapName = map!!.file.getName()
            ...
            controller.controller!!.errorMessage(
                    Resources.getInstance().format(
                            SocketBasics.Companion.UNKNWON_HOST_EXCEPTION_MESSAGE, arrayOf<Any?>(e.message)))
            return
        } catch (e: ConnectException) {
            Resources.getInstance().logException(e)
            controller.controller!!.errorMessage(
                    Resources.getInstance().format(
                            SocketBasics.Companion.CONNECT_EXCEPTION_MESSAGE, arrayOf<Any?>(portProperty!!.value,
                            hostProperty.value, e.message)))
            return
        } catch (e: Exception) {
            Resources.getInstance().logException(e)
    ...
    private val isConnected: SocketConnectionHook?
        private get() {
            val activatedHooks = mindMapController!!.rootNode
                    .activatedHooks
    ...
    override val role: Int?
        get() = SocketBasics.Companion.ROLE_SLAVE
    ...
    override fun lock(pUserName: String?, pController: ExtendedMapFeedback?): String? {
        return null
    }
    ...
    override fun broadcastCommand(pDoAction: String?, pUndoAction: String?,
	 ...
	 */
    override fun unlock(pController: ExtendedMapFeedback?) {}
    ...
    companion object {
        private const val HOST_PROPERTY = "plugins.collaboration.database.host"
    }
}
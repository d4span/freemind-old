 ...
 * @author foltin
 */
class MindMapMaster : SocketMaster(), PermanentNodeHook, DontSaveMarker {
    var mListener: MasterThread? = null
    var mServer: ServerSocket? = null
    private var mMasterStarted = false

    inner class MasterThread
    /**
     * @param pName
     */
        : TerminateableThread("Master") {
        private var mLastTimeUserInformationSent: Long = 0
        ...
        override fun processAction(): Boolean {
            try {
                TerminateableThread.Companion.logger!!.finest("Waiting for message")
                val client = mServer!!.accept()
                TerminateableThread.Companion.logger!!.info("Received new client.")
                client.soTimeout = SocketBasics.Companion.SOCKET_TIMEOUT_IN_MILLIES
                val c = ServerCommunication(
                        this@MindMapMaster, client, mindMapController, true)
                c.start()
            } catch (e: SocketTimeoutException) {
            }
            val now = System.currentTimeMillis()
            for (mapName in mFileMap.keys) {
                val extendedMapFeedback = mFileMap[mapName]
                val sessionData = getSessionData(extendedMapFeedback)
                if (now - mLastTimeUserInformationSent > Companion.TIME_BETWEEN_USER_INFORMATION_IN_MILLIES) {
                    mLastTimeUserInformationSent = now
                    val userInfo = getMasterInformation(extendedMapFeedback)
                    synchronized(sessionData!!.mConnections) {
                        for (i in sessionData.mConnections.indices) {
                            try {
                                val connection = sessionData.mConnections
                                        .elementAt(i)
                                /*
								 ...
								 * reported.
								 */userInfo!!.masterIp = connection.ipToSocket
                ...
                synchronized(sessionData!!.mLockMutex) {
                    if (sessionData.mLockEnabled && now - sessionData.mLockedAt > Companion.TIME_FOR_ORPHANED_LOCK) {
                        TerminateableThread.Companion.logger!!.warning("Release lock " + sessionData.mLockId + " held by "
        ...
        companion object {
            private const val TIME_BETWEEN_USER_INFORMATION_IN_MILLIES: Long = 5000
            private const val TIME_FOR_ORPHANED_LOCK: Long = 5000
        }
    }
    ...
    override fun startupMapHook() {
        ...
        if (mapFeedback.map!!.file == null) {
            getController()!!.out(getController()!!.getText("map_not_saved"))
        }
        val controller = mindMapController!!
        val passwordProperty = StringProperty(
                SocketBasics.Companion.PASSWORD_DESCRIPTION, SocketBasics.Companion.PASSWORD)
        val passwordProperty2 = StringProperty(
                SocketBasics.Companion.PASSWORD_VERIFICATION_DESCRIPTION, SocketBasics.Companion.PASSWORD_VERIFICATION)
        // StringProperty bindProperty = new StringProperty(
        // "IP address of the local machine, or 0.0.0.0 if ", "Host");
        val portProperty = portProperty
        ...
        val dialog = FormDialog(controller)
        dialog.setUp(controls, object : FormDialogValidator {
            override val isValid: Boolean
                get() {
                    logger!!.finest("Output valid?")
                    return safeEquals(passwordProperty.value,
                            passwordProperty2.value)
                }
        })
        if (!dialog.isSuccess) {
            switchMeOff()
        ...
        /* Store port value in preferences. */setPortProperty(portProperty)
        mPassword = passwordProperty.value
        // start server:
        logger!!.info("Start server...")
        mMasterStarted = true
        try {
            mPort = getPortProperty().intValue
            mServer = ServerSocket(mPort)
            mServer!!.soTimeout = SocketBasics.Companion.SOCKET_TIMEOUT_IN_MILLIES
            mListener = MasterThread()
            mListener!!.start()
        } catch (e: Exception) {
            Resources.getInstance().logException(e)
            controller.controller!!.errorMessage(
                    Resources.getInstance().format(
                            SocketBasics.Companion.SOCKET_CREATION_EXCEPTION_MESSAGE, arrayOf<Any?>(portProperty!!.value,
                            e.message)))
            switchMeOff()
            return
        }
        addSession(mapFeedback.map!!.file.getName(), mapFeedback as ExtendedMapFeedback?)
        registerFilter()
    ...
    fun switchMeOff() {
        ...
        SwingUtilities.invokeLater { SocketBasics.Companion.togglePermanentHook(mindMapController, SocketBasics.Companion.MASTER_HOOK_LABEL) }
    }

    override fun loadFrom(pChild: XMLElement?) {
        // this plugin should not be saved.
    }

    override fun save(pXml: XMLElement) {
        // this plugin should not be saved.
        // nothing to do.
    }
     ...
     * @param pMapFeedback
     */
    private fun signalEndOfSession(pMapFeedback: ExtendedMapFeedback?) {
        val goodbye = CollaborationGoodbye()
        goodbye.userId = Tools.userName
        val sessionData = getSessionData(pMapFeedback)
        synchronized(sessionData!!.mConnections) {
            for (i in sessionData.mConnections.indices) {
                val serverCommunication = sessionData.mConnections
                        .elementAt(i)
                try {
    ...
    override fun onAddChild(pAddedChildNode: MindMapNode?) {}
    override fun onAddChildren(pAddedChild: MindMapNode?) {}
    override fun onLostFocusNode(pNodeView: NodeView?) {}
    override fun onNewChild(pNewChildNode: MindMapNode?) {}
    override fun onRemoveChild(pOldChildNode: MindMapNode?) {}
    override fun onRemoveChildren(pOldChildNode: MindMapNode?, pOldDad: MindMapNode?) {}
    override fun onFocusNode(pNodeView: NodeView?) {}
    override fun onUpdateChildrenHook(pUpdatedNode: MindMapNode?) {}
    override fun onUpdateNodeHook() {}
    override fun onViewCreatedHook(pNodeView: NodeView?) {}
    override fun onViewRemovedHook(pNodeView: NodeView?) {}
	 ...
	 */
    protected override val baseFile: File?
        protected get() {
            throw IllegalArgumentException("No base file here.")
        }

    companion object {
        const val LABEL = "plugins/collaboration/socket/socket_master_plugin"
    }
}
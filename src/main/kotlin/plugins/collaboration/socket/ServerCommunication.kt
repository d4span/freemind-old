 ...
 * @date 13.09.2012
 */
class ServerCommunication(pSocketStarter: SocketMaster?, pClient: Socket,
                          pMindMapController: ExtendedMapFeedback?, isSingleMapServer: Boolean) : CommunicationBase("Server Communication", pClient, pMindMapController,
        DataOutputStream(pClient.getOutputStream()),
        DataInputStream(pClient.getInputStream())) {
    protected var mMindMapMaster: SocketMaster? = null
    private val mIsSingleMapServer: Boolean

    init {
        mMindMapMaster = pSocketStarter
        mIsSingleMapServer = isSingleMapServer
        val commandWho = CollaborationWhoAreYou()
        commandWho.serverVersion = SERVER_VERSION
        send(commandWho)
        currentState = CommunicationBase.Companion.STATE_WAIT_FOR_GET_OFFERS
    }
    ...
    override fun processCommand(pCommand: CollaborationActionBase) {
        ...
        if (pCommand is CollaborationGetOffers) {
            if (currentState != CommunicationBase.Companion.STATE_WAIT_FOR_GET_OFFERS) {
                printWrongState(pCommand)
            } else {
                val commandGetOffers = pCommand
                commandHandled = true
                name = commandGetOffers.userId
                // verify password:
                if (mMindMapMaster.getPassword() ==
                        commandGetOffers.password) {
                    val commandOffers = CollaborationOffers()
                    commandOffers.setIsSingleOffer(mIsSingleMapServer)
                    for (fileName in mMindMapMaster.getFileMap().keys) {
                        val offer = CollaborationMapOffer()
                        offer.map = fileName
                        commandOffers.addCollaborationMapOffer(offer)
                    }
                    send(commandOffers)
                    currentState = CommunicationBase.Companion.STATE_WAIT_FOR_HELLO
                } else {
                    // Send error message
                    val wrongMessage = CollaborationWrongCredentials()
                    try {
                        send(wrongMessage)
                    } finally {
                        terminateSocket()
        ...
        if (pCommand is CollaborationPublishNewMap) {
            val commandPublish = pCommand
            if (currentState != CommunicationBase.Companion.STATE_WAIT_FOR_GET_OFFERS || mIsSingleMapServer) {
                printWrongState(pCommand)
            } else {
                // we got a new map to be published on the server
                commandHandled = true
                name = commandPublish.userId
                // verify password:
                if (mMindMapMaster.getPassword() ==
                        commandPublish.password) {
                    val map = commandPublish.mapName
                    val baseFile = mMindMapMaster.getBaseFile()
                    val file = File(baseFile, map)
                    // check for correct name
                    var notCorrectName = false
                    ...
                    if (mMindMapMaster.getFileMap().containsKey(map) || notCorrectName) {
                        // Send error message
                        val wrongMessage = CollaborationWrongMap()
                        try {
                            send(wrongMessage)
                        } finally {
                            terminateSocket()
                    ...
                    TerminateableThread.Companion.logger!!.info("New map $map published.")
                    // create new controller and load map
                    mController = mMindMapMaster!!.createMapOnServer(
                            map,
                            StringReaderCreator(commandPublish
                                    .map!!), baseFile)
                    // save:
                    mController.map.save(file)
        ...
        if (pCommand is CollaborationHello) {
            // here, we are strict in checking the state, as not the whole world
            // should get access to the map!
            if (currentState != CommunicationBase.Companion.STATE_WAIT_FOR_HELLO) {
                printWrongState(pCommand)
            } else {
                // get map:
                val map = pCommand.map
                ...
                TerminateableThread.Companion.logger!!.info("Map $map requested.")
                mController = mMindMapMaster.getFileMap()[map]
                mMindMapMaster!!.addConnection(this, mController)
                // send map:
                val welcomeCommand = CollaborationWelcome()
                TerminateableThread.Companion.logger!!.info("Store map in welcome command...")
                val writer = StringWriter()
                mController.map.getXml(writer)
                welcomeCommand.map = writer.toString()
                welcomeCommand.filename = mController.map.file
                        .getName()
                send(welcomeCommand)
                currentState = CommunicationBase.Companion.STATE_IDLE
                commandHandled = true
        ...
        if (pCommand is CollaborationTransaction) {
            if (currentState != CommunicationBase.Companion.STATE_WAIT_FOR_COMMAND) {
                printWrongState(pCommand)
            } else {
                val trans = pCommand
                try {
                    if (!safeEquals(mMindMapMaster!!.getLockId(controller), trans.id)) {
                        TerminateableThread.Companion.logger!!.severe("Wrong transaction id received. Expected: "
                                + mMindMapMaster!!.getLockId(controller) + ", received: "
                                + trans.id)
                    } else {
                        mMindMapMaster!!.broadcastCommand(trans.doAction,
                                trans.undoAction, trans.id, controller)
                        mMindMapMaster!!.executeTransaction(getActionPair(trans), controller)
                    }
                } finally {
        ...
        if (pCommand is CollaborationRequireLock) {
            if (currentState != CommunicationBase.Companion.STATE_IDLE) {
                printWrongState(pCommand)
            }
            try {
                val lockId = mMindMapMaster!!.lock(name, controller)
                TerminateableThread.Companion.logger!!.info("Got lock for $name")
                val lockCommand = CollaborationReceiveLock()
                lockCommand.id = lockId
                send(lockCommand)
                currentState = CommunicationBase.Companion.STATE_WAIT_FOR_COMMAND
                commandHandled = true
            } catch (e: UnableToGetLockException) {
                Resources.getInstance().logException(e)
                val unableToLock = CollaborationUnableToLock()
                send(unableToLock)

 ...
 * @date 03.05.2014
 */
abstract class SocketMaster : SocketBasics() {
    class SessionData {
        var mLockEnabled = false
        var mLockId: String? = null
        var mLockedAt: Long = 0
        var mLockUserName: String? = null
        var mConnections = Vector<ServerCommunication?>()
        var mLockMutex = String()
    }

    protected var mSessions = HashMap<ExtendedMapFeedback?, SessionData>()
     ...
     * @return the fileMap
     */
    var fileMap = HashMap<String?, ExtendedMapFeedback?>()
        protected set

    @Synchronized
    fun removeConnection(client: ServerCommunication) {
        synchronized(mSessions) {
            val controller = client.controller
            if (mSessions.containsKey(controller)) {
                mSessions[controller]!!.mConnections.remove(client)
            }
        }
     ...
     */
    abstract val baseFile: File?
    override val role: Int?
        get() = SocketBasics.Companion.ROLE_MASTER
    ...
    public override fun lock(pUserName: String?, pController: ExtendedMapFeedback?): String? {
        val sessionData = getSessionData(pController)
     ...
     * @param pController
     */
    protected fun getSessionData(pController: ExtendedMapFeedback?): SessionData? {
        if (mSessions.containsKey(pController)) {
            return mSessions[pController]
        }
        throw IllegalArgumentException("Session for $pController not present.")
    }
    ...
    public override fun broadcastCommand(pDoAction: String?, pUndoAction: String?,
                                         pLockId: String?, pController: ExtendedMapFeedback?) {
        val sessionData = getSessionData(pController)
        synchronized(sessionData!!.mConnections) {
            for (i in sessionData.mConnections.indices) {
                sessionData.mConnections.elementAt(i)!!.sendCommand(
	 ...
	 */
    public override fun unlock(pController: ExtendedMapFeedback?) {
        val sessionData = getSessionData(pController)
        synchronized(sessionData!!.mLockMutex) {
            check(sessionData.mLockEnabled)
            logger!!.fine("Release lock " + sessionData.mLockId + " held by " + sessionData.mLockUserName)
            clearLock(pController)
        }
    }

    fun clearLock(pController: ExtendedMapFeedback?) {
        val sessionData = getSessionData(pController)
    ...
    fun getLockId(pController: ExtendedMapFeedback?): String? {
        val sessionData = getSessionData(pController)
        synchronized(sessionData!!.mLockMutex) {
            check(sessionData.mLockEnabled)
	 ...
	 */
    fun getUsers(pController: ExtendedMapFeedback?): String {
        val sessionData = getSessionData(pController)
        val users = StringBuffer(Tools.userName)
        synchronized(sessionData!!.mConnections) {
            for (i in sessionData.mConnections.indices) {
                users.append(',')
                users.append(' ')
                users.append(sessionData.mConnections.elementAt(i)!!.name)
            }
        }
        return users.toString()
    }

    public override fun executeTransaction(pair: ActionPair?, pController: ExtendedMapFeedback?) {
    ...
    override fun getMasterInformation(pController: ExtendedMapFeedback?): CollaborationUserInformation? {
        val userInfo = CollaborationUserInformation()
        userInfo.userIds = getUsers(pController)
        userInfo.masterHostname = hostName
        userInfo.masterPort = port
        userInfo.masterIp = hostIpAsString
        return userInfo
    }
     ...
     * @param pController
     */
    fun addConnection(pServerCommunication: ServerCommunication?,
                      pController: ExtendedMapFeedback?) {
        synchronized(mSessions) { mSessions[pController]!!.mConnections.addElement(pServerCommunication) }
    }

    @Throws(IOException::class)
    fun createMapOnServer(fileName: String?,
                          readerCreator: ReaderCreator?, pFile: File?): ExtendedMapFeedback {
        val mapFeedback = ExtendedMapFeedbackImpl()
        val map = MindMapMapModel(mapFeedback)
        mapFeedback.map = map
        val root = map.loadTree(readerCreator!!,
    ...
    protected fun addSession(fileName: String?,
                             mapFeedback: ExtendedMapFeedback?) {
        synchronized(mSessions) {
            fileMap[fileName] = mapFeedback
            mSessions.put(mapFeedback, SessionData())
        }
    }
}
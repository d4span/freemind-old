...
class SocketRegistration(controller: ModeController, map: MindMap?) : HookRegistration, MenuItemSelectedListener, MenuItemEnabledListener {
    private val mController: MindMapController
    private val logger: Logger

    init {
        mController = controller as MindMapController
    ...
    override fun isSelected(pCheckItem: JMenuItem?, pAction: Action?): Boolean {
        ...
        if (pAction is NodeHookAction) {
            val action = pAction
            if (action.hookName == MindMapMaster.Companion.LABEL) {
                return isMaster
            }
            if (action.hookName == SocketBasics.Companion.SLAVE_STARTER_LABEL) {
                return isSlave
            }
            if (action.hookName == SocketBasics.Companion.SLAVE_PUBLISH_LABEL) {
    ...
    private val isMaster: Boolean
        private get() {
            val activatedHooks = mController.rootNode
                    .activatedHooks
            for (hook in activatedHooks) {
                if (hook is MindMapMaster) {
                    return true
                }
            }
            return false
        }
    private val isSlave: Boolean
        private get() {
            val activatedHooks = mController.rootNode
                    .activatedHooks
            for (hook in activatedHooks) {
    ...
    override fun isEnabled(pItem: JMenuItem?, pAction: Action?): Boolean {
        ...
        throw IllegalArgumentException("Unknown action: $pAction")
    }
}
 ...
 * @date 05.09.2012
 */
abstract class TerminateableThread(pName: String?) : Thread(pName) {
    protected var mShouldTerminate = false
    protected var mIsTerminated = false
    protected var mSleepTime: Int
    ...
    override fun run() {
        while (!mShouldTerminate) {
            var shouldBeCalledDirectlyAgain = false
            try {
    ...
    abstract fun processAction(): Boolean
    fun commitSuicide() {
        ...
        if (timeout == 0) {
            logger!!.warning("Can't stop thread $name!")
        } else {
            logger!!.info("Shutting down thread $name complete.")
        }
    }

 ...
 * @author foltin
 */
class FreemindHelpStarter : ModeControllerHookAdapter() {
    /**
     *
     */
    override fun startupMapHook() {
        ...
        val helpHS = "plugins/help/doc/freemind.hs"
        try {
            val classLoader = this.javaClass.classLoader
            val hsURL = HelpSet.findHelpSet(classLoader, helpHS)
            val hs = HelpSet(classLoader, hsURL)
            val hb = hs.createHelpBroker()
            hb.initPresentation()
            hb.isDisplayed = true
            hb.isViewDisplayed = true
        } catch (ee: Exception) {
            // Say what the exception really is
            Resources.getInstance().logException(ee)
            logger!!.warning("HelpSet " + ee.message + ee)

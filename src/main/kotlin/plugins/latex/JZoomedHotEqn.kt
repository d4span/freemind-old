...
class JZoomedHotEqn internal constructor(model: LatexNodeHook) : sHotEqn() {
    private var model: LatexNodeHook
    ...
    init {
        ...
        isBorder = true
        this.model = model
        equation = model.getContent(null)
        if (editorTitle == null) {
            editorTitle = model.mindMapController!!.getText(
                    "plugins/latex/LatexNodeHook.editorTitle")
        }
    }

    override fun getPreferredSize(): Dimension {
        val dimension = if (isValid) super.getPreferredSize() else getSizeof(equation)
        dimension.height *= zoom.toInt()
        dimension.width *= zoom.toInt()
        return dimension
    }
    ...
    override fun setBounds(x: Int, y: Int, w: Int, h: Int) {
        if (zoom < 1f) {
            super.setBounds(x, y, (w / zoom).toInt(), (h / zoom).toInt())
        } else {
    ...
    private fun edit() {
        val textArea = JTextArea(equation)
        ...
        editorScrollPane.preferredSize = Dimension(500, 160)
        val edit = JDialog(JOptionPane.getFrameForComponent(this),
                editorTitle, true)
        edit.contentPane.add(editorScrollPane)
        edit.defaultCloseOperation = JDialog.DISPOSE_ON_CLOSE
        edit.pack()
        edit.setLocationRelativeTo(this)
        edit.isVisible = true
        val eq = textArea.text
        model.setContent(null, eq)
    }

    fun setModel(model: LatexNodeHook) {
        this.model = model
        equation = model.getContent(null)
        revalidate()
        repaint()

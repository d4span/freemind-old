 ...
 * @date 27.10.2011
 */
class MapNodePositionHolder : MapNodePositionHolderBase() {
    var position = Coordinate(0, 0)
    var mapCenter = Coordinate(0, 0)
    /**
     * @return
     */
    /**
    ...
    override fun invoke(pNode: MindMapNode?) {
        super.invoke(pNode)
        registration.registerMapNode(this)
    }
    ...
    override fun shutdownMapHook() {
        registration.deregisterMapNode(this)
    ...
    override fun save(xml: XMLElement) {
        ...
        values[XML_STORAGE_POS_LON] = toString(position.lon)
        values[XML_STORAGE_POS_LAT] = toString(position.lat)
        values[XML_STORAGE_MAP_LON] = toString(mapCenter.lon)
        values[XML_STORAGE_MAP_LAT] = toString(mapCenter.lat)
        values[XML_STORAGE_ZOOM] = toString(zoom)
        if (tileSource != null) {
            values[XML_STORAGE_TILE_SOURCE] = tileSource
        }
        if (mTooltipLocation != null) {
            values[XML_STORAGE_MAP_TOOLTIP_LOCATION] = mTooltipLocation
        }
     ...
     * @return
     */
    private fun toString(pDouble: Double): String {
        return "" + pDouble
    }
     ...
     * @return
     */
    private fun toString(pInt: Int): String {
        return "" + pInt
    }
    ...
    override fun loadFrom(pChild: XMLElement?) {
        ...
        val values = loadNameValuePairs(pChild!!)
        position.lat = fromString(values[XML_STORAGE_POS_LAT])
        position.lon = fromString(values[XML_STORAGE_POS_LON])
        mapCenter.lat = fromString(values[XML_STORAGE_MAP_LAT])
        mapCenter.lon = fromString(values[XML_STORAGE_MAP_LON])
        zoom = intFromString(values[XML_STORAGE_ZOOM])
        ...
        tileSource = values[XML_STORAGE_TILE_SOURCE]
    }
     ...
     * @return
     */
    private fun fromString(pObject: Any?): Double {
        return if (pObject == null) {
            0.0
        } else try {
            pObject as String?. toDouble ()
        } catch (e: Exception) {
     ...
     * @return
     */
    private fun intFromString(pObject: Any?): Int {
        return if (pObject == null) {
            1
        } else try {
            pObject as String?. toInt ()
        } catch (e: Exception) {
            Resources.getInstance().logException(e)
            1
        }
     ...
     * @param pTileSource
     */
    fun changePosition(pPosition: Coordinate?, pMapCenter: Coordinate?,
                       pZoom: Int, pTileSource: String?) {
        registration.changePosition(this, pPosition, pMapCenter, pZoom,
                pTileSource)
    }

    interface MapNodePositionListener {
    ...
    override fun getNode(): MindMapNode? {
        return super.getNode()
    }

    override val nodeId: String
        get() = mindMapController!!.getNodeID(getNode())!!

    /**
     * @return
     */
    private val mindMapController: MindMapController?
        private get() = getController() as MindMapController?
    ...
    override fun onViewCreatedHook(pNodeView: NodeView?) {
        ...
        registration.fireNodeVisibilityChanged(true, this)
    }
    ...
    override fun onViewRemovedHook(pNodeView: NodeView?) {
        ...
        registration.fireNodeVisibilityChanged(false, this)
    }

    override fun toString(): String {
        return ("MapNodePositionHolder [mPosition=" + position
                + ", mMapCenter=" + mapCenter + ", mTileSource=" + tileSource
                + ", mZoom=" + zoom + ", getNode()=" + getNode() + "]")
    }
    ...
    override fun saveHtml(pFileout: Writer?) {
        ...
        if (sMapLocationGif == null) {
            sMapLocationGif = getController()!!.frame.getProperty(
                    MAP_LOCATION)
        }
        val link: String = FreeMindMapController.Companion.getLink(tileSource, position,
                mapCenter, zoom)
...
// determine other surrounding tiles that are close to the exact
     ...
     * @return
     */
    override val imageHtml: String
        get() {
            if (tileSource == null) {
                return super.imageHtml
            }
            val tileSource: TileSource = FreeMindMapController.Companion.getTileSourceByName(tileSource)!!.mTileSource
            val tileSize = tileSource.tileSize
            val exactx = OsmMercator.LonToX(position.lon, zoom).toInt()
            val exacty = OsmMercator.LatToY(position.lat, zoom).toInt()
            var x = exactx / tileSize
            var y = exacty / tileSize
            // determine other surrounding tiles that are close to the exact
            // point.
            var dx = exactx % tileSize
            var dy = exacty % tileSize
            // determine quadrant of cursor in tile:
            var posx = 0
            var posy = 0
            if (dx < tileSize / 2) {
            ...
            var imageHtml = "<html><body><table cellspacing='0' cellpadding='0'>"
            for (j in 0..1) {
                imageHtml += "<tr>"
                for (i in 0..1) {
                    try {
                        ...
                        imageHtml += ("<td valign='top' align='top' width='" + tileSize + "' height='"
                                + tileSize + "' BACKGROUND=\""
                                + tileSource.getTileUrl(zoom, x + i, y + j)
    ...
    companion object {
         ...
         *
         */
        private const val MAP_LOCATION = "map_location"
        private var sMapLocationGif: String? = null
        fun getHook(node: MindMapNode?): MapNodePositionHolder? {
            return getBaseHook(node) as MapNodePositionHolder?
        }
    }
}
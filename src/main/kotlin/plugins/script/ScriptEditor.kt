 ...
 * @author foltin
 */
class ScriptEditor : MindMapHookAdapter() {
    private inner class AttributeHolder(var mAttribute: Attribute, var mPosition: Int)
    private inner class NodeScriptModel(
            /**
             * Of AttributeHolder
             */
            private val mScripts: Vector<AttributeHolder>, private val mNode: MindMapNode,
            private val mMindMapController: MindMapController) : ScriptModel {
        ...
        override fun executeScript(pIndex: Int, pOutStream: PrintStream,
            ...
            val script = getScript(pIndex).script
            // get cookies from base plugin:
            val reg = getPluginBaseClass() as ScriptingRegistration
            return ScriptingEngine.Companion.executeScript(
                    mMindMapController.selected, BooleanHolder(true),
        ...
        override fun getScript(pIndex: Int): ScriptHolder {
            val attribute = (mScripts[pIndex] as AttributeHolder).mAttribute
            return ScriptHolder(attribute.name, attribute.value)
        }

        override fun setScript(pIndex: Int, pScript: ScriptHolder) {
            val holder = mScripts[pIndex] as AttributeHolder
            if (pScript.mScriptName != holder.mAttribute.name) {
                isDirty = true
            }
            if (pScript.mScript != holder.mAttribute.value) {
                isDirty = true
            }
            holder.mAttribute.name = pScript.mScriptName
            holder.mAttribute.value = pScript.mScript
        }
        ...
        override fun storeDialogPositions(pPanel: ScriptEditorPanel?,
            ...
            mindMapController!!.storeDialogPositions(pPanel, pStorage,
        ...
        override fun endDialog(pIsCanceled: Boolean) {
            if (!pIsCanceled) {
                ...
                // position.
                val attributeTableLength = mNode.attributeTableLength
                // store node attributes back
                for (holder in mScripts) {
                    val attribute = holder.mAttribute
        ...
        override fun addNewScript(): Int {
            val index = mScripts.size

            /**
             * is in general different from index, as not all attributes need to
             * be scripts.
             */
            val attributeIndex = mNode.attributeTableLength
            val scriptName: String = ScriptingEngine.Companion.SCRIPT_PREFIX
            var scriptNameSuffix = 1
            var found: Boolean
            do {
                found = false
                for (holder in mScripts) {
                    if (scriptName + scriptNameSuffix
                            == holder.mAttribute.name) {
                        found = true
                        scriptNameSuffix++
                        break
                    }
            ...
            mScripts.add(AttributeHolder(Attribute(scriptName
    ...
    override fun startupMapHook() {
        ...
        val node = mindMapController!!.selected!!
        val scripts = Vector<AttributeHolder>()
        for (position in 0 until node.attributeTableLength) {
            val attribute: Attribute = node.getAttribute(position)
            if (attribute.name!!.startsWith(ScriptingEngine.Companion.SCRIPT_PREFIX)) {
                scripts.add(AttributeHolder(attribute, position))
            }
        }
        val nodeScriptModel = NodeScriptModel(scripts, node,
                mindMapController!!)
        val scriptEditorPanel = ScriptEditorPanel(
                nodeScriptModel, getController()!!.frame, true)

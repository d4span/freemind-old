 ...
 *
 *  *
 *  * new script/delete script buttons
 *  * rename script button
 *  * undo feature?
 *  * show line/column numbers in status bar
 *
 */
class ScriptEditorPanel(pScriptModel: ScriptModel, pFrame: FreeMindMain,
                        pHasNewScriptFunctionality: Boolean) : JDialog(pFrame.jFrame, true /* modal */), MenuListener {
    private val mFrame: FreeMindMain
    private val mScriptModel: ScriptModel
    private val mScriptList: JList<*>
    private val mScriptTextField: JTextArea
    private val mListModel: DefaultListModel<String?>
    private var mLastSelected: Int? = null
    private val mScriptResultField: JTextArea
    private val mCentralUpperPanel: JSplitPane
    private val mCentralPanel: JSplitPane
    private val logger: Logger
    private val mStatus: JLabel
    private val mRunAction: AbstractAction
    private val mSignAction: SignAction
    private var mScriptThread: Thread? = null
    private val mStopAction: StopAction
    private val mMenuActions = Vector<AbstractAction>()

    private inner class ResultFieldStream : OutputStream() {
        @Throws(IOException::class)
        override fun write(pByte: Int) {
            write(byteArrayOf(pByte.toByte()))
        }

        @Throws(IOException::class)
        override fun write(pB: ByteArray) {
            EventQueue.invokeLater { mScriptResultField.append(String(pB)) }
        }
    }

    private inner class RunAction(pArg0: String?) : AbstractAction(pArg0), MenuItemEnabledListener {
        override fun actionPerformed(arg0: ActionEvent) {
            storeCurrent()
            if (isSomethingSelected) {
                mScriptResultField.text = ""
                mScriptThread = Thread {
                    mScriptModel.executeScript(
                            mScriptList.selectedIndex,
                            printStream, errorHandler)
                }
                mScriptThread!!.start()
            }
        }
    ...
    private inner class StopAction(pArg0: String?) : AbstractAction(pArg0), MenuItemEnabledListener {
        override fun actionPerformed(arg0: ActionEvent) {
		 ...
		 */
        override fun isEnabled(pItem: JMenuItem?, pAction: Action?): Boolean {
            return isSomethingSelected && mScriptThread != null && !mScriptThread!!.isInterrupted
        }
    }

    private inner class SignAction(pArg0: String?) : AbstractAction(pArg0) {
        override fun actionPerformed(arg0: ActionEvent) {
            storeCurrent()
            if (!mScriptList.isSelectionEmpty) {
                val selectedIndex = mScriptList.selectedIndex
                val script = mScriptModel.getScript(selectedIndex)
                val signedScript = SignedScriptHandler().signScript(
                        script.script, Resources.getInstance(), mFrame)
                script.setScript(signedScript)
                mScriptModel.setScript(selectedIndex, script)
                mScriptTextField.text = signedScript
            }
        }
    }

    private inner class CancelAction private constructor(pArg0: String) : AbstractAction(pArg0) {
        override fun actionPerformed(arg0: ActionEvent) {
            disposeDialog(true)
        }
    }

    private inner class ExitAction private constructor(pArg0: String) : AbstractAction(pArg0) {
        override fun actionPerformed(arg0: ActionEvent) {
            storeCurrent()
            disposeDialog(false)
        }
    ...
    private inner class NewScriptAction(pArg0: String?) : AbstractAction(pArg0) {
        override fun actionPerformed(arg0: ActionEvent) {
            storeCurrent()
            mLastSelected = null
            val scriptIndex = mScriptModel.addNewScript()
            updateFields()
            select(scriptIndex)
        }
    }
     ...
     * @param pScript
     * script content
     */(var scriptName: String?, var script: String?) {

        fun setScript(pScript: String?): ScriptHolder {
            script = pScript
            return this
        }

        fun setScriptName(pScriptName: String?): ScriptHolder {
    ...
    interface ScriptModel {
         ...
         * @return a script
         */
        fun getScript(pIndex: Int): ScriptHolder
        fun setScript(pIndex: Int, pScript: ScriptHolder)
        fun executeScript(pIndex: Int, outStream: PrintStream,
                          pErrorHandler: ScriptingEngine.ErrorHandler): Boolean

        fun storeDialogPositions(pPanel: ScriptEditorPanel?,
                                 pStorage: ScriptEditorWindowConfigurationStorage?,
                                 pWindow_preference_storage_property: String?)

        fun decorateDialog(
                pPanel: ScriptEditorPanel?,
                pWindow_preference_storage_property: String?): ScriptEditorWindowConfigurationStorage

        fun endDialog(pIsCanceled: Boolean)
        val isDirty: Boolean
         ...
         */
        fun addNewScript(): Int
    }

    init {
        logger = pFrame.getLogger(this.javaClass.name)
        ...
        addEscapeActionToDialog(this, object : AbstractAction() {
            override fun actionPerformed(arg0: ActionEvent) {
                disposeDialog(true)
            }
        })
        ...
        mListModel = DefaultListModel()
        mScriptList = JList(mListModel)
        mScriptList.selectionMode = ListSelectionModel.SINGLE_SELECTION
        mScriptList.addListSelectionListener(ListSelectionListener { pEvent ->
            if (pEvent.valueIsAdjusting) return@ListSelectionListener
            // System.out.println("List selection:" + pEvent);
            select(mScriptList.selectedIndex)
        })
        // add(mScriptList, BorderLayout.WEST);
        mScriptTextField = JTextArea()
        mScriptTextField.font = Font("Monospaced", Font.PLAIN, 12)
        mScriptTextField.isEnabled = false
        mScriptTextField.tabSize = 2
        mCentralUpperPanel = JSplitPane(JSplitPane.HORIZONTAL_SPLIT, mScriptList, JScrollPane(mScriptTextField))
        mCentralUpperPanel.isContinuousLayout = true
        mScriptResultField = JTextArea()
        mScriptResultField.isEditable = false
        mScriptResultField.wrapStyleWord = true
        mCentralPanel = JSplitPane(JSplitPane.VERTICAL_SPLIT,
                mCentralUpperPanel, JScrollPane(mScriptResultField))
        mCentralPanel.setDividerLocation(0.8)
        mCentralPanel.isContinuousLayout = true
        contentPane.add(mCentralPanel, BorderLayout.CENTER)
        mStatus = JLabel()
        contentPane.add(mStatus, BorderLayout.SOUTH)
        mScriptTextField.addCaretListener {
            val caretPosition = mScriptTextField.caretPosition
            try {
                val lineOfOffset = mScriptTextField
        ...
        mStopAction.isEnabled = false
        addAction(menu, mStopAction)
        mSignAction = SignAction(
                pFrame.getResourceString("plugins/ScriptEditor.sign"))
        mSignAction.isEnabled = false
        addAction(menu, mSignAction)
        val cancelAction: AbstractAction = CancelAction(
                pFrame.getResourceString("plugins/ScriptEditor.cancel"))
        addAction(menu, cancelAction)
        val exitAction: AbstractAction = ExitAction(
                pFrame.getResourceString("plugins/ScriptEditor.exit"))
        addAction(menu, exitAction)
        menuBar.add(menu)
        this.jMenuBar = menuBar
        // Retrieve window size and column positions.
        val storage = mScriptModel
                .decorateDialog(this, WINDOW_PREFERENCE_STORAGE_PROPERTY)
        if (storage != null) {
            mCentralUpperPanel.dividerLocation = storage.leftRatio
            mCentralPanel.dividerLocation = storage.topRatio
        } else {
            // bug fix: for new users, this is set to some reasonable values.
            mCentralUpperPanel.dividerLocation = 100
            mCentralPanel.dividerLocation = 240
        }
    }

    private fun addAction(menu: JMenu, action: AbstractAction) {
        mMenuActions.add(action)
        val item = menu.add(action)
        setLabelAndMnemonic(item,
                (action.getValue(AbstractAction.NAME) as String))
        item.icon = BlindIcon(StructuredMenuHolder.ICON_SIZE)
    }

    private fun updateFields() {
        mListModel.clear()
        for (i in 0 until mScriptModel.amountOfScripts) {
            val script = mScriptModel.getScript(i)
            mListModel.addElement(script.scriptName)
        }
    }

    private fun select(pIndex: Int) {
        mScriptTextField.isEnabled = pIndex >= 0
        mSignAction.isEnabled = pIndex >= 0
        if (pIndex < 0) {
            mScriptTextField.text = ""
            return
        }
        storeCurrent()
        // set new script
        mScriptTextField.text = mScriptModel.getScript(pIndex).script
        // set last one:
        mLastSelected = pIndex
        if (pIndex >= 0 && mScriptList.selectedIndex != pIndex) {
            mScriptList.selectedIndex = pIndex
        }
    }
    ...
    private fun storeCurrent() {
        if (mLastSelected != null) {
            // store old value:
            val oldIndex = mLastSelected!!.toInt()
            mScriptModel.setScript(oldIndex, mScriptModel.getScript(oldIndex)
                    .setScript(mScriptTextField.text))
        }
    }
     ...
     * TODO
     */
    private fun disposeDialog(pIsCanceled: Boolean) {
        // the script should be stopped.
        var pIsCanceled = pIsCanceled
        interruptScript()
        ...
        val storage = ScriptEditorWindowConfigurationStorage()
        storage.leftRatio = mCentralUpperPanel.dividerLocation
        storage.topRatio = mCentralPanel.dividerLocation
    ...
    val printStream: PrintStream
        get() = PrintStream(ResultFieldStream())
    val errorHandler: ScriptingEngine.ErrorHandler
        get() = ScriptingEngine.ErrorHandler { pLineNumber ->
            logger.info("Line number: $pLineNumber")
            EventQueue.invokeLater {
                if (pLineNumber > 0
                        && pLineNumber <= mScriptTextField
                                .lineCount) {
                    val element3 = mScriptTextField.document
                            .defaultRootElement
                    val element4 = element3
                            .getElement(pLineNumber - 1)
                    if (element4 != null) {
                        mScriptTextField.select(
                                element4.startOffset,
                                element4.endOffset)
                    }
                }
            }
	 ...
	 */
    override fun menuSelected(pE: MenuEvent) {
        for (action in mMenuActions) {
            if (action is MenuItemEnabledListener) {
                val listener = action as MenuItemEnabledListener
                action.isEnabled = listener.isEnabled(null, action)
            }
        }
    }
	 ...
	 * )
	 */
    override fun menuDeselected(pE: MenuEvent) {
	 ...
	 */
    override fun menuCanceled(pE: MenuEvent) {
        // TODO Auto-generated method stub
    }

    protected val isSomethingSelected: Boolean
        protected get() = !mScriptList.isSelectionEmpty

    companion object {
        /**
         *
         */
        private const val serialVersionUID = 3221975191441136520L
        private const val WINDOW_PREFERENCE_STORAGE_PROPERTY = "plugins.script.ScriptEditorPanel/window_positions"
    }
}
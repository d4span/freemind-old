...
class ScriptingRegistration(controller: ModeController, map: MindMap?) : HookRegistration, ExternalPatternAction, StartupDoneListener {
    private class ScriptingPluginPropertyContributor(
            modeController: MindMapController?) : FreemindPropertyContributor {
        override fun getControls(pTextTranslator: TextTranslator?): List<PropertyControl?>? {
            ...
            controls.add(BooleanProperty(FreeMind.RESOURCES_EXECUTE_SCRIPTS_WITHOUT_FILE_RESTRICTION
                    + ".tooltip",
                    FreeMind.RESOURCES_EXECUTE_SCRIPTS_WITHOUT_FILE_RESTRICTION))
            controls.add(BooleanProperty(FreeMind.RESOURCES_EXECUTE_SCRIPTS_WITHOUT_NETWORK_RESTRICTION
                    + ".tooltip",
                    FreeMind.RESOURCES_EXECUTE_SCRIPTS_WITHOUT_NETWORK_RESTRICTION))
            controls.add(BooleanProperty(FreeMind.RESOURCES_EXECUTE_SCRIPTS_WITHOUT_EXEC_RESTRICTION
                    + ".tooltip",
            ...
            controls.add(StringProperty(FreeMind.RESOURCES_SCRIPT_USER_KEY_NAME_FOR_SIGNING
                    + ".tooltip",
    ...
    private inner class PatternScriptModel(pScript: String?) : ScriptModel {
        var script: String?
            private set
        private val mOriginalScript: String?
        ...
        override fun endDialog(pIsCanceled: Boolean) {
            if (pIsCanceled) {
                mScript = mOriginalScript
            }
        }

        override fun executeScript(pIndex: Int, pOutStream: PrintStream,
                                   pErrorHandler: ScriptingEngine.ErrorHandler): Boolean {
            return ScriptingEngine.Companion.executeScript(controller.selected,
                    BooleanHolder(true), mScript, controller,
                    pErrorHandler, pOutStream, scriptCookies)
        }

        override val amountOfScripts: Int
            get() = 1

        override fun getScript(pIndex: Int): ScriptHolder {
            return ScriptHolder("Script", mScript)
        }

        override val isDirty: Boolean
            get() = !safeEquals(mScript, mOriginalScript)

        override fun setScript(pIndex: Int, pScript: ScriptHolder) {
            mScript = pScript.script
        }

        override fun storeDialogPositions(pPanel: ScriptEditorPanel?,
        ...
        override fun addNewScript(): Int {
            return 0
        }
    }

    private val controller: MindMapController
    private var mScriptEditorStarter: ScriptEditorStarter? = null
    val scriptCookies: HashMap<*, *> = HashMap<Any, Any>()
    private var mScriptingPluginPropertyContributor: ScriptingPluginPropertyContributor? = null

    init {
        this.controller = controller as MindMapController
        ScriptingEngine.Companion.logger = controller.frame.getLogger(
                this.javaClass.name)
    }

    override fun register() {
        controller.registerPlugin(this)
        mScriptEditorStarter = object : ScriptEditorStarter {
            override fun startEditor(pScriptInput: String?): String? {
                ScriptingEngine.Companion.logger!!.info("Start to edit script..."
                        + pScriptInput)
                val patternScriptModel = PatternScriptModel(
                        pScriptInput)
                val scriptEditorPanel = ScriptEditorPanel(
                        patternScriptModel, controller.frame, false)
                scriptEditorPanel.isVisible = true
                return patternScriptModel.getScript()
            }
        }
        controller.registerPlugin(mScriptEditorStarter)
        mScriptingPluginPropertyContributor = ScriptingPluginPropertyContributor(
                controller)
        OptionPanel.addContributor(mScriptingPluginPropertyContributor!!)
        controller.frame.registerStartupDoneListener(this)
    }
    ...
    override fun act(node: MindMapNode?, pattern: Pattern?) {
        if (pattern!!.patternScript != null
                && pattern.patternScript!!.value != null) {
            val scriptString = HtmlTools.toXMLUnescapedText(HtmlTools.unescapeHTMLUnicodeEntity(pattern
                    .patternScript!!.value!!))
            ScriptingEngine.Companion.logger!!.info("Executing script: $scriptString")
            executeScript(node, scriptString)
        }
    }

    private fun executeScript(node: MindMapNode?, scriptString: String) {
        ScriptingEngine.Companion.executeScript(node, BooleanHolder(false),
                scriptString, controller, ScriptingEngine.ErrorHandler { }, System.out, scriptCookies)
    }

    override fun startupDone() {
        /* Is there a startup groovy script? */
        val startupScriptFile = System.getProperty("startup_groovy_script")
        if (startupScriptFile != null && !startupScriptFile.isEmpty()) {
            val expandFileName = expandFileName(startupScriptFile)
            ScriptingEngine.Companion.logger!!.info("Starting script at $expandFileName")
            val scriptString = getFile(File(expandFileName))
            if (scriptString != null && !scriptString.isEmpty()) {
                ScriptingEngine.Companion.logger!!.info("Starting script $scriptString")
                try {
                    executeScript(controller.rootNode, scriptString)
                } catch (e: Exception) {
                    Resources.getInstance().logException(e)
                }
            } else {
                ScriptingEngine.Companion.logger!!.warning("Starting script not found!")
            }
        }
    }
}
 ...
 * @author foltin
 */
class ScriptingSecurityManager(pWithoutFileRestriction: Boolean,
                               pWithoutNetworkRestriction: Boolean, pWithoutExecRestriction: Boolean) : SecurityManager() {
    private val mWithoutFileRestriction: Boolean
    private val mWithoutNetworkRestriction: Boolean
    private val mWithoutExecRestriction: Boolean
    ...
    private fun getException(pPermissionGroup: Int, pPermission: Int): SecurityException {
        return SecurityException(Resources.getInstance().format(
                "plugins/ScriptEditor.FORBIDDEN_ACTION", arrayOf(pPermissionGroup, pPermission)))
    }
    ...
    override fun checkPermission(pPerm: Permission, pContext: Any) {
        logger!!.fine("Check Permission with Context: " + pPerm.javaClass)
    }

    override fun checkPermission(pPerm: Permission) {
        logger!!.fine("Check Permission: " + pPerm.javaClass)
    }
    ...
    companion object {
        private const val PERM_Accept = 0
        private const val PERM_Connect = 1
        private const val PERM_Listen = 2
        private const val PERM_Multicast = 3
        private const val PERM_SetFactory = 4
        private const val PERM_Exec = 5
        private const val PERM_Link = 6
        private const val PERM_Delete = 7
        private const val PERM_Read = 8
        private const val PERM_Write = 9
        private const val PERM_GROUP_FILE = 0
        private const val PERM_GROUP_NETWORK = 1
        private const val PERM_GROUP_EXEC = 2

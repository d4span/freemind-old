 ...
 */
class SearchControllerHook : MindMapHookAdapter(), ISearchController {
    /**
     */
    init {
        if (logger == null) {
            logger = getController()!!.frame.getLogger(
                    this.javaClass.name)
        }
    }

    override fun getLogger(className: Class<*>): Logger? {
        return getController()!!.frame.getLogger(className.name)
    }
    ...
    override fun startupMapHook() {
        ...
        val panel = SearchViewPanel(this)
    ...
    override fun openMap(mapModule: String?) {
        logger!!.fine("open map :$mapModule")
        getController()!!.loadURL(mapModule)
    }

    override val filesOfOpenTabs: Array<File?>
        get() {
            val maps: List<MapModule> = getController()!!.frame.getController()
                    .getMapModuleManager().getMapModuleVector()
            val mapFiles = arrayOfNulls<File>(maps.size)
            for (i in mapFiles.indices) {
                mapFiles[i] = maps[i].model.file
            }
            return mapFiles
        }

    override fun setWaitingCursor(waiting: Boolean) {
        getController()!!.frame.setWaitingCursor(waiting)
    }
}
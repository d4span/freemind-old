 ...
 * @author foltin
 */
class ExportPdf : ExportVectorGraphic() {
    var papers = ExportPdfPapers()
    override fun startupMapHook() {
        super.startupMapHook()
        val nodeExport = safeEquals("node",
                getResourceString("export_type"))
        var transcodingHints: HashMap<TranscodingHints.Key?, Float?>? = null
        val selecteds = getController()!!.selecteds
        val documentsToOpen = Vector<File>()
        while (!selecteds.isEmpty()) {
            val selectedNode = selecteds.removeAt(0) as MindMapNode?
            var nameExtension: String? = null
            if (nodeExport) {
                nameExtension = (" "
                        + selectedNode!!.getShortText(getController()))
            }
            val chosenFile = chooseFile("pdf",
                    getResourceString("export_pdf_text"), nameExtension) ?: return
        ...
        try {
            for (fileToOpen in documentsToOpen) {
                getController()!!.frame.openDocument(
                        fileToUrl(fileToOpen))
            }
        } catch (e: Exception) {
            Resources.getInstance().logException(e)
        }
     ...
     */
    fun choosePaper(): HashMap<TranscodingHints.Key?, Float?>? {
        val retValue = HashMap<TranscodingHints.Key?, Float?>()
        // user dialog
        val paperNames = papers.paperNames
        val controller: Controller? = getController()!!.controller
        val dialog = ExportPdfDialog(getController()!!.frame
        ...
        val orientation = dialog.orientation
        ...
        val paper = papers.determinePaper(format)
        if (paper != null) {
            pageFormat.paper = paper
            if (pageFormat.orientation == PageFormat.PORTRAIT) {
                logger!!.info("Orientation: Portrait")
                // portrait
                retValue[PDFTranscoder.KEY_HEIGHT] = pageFormat
                        .paper.height
                retValue[PDFTranscoder.KEY_WIDTH] = pageFormat
                        .paper.width
            } else {
    ...
    fun exportAsPdf(nodeExport: Boolean, selectedNode: MindMapNode?,
                    chosenFile: File?, pTranscoderHints: HashMap<TranscodingHints.Key?, Float?>? = null): Boolean {
        val view: MapView = getController()!!.view ?: return false
        val g2d: SVGGraphics2D?
        g2d = if (nodeExport) {
            fillSVGGraphics2D(view, selectedNode)
        } else {
            fillSVGGraphics2D(view)
        }
        val pdfTranscoder = PDFTranscoder()
        /*
		 * according to
		 ...
		 */pdfTranscoder.addTranscodingHint(PDFTranscoder.KEY_MAX_HEIGHT, 19200)
        pdfTranscoder.addTranscodingHint(PDFTranscoder.KEY_MAX_WIDTH, 19200)
        if (pTranscoderHints != null) {
            for (key in pTranscoderHints.keys) {
                pdfTranscoder.addTranscodingHint(key, pTranscoderHints[key])
            }
        }
        /* end patch */
        val doc = g2d.domFactory
        val rootE = doc.documentElement
        g2d.getRoot(rootE)
        val input = TranscoderInput(doc)
        val ostream = FileOutputStream(chosenFile)
        val bufStream = BufferedOutputStream(ostream)
        val output = TranscoderOutput(bufStream)
        // save the image

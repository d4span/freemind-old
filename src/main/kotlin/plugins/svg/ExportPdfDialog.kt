 ...
 */
class ExportPdfDialog(owner: Frame?,
                      /**
                       * list of available formats
                       */
                      private val formatsCollection: Array<String?>?,
                      /**
                       * reference to the freemind controller
                       */
                      private val controller: Controller?) : JDialog(owner) {
    /**
     *
     ...
     */
    var orientation = 0
    private var listBox: JComboBox<*>? = null
    private var birdButtonLandscape: JRadioButton? = null
    private var birdButtonPortrait: JRadioButton? = null
    ...
    private fun initialize() {
        this.isModal = true
        title = controller!!.getResourceString("ExportPdfDialog.PDF_Export_Settings")
        this.preferredSize = Dimension(400, 150)
        // Page size
        val labelPaper = JLabel(controller.getResourceString("ExportPdfDialog.Size_"))
        Arrays.sort(formatsCollection)
        listBox = JComboBox<Any?>(formatsCollection)
        listBox.setEditable(false)
        listBox.setPreferredSize(Dimension(100, 25))

        // Page orientation
        birdButtonLandscape = JRadioButton(controller.getResourceString("ExportPdfDialog.Landscape"))
        birdButtonLandscape!!.mnemonic = KeyEvent.VK_L
        birdButtonLandscape!!.isSelected = true
        birdButtonPortrait = JRadioButton(controller.getResourceString("ExportPdfDialog.Portrait"))
        birdButtonPortrait!!.mnemonic = KeyEvent.VK_P
        group = ButtonGroup()
        group!!.add(birdButtonLandscape)
        group!!.add(birdButtonPortrait)

        // ok button
        val jOKButton = JButton()
        jOKButton.preferredSize = Dimension(100, 20)
        jOKButton.action = object : AbstractAction() {
            override fun actionPerformed(e: ActionEvent) {
                okPressed()
            }
        }
        jOKButton.text = controller.getResourceString("ExportPdfDialog.OK")

        // panels and layout
        val panel1 = JPanel()
        panel1.border = BorderFactory.createTitledBorder(controller.getResourceString("ExportPdfDialog.Paper"))
        val gridbag1 = GridBagLayout()
        panel1.layout = gridbag1
        val panel2 = JPanel()
        panel2.border = BorderFactory.createTitledBorder(controller.getResourceString("ExportPdfDialog.Orientation"))
        ...
        getRootPane().defaultButton = jOKButton
        pack()
    }
    ...
    private fun initDefaults() {
        // page orientation
        val storedOrientation = controller
                .getProperty(PROP_PDF_PAGE_ORIENTATION)
        if (storedOrientation != null) // property found
        {
            if (safeEquals(storedOrientation, LANDSCAPE)) {
                birdButtonLandscape!!.isSelected = true
            } else {
                birdButtonPortrait!!.isSelected = true
            }
        }

        // page format
        val storedFormat = controller!!.getProperty(PROP_PDF_PAGE_FORMAT)
        if (storedFormat != null) // property found
        {
            listBox!!.selectedItem = storedFormat
        }
    }
     ...
     */
    private fun storeDefaults() {
        // page orientation
        if (orientation == PageFormat.LANDSCAPE) controller!!.setProperty(PROP_PDF_PAGE_ORIENTATION, LANDSCAPE) else if (orientation == PageFormat.PORTRAIT) controller!!.setProperty(PROP_PDF_PAGE_ORIENTATION, PORTRAIT)
    ...
    private fun okPressed() {
        format = listBox!!.selectedItem as String
        orientation = if (birdButtonLandscape!!.isSelected) {
    ...
    companion object {
        ...
        private const val PROP_PDF_PAGE_FORMAT = "pdf_page_format"
        private const val PROP_PDF_PAGE_ORIENTATION = "pdf_page_orientation"
    }
}
...
class ExportPdfPapers {
     ...
     */
    var paperFormats: MutableMap<String?, Paper> = HashMap()
     ...
     */
    val paperNames: Array<String?>
        get() {
            val o_names: Array<Any> = paperFormats.keys.toTypedArray()!!
            val names = arrayOfNulls<String>(o_names.size)
            for (i in 0 until paperFormats.size) {
                names[i] = o_names[i] as String
            }
            return names
        }
     ...
     *
     * for more papers see [://en.wikipedia.org/wiki/Paper_size][http]
     */
     ...
     * @param displayName
     */
    private fun addPaper(name: MediaSizeName, displayName: String) {
        val mSize = MediaSize.getMediaSizeForName(name)
        val paper = Paper()
        paper.setSize(mSize.getX(MediaSize.MM) * ppmm, mSize.getY(MediaSize.MM)
                * ppmm)
        paperFormats[displayName] = paper
    }
     ...
     */
    fun determinePaper(format: String?): Paper? {
        val o: Any? = paperFormats[format]
        return if (o != null && o is Paper) {
            o
        } else {
            null
        }
	 ...
	 */
        private const val ppmm = 96 / 25.4
    }
}
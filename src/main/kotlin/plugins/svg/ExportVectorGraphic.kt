 ...
 * @author foltin
 */
open class ExportVectorGraphic : ExportHook() {
    /**
     */
    protected fun fillSVGGraphics2D(view: MapView): SVGGraphics2D {
        // NodeAdapter root = (NodeAdapter) getController().getMap().getRoot();
        val g2d = createSvgGraphics2D()
        try {
            view.preparePrinting()
            val innerBounds = view.innerBounds
            g2d.svgCanvasSize = Dimension(innerBounds.width,
                    innerBounds.height)
    ...
    protected fun fillSVGGraphics2D(view: MapView, pNode: MindMapNode?): SVGGraphics2D {
        val g2d = createSvgGraphics2D()
        try {
            view.preparePrinting()
            var innerBounds: Rectangle? = null
            for (nodeView in view.getViewers(pNode)) {
                if (innerBounds == null) {
                    innerBounds = nodeView.innerBounds
                } else {
                    innerBounds.add(nodeView.innerBounds)
                }
            }
            g2d.svgCanvasSize = Dimension(innerBounds!!.width,
                    innerBounds.height)
    ...
    fun createSvgGraphics2D(): SVGGraphics2D {
        val impl = GenericDOMImplementation
                .getDOMImplementation()
        val namespaceURI = SVGConstants.SVG_NAMESPACE_URI
        val domFactory = impl.createDocument(namespaceURI, "svg", null)
        val ctx = SVGGeneratorContext.createDefault(domFactory)
        ctx.isEmbeddedFontsOn = true
        val defaults = GraphicContextDefaults()
        defaults.font = Font("Arial", Font.PLAIN, 12)
        ctx.graphicContextDefaults = defaults
        ctx.precision = 12
        val g2d = SVGGraphics2D(ctx, false)
        // This prevents the
    ...
    fun transForm(xmlSource: Source?, xsltStream: InputStream?,
                  resultFile: File, areaCode: String?) {
        ...
        try {
            ...
            trans.setParameter("destination_dir", resultFile.name
                    + "_files/")

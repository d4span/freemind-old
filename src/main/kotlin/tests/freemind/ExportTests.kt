 ...
 * @date 12.08.2011
 */
class ExportTests : FreeMindTestBase() {
    ...
    companion object {
        ...
        @Throws(FileNotFoundException::class, XMLParseException::class, IOException::class, URISyntaxException::class)
        @JvmStatic
        fun main(args: Array<String>) {
            val mFreeMindMain = FreeMindMainMock()
            val fm = JDialog()
            fm.title = "Title"
            fm.isModal = true
            val dim = Rectangle()
            val parent: JPanel = object : JPanel() {
                override fun paintChildren(pG: Graphics) {
            ...
            val creator = IndependantMapViewCreator()
            val mapView = creator.createMapViewForFile(TESTMAP_MM, parent,
                    mFreeMindMain)
            parent.add(mapView, BorderLayout.CENTER)
            mapView.doLayout()
            val innerBounds = mapView.innerBounds
            val bounds = mapView.bounds
            dim.y = bounds.y + innerBounds.y
            // System.out.println(bounds);
            // System.out.println(innerBounds);
            fm.bounds = mapView.innerBounds
            fm.defaultCloseOperation = WindowConstants.DISPOSE_ON_CLOSE
            fm.addWindowListener(object : WindowAdapter() {

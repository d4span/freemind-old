 ...
 * @author foltin
 */
class ScriptEditorPanelTest : FreeMindTestBase() {
    private class TestScriptModel : ScriptModel {
        var scripts = arrayOf(
                "j=0;\nfor(i in (0..6))\n	{\n		j+=i; \n		print i;\n	}; \nreturn j;\n",
                "for(i=0;i<5;++i){print i;}")

        override fun getAmountOfScripts(): Int {
            // TODO Auto-generated method stub
            return 2
        }

        override fun getScript(pIndex: Int): ScriptHolder {
            return ScriptHolder("script$pIndex", scripts[pIndex])
        }

        override fun setScript(pIndex: Int, pScript: ScriptHolder) {
            scripts[pIndex] = pScript.script
        }

        override fun executeScript(pIndex: Int, outStream: PrintStream,
        ...
        override fun decorateDialog(
            ...
            storage.height = 800
            storage.width = 400
            pPanel.rootPane.preferredSize = Dimension(storage.width, storage.height)
            storage.leftRatio = 100
            storage.topRatio = 500
            return storage
        }
    ...
    fun _testPanel() {
        val scriptEditor = ScriptEditorPanel(
                TestScriptModel(), FreeMindMainMock(), true)
        scriptEditor.isVisible = true
    }

    @Throws(Exception::class)
    fun testErrorLineNumbers() {
        val error = "startup failed, Script1.groovy: 1: For statement contains unexpected tokens. Possible attempt to use unsupported Java-style for loop. at line: 1 column: 1. File: Script1.groovy @ line 1, column 1.\n1 error"

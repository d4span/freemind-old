 ...
 * @date 13.08.2013
 */
class LayoutTests : FreeMindTestBase() {
    private var mRoot: MindMapNodeModel? = null
    private var mChild1: MindMapNodeModel? = null
    private var mChild2: MindMapNodeModel? = null
    private var mMapView: MapView? = null
    private var mModel: MindMapMapModel? = null
    private var mScrollPane: MapView.ScrollPane? = null
    ...
    override fun setUp() {
        ...
        mScrollPane = MapView.ScrollPane()
        parent.add(mScrollPane, BorderLayout.CENTER)
        val controller = Controller(mFreeMindMain)
        controller.initialization()
        val mode: MindMapMode = object : MindMapMode() {
            override fun createModeController(): ModeController {
                return object : MindMapController(this) {
                    override fun init() {}
                }
            }
        }
        mode.init(controller)
        val mc = mode.createModeController() as MindMapController
        mModel = MindMapMapModel(mc)
        mc.setModel(mModel)
        mRoot = MindMapNodeModel("ROOT", mModel)
        mChild1 = MindMapNodeModel("CHILD1", mModel)
        mRoot!!.insert(mChild1!!, 0)
        mChild2 = MindMapNodeModel("CHILD2", mModel)
        mRoot!!.insert(mChild2!!, 1)
        mModel!!.root = mRoot
        mMapView = object : MapView(mModel!!, mc) {
            override fun selectAsTheOnlyOneSelected(pNewSelected: NodeView?) {}
        }
        mScrollPane!!.setViewportView(mMapView)
        mc.view = mMapView
        mMapView.setBounds(parent.bounds)
    ...
    fun testYShift() {
        layout(mMapView)
        val yCoordinate = getYCoordinate(mChild2)
        val yCoordinateRoot = getYCoordinate(mRoot)
        mChild2!!.shiftY = 10
        layout(mMapView)
    ...
    @Throws(Exception::class)
    fun testYShiftNegative() {
        layout(mMapView)
        val yCoordinate = getYCoordinate(mChild2)
        val yCoordinateRoot = getYCoordinate(mRoot)
        val yCoordinateChild1 = getYCoordinate(mChild1)
        val delta = -10
        mChild2!!.shiftY = delta
        layout(mMapView)
        assertTrue(getYCoordinate(mChild1) != getYCoordinate(mChild2))
    ...
    @Throws(Exception::class)
    fun testYShiftNegativeWith3Childs() {
        val child3 = MindMapNodeModel("CHILD3", mModel)
        mModel!!.insertNodeInto(child3, mRoot, 2)
        layout(mMapView)
        val yCoordinate = getYCoordinate(mChild2)
        val yCoordinateRoot = getYCoordinate(mRoot)
        val yCoordinateChild1 = getYCoordinate(mChild1)
        val yCoordinate3 = getYCoordinate(child3)
        val delta = -10
        mChild2!!.shiftY = delta
    ...
    fun testScrollMap() {
        layout(mMapView)
        val yCoordinateRoot = getYCoordinateToViewport(mRoot)
        val delta = 10
        val viewPosition = mMapView!!.viewPosition
        mMapView!!.scrollBy(0, delta)
        layout(mMapView)
        assertEquals(viewPosition!!.y + delta, mMapView!!.viewPosition!!.y)
        assertEquals(yCoordinateRoot - delta, getYCoordinateToViewport(mRoot))
    }

    @Throws(Exception::class)
    fun testYShiftNegativeWith3ChildsWithRootMovement() {
        val child3 = MindMapNodeModel("CHILD3", mModel)
        mModel!!.insertNodeInto(child3, mRoot, 2)
        layout(mMapView)
        val yCoordinate = getYCoordinateToViewport(mChild2)
        val yCoordinateRoot = getYCoordinateToViewport(mRoot)
        val yCoordinateChild1 = getYCoordinateToViewport(mChild1)
        val yCoordinate3 = getYCoordinateToViewport(child3)
        val delta = -10
        mChild2!!.shiftY = delta
        mMapView!!.scrollBy(0, -delta)
        layout(mMapView)
    ...
    @Throws(Exception::class)
    fun testYShiftNegativeWith3ChildsYCalcToRoot() {
        val child3 = MindMapNodeModel("CHILD3", mModel)
        mModel!!.insertNodeInto(child3, mRoot, 2)
        layout(mMapView)
        val yCoordinateRoot = getYCoordinate(mRoot)
        val yCoordinate = getYCoordinate(mChild2) - yCoordinateRoot
        val yCoordinateChild1 = getYCoordinate(mChild1) - yCoordinateRoot
        val yCoordinate3 = getYCoordinate(child3) - yCoordinateRoot
        val delta = -10
        mChild2!!.shiftY = delta
        layout(mMapView)
        val yCoordinateRoot2 = getYCoordinate(mRoot)
    ...
    protected fun layout(mapView: MapView?) {
        val root = mapView!!.root
        val layout = root!!.layout
        layout.layoutContainer(root)
        root.getMainView()!!.doLayout()
        val nodes = getVectorWithSingleElement<NodeView?>(root)
        // print summary
        println("------------------")
        while (!nodes.isEmpty()) {
            val view = nodes.lastElement()
            nodes.remove(view)
            nodes.addAll(view!!.childrenViews)
            val yCoordinate = getYCoordinate(view.model)
            val yCoordinateRoot = getYCoordinate(root.model)
            println("Y coordinate of node " + view.model + " is " + (yCoordinate - yCoordinateRoot))
        }
        println("------------------")
    }

    protected fun getYCoordinate(child2: MindMapNode?): Int {
        assertTrue(mMapView!!.getViewers(child2).size > 0)
        val nodeView = mMapView!!.getViewers(child2).iterator().next()
        val point = nodeView.getMainView()!!.location
        convertPointToAncestor(nodeView, point, mMapView!!)
        return point.y
    }

    protected fun getYCoordinateToViewport(child2: MindMapNode?): Int {
        assertTrue(mMapView!!.getViewers(child2).size > 0)
        val nodeView = mMapView!!.getViewers(child2).iterator().next()
        val point = nodeView.getMainView()!!.location
        convertPointToAncestor(nodeView, point, mScrollPane!!)
        return point.y
    }
}
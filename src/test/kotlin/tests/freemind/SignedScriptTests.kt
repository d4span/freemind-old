...
class SignedScriptTests : TestCase() {
    @Throws(Exception::class)
    fun testSignedInitialization() {
        val out = ByteArrayOutputStream()
        val signedScriptHandler = SignedScriptHandler()
        assertEquals(true,
  ...
  """, out))
    ...
    @Throws(Exception::class)
    fun testScriptContents() {
        var scriptContent = ScriptContents(SIGNED_SCRIPT)
        ...
        scriptContent = ScriptContents(SIGNED_SCRIPT_OTHER_KEY)
        assertEquals(SCRIPTS_CONTENT, scriptContent.mScript)
        assertEquals(SCRIPTS_SIGNATURE, scriptContent.mSignature)
    ...
    companion object {
        private const val SCRIPTS_CONTENT = "test"
        private const val SCRIPTS_SIGNATURE = "MCwCFCllrN6Xig7V0nRFGmWBLoBauMiGAhQLoYbNRTjVS1c7A2ev3bvJqUqg8Q=="
        private const val SIGNED_SCRIPT = (SCRIPTS_CONTENT + "//SIGN:"
                + SCRIPTS_SIGNATURE)
        private const val SIGNED_SCRIPT_OTHER_KEY = (SCRIPTS_CONTENT
                + "//SIGN(mykey):" + SCRIPTS_SIGNATURE)
        private const val SIGNED_SCRIPT_FREEMIND_KEY = (SCRIPTS_CONTENT

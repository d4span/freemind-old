...
class FindTextTests : FreeMindTestBase() {
    @Throws(Exception::class)
    fun testTagRemoval() {
        val input = "<html>test</html>"
        assertEquals("tags are removed:", "test",
                HtmlTools.removeHtmlTagsFromString(input))
    }

    @Throws(Exception::class)
    fun testTagRemovalWithNewlines() {
        val input = "<html><strong\nref=\"test2\">test</strong></html\n>"
        assertEquals("tags are removed:", "test",
                HtmlTools.removeHtmlTagsFromString(input))
    }

    @Throws(Exception::class)
    fun testTagRemovalOnlyForHtmlText() {
        val input = "test<test>"
     ...
     */
    @Throws(Exception::class)
    fun testFlatNodeTableFilter() {
        val tm: AbstractTableModel = object : AbstractTableModel() {
            ...
            override fun getValueAt(rowIndex: Int, columnIndex: Int): Any {
                val testMindMapNode = TestMindMapNode()
                testMindMapNode
                        .setText("<html><strong>test</strong></html>")
                return if (columnIndex == 0) {
                    TimeList.NodeHolder(testMindMapNode)
                } else {
                    NotesHolder(testMindMapNode)
                }
            }
        }
        val dut = FlatNodeTableFilterModel(tm, 0, 1)
        assertEquals("One row:", 1, dut.rowCount)
        dut.setFilter("strong")
        assertEquals("No row:", 0, dut.rowCount)
        dut.setFilter("test")
        assertEquals("One row:", 1, dut.rowCount)
    }
    ...
    @Throws(Exception::class)
    fun testPositions() {
        val pair1 = IndexPair(0, 6, 0, 0, false)
        val pair2 = IndexPair(6, 14, 0, 0, false)
        val pair3 = IndexPair(14, 27, 0, 4, false)
        val pair4 = IndexPair(27, 34, 4, 4, false)
        val pair5 = IndexPair(34, 34, 4, 4, false)
        val list = ArrayList<IndexPair>()
        list.add(pair1)
        list.add(pair2)
        list.add(pair3)
        list.add(pair4)
    ...
    fun testGetPureRegularExpression() {
    ...
    private fun executeRegExpRemovalAndTest(input: String, outputExpected: String) {
        val result = TimeList.getPureRegularExpression(input)
    ...
    @Throws(Exception::class)
    fun testReplaceNodeText() {
        // normal text is replaced,
        var info: IReplaceInputInformation = TestReplaceInputInfo(
                "<html><strong>test</strong></html>",
                "<html><strong>blabla</strong></html>")
        TimeList.replace(info, "test", "blabla")
        // but tags not:
        info = TestReplaceInputInfo("<html><strong>test</strong></html>",
                "<html><strong>test</strong></html>")
        TimeList.replace(info, "strong", "strang")
        // and replaces <tag>tes</tag>t by <tag>text</tag>:
        info = TestReplaceInputInfo("<html><strong>tes</strong>t</html>",
                "<html><strong>blabla</strong></html>")
        TimeList.replace(info, "test", "blabla")
        // and replaces tex<tag>t</tag> by text<tag></tag>:
        info = TestReplaceInputInfo("<html>tes<strong>t</strong></html>",
                "<html>blabla<strong></strong></html>")
        TimeList.replace(info, "test", "blabla")
        // replace with environment
        info = TestReplaceInputInfo(
        ...
        // no html at all
        info = TestReplaceInputInfo("beforetestafter", "beforeblablaafter")
        TimeList.replace(info, "test", "blabla")
    }

    private inner class TestReplaceInputInfo(private val input: String, private val output: String) : IReplaceInputInformation {
        override fun getLength(): Int {
            return 1
        }

        override fun getNodeHolderAt(i: Int): TimeList.NodeHolder {
            val testMindMapNode = TestMindMapNode()
            testMindMapNode.setText(input)
            return TimeList.NodeHolder(testMindMapNode)
        }

        override fun changeString(holder: TimeList.NodeHolder, newText: String) {
